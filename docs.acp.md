# SiriClaw-Instruct ACP Bridge

This document describes how the SiriClaw-Instruct ACP (Agent Client Protocol) bridge works,
how it maps ACP sessions to Gateway sessions, and how IDEs should invoke it.

## Overview

`SiriClaw-Instruct acp` exposes an ACP agent over stdio and forwards prompts to a running
SiriClaw-Instruct Gateway over WebSocket. It keeps ACP session ids mapped to Gateway
...
- ACP client spawns `SiriClaw-Instruct acp` and speaks ACP messages over stdio.
...
SiriClaw-Instruct acp --reset-session
```
...
`SiriClaw-Instruct acp` resolves the Gateway URL and auth from CLI flags or config:
