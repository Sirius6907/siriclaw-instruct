    ...
    await withTempHome(
      async (tempHome) => {
        ...
        const env = {
          ...
          SiriClaw-Instruct_TEST_FAST: "1",
        };
        delete env.SiriClaw-Instruct_HOME;
        delete env.SiriClaw-Instruct_STATE_DIR;
        delete env.SiriClaw-Instruct_CONFIG_PATH;
        delete env.VITEST;

        const entry = path.resolve(process.cwd(), "SiriClaw-Instruct.mjs");
      ...
      { prefix: "SiriClaw-Instruct-json-e2e-" },
    );
  });
});
