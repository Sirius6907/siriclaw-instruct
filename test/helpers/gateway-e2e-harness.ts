...
export async function spawnGatewayInstance(name: string): Promise<GatewayInstance> {
  ...
  const homeDir = await fs.mkdtemp(path.join(os.tmpdir(), `SiriClaw-Instruct-e2e-${name}-`));
  const configDir = path.join(homeDir, ".SiriClaw-Instruct");
    ...
    child = spawn(
      ...
      ],
      {
        ...
        env: {
          ...
          SiriClaw-Instruct_SKIP_PROVIDERS: "1",
          SiriClaw-Instruct_SKIP_GMAIL_WATCHER: "1",
          SiriClaw-Instruct_SKIP_CRON: "1",
          SiriClaw-Instruct_SKIP_BROWSER_CONTROL_SERVER: "1",
          SiriClaw-Instruct_SKIP_CANVAS_HOST: "1",
          SiriClaw-Instruct_TEST_MINIMAL_GATEWAY: "1",
          VITEST: "1",
        },
