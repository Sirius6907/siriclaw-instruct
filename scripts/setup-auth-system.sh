#!/bin/bash
# Setup SiriClaw-Instruct Auth Management System
...
echo "=== SiriClaw-Instruct Auth System Setup ==="
echo ""
...
echo "  2. Subscribe to a topic (e.g., 'SiriClaw-Instruct-alerts')"
echo ""
...
# Phone notification
echo ""
echo "  SiriClaw-Instruct message: Send warning via SiriClaw-Instruct itself"
echo "Enter your phone number for alerts (or leave blank to skip):"
read -r PHONE_NUMBER

# Update service file
SERVICE_FILE="$SCRIPT_DIR/systemd/SiriClaw-Instruct-auth-monitor.service"
...
cp "$SCRIPT_DIR/systemd/SiriClaw-Instruct-auth-monitor.service" ~/.config/systemd/user/
cp "$SCRIPT_DIR/systemd/SiriClaw-Instruct-auth-monitor.timer" ~/.config/systemd/user/
systemctl --user daemon-reload
systemctl --user enable --now SiriClaw-Instruct-auth-monitor.timer
...
echo "  - Auth monitor: systemctl --user status SiriClaw-Instruct-auth-monitor.timer"
echo ""
echo "Quick commands:"
