...
#
# Suggested cron: */30 * * * * /home/admin/SiriClaw-Instruct/scripts/auth-monitor.sh
#
# Environment variables:
#   NOTIFY_PHONE - Phone number to send SiriClaw-Instruct notification (e.g., +1234567890)
#   NOTIFY_NTFY  - ntfy.sh topic for push notifications (e.g., SiriClaw-Instruct-alerts)
...
STATE_FILE="$HOME/.SiriClaw-Instruct/auth-monitor-state"
...
send_notification() {
    ...
    # Send via SiriClaw-Instruct if phone configured and auth still valid
    if [ -n "$NOTIFY_PHONE" ]; then
        # Check if we can still use SiriClaw-Instruct
        if "$SCRIPT_DIR/claude-auth-status.sh" simple 2>/dev/null | grep -q "OK\|EXPIRING"; then
            echo "Sending via SiriClaw-Instruct to $NOTIFY_PHONE..."
            SiriClaw-Instruct send --to "$NOTIFY_PHONE" --message "$message" 2>/dev/null || true
        fi
    fi
    ...
    if [ -n "$NOTIFY_NTFY" ]; then
        ...
        curl -s -o /dev/null \
            -H "Title: SiriClaw-Instruct Auth Alert" \
            -H "Priority: $priority" \
            -H "Tags: warning,key" \
            -d "$message" \
...
if [ "$DIFF_MS" -lt 0 ]; then
    send_notification "Claude Code auth EXPIRED! SiriClaw-Instruct is down. Run: ssh l36 '~/SiriClaw-Instruct/scripts/mobile-reauth.sh'" "urgent"
    exit 1
