...
# Checks both Claude Code and SiriClaw-Instruct auth status
...
fetch_models_status_json() {
    SiriClaw-Instruct models status --json 2>/dev/null || true
}
...
echo ""
echo "SiriClaw-Instruct Auth (~/.SiriClaw-Instruct/agents/main/agent/auth-profiles.json):"
if [ "$USE_JSON" -eq 1 ]; then
...
if [ "$expires" -le 0 ] && [ "$api_keys" -gt 0 ]; then
    ...
    echo -e "  Status: ${RED}NOT FOUND${NC}"
    echo "  Note: Run 'SiriClaw-Instruct doctor --yes' to sync from Claude Code"
else
    now_ms=$(( $(date +%s) * 1000 ))
...
echo ""
echo "=== Service Status ==="
if systemctl --user is-active SiriClaw-Instruct >/dev/null 2>&1; then
    echo -e "SiriClaw-Instruct service: ${GREEN}running${NC}"
else
    echo -e "SiriClaw-Instruct service: ${RED}NOT running${NC}"
fi
