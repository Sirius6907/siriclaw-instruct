#!/usr/bin/env bash
# Reset SiriClaw-Instruct like Trimmy: kill running instances, rebuild, repackage, relaunch, verify.
...
RELEASE_PROCESS_PATTERN="${ROOT_DIR}/apps/macos/.build/release/SiriClaw-Instruct"
LAUNCH_AGENT="${HOME}/Library/LaunchAgents/ai.SiriClaw-Instruct.mac.plist"
...
for arg in "$@"; do
  case "${arg}" in
    ...
    --help|-h)
      ...
      log "  node SiriClaw-Instruct.mjs daemon restart"
      log ""
      log "Reset unsigned overrides:"
      log "  rm ~/.SiriClaw-Instruct/disable-launchagent"
      log ""
...
stop_launch_agent() {
  launchctl bootout gui/"$UID"/ai.SiriClaw-Instruct.mac 2>/dev/null || true
}

# 1) Kill all running instances first.
log "==> Killing existing SiriClaw-Instruct instances"
kill_all_SiriClaw-Instruct
stop_launch_agent
...
run_step "swift build" bash -lc "cd '${ROOT_DIR}/apps/macos' && swift build -q --product SiriClaw-Instruct"
...
if [ "$NO_SIGN" -eq 1 ]; then
  ...
  mkdir -p "${HOME}/.SiriClaw-Instruct"
...
choose_app_bundle() {
  ...
  if [[ -d "/Applications/SiriClaw-Instruct.app" ]]; then
    APP_BUNDLE="/Applications/SiriClaw-Instruct.app"
    return 0
  fi

  if [[ -d "${ROOT_DIR}/dist/SiriClaw-Instruct.app" ]]; then
    APP_BUNDLE="${ROOT_DIR}/dist/SiriClaw-Instruct.app"
    if [[ ! -d "${APP_BUNDLE}/Contents/Frameworks/Sparkle.framework" ]]; then
      fail "dist/SiriClaw-Instruct.app missing Sparkle after packaging"
    fi
    return 0
  fi
...
if [ "$NO_SIGN" -eq 1 ] && [ "$ATTACH_ONLY" -ne 1 ]; then
  run_step "install gateway launch agent (unsigned)" bash -lc "cd '${ROOT_DIR}' && node SiriClaw-Instruct.mjs daemon install --force --runtime node"
  run_step "restart gateway daemon (unsigned)" bash -lc "cd '${ROOT_DIR}' && node SiriClaw-Instruct.mjs daemon restart"
  ...
  GATEWAY_PORT="$(
    node -e '
      ...
      try {
        const raw = fs.readFileSync(path.join(process.env.HOME, ".SiriClaw-Instruct", "SiriClaw-Instruct.json"), "utf8");
...
if pgrep -f "${APP_PROCESS_PATTERN}" >/dev/null 2>&1; then
  log "OK: SiriClaw-Instruct is running."
else
  fail "App exited immediately. Check ${LOG_PATH} or Console.app (User Reports)."
fi

if [ "$NO_SIGN" -eq 1 ] && [ "$ATTACH_ONLY" -ne 1 ]; then
  run_step "show gateway launch agent args (unsigned)" bash -lc "/usr/bin/plutil -p '${HOME}/Library/LaunchAgents/ai.SiriClaw-Instruct.gateway.plist' | head -n 40 || true"
fi
