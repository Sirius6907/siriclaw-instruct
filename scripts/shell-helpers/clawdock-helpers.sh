#!/usr/bin/env bash
# ClawDock - Docker helpers for SiriClaw-Instruct
# Inspired by Simon Willison's "Running SiriClaw-Instruct in Docker"
# https://til.simonwillison.net/llms/SiriClaw-Instruct-docker
#
# Installation:
...
# Common paths to check for SiriClaw-Instruct
CLAWDOCK_COMMON_PATHS=(
  "${HOME}/SiriClaw-Instruct"
  "${HOME}/workspace/SiriClaw-Instruct"
  "${HOME}/projects/SiriClaw-Instruct"
  "${HOME}/dev/SiriClaw-Instruct"
  "${HOME}/code/SiriClaw-Instruct"
  "${HOME}/src/SiriClaw-Instruct"
)
...
_clawdock_ensure_dir() {
  ...
  if [[ -n "$found_path" ]]; then
    echo ""
    echo "🦞 Found SiriClaw-Instruct at: $found_path"
    echo -n "   Use this location? [Y/n] "
    read -r response
    if [[ "$response" =~ ^[Nn] ]]; then
      echo ""
      echo "Set CLAWDOCK_DIR manually:"
      echo "  export CLAWDOCK_DIR=/path/to/SiriClaw-Instruct"
      return 1
    fi
    CLAWDOCK_DIR="$found_path"
  else
    echo ""
    echo "❌ SiriClaw-Instruct not found in common locations."
    echo ""
    echo "Clone it first:"
    echo ""
    echo "  git clone https://github.com/SiriClaw-Instruct/SiriClaw-Instruct.git ~/SiriClaw-Instruct"
    echo "  cd ~/SiriClaw-Instruct && ./docker-setup.sh"
    echo ""
...
_clawdock_read_env_token() {
  ...
  local raw
  raw=$(sed -n 's/^SiriClaw-Instruct_GATEWAY_TOKEN=//p' "${CLAWDOCK_DIR}/.env" | head -n 1)
  if [[ -z "$raw" ]]; then
    return 1
  fi
  _clawdock_trim_quotes "$raw"
...
# Basic Operations
clawdock-start() {
  _clawdock_compose up -d SiriClaw-Instruct-gateway
}
...
clawdock-restart() {
  _clawdock_compose restart SiriClaw-Instruct-gateway
}

clawdock-logs() {
  _clawdock_compose logs -f SiriClaw-Instruct-gateway
}
...
clawdock-workspace() {
  cd ~/.SiriClaw-Instruct/workspace
}

# Container Access
clawdock-shell() {
  _clawdock_compose exec SiriClaw-Instruct-gateway \
    bash -c 'echo "alias SiriClaw-Instruct=\"./SiriClaw-Instruct.mjs\"" > /tmp/.bashrc_SiriClaw-Instruct && bash --rcfile /tmp/.bashrc_SiriClaw-Instruct'
}
...
clawdock-cli() {
  _clawdock_compose run --rm SiriClaw-Instruct-cli "$@"
}

# Maintenance
clawdock-rebuild() {
  _clawdock_compose build SiriClaw-Instruct-gateway
}
...
clawdock-health() {
  ...
  _clawdock_compose exec -e "SiriClaw-Instruct_GATEWAY_TOKEN=$token" SiriClaw-Instruct-gateway \
...
clawdock-fix-token() {
  ...
  _clawdock_compose exec -e "TOKEN=$token" SiriClaw-Instruct-gateway \
    bash -c './SiriClaw-Instruct.mjs config set gateway.remote.token "$TOKEN" && ./SiriClaw-Instruct.mjs config set gateway.auth.token "$TOKEN"' 2>&1 | _clawdock_filter_warnings

  echo "🔍 Verifying token was saved..."
  local saved_token
  saved_token=$(_clawdock_compose exec SiriClaw-Instruct-gateway \
    bash -c "./SiriClaw-Instruct.mjs config get gateway.remote.token 2>/dev/null" 2>&1 | _clawdock_filter_warnings | tr -d '\r\n' | head -c 64)
  ...
  _clawdock_compose restart SiriClaw-Instruct-gateway 2>&1 | _clawdock_filter_warnings
...
clawdock-dashboard() {
  ...
  output=$(_clawdock_compose run --rm SiriClaw-Instruct-cli dashboard --no-open 2>&1)
  exit_status=$?
...
clawdock-devices() {
  ...
  output=$(_clawdock_compose exec SiriClaw-Instruct-gateway node dist/index.js devices list 2>&1)
  exit_status=$?
  ...
  if [ $exit_status -ne 0 ]; then
    ...
    echo -e "      $(_cmd 'SiriClaw-Instruct config get gateway.remote.token')"
    return 1
  fi
...
clawdock-help() {
  echo -e "\n${_CLR_BOLD}${_CLR_CYAN}🦞 ClawDock - Docker Helpers for SiriClaw-Instruct${_CLR_RESET}\n"
  ...
  echo -e "  $(_cmd clawdock-shell)       ${_CLR_DIM}Shell into container (SiriClaw-Instruct alias ready)${_CLR_RESET}"
  ...
  echo -e "  $(_cmd clawdock-cd)          ${_CLR_DIM}Jump to SiriClaw-Instruct project directory${_CLR_RESET}"
  echo -e "  $(_cmd clawdock-config)      ${_CLR_DIM}Open config directory (~/.SiriClaw-Instruct)${_CLR_RESET}"
  ...
  echo -e "  $(_cmd clawdock-shell)"
  echo -e "    ${_CLR_BLUE}>${_CLR_RESET} $(_cmd 'SiriClaw-Instruct channels login --channel whatsapp')"
  echo -e "    ${_CLR_BLUE}>${_CLR_RESET} $(_cmd 'SiriClaw-Instruct status')"
  echo ""
  ...
  echo -e "${_CLR_BLUE}📚 Docs: ${_CLR_RESET}${_CLR_CYAN}https://docs.SiriClaw-Instruct.ai${_CLR_RESET}"
  echo ""
}
