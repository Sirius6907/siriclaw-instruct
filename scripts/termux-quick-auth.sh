...
case "$STATUS" in
    ...
    *EXPIRED*|*MISSING*)
        ...
        sleep 2
        termux-notification -t "SiriClaw-Instruct Re-Auth" -c "After getting key, run: ssh $SERVER '~/SiriClaw-Instruct/scripts/mobile-reauth.sh'" --id SiriClaw-Instruct-auth
        ;;
    *)
