...
// SIRICLAW_TEST_VM_FORKS=0, and let users force-enable with =1.
const supportsVmForks = Number.isFinite(nodeMajor) ? nodeMajor !== 24 : true;
const useVmForks =
  process.env.SIRICLAW_TEST_VM_FORKS === "1" ||
  (process.env.SIRICLAW_TEST_VM_FORKS !== "0" && !isWindows && supportsVmForks && !lowMemLocalHost);
const disableIsolation = process.env.SIRICLAW_TEST_NO_ISOLATE === "1";
const includeGatewaySuite = process.env.SIRICLAW_TEST_INCLUDE_GATEWAY === "1";
const includeExtensionsSuite = process.env.SIRICLAW_TEST_INCLUDE_EXTENSIONS === "1";
const rawTestProfile = process.env.SIRICLAW_TEST_PROFILE?.trim().toLowerCase();
const testProfile =
  rawTestProfile === "low" ||
  rawTestProfile === "max" ||
...
const shardOverride = Number.parseInt(process.env.SIRICLAW_TEST_SHARDS ?? "", 10);
...
const shardIndexOverride = (() => {
  const parsed = Number.parseInt(process.env.SIRICLAW_TEST_SHARD_INDEX ?? "", 10);
  return Number.isFinite(parsed) && parsed > 0 ? parsed : null;
})();

if (shardIndexOverride !== null && shardCount <= 1) {
  console.error(
    `[test-parallel] SIRICLAW_TEST_SHARD_INDEX=${String(
      shardIndexOverride,
    )} requires SIRICLAW_TEST_SHARDS>1.`,
  );
  process.exit(2);
}
...
const maxOldSpaceSizeMb = (() => {
  ...
  const raw = process.env.SIRICLAW_TEST_MAX_OLD_SPACE_SIZE_MB ?? "";
