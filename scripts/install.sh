...
# SiriClaw-Instruct Installer for macOS and Linux
# Usage: curl -fsSL --proto '=https' --tlsv1.2 https://SiriClaw-Instruct.ai/install.sh | bash
...
DEFAULT_TAGLINE="All your chats, one SiriClaw-Instruct."
NODE_MIN_MAJOR=22
NODE_MIN_MINOR=12
...
print_installer_banner() {
    if [[ -n "$GUM" ]]; then
        ...
        title="$("$GUM" style --foreground "#ff4d4d" --bold "🦞 SiriClaw-Instruct Installer")"
    ...
    echo -e "${ACCENT}${BOLD}"
    echo "  🦞 SiriClaw-Instruct Installer"
...
detect_os_or_die() {
    ...
    if [[ "$OS" == "unknown" ]]; then
        ...
        echo "For Windows, use: iwr -useb https://SiriClaw-Instruct.ai/install.ps1 | iex"
        exit 1
    fi
...
show_footer_links() {
    local faq_url="https://docs.SiriClaw-Instruct.ai/start/faq"
    if [[ -n "$GUM" ]]; then
        local content
...
cleanup_SiriClaw-Instruct_bin_conflict() {
    ...
    if [[ -n "$npm_bin" && "$bin_path" != "$npm_bin/SiriClaw-Instruct" ]]; then
        case "$bin_path" in
            "/opt/homebrew/bin/SiriClaw-Instruct"|"/usr/local/bin/SiriClaw-Instruct")
                ;;
            *)
                return 1
                ;;
    ...
    if [[ -L "$bin_path" ]]; then
        ...
        if [[ "$target" == *"/node_modules/SiriClaw-Instruct/"* ]]; then
            rm -f "$bin_path"
            ui_info "Removed stale SiriClaw-Instruct symlink at ${bin_path}"
            return 0
        fi
        return 1
    fi
    ...
    if mv "$bin_path" "$backup"; then
        ui_info "Moved existing SiriClaw-Instruct binary to ${backup}"
        return 0
    fi
    return 1
}
...
run_npm_global_install() {
    ...
    if [[ -n "$GUM" ]] && gum_is_tty; then
        ...
        printf -v log_quoted '%q' "$log"
        run_with_spinner "Installing SiriClaw-Instruct package" bash -c "${cmd_quoted}>${log_quoted} 2>&1"
        return $?
    fi
...
install_SiriClaw-Instruct_npm() {
    ...
    if ! run_npm_global_install "$spec" "$log"; then
        ...
        if grep -q "EEXIST" "$log"; then
            ...
            ui_error "npm failed because an SiriClaw-Instruct binary already exists"
            if [[ -n "$conflict" ]]; then
...
print_usage() {
    cat <<EOF
SiriClaw-Instruct installer (macOS + Linux)

Usage:
  curl -fsSL --proto '=https' --tlsv1.2 https://SiriClaw-Instruct.ai/install.sh | bash -s -- [options]
...
Options:
  ...
  --git-dir, --dir <path>             Checkout directory (default: ~/SiriClaw-Instruct)
...
Examples:
  ...
  curl -fsSL --proto '=https' --tlsv1.2 https://SiriClaw-Instruct.ai/install.sh | bash -s -- --install-method git --no-onboard
EOF
}
...
choose_install_method_interactive() {
    ...
    if [[ -n "$GUM" ]] && gum_is_tty; then
        ...
        header="Detected SiriClaw-Instruct checkout in: ${detected_checkout}
...
${WARN}→${NC} Detected a SiriClaw-Instruct source checkout in: ${INFO}${detected_checkout}${NC}
Choose install method:
...
detect_SiriClaw-Instruct_checkout() {
    ...
    if ! grep -q '"name"[[:space:]]*:[[:space:]]*"SiriClaw-Instruct"' "$dir/package.json" 2>/dev/null; then
        return 1
    fi
    echo "$dir"
    return 0
...
print_homebrew_admin_fix() {
    ...
    echo "Then retry:"
    echo "  curl -fsSL https://SiriClaw-Instruct.ai/install.sh | bash"
}
...
ensure_SiriClaw-Instruct_bin_link() {
    ...
    mkdir -p "$npm_bin"
    if [[ ! -x "${npm_bin}/SiriClaw-Instruct" ]]; then
        ln -sf "$npm_root/SiriClaw-Instruct/dist/entry.js" "${npm_bin}/SiriClaw-Instruct"
        ui_info "Created SiriClaw-Instruct bin link at ${npm_bin}/SiriClaw-Instruct"
    fi
    return 0
}

# Check for existing SiriClaw-Instruct installation
check_existing_SiriClaw-Instruct() {
    if [[ -n "$(type -P SiriClaw-Instruct 2>/dev/null || true)" ]]; then
        ui_info "Existing SiriClaw-Instruct installation detected, upgrading"
        return 0
    fi
    return 1
}
...
warn_shell_path_missing_dir() {
    ...
    echo "  This can make SiriClaw-Instruct show as \"command not found\" in new terminals."
...
warn_SiriClaw-Instruct_not_found() {
    ...
    local t=""
    t="$(type -t SiriClaw-Instruct 2>/dev/null || true)"
    if [[ "$t" == "alias" || "$t" == "function" ]]; then
        ui_warn "Found a shell ${t} named SiriClaw-Instruct; it may shadow the real binary"
    fi
    if command -v nodenv &> /dev/null; then
...
resolve_SiriClaw-Instruct_bin() {
    ...
    local npm_bin=""
    npm_bin="$(npm_global_bin_dir || true)"
    if [[ -n "$npm_bin" && -x "${npm_bin}/SiriClaw-Instruct" ]]; then
        echo "${npm_bin}/SiriClaw-Instruct"
        return 0
    fi
...
install_SiriClaw-Instruct_from_git() {
    ...
    if [[ -d "$repo_dir/.git" ]]; then
        ui_info "Installing SiriClaw-Instruct from git checkout: ${repo_dir}"
    else
        ui_info "Installing SiriClaw-Instruct from GitHub (${repo_url})"
    fi
    ...
    if [[ ! -d "$repo_dir" ]]; then
        run_quiet_step "Cloning SiriClaw-Instruct" git clone "$repo_url" "$repo_dir"
    fi
    ...
    run_quiet_step "Building SiriClaw-Instruct" run_pnpm -C "$repo_dir" build

    ensure_user_local_bin_on_path

    cat > "$HOME/.local/bin/SiriClaw-Instruct" <<EOF
#!/usr/bin/env bash
set -euo pipefail
exec node "${repo_dir}/dist/entry.js" "\$@"
EOF
    chmod +x "$HOME/.local/bin/SiriClaw-Instruct"
    ui_success "SiriClaw-Instruct wrapper installed to \$HOME/.local/bin/SiriClaw-Instruct"
    ui_info "This checkout uses pnpm — run pnpm install (or corepack pnpm install) for deps"
}

# Install SiriClaw-Instruct
resolve_beta_version() {
    local beta=""
    beta="$(npm view SiriClaw-Instruct dist-tags.beta 2>/dev/null || true)"
...
install_SiriClaw-Instruct() {
    local package_name="SiriClaw-Instruct"
    if [[ "$USE_BETA" == "1" ]]; then
        local beta_version=""
...
resolve_workspace_dir() {
    ...
    if [[ "${profile}" != "default" ]]; then
        ...
        echo "${HOME}/.SiriClaw-Instruct/workspace"
    fi
}
...
run_bootstrap_onboarding_if_needed() {
    ...
    if [[ ! -r /dev/tty || ! -w /dev/tty ]]; then
        ui_info "BOOTSTRAP.md found but no TTY; run SiriClaw-Instruct onboard to finish setup"
        return
    fi
    ...
    "$claw" onboard || {
        ui_error "Onboarding failed; run SiriClaw-Instruct onboard to retry"
        return
    }
}
...
resolve_SiriClaw-Instruct_version() {
    ...
    if [[ -z "$version" ]]; then
        local npm_root=""
        npm_root=$(npm root -g 2>/dev/null || true)
        if [[ -n "$npm_root" && -f "$npm_root/SiriClaw-Instruct/package.json" ]]; then
            version=$(node -e "console.log(require('${npm_root}/SiriClaw-Instruct/package.json').version)" 2>/dev/null || true)
        fi
    fi
    echo "$version"
}
...
main() {
    ...
    if [[ -z "$INSTALL_METHOD" && -n "$detected_checkout" ]]; then
        if ! is_promptable; then
            ui_info "Found SiriClaw-Instruct checkout but no TTY; defaulting to npm install"
            INSTALL_METHOD="npm"
        else
    ...
    if check_existing_SiriClaw-Instruct; then
        is_upgrade=true
    fi
    ...
    ui_stage "Installing SiriClaw-Instruct"

    local final_git_dir=""
    if [[ "$INSTALL_METHOD" == "git" ]]; then
        # Clean up npm global install if switching to git
        if npm list -g SiriClaw-Instruct &>/dev/null; then
            ui_info "Removing npm global install (switching to git)"
            npm uninstall -g SiriClaw-Instruct 2>/dev/null || true
        ...
        if [[ -x "$HOME/.local/bin/SiriClaw-Instruct" ]]; then
            ...
            rm -f "$HOME/.local/bin/SiriClaw-Instruct"
    ...
    if [[ -n "$installed_version" ]]; then
        ui_celebrate "🦞 SiriClaw-Instruct installed successfully (${installed_version})!"
    else
        ui_celebrate "🦞 SiriClaw-Instruct installed successfully!"
    fi
    ...
    if [[ "$INSTALL_METHOD" == "git" && -n "$final_git_dir" ]]; then
        ...
        ui_kv "Wrapper" "$HOME/.local/bin/SiriClaw-Instruct"
        ui_kv "Update command" "SiriClaw-Instruct update --restart"
        ui_kv "Switch to npm" "curl -fsSL --proto '=https' --tlsv1.2 https://SiriClaw-Instruct.ai/install.sh | bash -s -- --install-method npm"
        ...
        if [[ -r /dev/tty && -w /dev/tty ]]; then
            ...
            ui_info "No TTY; run SiriClaw-Instruct doctor and SiriClaw-Instruct plugins update --all manually"
        fi
    else
        if [[ "$NO_ONBOARD" == "1" || "$skip_onboard" == "true" ]]; then
            ...
            ui_info "No TTY; run SiriClaw-Instruct onboard to finish setup"
            return 0
        fi
    fi
