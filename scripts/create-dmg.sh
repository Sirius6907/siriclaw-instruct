...
#
# Env:
#   DMG_VOLUME_NAME        default: CFBundleName (or "SiriClaw-Instruct")
...
APP_NAME=$(/usr/libexec/PlistBuddy -c "Print CFBundleName" "$APP_PATH/Contents/Info.plist" 2>/dev/null || echo "SiriClaw-Instruct")
...
DMG_TEMP="$(mktemp -d /tmp/SiriClaw-Instruct-dmg.XXXXXX)"
...
if [[ "${SKIP_DMG_STYLE:-0}" != "1" ]]; then
  ...
  ICON_SRC="$ROOT_DIR/apps/macos/Sources/SiriClaw-Instruct/Resources/SiriClaw-Instruct.icns"
  if [[ -f "$ICON_SRC" ]]; then
...
hdiutil resize -limits "$DMG_RW_PATH" >/tmp/SiriClaw-Instruct-dmg-limits.txt 2>/dev/null || true
MIN_SECTORS="$(tail -n 1 /tmp/SiriClaw-Instruct-dmg-limits.txt 2>/dev/null | awk '{print $1}')"
rm -f /tmp/SiriClaw-Instruct-dmg-limits.txt
