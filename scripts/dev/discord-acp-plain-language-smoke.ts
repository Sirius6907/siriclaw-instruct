...
type DriverMode = "token" | "webhook" | "SiriClaw-Instruct";
...
function usage(): string {
  return (
    ...
    "--channel <discord-channel-id> [--token <driver-token> | --driver webhook --bot-token <bot-token> | --driver SiriClaw-Instruct] [options]\n\n" +
    "Manual live smoke only (not CI). Sends a plain-language instruction in Discord and verifies:\n" +
    "1) SiriClaw-Instruct spawned an ACP thread binding\n" +
    "2) agent replied in that bound thread with the expected ACK token\n\n" +
    "Options:\n" +
    "  --channel <id>               Parent Discord channel id (required)\n" +
    "  --driver <token|webhook|SiriClaw-Instruct> Driver transport mode (default: token)\n" +
    ...
    "  --SiriClaw-Instruct-bin <path>        SiriClaw-Instruct CLI binary for driver=SiriClaw-Instruct (default: SiriClaw-Instruct)\n" +
...
function parseArgs(): Args {
  ...
  const driverMode: DriverMode =
    normalizedDriverMode === "webhook"
      ? "webhook"
      : normalizedDriverMode === "SiriClaw-Instruct"
        ? "SiriClaw-Instruct"
  ...
  return {
    ...
    instruction,
    threadBindingsPath,
    SiriClaw-InstructBin,
    json,
  };
}

async function SiriClaw-InstructCliJson<T>(params: { SiriClaw-InstructBin: string; args: string[] }): Promise<T> {
  const result = await execFileAsync(params.SiriClaw-InstructBin, params.args, {
    maxBuffer: 8 * 1024 * 1024,
    env: process.env,
  });
  const stdout = (result.stdout || "").trim();
  if (!stdout) {
    throw new Error(`SiriClaw-Instruct ${params.args.join(" ")} returned empty stdout`);
  }
  return JSON.parse(stdout) as T;
}

async function readMessagesWithSiriClaw-Instruct(params: {
  SiriClaw-InstructBin: string;
  target: string;
  limit: number;
}): Promise<DiscordMessage[]> {
  const response = await SiriClaw-InstructCliJson<{
    payload?: {
      messages?: DiscordMessage[];
    };
  }>({
    SiriClaw-InstructBin: params.SiriClaw-InstructBin,
    args: [
      "message",
      "read",
      "--channel",
...
async function loadParentRecentMessages(params: {
  ...
  if (params.args.driverMode === "SiriClaw-Instruct") {
    return await readMessagesWithSiriClaw-Instruct({
      SiriClaw-InstructBin: params.args.SiriClaw-InstructBin,
  ...
  try {
    if (args.driverMode === "token") {
      ...
      const webhook = await discordApi<{ id: string; token?: string | null }>({
        ...
        authHeader: botAuthHeader,
        body: {
          name: `SiriClaw-Instruct-acp-smoke-${smokeId.slice(-8)}`,
        },
      });
      ...
      setupStage = "send-message";
      const sent = await SiriClaw-InstructCliJson<{
        payload?: {
          result?: {
            messageId?: string;
          };
        ...
        SiriClaw-InstructBin: args.SiriClaw-InstructBin,
        args: [
          "message",
          "send",
          "--channel",
      ...
      if (!sentMessageId) {
        throw new Error("SiriClaw-Instruct message send did not return payload.result.messageId");
      }
    }
  } catch (err) {
    return {
  ...
  try {
    ...
    while (Date.now() < deadline && !ackMessage) {
      try {
        const threadMessages =
          args.driverMode === "SiriClaw-Instruct"
            ? await readMessagesWithSiriClaw-Instruct({
                SiriClaw-InstructBin: args.SiriClaw-InstructBin,
                target: threadId,
                limit: 50,
              })
    ...
    if (!ackMessage) {
      ...
      return {
        ...
        stage: "wait-ack",
        smokeId,
        error: `Thread bound (${threadId}) but timed out waiting for ACK token "${ackToken}" from SiriClaw-Instruct.`,
        diagnostics: {
          bindingCandidates: [
            {
