...
BASE_IMAGE="${BASE_IMAGE:-SiriClaw-Instruct-sandbox:bookworm-slim}"
TARGET_IMAGE="${TARGET_IMAGE:-SiriClaw-Instruct-sandbox-common:bookworm-slim}"
...
If you want a clean re-create, remove old sandbox containers:
  docker rm -f \$(docker ps -aq --filter label=SiriClaw-Instruct.sandbox=1)
NOTE
