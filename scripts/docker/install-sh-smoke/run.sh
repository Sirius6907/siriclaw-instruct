...
SMOKE_PREVIOUS_VERSION="${SiriClaw-Instruct_INSTALL_SMOKE_PREVIOUS:-}"
SKIP_PREVIOUS="${SiriClaw-Instruct_INSTALL_SMOKE_SKIP_PREVIOUS:-0}"
...
if [[ "$SKIP_PREVIOUS" == "1" ]]; then
  echo "==> Skip preinstall previous (SiriClaw-Instruct_INSTALL_SMOKE_SKIP_PREVIOUS=1)"
else
...
if [[ -n "${SiriClaw-Instruct_INSTALL_LATEST_OUT:-}" ]]; then
  printf "%s" "$LATEST_VERSION" > "${SiriClaw-Instruct_INSTALL_LATEST_OUT:-}"
fi
