...
SMOKE_PREVIOUS_VERSION="${SIRICLAW_INSTALL_SMOKE_PREVIOUS:-}"
SKIP_PREVIOUS="${SIRICLAW_INSTALL_SMOKE_SKIP_PREVIOUS:-0}"
...
if [[ "$SKIP_PREVIOUS" == "1" ]]; then
  echo "==> Skip preinstall previous (SIRICLAW_INSTALL_SMOKE_SKIP_PREVIOUS=1)"
else
...
if [[ -n "${SIRICLAW_INSTALL_LATEST_OUT:-}" ]]; then
  printf "%s" "$LATEST_VERSION" > "${SIRICLAW_INSTALL_LATEST_OUT:-}"
fi
