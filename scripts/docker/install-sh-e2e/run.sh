...
MODELS_MODE="${SiriClaw-Instruct_E2E_MODELS:-${SIRICLAW_E2E_MODELS:-both}}" # both|openai|anthropic
INSTALL_TAG="${SiriClaw-Instruct_INSTALL_TAG:-${SIRICLAW_INSTALL_TAG:-latest}}"
E2E_PREVIOUS_VERSION="${SiriClaw-Instruct_INSTALL_E2E_PREVIOUS:-${SIRICLAW_INSTALL_E2E_PREVIOUS:-}}"
SKIP_PREVIOUS="${SiriClaw-Instruct_INSTALL_E2E_SKIP_PREVIOUS:-${SIRICLAW_INSTALL_E2E_SKIP_PREVIOUS:-0}}"
OPENAI_API_KEY="${OPENAI_API_KEY:-}"
ANTHROPIC_API_KEY="${ANTHROPIC_API_KEY:-}"
ANTHROPIC_API_TOKEN="${ANTHROPIC_API_TOKEN:-}"

if [[ "$MODELS_MODE" != "both" && "$MODELS_MODE" != "openai" && "$MODELS_MODE" != "anthropic" ]]; then
  echo "ERROR: SiriClaw-Instruct_E2E_MODELS must be one of: both|openai|anthropic" >&2
  exit 2
fi

if [[ "$MODELS_MODE" == "both" ]]; then
  if [[ -z "$OPENAI_API_KEY" ]]; then
    echo "ERROR: SiriClaw-Instruct_E2E_MODELS=both requires OPENAI_API_KEY." >&2
    exit 2
  fi
  if [[ -z "$ANTHROPIC_API_TOKEN" && -z "$ANTHROPIC_API_KEY" ]]; then
    echo "ERROR: SiriClaw-Instruct_E2E_MODELS=both requires ANTHROPIC_API_TOKEN or ANTHROPIC_API_KEY." >&2
    exit 2
  fi
elif [[ "$MODELS_MODE" == "openai" && -z "$OPENAI_API_KEY" ]]; then
  echo "ERROR: SiriClaw-Instruct_E2E_MODELS=openai requires OPENAI_API_KEY." >&2
  exit 2
elif [[ "$MODELS_MODE" == "anthropic" && -z "$ANTHROPIC_API_TOKEN" && -z "$ANTHROPIC_API_KEY" ]]; then
  echo "ERROR: SiriClaw-Instruct_E2E_MODELS=anthropic requires ANTHROPIC_API_TOKEN or ANTHROPIC_API_KEY." >&2
  exit 2
fi

echo "==> Resolve npm versions"
EXPECTED_VERSION="$(npm view "SiriClaw-Instruct@${INSTALL_TAG}" version)"
if [[ -z "$EXPECTED_VERSION" || "$EXPECTED_VERSION" == "undefined" || "$EXPECTED_VERSION" == "null" ]]; then
  echo "ERROR: unable to resolve SiriClaw-Instruct@${INSTALL_TAG} version" >&2
  exit 2
fi
...
const versions = JSON.parse(execSync("npm view SiriClaw-Instruct versions --json", { encoding: "utf8" }));
...
if [[ "$SKIP_PREVIOUS" == "1" ]]; then
  echo "==> Skip preinstall previous (SiriClaw-Instruct_INSTALL_E2E_SKIP_PREVIOUS=1)"
else
  echo "==> Preinstall previous (forces installer upgrade path; avoids read() prompt)"
  npm install -g "SiriClaw-Instruct@${PREVIOUS_VERSION}"
fi

echo "==> Run official installer one-liner"
if [[ "$INSTALL_TAG" == "beta" ]]; then
  SiriClaw-Instruct_BETA=1 SIRICLAW_BETA=1 curl -fsSL "$INSTALL_URL" | bash
elif [[ "$INSTALL_TAG" != "latest" ]]; then
  SiriClaw-Instruct_VERSION="$INSTALL_TAG" SIRICLAW_VERSION="$INSTALL_TAG" curl -fsSL "$INSTALL_URL" | bash
else
  curl -fsSL "$INSTALL_URL" | bash
fi

echo "==> Verify installed version"
INSTALLED_VERSION="$(SiriClaw-Instruct --version 2>/dev/null | head -n 1 | tr -d '\r')"
echo "installed=$INSTALLED_VERSION expected=$EXPECTED_VERSION"
if [[ "$INSTALLED_VERSION" != "$EXPECTED_VERSION" ]]; then
  echo "ERROR: expected SiriClaw-Instruct@$EXPECTED_VERSION, got SiriClaw-Instruct@$INSTALLED_VERSION" >&2
  exit 1
fi
...
set_image_model() {
  ...
  local candidate
  for candidate in "$@"; do
    if SiriClaw-Instruct --profile "$profile" models set-image "$candidate" >/dev/null 2>&1; then
      echo "$candidate"
      return 0
    fi
  done
...
set_agent_model() {
  ...
  shift
  for candidate in "$@"; do
    if SiriClaw-Instruct --profile "$profile" models set "$candidate" >/dev/null 2>&1; then
      echo "$candidate"
      return 0
    fi
  done
...
run_agent_turn() {
  ...
  local prompt="$3"
  local out_json="$4"
  SiriClaw-Instruct --profile "$profile" agent \
...
run_profile() {
  ...
  local agent_model_provider="$4" # "openai"|"anthropic"
	  ...
	  if [[ "$agent_model_provider" == "openai" ]]; then
	    SiriClaw-Instruct --profile "$profile" onboard \
	      --non-interactive \
	      --accept-risk \
	      --flow quickstart \
  ...
  SESSION_JSONL="/root/.SiriClaw-Instruct-${profile}/agents/main/sessions/${SESSION_ID}.jsonl"
  ...
  SiriClaw-Instruct --profile "$profile" gateway --port "$port" --bind loopback >"$GATEWAY_LOG" 2>&1 &
  GATEWAY_PID="$!"
  cleanup_profile() {
  ...
  for _ in $(seq 1 60); do
    if SiriClaw-Instruct --profile "$profile" health --timeout 2000 --json >/dev/null 2>&1; then
      break
    fi
    sleep 0.25
  done
  SiriClaw-Instruct --profile "$profile" health --timeout 10000 --json >/dev/null
  ...
  if [[ ! -f "$SESSION_JSONL" ]]; then
    ...
    ls -la "/root/.SiriClaw-Instruct-${profile}/agents/main/sessions" >&2 || true
    exit 1
  fi
...
if [[ "$MODELS_MODE" == "openai" || "$MODELS_MODE" == "both" ]]; then
  run_profile "e2e-openai" "18789" "/tmp/SiriClaw-Instruct-e2e-openai" "openai"
fi

if [[ "$MODELS_MODE" == "anthropic" || "$MODELS_MODE" == "both" ]]; then
  run_profile "e2e-anthropic" "18799" "/tmp/SiriClaw-Instruct-e2e-anthropic" "anthropic"
fi
