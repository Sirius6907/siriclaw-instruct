#!/usr/bin/env bash
# Rootless SiriClaw-Instruct in Podman: run after one-time setup.
#
# One-time setup (from repo root): ./setup-podman.sh
# Then:
#   ./scripts/run-SiriClaw-Instruct-podman.sh launch           # Start gateway
#   ./scripts/run-SiriClaw-Instruct-podman.sh launch setup      # Onboarding wizard
#
# As the SiriClaw-Instruct user (no repo needed):
...
CONFIG_JSON="$CONFIG_DIR/SiriClaw-Instruct.json"
