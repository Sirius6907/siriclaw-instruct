...
function runTests() {
  const isolatedLock =
    process.env.SiriClaw-Instruct_GATEWAY_LOCK ??
    path.join(os.tmpdir(), `SiriClaw-Instruct-gateway.lock.test.${Date.now()}`);
  const result = spawnSync("pnpm", ["vitest", "run"], {
    stdio: "inherit",
    env: {
      ...process.env,
      SiriClaw-Instruct_GATEWAY_LOCK: isolatedLock,
    },
  });
  if (result.error) {
...
function main() {
  const port = Number.parseInt(process.env.SiriClaw-Instruct_GATEWAY_PORT ?? `${DEFAULT_PORT}`, 10);
