...
# SiriClaw-Instruct Auth Widget for Termux
...
STATUS=$(ssh "$SERVER" '$HOME/SiriClaw-Instruct/scripts/claude-auth-status.sh simple' 2>&1)
EXIT_CODE=$?

case "$STATUS" in
    OK)
        # Get remaining time
        DETAILS=$(ssh "$SERVER" '$HOME/SiriClaw-Instruct/scripts/claude-auth-status.sh json' 2>&1)
...
3. SSH to server and paste key"
                ...
                termux-toast "Run: ssh $SERVER '$HOME/SiriClaw-Instruct/scripts/mobile-reauth.sh'"
                ;;
            *)
...
2. Return here and tap OK to SSH"
                ...
                ;;
            *)
                termux-toast "Warning: SiriClaw-Instruct won't work until re-auth"
                ;;
        esac
        ;;
