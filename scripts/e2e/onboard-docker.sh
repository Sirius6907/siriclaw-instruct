...
IMAGE_NAME="SiriClaw-Instruct-onboard-e2e"
...
docker run --rm -t "$IMAGE_NAME" bash -lc '
  ...
  cat > /tmp/SiriClaw-Instruct-bin/trash <<'"'"'TRASH'"'"'
#!/usr/bin/env bash
set -euo pipefail
...
TRASH
  chmod +x /tmp/SiriClaw-Instruct-bin/trash
  ...
  run_wizard_cmd() {
    ...
    mkfifo "$input_fifo"
    local log_path="/tmp/SiriClaw-Instruct-onboard-${case_name}.log"
