...
IMAGE_NAME="SiriClaw-Instruct-doctor-install-switch-e2e"
...
docker run --rm -e COREPACK_ENABLE_DOWNLOAD_PROMPT=0 "$IMAGE_NAME" bash -lc '
  ...
  mkdir -p /tmp/SiriClaw-Instruct-bin

  cat > /tmp/SiriClaw-Instruct-bin/systemctl <<"SYSTEMCTL"
#!/usr/bin/env bash
set -euo pipefail
...
SYSTEMCTL
  chmod +x /tmp/SiriClaw-Instruct-bin/systemctl

  cat > /tmp/SiriClaw-Instruct-bin/loginctl <<"LOGINCTL"
#!/usr/bin/env bash
set -euo pipefail
...
exit 0
LOGINCTL
  chmod +x /tmp/SiriClaw-Instruct-bin/loginctl
  ...
  npm install -g --prefix /tmp/npm-prefix "/app/$pkg_tgz"
	  ...
	  npm_bin="/tmp/npm-prefix/bin/SiriClaw-Instruct"
	  npm_root="/tmp/npm-prefix/lib/node_modules/SiriClaw-Instruct"
	  ...
	  git_cli="/app/SiriClaw-Instruct.mjs"
  ...
  run_flow() {
    ...
    echo "== Flow: $name =="
    home_dir=$(mktemp -d "/tmp/SiriClaw-Instruct-switch-${name}.XXXXXX")
    export HOME="$home_dir"
    export USER="testuser"

    eval "$install_cmd"

    unit_path="$HOME/.config/systemd/user/SiriClaw-Instruct-gateway.service"
    if [ ! -f "$unit_path" ]; then
