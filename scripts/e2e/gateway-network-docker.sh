...
IMAGE_NAME="SiriClaw-Instruct-gateway-network-e2e"

PORT="18789"
TOKEN="e2e-$(date +%s)-$$"
NET_NAME="SiriClaw-Instruct-net-e2e-$$"
GW_NAME="SiriClaw-Instruct-gateway-e2e-$$"
...
docker run -d \
  --name "$GW_NAME" \
  --network "$NET_NAME" \
  -e "SiriClaw-Instruct_GATEWAY_TOKEN=$TOKEN" \
  -e "SiriClaw-Instruct_SKIP_CHANNELS=1" \
  -e "SiriClaw-Instruct_SKIP_GMAIL_WATCHER=1" \
  -e "SiriClaw-Instruct_SKIP_CRON=1" \
  -e "SiriClaw-Instruct_SKIP_CANVAS_HOST=1" \
  "$IMAGE_NAME" \
