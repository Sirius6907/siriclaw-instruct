...
docker run --rm -t \
  ...
  -e SIRICLAW_LIVE_GATEWAY_MODELS="${SIRICLAW_LIVE_GATEWAY_MODELS:-${SIRICLAW_LIVE_GATEWAY_MODELS:-modern}}" \
  -e SIRICLAW_LIVE_GATEWAY_PROVIDERS="${SIRICLAW_LIVE_GATEWAY_PROVIDERS:-${SIRICLAW_LIVE_GATEWAY_PROVIDERS:-}}" \
  -e SIRICLAW_LIVE_GATEWAY_MAX_MODELS="${SIRICLAW_LIVE_GATEWAY_MAX_MODELS:-${SIRICLAW_LIVE_GATEWAY_MAX_MODELS:-24}}" \
  -e SIRICLAW_LIVE_GATEWAY_MODEL_TIMEOUT_MS="${SIRICLAW_LIVE_GATEWAY_MODEL_TIMEOUT_MS:-${SIRICLAW_LIVE_GATEWAY_MODEL_TIMEOUT_MS:-}}" \
  -v "$ROOT_DIR":/src:ro \
