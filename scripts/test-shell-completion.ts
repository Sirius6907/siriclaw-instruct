 ...
 * `SiriClaw-Instruct update`. Use it to verify the completion installation flow
...
const CLI_NAME = "SiriClaw-Instruct";
...
\`SiriClaw-Instruct update\`, \`SiriClaw-Instruct doctor\`, and \`SiriClaw-Instruct onboard\`.
...
async function main() {
  ...
  if (status.profileInstalled && status.cacheExists && !options.force) {
    ...
    console.log(
      theme.muted("  1. Remove the '# SiriClaw-Instruct Completion' block from your shell profile"),
    );
  ...
  if (isCancel(shouldInstall) || !shouldInstall) {
    console.log(theme.muted(`Skipped. Run \`SiriClaw-Instruct completion --install\` later to enable.`));
    return;
  }
