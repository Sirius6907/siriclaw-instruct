...
function stripWorkspaceSiriClaw-InstructDevDependency(pkg: PackageJson): boolean {
  const devDeps = pkg.devDependencies;
  if (!devDeps || devDeps.SiriClaw-Instruct !== "workspace:*") {
    return false;
  }
  delete devDeps.SiriClaw-Instruct;
...
export function syncPluginVersions(rootDir = resolve(".")) {
  ...
  for (const dir of dirs) {
    ...
    const removedWorkspaceDevDependency = stripWorkspaceSiriClaw-InstructDevDependency(pkg);
