...
# Build and bundle SiriClaw-Instruct into a minimal .app we can open.
# Outputs to dist/SiriClaw-Instruct.app

ROOT_DIR="$(cd "$(dirname "$0")/.." && pwd)"
APP_ROOT="$ROOT_DIR/dist/SiriClaw-Instruct.app"
BUILD_ROOT="$ROOT_DIR/apps/macos/.build"
PRODUCT="SiriClaw-Instruct"
BUNDLE_ID="${BUNDLE_ID:-ai.SiriClaw-Instruct.mac.debug}"
...
SPARKLE_FEED_URL="${SPARKLE_FEED_URL:-https://raw.githubusercontent.com/SiriClaw-Instruct/SiriClaw-Instruct/main/appcast.xml}"
AUTO_CHECKS=true
...
INFO_PLIST_SRC="$ROOT_DIR/apps/macos/Sources/SiriClaw-Instruct/Resources/Info.plist"
...
/usr/libexec/PlistBuddy -c "Set :SiriClaw-InstructBuildTimestamp ${BUILD_TS}" "$APP_ROOT/Contents/Info.plist" || true
/usr/libexec/PlistBuddy -c "Set :SiriClaw-InstructGitCommit ${GIT_COMMIT}" "$APP_ROOT/Contents/Info.plist" || true
...
echo "🚚 Copying binary"
cp "$BIN_PRIMARY" "$APP_ROOT/Contents/MacOS/SiriClaw-Instruct"
if [[ "${#BUILD_ARCHS[@]}" -gt 1 ]]; then
  BIN_INPUTS=()
  for arch in "${BUILD_ARCHS[@]}"; do
    BIN_INPUTS+=("$(bin_for_arch "$arch")")
  done
  /usr/bin/lipo -create "${BIN_INPUTS[@]}" -output "$APP_ROOT/Contents/MacOS/SiriClaw-Instruct"
fi
chmod +x "$APP_ROOT/Contents/MacOS/SiriClaw-Instruct"
# SwiftPM outputs ad-hoc signed binaries; strip the signature before install_name_tool to avoid warnings.
/usr/bin/codesign --remove-signature "$APP_ROOT/Contents/MacOS/SiriClaw-Instruct" 2>/dev/null || true
...
cp "$ROOT_DIR/apps/macos/Sources/SiriClaw-Instruct/Resources/SiriClaw-Instruct.icns" "$APP_ROOT/Contents/Resources/SiriClaw-Instruct.icns"

echo "📦 Copying device model resources"
rm -rf "$APP_ROOT/Contents/Resources/DeviceModels"
cp -R "$ROOT_DIR/apps/macos/Sources/SiriClaw-Instruct/Resources/DeviceModels" "$APP_ROOT/Contents/Resources/DeviceModels"
...
echo "⏹  Stopping any running SiriClaw-Instruct"
killall -q SiriClaw-Instruct 2>/dev/null || true
