...
# Configuration
SUBSYSTEM="ai.SiriClaw-Instruct"
DEFAULT_LEVEL="info"
...
show_usage() {
    cat << EOF
clawlog - SiriClaw-Instruct Logging Utility

USAGE:
    clawlog [OPTIONS]

DESCRIPTION:
    View SiriClaw-Instruct logs with full details (bypasses Apple's privacy redaction).
    Requires sudo access configured for /usr/bin/log command.

LOG FLOW ARCHITECTURE:
    SiriClaw-Instruct logs flow through the macOS unified log (subsystem: ai.SiriClaw-Instruct).
