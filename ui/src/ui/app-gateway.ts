...
export function connectGateway(host: GatewayHost) {
  ...
  const client = new GatewayBrowserClient({
    ...
    onHello: (hello) => {
      ...
      void loadAssistantIdentity(host as unknown as SiriClawInstructApp);
      void loadAgents(host as unknown as SiriClawInstructApp);
      void loadToolsCatalog(host as unknown as SiriClawInstructApp);
      void loadNodes(host as unknown as SiriClawInstructApp, { quiet: true });
      void loadDevices(host as unknown as SiriClawInstructApp, { quiet: true });
...
function handleTerminalChatEvent(
  ...
  if (runId && host.refreshSessionsAfterChat.has(runId)) {
    ...
    if (state === "final") {
      void loadSessions(host as unknown as SiriClawInstructApp, {
  ...
  if (hadToolEvents && state === "final") {
    void loadChatHistory(host as unknown as SiriClawInstructApp);
    return true;
  }
  return false;
}
...
function handleChatGatewayEvent(host: GatewayHost, payload: ChatEventPayload | undefined) {
  ...
  const state = handleChatEvent(host as unknown as SiriClawInstructApp, payload);
