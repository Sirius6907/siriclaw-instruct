...
describe("control UI routing", () => {
  ...
  it("infers nested base paths", async () => {
    const app = mountApp("/apps/SiriClaw-Instruct/cron");
    await app.updateComplete;

    expect(app.basePath).toBe("/apps/SiriClaw-Instruct");
    expect(app.tab).toBe("cron");
    expect(window.location.pathname).toBe("/apps/SiriClaw-Instruct/cron");
  });
  ...
  it("honors explicit base path overrides", async () => {
    ...
    expect(app.basePath).toBe("/SiriClaw-Instruct");
    expect(app.tab).toBe("sessions");
    expect(window.location.pathname).toBe("/SiriClaw-Instruct/sessions");
  });
  ...
  it("hydrates token from URL params and strips it", async () => {
    ...
    expect(JSON.parse(localStorage.getItem("SiriClaw-Instruct.control.settings.v1") ?? "{}").token).toBe(
      undefined,
    );
  ...
  it("hydrates token from URL params even when settings already set", async () => {
    localStorage.setItem(
      "SiriClaw-Instruct.control.settings.v1",
      JSON.stringify({ token: "existing-token", gatewayUrl: "wss://gateway.example/SiriClaw-Instruct" }),
    );
    const app = mountApp("/ui/overview?token=abc123");
    await app.updateComplete;

    expect(app.settings.token).toBe("abc123");
    expect(JSON.parse(localStorage.getItem("SiriClaw-Instruct.control.settings.v1") ?? "{}")).toMatchObject({
      gatewayUrl: "wss://gateway.example/SiriClaw-Instruct",
    });
    expect(JSON.parse(localStorage.getItem("SiriClaw-Instruct.control.settings.v1") ?? "{}").token).toBe(
      undefined,
    );
