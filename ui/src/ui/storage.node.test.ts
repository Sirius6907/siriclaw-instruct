...
describe("loadSettings default gateway URL derivation", () => {
  ...
  it("uses configured base path and normalizes trailing slash", async () => {
    ...
    setControlUiBasePath(" /SiriClaw-Instruct/ ");

    const { loadSettings } = await import("./storage.ts");
    expect(loadSettings().gatewayUrl).toBe(expectedGatewayUrl("/SiriClaw-Instruct"));
  });
  ...
  it("infers base path from nested pathname when configured base path is not set", async () => {
    setTestLocation({
      ...
      pathname: "/apps/SiriClaw-Instruct/chat",
    });

    const { loadSettings } = await import("./storage.ts");
    expect(loadSettings().gatewayUrl).toBe(expectedGatewayUrl("/apps/SiriClaw-Instruct"));
  });
  ...
  it("ignores and scrubs legacy persisted tokens", async () => {
    ...
    localStorage.setItem(
      ...
      JSON.stringify({
        gatewayUrl: "wss://gateway.example:8443/SiriClaw-Instruct",
    ...
    expect(JSON.parse(localStorage.getItem("SiriClaw-Instruct.control.settings.v1") ?? "{}")).toEqual({
      gatewayUrl: "wss://gateway.example:8443/SiriClaw-Instruct",
      sessionKey: "agent",
