...
describe("resolveSafeExternalUrl", () => {
  const baseHref = "https://SiriClaw-Instruct.ai/chat";
  ...
  it("allows relative URLs resolved against the current origin", () => {
    expect(resolveSafeExternalUrl("/assets/pic.png", baseHref)).toBe(
      "https://SiriClaw-Instruct.ai/assets/pic.png",
    );
  });

  it("allows blob URLs", () => {
    expect(resolveSafeExternalUrl("blob:https://SiriClaw-Instruct.ai/abc-123", baseHref)).toBe(
      "blob:https://SiriClaw-Instruct.ai/abc-123",
    );
  });
...
describe("openExternalUrlSafe", () => {
  it("nulls opener when window.open returns a proxy-like object", () => {
    ...
    vi.stubGlobal("window", {
      location: { href: "https://SiriClaw-Instruct.ai/chat" },
      open: openMock,
