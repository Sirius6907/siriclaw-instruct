...
export function resolveBundledHooksDir(): string | undefined {
  const override = process.env.SiriClaw-Instruct_BUNDLED_HOOKS_DIR?.trim();
  if (override) {
    return override;
  }
  ...
  // This path works when installed via npm: node_modules/SiriClaw-Instruct/dist/hooks/bundled-dir.js
  try {
