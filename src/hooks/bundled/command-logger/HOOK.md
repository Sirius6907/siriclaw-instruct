...
homepage: https://docs.SiriClaw-Instruct.ai/automation/hooks#command-logger
metadata:
  {
    "SiriClaw-Instruct":
      {
...
2. **Appends to log file** - Writes a JSON line to `~/.SiriClaw-Instruct/logs/commands.log`
...
`~/.SiriClaw-Instruct/logs/commands.log`
...
1. **Manual rotation**:
   ...
   ```bash
   mv ~/.SiriClaw-Instruct/logs/commands.log ~/.SiriClaw-Instruct/logs/commands.log.old
   ```

2. **Use logrotate** (Linux):
   Create `/etc/logrotate.d/SiriClaw-Instruct`:
   ```
   /home/username/.SiriClaw-Instruct/logs/commands.log {
       weekly
       rotate 4
       compress
       missingok
