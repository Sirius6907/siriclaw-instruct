...
import { type SiriClaw-InstructConfig, DEFAULT_GATEWAY_PORT } from "../config/config.js";
import {
  buildDefaultHookUrl,
  buildTopicPath,
  parseTopicPath,
...
describe("gmail hook config", () => {
  function resolveWithGmailOverrides(
    overrides: Partial<NonNullable<SiriClaw-InstructConfig["hooks"]>["gmail"]>,
  ) {
  ...
  it("resolves runtime config with defaults", () => {
    ...
    expect(result.ok).toBe(true);
    if (result.ok) {
      expect(result.value.account).toBe("SiriClaw-Instruct@gmail.com");
