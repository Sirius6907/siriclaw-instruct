...
import { type SiriClawInstructConfig, DEFAULT_GATEWAY_PORT } from "../config/config.js";
import {
  buildDefaultHookUrl,
  buildTopicPath,
  parseTopicPath,
...
const baseConfig = {
  hooks: {
    token: "hook-token",
    gmail: {
      account: "SiriClawInstruct@gmail.com",
...
describe("gmail hook config", () => {
  function resolveWithGmailOverrides(
    overrides: Partial<NonNullable<SiriClawInstructConfig["hooks"]>["gmail"]>,
  ) {
  ...
  it("resolves runtime config with defaults", () => {
    ...
    expect(result.ok).toBe(true);
    if (result.ok) {
      expect(result.value.account).toBe("SiriClawInstruct@gmail.com");
