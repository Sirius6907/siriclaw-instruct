...
function setupHardlinkHookWorkspace(hookName: string): {
  ...
  const root = fs.mkdtempSync(path.join(os.tmpdir(), "SiriClaw-Instruct-hooks-workspace-hardlink-"));
...
describe("hooks workspace", () => {
  it("ignores package.json hook paths that traverse outside package directory", () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "SiriClaw-Instruct-hooks-workspace-"));
    const hooksRoot = path.join(root, "hooks");
    fs.mkdirSync(hooksRoot, { recursive: true });
    ...
    const entries = loadHookEntriesFromDir({ dir: hooksRoot, source: "SiriClaw-Instruct-workspace" });
    expect(entries.some((e) => e.hook.name === "outside")).toBe(false);
  });

  it("accepts package.json hook paths within package directory", () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "SiriClaw-Instruct-hooks-workspace-ok-"));
    const hooksRoot = path.join(root, "hooks");
    fs.mkdirSync(hooksRoot, { recursive: true });
  ...
  it("ignores package.json hook paths that escape via symlink", () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "SiriClaw-Instruct-hooks-workspace-link-"));
    const hooksRoot = path.join(root, "hooks");
    fs.mkdirSync(hooksRoot, { recursive: true });
