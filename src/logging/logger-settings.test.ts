...
let originalSiriClawInstructLogLevel: string | undefined;
...
beforeEach(() => {
  originalTestFileLog = process.env.SiriClawInstruct_TEST_FILE_LOG;
  originalSiriClawInstructLogLevel = process.env.SiriClawInstruct_LOG_LEVEL;
  delete process.env.SiriClawInstruct_TEST_FILE_LOG;
  delete process.env.SiriClawInstruct_LOG_LEVEL;
...
afterEach(() => {
  if (originalTestFileLog === undefined) {
    ...
    process.env.SiriClawInstruct_TEST_FILE_LOG = originalTestFileLog;
  }
  if (originalSiriClawInstructLogLevel === undefined) {
    delete process.env.SiriClawInstruct_LOG_LEVEL;
  } else {
    process.env.SiriClawInstruct_LOG_LEVEL = originalSiriClawInstructLogLevel;
  }
  logging.resetLogger();
  logging.setLoggerOverride(null);
  vi.restoreAllMocks();
