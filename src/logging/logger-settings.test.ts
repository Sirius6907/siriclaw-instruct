...
let originalSiriClaw-InstructLogLevel: string | undefined;
...
beforeEach(() => {
  originalTestFileLog = process.env.SiriClaw-Instruct_TEST_FILE_LOG;
  originalSiriClaw-InstructLogLevel = process.env.SiriClaw-Instruct_LOG_LEVEL;
  delete process.env.SiriClaw-Instruct_TEST_FILE_LOG;
  delete process.env.SiriClaw-Instruct_LOG_LEVEL;
...
afterEach(() => {
  if (originalTestFileLog === undefined) {
    ...
    process.env.SiriClaw-Instruct_TEST_FILE_LOG = originalTestFileLog;
  }
  if (originalSiriClaw-InstructLogLevel === undefined) {
    delete process.env.SiriClaw-Instruct_LOG_LEVEL;
  } else {
    process.env.SiriClaw-Instruct_LOG_LEVEL = originalSiriClaw-InstructLogLevel;
  }
  logging.resetLogger();
  logging.setLoggerOverride(null);
  vi.restoreAllMocks();
