...
describe("runCapability video provider wiring", () => {
  it("merges video baseUrl and headers with entry precedence", async () => {
    ...
    await withVideoFixture("SiriClaw-Instruct-video-merge", async ({ ctx, media, cache }) => {
      const cfg = {
        models: {
          providers: {
            moonshot: {
  ...
  it("auto-selects moonshot for video when google is unavailable", async () => {
    await withEnvAsync(
      ...
      async () => {
        await withVideoFixture("SiriClaw-Instruct-video-auto-moonshot", async ({ ctx, media, cache }) => {
          const cfg = {
            models: {
              providers: {
                moonshot: {
