...
export function registerWebhooksCli(program: Command) {
  const webhooks = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/webhooks", "docs.SiriClawInstruct.ai/cli/webhooks")}\n`,
    );
  ...
  gmail
    .command("setup")
    .description("Configure Gmail watch + Pub/Sub + SiriClawInstruct hooks")
    ...
    .option("--hook-url <url>", "SiriClawInstruct hook URL")
    .option("--hook-token <token>", "SiriClawInstruct hook token")
