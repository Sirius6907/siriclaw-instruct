...
export function registerWebhooksCli(program: Command) {
  const webhooks = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/webhooks", "docs.SiriClaw-Instruct.ai/cli/webhooks")}\n`,
    );
  ...
  gmail
    .command("setup")
    .description("Configure Gmail watch + Pub/Sub + SiriClaw-Instruct hooks")
    ...
    .option("--hook-url <url>", "SiriClaw-Instruct hook URL")
    .option("--hook-token <token>", "SiriClaw-Instruct hook token")
