...
async function runGatewayCommand(opts: GatewayRunOpts) {
  const isDevProfile = process.env.SiriClawInstruct_PROFILE?.trim().toLowerCase() === "dev";
  ...
  if (opts.claudeCliLogs) {
    ...
    process.env.SiriClawInstruct_CLAUDE_CLI_LOG_OUTPUT = "1";
  }
  ...
  if (opts.rawStream) {
    process.env.SiriClawInstruct_RAW_STREAM = "1";
  }
  const rawStreamPath = toOptionString(opts.rawStreamPath);
  if (rawStreamPath) {
    process.env.SiriClawInstruct_RAW_STREAM_PATH = rawStreamPath;
  }
  ...
  if (process.env.SiriClawInstruct_SERVICE_MARKER?.trim()) {
  ...
  if (!opts.allowUnconfigured && mode !== "local") {
    if (!configExists) {
      defaultRuntime.error(
        `Missing config. Run \`${formatCliCommand("SiriClawInstruct setup")}\` or set gateway.mode=local (or pass --allow-unconfigured).`,
      );
    } else {
      defaultRuntime.error(
  ...
  if (resolvedAuthMode === "password" && !passwordConfigured) {
    defaultRuntime.error(
      [
        ...
        "Set gateway.auth.password (or SiriClawInstruct_GATEWAY_PASSWORD), or pass --password.",
        ...authHints,
      ]
        .filter(Boolean)
        .join("\n"),
  ...
  if (
    ...
    defaultRuntime.error(
      [
        ...
        "Set gateway.auth.token/password (or SiriClawInstruct_GATEWAY_TOKEN/SiriClawInstruct_GATEWAY_PASSWORD) or pass --token/--password.",
        ...authHints,
      ]
        .filter(Boolean)
        .join("\n"),
  ...
  try {
    ...
    if (
      ...
      defaultRuntime.error(
        `Gateway failed to start: ${errMessage}\nIf the gateway is supervised, stop it with: ${formatCliCommand("SiriClawInstruct gateway stop")}`,
      );
      try {
...
export function addGatewayRunCommand(cmd: Command): Command {
  return cmd
    ...
    .option(
      "--token <token>",
      "Shared token required in connect.params.auth.token (default: SiriClawInstruct_GATEWAY_TOKEN env if set)",
    )
