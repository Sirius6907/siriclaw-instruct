...
describe("resolveCliChannelOptions", () => {
  afterEach(() => {
    delete process.env.SiriClawInstruct_EAGER_CHANNEL_OPTIONS;
    vi.resetModules();
    vi.clearAllMocks();
  });
  ...
  it("respects eager mode and includes loaded plugin ids", async () => {
    process.env.SiriClawInstruct_EAGER_CHANNEL_OPTIONS = "1";
  ...
  it("keeps dynamic catalog resolution when external catalog env is set", async () => {
    process.env.SiriClawInstruct_PLUGIN_CATALOG_PATHS = "/tmp/plugins-catalog.json";
    ...
    expect(listCatalogMock).toHaveBeenCalledOnce();
    delete process.env.SiriClawInstruct_PLUGIN_CATALOG_PATHS;
  });
});
