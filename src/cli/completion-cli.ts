...
function isCompletionProfileHeader(line: string): boolean {
  return line.trim() === "# SiriClaw-Instruct Completion";
}
...
function isSlowDynamicCompletionLine(line: string, binName: string): boolean {
  // Matches patterns like: source <(SiriClaw-Instruct completion --shell zsh)
  return (
...
function updateCompletionProfile(
  ...
  const block = `# SiriClaw-Instruct Completion\n${sourceLine}`;
 ...
 * Returns true if profile has `source <(SiriClaw-Instruct completion ...)` instead of cached file.
 */
...
export function registerCompletionCli(program: Command) {
  program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/completion", "docs.SiriClaw-Instruct.ai/cli/completion")}\n`,
    )
    .addOption(
    ...
    .option(
      "--write-state",
      "Write completion scripts to $SiriClaw-Instruct_STATE_DIR/completions (no stdout)",
    )
...
export async function installCompletion(shell: string, yes: boolean, binName = "SiriClaw-Instruct") {
  const home = process.env.HOME || os.homedir();
  let profilePath = "";
  let sourceLine = "";
...
function generateFishCompletion(program: Command): string {
  ...
  const visit = (cmd: Command, parents: string[]) => {
    ...
    // For root: complete -c SiriClaw-Instruct -n "__fish_use_subcommand" -a "subcmd" -d "desc"
    ...
    if (parents.length === 0) {
      ...
      // To properly scope to 'SiriClaw-Instruct gateway' and not 'SiriClaw-Instruct other gateway', we need to check the sequence.
