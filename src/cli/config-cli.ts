...
function formatDoctorHint(message: string): string {
  return `Run \`${formatCliCommand("SiriClaw-Instruct doctor")}\` ${message}`;
}
...
export async function runConfigValidate(opts: { json?: boolean; runtime?: RuntimeEnv } = {}) {
  const runtime = opts.runtime ?? defaultRuntime;
  let outputPath = CONFIG_PATH ?? "SiriClaw-Instruct.json";
...
export function registerConfigCli(program: Command) {
  const cmd = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/config", "docs.SiriClaw-Instruct.ai/cli/config")}\n`,
    )
    .option(
      "--section <section>",
