...
describe("resolveInitialTuiAgentId", () => {
  const cfg: SiriClawInstructConfig = {
    agents: {
      list: [
        { id: "main", workspace: "/tmp/SiriClawInstruct" },
        { id: "ops", workspace: "/tmp/SiriClawInstruct/projects/ops" },
      ],
    },
  };
  ...
  it("infers agent from cwd when session is not agent-prefixed", () => {
    expect(
      resolveInitialTuiAgentId({
        ...
        initialSessionInput: "",
        cwd: "/tmp/SiriClawInstruct/projects/ops/src",
      }),
    ).toBe("ops");
  });
...
describe("resolveGatewayDisconnectState", () => {
  it("returns pairing recovery guidance when disconnect reason requires pairing", () => {
    ...
    expect(state.activityStatus).toBe("pairing required: run SiriClawInstruct devices list");
    expect(state.pairingHint).toContain("SiriClawInstruct devices list");
  });
