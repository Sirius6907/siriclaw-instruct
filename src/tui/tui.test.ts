...
describe("resolveInitialTuiAgentId", () => {
  const cfg: SiriClaw-InstructConfig = {
    agents: {
      list: [
        { id: "main", workspace: "/tmp/SiriClaw-Instruct" },
        { id: "ops", workspace: "/tmp/SiriClaw-Instruct/projects/ops" },
      ],
    },
  };
  ...
  it("infers agent from cwd when session is not agent-prefixed", () => {
    expect(
      resolveInitialTuiAgentId({
        ...
        initialSessionInput: "",
        cwd: "/tmp/SiriClaw-Instruct/projects/ops/src",
      }),
    ).toBe("ops");
  });
...
describe("resolveGatewayDisconnectState", () => {
  it("returns pairing recovery guidance when disconnect reason requires pairing", () => {
    ...
    expect(state.activityStatus).toBe("pairing required: run SiriClaw-Instruct devices list");
    expect(state.pairingHint).toContain("SiriClaw-Instruct devices list");
  });
