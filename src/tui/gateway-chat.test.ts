...
async function withModeExecProviderFixture(
  ...
  const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), `SiriClawInstruct-tui-mode-${label}-`));
...
describe("resolveGatewayConnection", () => {
  ...
  beforeEach(() => {
    envSnapshot = captureEnv(["SiriClawInstruct_GATEWAY_TOKEN", "SiriClawInstruct_GATEWAY_PASSWORD"]);
  ...
  it.each([
    ...
    const result = await withEnvAsync({ SiriClawInstruct_GATEWAY_TOKEN: "env-token" }, async () => {
  ...
  it("falls back to SiriClawInstruct_GATEWAY_TOKEN when config token is missing", async () => {
    loadConfig.mockReturnValue({ gateway: { mode: "local" } });
  ...
  it("prefers SiriClawInstruct_GATEWAY_PASSWORD over remote password fallback", async () => {
    loadConfig.mockReturnValue({
      gateway: {
        mode: "remote",
    ...
    const gatewayPasswordEnv = "SiriClawInstruct_GATEWAY_PASSWORD"; // pragma: allowlist secret
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "SiriClawInstruct-tui-file-secret-"));
