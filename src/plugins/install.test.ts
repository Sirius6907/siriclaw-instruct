...
const DYNAMIC_ARCHIVE_TEMPLATE_PRESETS = [
  {
    ...
    packageJson: {
      name: "@evil/..",
      version: "0.0.1",
      SiriClaw-Instruct: { extensions: ["./dist/index.js"] },
...
function ensureSuiteTempRoot() {
  ...
  suiteTempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "SiriClaw-Instruct-plugin-install-"));
  return suiteTempRoot;
}
...
beforeAll(async () => {
  ...
  fs.writeFileSync(
    ...
    JSON.stringify({
      name: "@SiriClaw-Instruct/test-plugin",
      version: "0.0.1",
      SiriClaw-Instruct: { extensions: ["./dist/index.js"] },
  ...
  fs.writeFileSync(
    ...
    JSON.stringify({
      name: "@SiriClaw-Instruct/cognee-SiriClaw-Instruct",
      version: "0.0.1",
      SiriClaw-Instruct: { extensions: ["./dist/index.js"] },
    }),
    "utf-8",
  ...
  fs.writeFileSync(
    path.join(manifestInstallTemplateDir, "SiriClaw-Instruct.plugin.json"),
    JSON.stringify({
...
describe("installPluginFromArchive", () => {
  it("installs into ~/.SiriClaw-Instruct/extensions and uses unscoped id", async () => {
  ...
  it("rejects packages without SiriClaw-Instruct.extensions", async () => {
    const result = await installArchivePackageAndReturnResult({
      packageJson: { name: "@SiriClaw-Instruct/nope", version: "0.0.1" },
      outName: "bad.tgz",
    });
    expect(result.ok).toBe(false);
    if (result.ok) {
    ...
    expect(result.error).toContain("SiriClaw-Instruct.extensions");
    expect(result.code).toBe(PLUGIN_INSTALL_ERROR_CODE.MISSING_SiriClaw-Instruct_EXTENSIONS);
  });

  it("rejects legacy plugin package shape when SiriClaw-Instruct.extensions is missing", async () => {
    const { pluginDir, extensionsDir } = setupPluginInstallDirs();
    fs.writeFileSync(
      path.join(pluginDir, "package.json"),
      JSON.stringify({
        name: "@SiriClaw-Instruct/legacy-entry-fallback",
        version: "0.0.1",
      }),
      "utf-8",
    );
    ...
    expect(result.ok).toBe(false);
    if (!result.ok) {
      expect(result.error).toContain("package.json missing SiriClaw-Instruct.extensions");
      expect(result.error).toContain("update the plugin package");
      expect(result.code).toBe(PLUGIN_INSTALL_ERROR_CODE.MISSING_SiriClaw-Instruct_EXTENSIONS);
      return;
    }
    expect.unreachable("expected install to fail without SiriClaw-Instruct.extensions");
  });
...
describe("installPluginFromDir", () => {
  ...
  it("strips workspace devDependencies before npm install", async () => {
    ...
    expect(manifest.devDependencies?.SiriClaw-Instruct).toBeUndefined();
    expect(manifest.devDependencies?.vitest).toBe("^3.0.0");
  });

  it("uses SiriClaw-Instruct.plugin.json id as install key when it differs from package name", async () => {
    ...
    expect(
      infoMessages.some((msg) =>
        msg.includes(
          'Plugin manifest id "memory-cognee" differs from npm package name "cognee-SiriClaw-Instruct"',
        ),
      ),
    ).toBe(true);
  });
...
describe("installPluginFromNpmSpec", () => {
  it("uses --ignore-scripts for npm pack and cleans up temp dir", async () => {
      ...
      if (argv[0] === "npm" && argv[1] === "pack") {
        ...
        return {
          ...
          stdout: JSON.stringify([
            {
              id: "@SiriClaw-Instruct/voice-call@0.0.1",
              name: "@SiriClaw-Instruct/voice-call",
              version: "0.0.1",
              filename: packedName,
    ...
    const result = await installPluginFromNpmSpec({
      spec: "@SiriClaw-Instruct/voice-call@0.0.1",
      extensionsDir,
      logger: { info: () => {}, warn: () => {} },
    });
    expect(result.ok).toBe(true);
    ...
    expect(result.npmResolution?.resolvedSpec).toBe("@SiriClaw-Instruct/voice-call@0.0.1");
    expect(result.npmResolution?.integrity).toBe("sha512-plugin-test");

    expectSingleNpmPackIgnoreScriptsCall({
      calls: run.mock.calls,
      expectedSpec: "@SiriClaw-Instruct/voice-call@0.0.1",
    });
  ...
  it("classifies npm package-not-found errors with a stable error code", async () => {
    ...
    const result = await installPluginFromNpmSpec({
      spec: "@SiriClaw-Instruct/not-found",
      logger: { info: () => {}, warn: () => {} },
    });
    expect(result.ok).toBe(false);
    if (!result.ok) {
  ...
  it("rejects bare npm specs that resolve to prerelease versions", async () => {
    const run = vi.mocked(runCommandWithTimeout);
    mockNpmPackMetadataResult(run, {
      id: "@SiriClaw-Instruct/voice-call@0.0.2-beta.1",
      name: "@SiriClaw-Instruct/voice-call",
    ...
    if (!result.ok) {
      ...
      expect(result.error).toContain('"@SiriClaw-Instruct/voice-call@beta"');
    }
  });
  ...
  it("allows explicit prerelease npm tags", async () => {
    ...
    const result = await installPluginFromNpmSpec({
      spec: "@SiriClaw-Instruct/voice-call@beta",
      extensionsDir,
      logger: { info: () => {}, warn: () => {} },
    });
    expect(result.ok).toBe(true);
    ...
    expect(result.npmResolution?.resolvedSpec).toBe("@SiriClaw-Instruct/voice-call@0.0.2-beta.1");
    expectSingleNpmPackIgnoreScriptsCall({
      calls: run.mock.calls,
      expectedSpec: "@SiriClaw-Instruct/voice-call@beta",
    });
    expect(packTmpDir).not.toBe("");
  });
});
