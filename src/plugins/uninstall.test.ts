...
function createSinglePluginWithEmptySlotsConfig(): SiriClaw-InstructConfig {
  return {
    plugins: {
...
function createSingleNpmInstallConfig(installPath: string): SiriClaw-InstructConfig {
  return {
    plugins: {
      entries: createSinglePluginEntries(),
      installs: {
...
describe("resolveUninstallDirectoryTarget", () => {
  ...
  it("falls back to default path when configured installPath is untrusted", () => {
    const extensionsDir = path.join(os.tmpdir(), "SiriClaw-Instruct-uninstall-safe");
    const target = resolveUninstallDirectoryTarget({
      ...
      installRecord: {
        source: "npm",
        spec: "my-plugin@1.0.0",
        installPath: "/tmp/not-SiriClaw-Instruct-extensions/my-plugin",
      },
      extensionsDir,
    });
