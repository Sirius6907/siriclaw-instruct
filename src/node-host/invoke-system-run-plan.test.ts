...
describe("hardenApprovedExecutionPaths", () => {
  ...
  for (const testCase of cases) {
    it.runIf(process.platform !== "win32")(testCase.name, () => {
      const tmp = fs.mkdtempSync(path.join(os.tmpdir(), "SiriClawInstruct-approval-hardening-"));
  ...
  it("captures mutable shell script operands in approval plans", () => {
    const tmp = fs.mkdtempSync(path.join(os.tmpdir(), "SiriClawInstruct-approval-script-plan-"));
