...
describe("handleControlUiHttpRequest", () => {
  async function withControlUiRoot<T>(params: {
    ...
    const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "SiriClaw-Instruct-ui-"));
    try {
  ...
  async function withBasePathRootFixture<T>(params: {
    ...
    const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "SiriClaw-Instruct-ui-root-"));
    try {
  ...
  it("serves bootstrap config JSON under basePath", async () => {
    await withControlUiRoot({
      fn: async (tmp) => {
        const { res, end } = makeMockHttpResponse();
        const handled = handleControlUiHttpRequest(
          { url: `/SiriClaw-Instruct${CONTROL_UI_BOOTSTRAP_CONFIG_PATH}`, method: "GET" } as IncomingMessage,
          res,
          {
        ...
        expect(handled).toBe(true);
        const parsed = parseBootstrapPayload(end);
        expect(parsed.basePath).toBe("/SiriClaw-Instruct");
        expect(parsed.assistantName).toBe("Ops");
        expect(parsed.assistantAvatar).toBe("/SiriClaw-Instruct/avatar/main");
        expect(parsed.assistantAgentId).toBe("main");
      },
    });
  });
  ...
  it("serves local avatar bytes through hardened avatar handler", async () => {
    const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "SiriClaw-Instruct-avatar-http-"));
    try {
  ...
  it("rejects avatar symlink paths from resolver", async () => {
    const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "SiriClaw-Instruct-avatar-http-link-"));
    const outside = await fs.mkdtemp(path.join(os.tmpdir(), "SiriClaw-Instruct-avatar-http-outside-"));
    try {
  ...
  it("rejects symlinked assets that resolve outside control-ui root", async () => {
    await withControlUiRoot({
      fn: async (tmp) => {
        ...
        const outsideDir = await fs.mkdtemp(path.join(os.tmpdir(), "SiriClaw-Instruct-ui-outside-"));
        try {
  ...
  it("rejects symlinked SPA fallback index.html outside control-ui root", async () => {
    await withControlUiRoot({
      fn: async (tmp) => {
        const outsideDir = await fs.mkdtemp(path.join(os.tmpdir(), "SiriClaw-Instruct-ui-index-outside-"));
        try {
    ...
    await withControlUiRoot({
      fn: async (tmp) => {
        ...
        const handled = handleControlUiHttpRequest(
          ...
          res,
          { basePath: "/SiriClaw-Instruct", root: { kind: "resolved", path: tmp } },
        );
  ...
  it("falls through POST requests under configured basePath (plugin webhook passthrough)", async () => {
    await withControlUiRoot({
      fn: async (tmp) => {
        for (const route of ["/SiriClaw-Instruct", "/SiriClaw-Instruct/", "/SiriClaw-Instruct/some-page"]) {
          const { handled, end } = runControlUiRequest({
            url: route,
            method: "POST",
            rootPath: tmp,
  ...
  it("rejects absolute-path escape attempts under basePath routes", async () => {
    await withBasePathRootFixture({
      ...
      fn: async ({ root, sibling }) => {
        ...
        const { res, end, handled } = runControlUiRequest({
          url: `/SiriClaw-Instruct/${absolutePathUrl}`,
          method: "GET",
          rootPath: root,
          basePath: "/SiriClaw-Instruct",
        });
  ...
  it("rejects symlink escape attempts under basePath routes", async () => {
    await withBasePathRootFixture({
      ...
      fn: async ({ root, sibling }) => {
        ...
        const { res, end, handled } = runControlUiRequest({
          url: "/SiriClaw-Instruct/assets/leak.txt",
          method: "GET",
          rootPath: root,
          basePath: "/SiriClaw-Instruct",
        });
