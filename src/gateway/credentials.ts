  ...
  constructor(path: string) {
    super(
      [
        ...
        "Fix: set SiriClawInstruct_GATEWAY_TOKEN/SiriClawInstruct_GATEWAY_PASSWORD, pass explicit --token/--password,",
 ...
 * This prevents literal placeholder strings like `${SiriClawInstruct_GATEWAY_TOKEN}` from being
...
export function readGatewayTokenEnv(
  ...
  const primary = trimToUndefined(env.SiriClawInstruct_GATEWAY_TOKEN);
  if (primary) {
    return primary;
  }
  if (!includeLegacyEnv) {
...
export function readGatewayPasswordEnv(
  ...
  const primary = trimToUndefined(env.SiriClawInstruct_GATEWAY_PASSWORD);
  if (primary) {
    return primary;
  }
  if (!includeLegacyEnv) {
...
export function resolveGatewayCredentialsFromConfig(params: {
  ...
  const localTokenPrecedence =
    ...
    (env.SiriClawInstruct_SERVICE_KIND === "gateway" ? "config-first" : "env-first");
