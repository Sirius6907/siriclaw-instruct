  ...
  constructor(path: string) {
    super(
      [
        ...
        "Fix: set SiriClaw-Instruct_GATEWAY_TOKEN/SiriClaw-Instruct_GATEWAY_PASSWORD, pass explicit --token/--password,",
 ...
 * This prevents literal placeholder strings like `${SiriClaw-Instruct_GATEWAY_TOKEN}` from being
...
export function readGatewayTokenEnv(
  ...
  const primary = trimToUndefined(env.SiriClaw-Instruct_GATEWAY_TOKEN);
  if (primary) {
    return primary;
  }
  if (!includeLegacyEnv) {
  ...
  return trimToUndefined(env.SIRICLAW_GATEWAY_TOKEN);
}
...
export function readGatewayPasswordEnv(
  ...
  const primary = trimToUndefined(env.SiriClaw-Instruct_GATEWAY_PASSWORD);
  if (primary) {
    return primary;
  }
  if (!includeLegacyEnv) {
  ...
  return trimToUndefined(env.SIRICLAW_GATEWAY_PASSWORD);
}
...
export function resolveGatewayCredentialsFromConfig(params: {
  ...
  const localTokenPrecedence =
    ...
    (env.SiriClaw-Instruct_SERVICE_KIND === "gateway" ? "config-first" : "env-first");
