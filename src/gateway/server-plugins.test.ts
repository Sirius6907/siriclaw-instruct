...
const loadSiriClaw-InstructPlugins = vi.hoisted(() => vi.fn());
...
function createSubagentRuntime(serverPlugins: ServerPluginsModule): PluginRuntime["subagent"] {
  ...
  loadSiriClaw-InstructPlugins.mockReturnValue(createRegistry([]));
  ...
  const call = loadSiriClaw-InstructPlugins.mock.calls.at(-1)?.[0] as
...
beforeEach(() => {
  loadSiriClaw-InstructPlugins.mockReset();
...
describe("loadGatewayPlugins", () => {
  test("logs plugin errors with details", async () => {
    ...
    loadSiriClaw-InstructPlugins.mockReturnValue(createRegistry(diagnostics));
  ...
  test("provides subagent runtime with sessions.get method aliases", async () => {
    ...
    const call = loadSiriClaw-InstructPlugins.mock.calls.at(-1)?.[0];
