...
const loadSiriClawInstructPlugins = vi.hoisted(() => vi.fn());
...
function createSubagentRuntime(serverPlugins: ServerPluginsModule): PluginRuntime["subagent"] {
  ...
  loadSiriClawInstructPlugins.mockReturnValue(createRegistry([]));
  ...
  const call = loadSiriClawInstructPlugins.mock.calls.at(-1)?.[0] as
...
beforeEach(() => {
  loadSiriClawInstructPlugins.mockReset();
...
describe("loadGatewayPlugins", () => {
  test("logs plugin errors with details", async () => {
    ...
    loadSiriClawInstructPlugins.mockReturnValue(createRegistry(diagnostics));
  ...
  test("provides subagent runtime with sessions.get method aliases", async () => {
    ...
    const call = loadSiriClawInstructPlugins.mock.calls.at(-1)?.[0];
