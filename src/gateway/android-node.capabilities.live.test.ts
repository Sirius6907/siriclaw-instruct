...
const LIVE_ANDROID_NODE = isTruthyEnvValue(process.env.SiriClaw-Instruct_LIVE_ANDROID_NODE);
...
function resolveGatewayConnection() {
  ...
  const urlOverride = readString(process.env.SiriClaw-Instruct_ANDROID_GATEWAY_URL);
  const details = buildGatewayConnectionDetails({
    config: cfg,
    ...(urlOverride ? { url: urlOverride } : {}),
  });
  const tokenOverride = readString(process.env.SiriClaw-Instruct_ANDROID_GATEWAY_TOKEN);
  const passwordOverride = readString(process.env.SiriClaw-Instruct_ANDROID_GATEWAY_PASSWORD);
...
function selectTargetNode(nodes: NodeListNode[]): NodeListNode {
  const nodeIdOverride = readString(process.env.SiriClaw-Instruct_ANDROID_NODE_ID);
  if (nodeIdOverride) {
    const match = nodes.find((node) => node.nodeId === nodeIdOverride);
    if (!match) {
      throw new Error(`SiriClaw-Instruct_ANDROID_NODE_ID not found in node.list: ${nodeIdOverride}`);
    }
    return match;
  }

  const nodeNameOverride = readString(process.env.SiriClaw-Instruct_ANDROID_NODE_NAME)?.toLowerCase();
  if (nodeNameOverride) {
    const match = nodes.find(
      (node) => readString(node.displayName)?.toLowerCase() === nodeNameOverride,
    );
    if (!match) {
      throw new Error(`SiriClaw-Instruct_ANDROID_NODE_NAME not found in node.list: ${nodeNameOverride}`);
    }
    return match;
  }
    ...
    if (!target.connected || !target.paired) {
      ...
      throw new Error(
        [
          ...
          pendingHint,
          "precondition: open app, keep foreground, ensure pairing approved (`SiriClaw-Instruct nodes pending` / `SiriClaw-Instruct nodes approve <requestId>`)",
        ].join("\n"),
      );
    }
