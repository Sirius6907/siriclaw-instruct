...
async function writeGatewayConfig(config: Record<string, unknown>) {
  ...
  if (!configPath) {
    throw new Error("SiriClaw-Instruct_CONFIG_PATH is required for gateway config tests");
  }
  ...
  it("handles OpenResponses request parsing and validation", async () => {
    ...
    try {
      ...
      const resMissingAuth = await fetch(`http://127.0.0.1:${port}/v1/responses`, {
        ...
        body: JSON.stringify({ model: "SiriClaw-Instruct", input: "hi" }),
      });
      ...
      const resHeader = await postResponses(
        port,
        { model: "SiriClaw-Instruct", input: "hi" },
      ...
      mockAgentOnce([{ text: "hello" }]);
      const resModel = await postResponses(port, { model: "SiriClaw-Instruct:beta", input: "hi" });
      ...
      const resChannelHeader = await postResponses(
        port,
        { model: "SiriClaw-Instruct", input: "hi" },
        { "x-SiriClaw-Instruct-message-channel": "custom-client-channel" },
      );
