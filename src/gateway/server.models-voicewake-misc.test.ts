...
describe("gateway server models + voicewake", () => {
  ...
  const withModelsConfig = async <T>(config: unknown, run: () => Promise<T>): Promise<T> => {
    ...
    if (!configPath) {
      throw new Error("Missing SiriClawInstruct_CONFIG_PATH");
    }
  ...
  const withTempHome = async <T>(fn: (homeDir: string) => Promise<T>): Promise<T> => {
    const tempHome = await createTempHomeEnv("SiriClawInstruct-home-");
    try {
  ...
  test(
    ...
    async () => {
      await withTempHome(async (homeDir) => {
        ...
        expect(initial.payload?.triggers).toEqual(["SiriClawInstruct", "claude", "computer"]);
        ...
        const onDisk = JSON.parse(
          await fs.readFile(path.join(homeDir, ".SiriClawInstruct", "settings", "voicewake.json"), "utf8"),
  ...
  test("pushes voicewake.changed to nodes on connect and on updates", async () => {
    await withTempHome(async () => {
      ...
      const setRes = await rpcReq<{ triggers: string[] }>(ws, "voicewake.set", {
        triggers: ["SiriClawInstruct", "computer"],
      });
      expect(setRes.ok).toBe(true);
...
describe("gateway server misc", () => {
  test("hello-ok advertises the gateway port for canvas host", async () => {
    await withEnvAsync({ SiriClawInstruct_GATEWAY_TOKEN: "secret" }, async () => {
      testTailnetIPv4.value = "100.64.0.1";
      testState.gatewayBind = "lan";
      const canvasPort = await getFreePort();
      testState.canvasHostPort = canvasPort;
      await withEnvAsync({ SiriClawInstruct_CANVAS_HOST_PORT: String(canvasPort) }, async () => {
