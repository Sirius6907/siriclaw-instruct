...
export function resolveHooksConfig(cfg: SiriClaw-InstructConfig): HooksConfigResolved | null {
...
function resolveKnownAgentIds(cfg: SiriClaw-InstructConfig, defaultAgentId: string): Set<string> {
...
export function extractHookToken(req: IncomingMessage): string | undefined {
  ...
  const headerToken =
    typeof req.headers["x-SiriClaw-Instruct-token"] === "string"
      ? req.headers["x-SiriClaw-Instruct-token"].trim()
      : "";
  if (headerToken) {
    return headerToken;
  }
