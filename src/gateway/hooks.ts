...
export function resolveHooksConfig(cfg: SiriClawInstructConfig): HooksConfigResolved | null {
...
function resolveKnownAgentIds(cfg: SiriClawInstructConfig, defaultAgentId: string): Set<string> {
...
export function extractHookToken(req: IncomingMessage): string | undefined {
  ...
  const headerToken =
    typeof req.headers["x-SiriClawInstruct-token"] === "string"
      ? req.headers["x-SiriClawInstruct-token"].trim()
      : "";
  if (headerToken) {
    return headerToken;
  }
