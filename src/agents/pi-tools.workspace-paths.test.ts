...
describe("workspace path resolution", () => {
  it("resolves relative read/write/edit paths against workspaceDir even after cwd changes", async () => {
    await withTempDir("SiriClaw-Instruct-ws-", async (workspaceDir) => {
      await withTempDir("SiriClaw-Instruct-cwd-", async (otherDir) => {
        const cwdSpy = vi.spyOn(process, "cwd").mockReturnValue(otherDir);
        try {
          const tools = createSiriClaw-InstructCodingTools({ workspaceDir });
  ...
  it("lets exec workdir override the workspace default", async () => {
    await withTempDir("SiriClaw-Instruct-ws-", async (workspaceDir) => {
      await withTempDir("SiriClaw-Instruct-override-", async (overrideDir) => {
        const execTool = createExecTool(workspaceDir);
  ...
  it("rejects @-prefixed absolute paths outside workspace when workspaceOnly is enabled", async () => {
    await withTempDir("SiriClaw-Instruct-ws-", async (workspaceDir) => {
      const cfg: SiriClaw-InstructConfig = { tools: { fs: { workspaceOnly: true } } };
      const tools = createSiriClaw-InstructCodingTools({ workspaceDir, config: cfg });
      const { readTool } = expectReadWriteEditTools(tools);

      const outsideAbsolute = path.resolve(path.parse(workspaceDir).root, "outside-SiriClaw-Instruct.txt");
      await expect(
...
describe("sandboxed workspace paths", () => {
  it("uses sandbox workspace for relative read/write/edit", async () => {
    await withTempDir("SiriClaw-Instruct-sandbox-", async (sandboxDir) => {
      await withTempDir("SiriClaw-Instruct-workspace-", async (workspaceDir) => {
        ...
        const tools = createSiriClaw-InstructCodingTools({ workspaceDir, sandbox });
        const { readTool, writeTool, editTool } = expectReadWriteEditTools(tools);
