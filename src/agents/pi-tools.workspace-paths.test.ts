...
describe("workspace path resolution", () => {
  it("resolves relative read/write/edit paths against workspaceDir even after cwd changes", async () => {
    await withTempDir("SiriClawInstruct-ws-", async (workspaceDir) => {
      await withTempDir("SiriClawInstruct-cwd-", async (otherDir) => {
        const cwdSpy = vi.spyOn(process, "cwd").mockReturnValue(otherDir);
        try {
          const tools = createSiriClawInstructCodingTools({ workspaceDir });
  ...
  it("lets exec workdir override the workspace default", async () => {
    await withTempDir("SiriClawInstruct-ws-", async (workspaceDir) => {
      await withTempDir("SiriClawInstruct-override-", async (overrideDir) => {
        const execTool = createExecTool(workspaceDir);
  ...
  it("rejects @-prefixed absolute paths outside workspace when workspaceOnly is enabled", async () => {
    await withTempDir("SiriClawInstruct-ws-", async (workspaceDir) => {
      const cfg: SiriClawInstructConfig = { tools: { fs: { workspaceOnly: true } } };
      const tools = createSiriClawInstructCodingTools({ workspaceDir, config: cfg });
      const { readTool } = expectReadWriteEditTools(tools);

      const outsideAbsolute = path.resolve(path.parse(workspaceDir).root, "outside-SiriClawInstruct.txt");
      await expect(
...
describe("sandboxed workspace paths", () => {
  it("uses sandbox workspace for relative read/write/edit", async () => {
    await withTempDir("SiriClawInstruct-sandbox-", async (sandboxDir) => {
      await withTempDir("SiriClawInstruct-workspace-", async (workspaceDir) => {
        ...
        const tools = createSiriClawInstructCodingTools({ workspaceDir, sandbox });
        const { readTool, writeTool, editTool } = expectReadWriteEditTools(tools);
