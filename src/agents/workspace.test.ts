...
describe("resolveDefaultAgentWorkspaceDir", () => {
  it("uses SiriClawInstruct_HOME for default workspace resolution", () => {
    const dir = resolveDefaultAgentWorkspaceDir({
      SiriClawInstruct_HOME: "/srv/SiriClawInstruct-home",
      HOME: "/home/other",
    } as NodeJS.ProcessEnv);

    expect(dir).toBe(path.join(path.resolve("/srv/SiriClawInstruct-home"), ".SiriClawInstruct", "workspace"));
  });
});

const WORKSPACE_STATE_PATH_SEGMENTS = [".SiriClawInstruct", "workspace-state.json"] as const;
...
describe("ensureAgentWorkspace", () => {
  it("creates BOOTSTRAP.md and records a seeded marker for brand new workspaces", async () => {
    const tempDir = await makeTempWorkspace("SiriClawInstruct-workspace-");
...
describe("loadWorkspaceBootstrapFiles", () => {
  ...
  it("treats hardlinked bootstrap aliases as missing", async () => {
    ...
    const rootDir = await fs.mkdtemp(path.join(os.tmpdir(), "SiriClawInstruct-workspace-hardlink-"));
    try {
