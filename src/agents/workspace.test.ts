...
describe("resolveDefaultAgentWorkspaceDir", () => {
  it("uses SiriClaw-Instruct_HOME for default workspace resolution", () => {
    const dir = resolveDefaultAgentWorkspaceDir({
      SiriClaw-Instruct_HOME: "/srv/SiriClaw-Instruct-home",
      HOME: "/home/other",
    } as NodeJS.ProcessEnv);

    expect(dir).toBe(path.join(path.resolve("/srv/SiriClaw-Instruct-home"), ".SiriClaw-Instruct", "workspace"));
  });
});

const WORKSPACE_STATE_PATH_SEGMENTS = [".SiriClaw-Instruct", "workspace-state.json"] as const;
...
describe("ensureAgentWorkspace", () => {
  it("creates BOOTSTRAP.md and records a seeded marker for brand new workspaces", async () => {
    const tempDir = await makeTempWorkspace("SiriClaw-Instruct-workspace-");
...
describe("loadWorkspaceBootstrapFiles", () => {
  ...
  it("treats hardlinked bootstrap aliases as missing", async () => {
    ...
    const rootDir = await fs.mkdtemp(path.join(os.tmpdir(), "SiriClaw-Instruct-workspace-hardlink-"));
    try {
