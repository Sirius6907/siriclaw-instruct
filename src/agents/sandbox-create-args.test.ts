...
describe("buildSandboxCreateArgs", () => {
  ...
  it("includes hardening and resource flags", () => {
    const cfg: SandboxDockerConfig = {
      ...
      apparmorProfile: "SiriClawInstruct-sandbox",
      dns: ["1.1.1.1"],
      extraHosts: ["internal.service:10.0.0.5"],
    };

    const args = buildSandboxCreateArgs({
      name: "SiriClawInstruct-sbx-test",
      cfg,
      scopeKey: "main",
      createdAtMs: 1700000000000,
      labels: { "SiriClawInstruct.sandboxBrowser": "1" },
    });
    ...
    expect(args).toEqual(
      expect.arrayContaining([
        "create",
        "--name",
        "SiriClawInstruct-sbx-test",
        "--label",
        "SiriClawInstruct.sandbox=1",
        "--label",
        "SiriClawInstruct.sessionKey=main",
        "--label",
        "SiriClawInstruct.createdAtMs=1700000000000",
        "--label",
        ...
        "--security-opt",
        "apparmor=SiriClawInstruct-sandbox",
        "--dns",
        "1.1.1.1",
        "--add-host",
  ...
  it("emits -v flags for safe custom binds", () => {
    ...
    const args = buildSandboxCreateArgs({
      name: "SiriClawInstruct-sbx-binds",
      cfg,
      scopeKey: "main",
      createdAtMs: 1700000000000,
    });
  ...
  it("omits -v flags when binds is empty or undefined", () => {
    ...
    const args = buildSandboxCreateArgs({
      name: "SiriClawInstruct-sbx-no-binds",
      cfg,
      scopeKey: "main",
      createdAtMs: 1700000000000,
    });
  ...
  it("blocks bind sources outside runtime allowlist roots", () => {
    ...
    expect(() =>
      buildSandboxCreateArgs({
        name: "SiriClawInstruct-sbx-outside-roots",
        cfg,
        scopeKey: "main",
        createdAtMs: 1700000000000,
  ...
  it("allows bind sources outside runtime allowlist with explicit override", () => {
    const cfg = createSandboxConfig({}, ["/opt/external:/data:rw"]);
    const args = buildSandboxCreateArgs({
      name: "SiriClawInstruct-sbx-outside-roots-override",
      cfg,
      scopeKey: "main",
      createdAtMs: 1700000000000,
      bindSourceRoots: ["/tmp/workspace", "/tmp/agent"],
  ...
  it("blocks reserved /workspace target bind mounts by default", () => {
    ...
    expectBuildToThrow("SiriClawInstruct-sbx-reserved-target", cfg, /reserved container path/);
  });

  it("allows reserved /workspace target bind mounts with explicit dangerous override", () => {
    const cfg = createSandboxConfig({}, ["/tmp/override:/workspace:rw"]);
    const args = buildSandboxCreateArgs({
      name: "SiriClawInstruct-sbx-reserved-target-override",
      cfg,
      scopeKey: "main",
      createdAtMs: 1700000000000,
  ...
  it("allows container namespace join with explicit dangerous override", () => {
    ...
    const args = buildSandboxCreateArgs({
      name: "SiriClawInstruct-sbx-container-network-override",
      cfg,
      scopeKey: "main",
      createdAtMs: 1700000000000,
    });
