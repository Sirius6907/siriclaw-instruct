...
function makeFallbacksOnlyCfg(): SiriClaw-InstructConfig {
  return {
    agents: {
      defaults: {
        model: {
...
function makeProviderFallbackCfg(provider: string): SiriClaw-InstructConfig {
  return makeCfg({
    agents: {
      defaults: {
        model: {
...
async function withTempAuthStore<T>(
  ...
  const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "SiriClaw-Instruct-auth-"));
...
// https://github.com/SiriClaw-Instruct/SiriClaw-Instruct/issues/23440
const INSUFFICIENT_QUOTA_PAYLOAD =
  '{"type":"error","error":{"type":"insufficient_quota","message":"Your account has insufficient quota balance to run this request."}}';
// Internal SiriClaw-Instruct compatibility marker, not a provider API contract.
...
describe("runWithModelFallback", () => {
  ...
  describe("fallback behavior with provider cooldowns", () => {
    async function makeAuthStoreWithCooldown(
      ...
      const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "SiriClaw-Instruct-test-"));
      const now = Date.now();
      const store: AuthProfileStore = {
        version: AUTH_STORE_VERSION,
        profiles: {
