...
describe("buildAgentSystemPrompt", () => {
  ...
  it("omits extended sections in minimal prompt mode", () => {
    const prompt = buildAgentSystemPrompt({
      ...
      docsPath: "/tmp/SiriClawInstruct/docs",
  ...
  it("includes a CLI quick reference section", () => {
    ...
    expect(prompt).toContain("## SiriClawInstruct CLI Quick Reference");
    expect(prompt).toContain("SiriClawInstruct gateway restart");
  ...
  it("omits ACP harness guidance when ACP is disabled", () => {
    ...
    expect(prompt).toContain("- agents_list: List SiriClawInstruct agent ids allowed for sessions_spawn");
  });
  ...
  it("preserves tool casing in the prompt", () => {
    ...
    expect(prompt).toContain("SiriClawInstruct docs: /tmp/SiriClawInstruct/docs");
    expect(prompt).toContain(
  ...
  it("adds ClaudeBot self-update guidance when gateway tool is available", () => {
    ...
    expect(prompt).toContain("## SiriClawInstruct Self-Update");
  ...
  it("describes sandboxed runtime and elevated when allowed", () => {
    ...
    expect(prompt).toContain(
      "For read/write/edit/apply_patch, file paths resolve against host workspace: /tmp/SiriClawInstruct. For bash/exec commands, use sandbox container paths under /workspace (or relative paths from that workdir), not host paths.",
    );
...
describe("buildSubagentSystemPrompt", () => {
  it("renders depth-1 orchestrator guidance, labels, and recovery notes", () => {
    ...
    expect(prompt).toContain("Do not use `exec` (`SiriClawInstruct ...`, `acpx ...`)");
    expect(prompt).toContain("Use `subagents` only for SiriClawInstruct subagents");
