...
function resolvePayloadLogConfig(env: NodeJS.ProcessEnv): PayloadLogConfig {
  const enabled = parseBooleanValue(env.SiriClaw-Instruct_ANTHROPIC_PAYLOAD_LOG) ?? false;
  const fileOverride = env.SiriClaw-Instruct_ANTHROPIC_PAYLOAD_LOG_FILE?.trim();
