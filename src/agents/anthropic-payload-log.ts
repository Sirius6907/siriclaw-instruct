...
function resolvePayloadLogConfig(env: NodeJS.ProcessEnv): PayloadLogConfig {
  const enabled = parseBooleanValue(env.SiriClawInstruct_ANTHROPIC_PAYLOAD_LOG) ?? false;
  const fileOverride = env.SiriClawInstruct_ANTHROPIC_PAYLOAD_LOG_FILE?.trim();
