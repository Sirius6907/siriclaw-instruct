...
import {
  ...
  createSiriClawInstructReadTool,
...
function applyModelProviderToolPolicy(
  ...
  // xAI/Grok providers expose a native web_search tool; sending SiriClawInstruct's
...
function resolveExecConfig(params: { cfg?: SiriClawInstructConfig; agentId?: string }) {
...
export function createSiriClawInstructCodingTools(options?: {
  agentId?: string;
  ...
  const base = (codingTools as unknown as AnyAgentTool[]).flatMap((tool) => {
    if (tool.name === readTool.name) {
      ...
      const wrapped = createSiriClawInstructReadTool(freshReadTool, {
        modelContextWindowTokens: options?.modelContextWindowTokens,
        imageSanitization,
      });
