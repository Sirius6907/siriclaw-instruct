...
const bundledPluginsDirSnapshot = captureEnv(["SiriClaw-Instruct_BUNDLED_PLUGINS_DIR"]);

beforeAll(() => {
  process.env.SiriClaw-Instruct_BUNDLED_PLUGINS_DIR = path.join(
    os.tmpdir(),
...
const { createSiriClaw-InstructCodingTools } = await import("./pi-tools.js");
...
describe("createSiriClaw-InstructCodingTools safeBins", () => {
  it("threads tools.exec.safeBins into exec allowlist checks", async () => {
    await withSafeBinsExecTool(
      {
        tmpPrefix: "SiriClaw-Instruct-safe-bins-",
        safeBins: ["echo"],
        safeBinProfiles: {
  ...
  it("rejects unprofiled custom safe-bin entries", async () => {
    await withSafeBinsExecTool(
      {
        tmpPrefix: "SiriClaw-Instruct-safe-bins-unprofiled-",
        safeBins: ["echo"],
      },
      async ({ tmpDir, execTool }) => {
        await expect(
  ...
  it("does not allow env var expansion to smuggle file args via safeBins", async () => {
    await withSafeBinsExecTool(
      {
        tmpPrefix: "SiriClaw-Instruct-safe-bins-expand-",
        safeBins: ["head", "wc"],
  ...
  it("blocks sort output/compress bypass attempts in safeBins mode", async () => {
    await withSafeBinsExecTool(
      {
        tmpPrefix: "SiriClaw-Instruct-safe-bins-sort-",
        safeBins: ["sort"],
  ...
  it("blocks shell redirection metacharacters in safeBins mode", async () => {
    await withSafeBinsExecTool(
      {
        tmpPrefix: "SiriClaw-Instruct-safe-bins-redirect-",
        safeBins: ["head"],
  ...
  it("blocks grep recursive flags from reading cwd via safeBins", async () => {
    await withSafeBinsExecTool(
      {
        tmpPrefix: "SiriClaw-Instruct-safe-bins-grep-",
        safeBins: ["grep"],
