...
describe("subagent registry persistence", () => {
  const envSnapshot = captureEnv(["SiriClawInstruct_STATE_DIR"]);
  ...
  const writePersistedRegistry = async (
    ...
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "SiriClawInstruct-subagent-"));
  ...
  it("uses isolated temp state when SiriClawInstruct_STATE_DIR is unset in tests", async () => {
    ...
    expect(registryPath).toContain(path.join(os.tmpdir(), "SiriClawInstruct-test-state"));
  });
});
