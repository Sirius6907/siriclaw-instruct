...
describe("subagent registry persistence", () => {
  const envSnapshot = captureEnv(["SiriClaw-Instruct_STATE_DIR"]);
  ...
  const writePersistedRegistry = async (
    ...
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "SiriClaw-Instruct-subagent-"));
  ...
  it("uses isolated temp state when SiriClaw-Instruct_STATE_DIR is unset in tests", async () => {
    ...
    expect(registryPath).toContain(path.join(os.tmpdir(), "SiriClaw-Instruct-test-state"));
  });
});
