...
describe("exec PATH login shell merge", () => {
  ...
  it("sets SiriClawInstruct_SHELL for host=gateway commands", async () => {
    if (isWin) {
      return;
    }

    const tool = createExecTool({ host: "gateway", security: "full", ask: "off" });
    const result = await tool.execute("call-SiriClawInstruct-shell", {
      command: 'printf "%s" "${SiriClawInstruct_SHELL:-}"',
    });
  ...
  it("does not apply login-shell PATH when probe rejects unregistered absolute SHELL", async () => {
    ...
    process.env.PATH = "/usr/bin";
    const shellDir = fs.mkdtempSync(path.join(os.tmpdir(), "SiriClawInstruct-shell-env-"));
...
describe("exec host env validation", () => {
  ...
  it("strips dangerous inherited env vars from host execution", async () => {
    ...
    process.env.SSLKEYLOGFILE = "/tmp/SiriClawInstruct-ssl-keys.log";
    try {
      const { createExecTool } = await import("./bash-tools.exec.js");
      const tool = createExecTool({ host: "gateway", security: "full", ask: "off" });
      ...
      expect(output).not.toContain("/tmp/SiriClawInstruct-ssl-keys.log");
    } finally {
