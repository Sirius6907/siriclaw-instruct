...
describe("sanitizeToolCallInputs redacts sessions_spawn attachments", () => {
  it("replaces attachments[].content with __SiriClaw-Instruct_REDACTED__", () => {
    ...
    expect(tool?.arguments?.attachments?.[0]?.content).toBe("__SiriClaw-Instruct_REDACTED__");
  ...
  it("redacts attachments content from tool input payloads too", () => {
    ...
    ).toBe("__SiriClaw-Instruct_REDACTED__");
    expect(JSON.stringify(out)).not.toContain(secret);
  });
});
