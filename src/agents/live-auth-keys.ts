...
const GOOGLE_LIVE_SINGLE_KEY = "SiriClaw-Instruct_LIVE_GEMINI_KEY";
...
const PROVIDER_API_KEY_CONFIG: Record<string, Omit<ProviderApiKeyConfig, "fallbackVars">> = {
  anthropic: {
    liveSingle: "SiriClaw-Instruct_LIVE_ANTHROPIC_KEY",
    listVar: "SiriClaw-Instruct_LIVE_ANTHROPIC_KEYS",
  ...
  openai: {
    liveSingle: "SiriClaw-Instruct_LIVE_OPENAI_KEY",
...
function resolveProviderApiKeyConfig(provider: string): ProviderApiKeyConfig {
  ...
  const liveSingle = custom?.liveSingle ?? `SiriClaw-Instruct_LIVE_${base}_KEY`;
