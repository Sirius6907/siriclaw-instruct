...
const GOOGLE_LIVE_SINGLE_KEY = "SiriClawInstruct_LIVE_GEMINI_KEY";
...
const PROVIDER_API_KEY_CONFIG: Record<string, Omit<ProviderApiKeyConfig, "fallbackVars">> = {
  anthropic: {
    liveSingle: "SiriClawInstruct_LIVE_ANTHROPIC_KEY",
    listVar: "SiriClawInstruct_LIVE_ANTHROPIC_KEYS",
  ...
  openai: {
    liveSingle: "SiriClawInstruct_LIVE_OPENAI_KEY",
...
function resolveProviderApiKeyConfig(provider: string): ProviderApiKeyConfig {
  ...
  const liveSingle = custom?.liveSingle ?? `SiriClawInstruct_LIVE_${base}_KEY`;
