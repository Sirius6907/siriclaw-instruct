...
describe("Agent-specific tool filtering", () => {
  ...
  async function withApplyPatchEscapeCase(
    ...
    const workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "SiriClawInstruct-pi-tools-"));
...
*** End Patch`;
  ...
  function createMainSessionTools(cfg: SiriClawInstructConfig) {
    return createSiriClawInstructCodingTools({
      config: cfg,
      sessionKey: "agent:main:main",
  ...
  function createMainAgentConfig(params: {
    tools: NonNullable<SiriClawInstructConfig["tools"]>;
    agentTools?: NonNullable<NonNullable<SiriClawInstructConfig["agents"]>["list"]>[number]["tools"];
  ...
  it("should allow different tool policies for different agents", () => {
    ...
    const mainTools = createSiriClawInstructCodingTools({
      config: cfg,
      sessionKey: "agent:main:main",
    ...
    const familyTools = createSiriClawInstructCodingTools({
      config: cfg,
  ...
  it("should apply group tool policy overrides (group-specific beats wildcard)", () => {
    ...
    const trustedTools = createSiriClawInstructCodingTools({
      config: cfg,
  ...
  it("should apply per-sender tool policies for group tools", () => {
    ...
    const aliceTools = createSiriClawInstructCodingTools({
      config: cfg,
    ...
    const bobTools = createSiriClawInstructCodingTools({
      config: cfg,
      sessionKey: "agent:main:whatsapp:group:family",
      senderId: "bob",
  ...
  it("should not let default sender policy override group tools", () => {
    ...
    const adminTools = createSiriClawInstructCodingTools({
      config: cfg,
  ...
  it("should apply agent-specific exec host defaults over global defaults", async () => {
    ...
    const helperTools = createSiriClawInstructCodingTools({
      config: cfg,
