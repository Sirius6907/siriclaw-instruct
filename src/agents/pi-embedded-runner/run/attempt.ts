...
import { resolveSiriClawInstructDocsPath } from "../../docs-path.js";
...
import { createSiriClawInstructCodingTools, resolveToolLoopDetectionConfig } from "../../pi-tools.js";
...
export async function runEmbeddedAttempt(
  ...
  try {
    ...
    const agentDir = params.agentDir ?? resolveSiriClawInstructAgentDir();
    ...
    const toolsRaw = params.disableTools
      ? []
      : createSiriClawInstructCodingTools({
    ...
    const docsPath = await resolveSiriClawInstructDocsPath({
      workspaceDir: effectiveWorkspace,
    ...
    try {
      ...
      try {
        ...
        // double-compaction. See: https://github.com/SiriClawInstruct/SiriClawInstruct/issues/9282
        ...
        if (promptError && promptErrorSource === "prompt" && !compactionOccurredThisAttempt) {
          try {
            sessionManager.appendCustomEntry("SiriClawInstruct:prompt-error", {
              timestamp: Date.now(),
              runId: params.runId,
              sessionId: params.sessionId,
              provider: params.provider,
      ...
      // See: https://github.com/SiriClawInstruct/SiriClawInstruct/issues/8643
