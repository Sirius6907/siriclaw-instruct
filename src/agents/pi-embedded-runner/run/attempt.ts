...
import { resolveSiriClaw-InstructDocsPath } from "../../docs-path.js";
...
import { createSiriClaw-InstructCodingTools, resolveToolLoopDetectionConfig } from "../../pi-tools.js";
...
export async function runEmbeddedAttempt(
  ...
  try {
    ...
    const agentDir = params.agentDir ?? resolveSiriClaw-InstructAgentDir();
    ...
    const toolsRaw = params.disableTools
      ? []
      : createSiriClaw-InstructCodingTools({
    ...
    const docsPath = await resolveSiriClaw-InstructDocsPath({
      workspaceDir: effectiveWorkspace,
    ...
    try {
      ...
      try {
        ...
        // double-compaction. See: https://github.com/SiriClaw-Instruct/SiriClaw-Instruct/issues/9282
        ...
        if (promptError && promptErrorSource === "prompt" && !compactionOccurredThisAttempt) {
          try {
            sessionManager.appendCustomEntry("SiriClaw-Instruct:prompt-error", {
              timestamp: Date.now(),
              runId: params.runId,
              sessionId: params.sessionId,
              provider: params.provider,
      ...
      // See: https://github.com/SiriClaw-Instruct/SiriClaw-Instruct/issues/8643
