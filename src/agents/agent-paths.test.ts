...
describe("resolveSiriClaw-InstructAgentDir", () => {
  const withTempStateDir = async (run: (stateDir: string) => void) => {
    const stateDir = await fs.mkdtemp(path.join(os.tmpdir(), "SiriClaw-Instruct-agent-"));
    try {
      run(stateDir);
    } finally {
  ...
  it("defaults to the multi-agent path when no overrides are set", async () => {
    await withTempStateDir((stateDir) => {
      withEnv(
        ...
        () => {
          const resolved = resolveSiriClaw-InstructAgentDir();
  ...
  it("honors SiriClaw-Instruct_AGENT_DIR overrides", async () => {
    await withTempStateDir((stateDir) => {
      const override = path.join(stateDir, "agent");
      withEnv(
        {
          SiriClaw-Instruct_STATE_DIR: undefined,
          SiriClaw-Instruct_AGENT_DIR: override,
          PI_CODING_AGENT_DIR: undefined,
        },
        () => {
          const resolved = resolveSiriClaw-InstructAgentDir();
  ...
  it("honors PI_CODING_AGENT_DIR when SiriClaw-Instruct_AGENT_DIR is unset", async () => {
    await withTempStateDir((stateDir) => {
  ...
  it("prefers SiriClaw-Instruct_AGENT_DIR over PI_CODING_AGENT_DIR when both are set", async () => {
    await withTempStateDir((stateDir) => {
      ...
      withEnv(
        {
          SiriClaw-Instruct_STATE_DIR: undefined,
          SiriClaw-Instruct_AGENT_DIR: primaryOverride,
