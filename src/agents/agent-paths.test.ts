...
describe("resolveSiriClawInstructAgentDir", () => {
  const withTempStateDir = async (run: (stateDir: string) => void) => {
    const stateDir = await fs.mkdtemp(path.join(os.tmpdir(), "SiriClawInstruct-agent-"));
    try {
      run(stateDir);
    } finally {
  ...
  it("defaults to the multi-agent path when no overrides are set", async () => {
    await withTempStateDir((stateDir) => {
      withEnv(
        ...
        () => {
          const resolved = resolveSiriClawInstructAgentDir();
  ...
  it("honors SiriClawInstruct_AGENT_DIR overrides", async () => {
    await withTempStateDir((stateDir) => {
      const override = path.join(stateDir, "agent");
      withEnv(
        {
          SiriClawInstruct_STATE_DIR: undefined,
          SiriClawInstruct_AGENT_DIR: override,
          PI_CODING_AGENT_DIR: undefined,
        },
        () => {
          const resolved = resolveSiriClawInstructAgentDir();
  ...
  it("honors PI_CODING_AGENT_DIR when SiriClawInstruct_AGENT_DIR is unset", async () => {
    await withTempStateDir((stateDir) => {
  ...
  it("prefers SiriClawInstruct_AGENT_DIR over PI_CODING_AGENT_DIR when both are set", async () => {
    await withTempStateDir((stateDir) => {
      ...
      withEnv(
        {
          SiriClawInstruct_STATE_DIR: undefined,
          SiriClawInstruct_AGENT_DIR: primaryOverride,
