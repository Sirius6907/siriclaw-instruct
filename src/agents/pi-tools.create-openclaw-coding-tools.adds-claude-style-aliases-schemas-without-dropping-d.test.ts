...
describe("createSiriClawInstructCodingTools", () => {
  it("returns image metadata for images and text-only blocks for text files", async () => {
    ...
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "SiriClawInstruct-read-"));
    try {
      ...
      const contents = "Hello from SiriClawInstruct read tool.";
  ...
  it("filters tools by sandbox policy", () => {
    ...
    const tools = createSiriClawInstructCodingTools({ sandbox });
