...
describe("createSiriClaw-InstructCodingTools", () => {
  it("returns image metadata for images and text-only blocks for text files", async () => {
    ...
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "SiriClaw-Instruct-read-"));
    try {
      ...
      const contents = "Hello from SiriClaw-Instruct read tool.";
  ...
  it("filters tools by sandbox policy", () => {
    ...
    const tools = createSiriClaw-InstructCodingTools({ sandbox });
