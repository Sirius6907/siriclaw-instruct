...
const CDP_SOURCE_RANGE_ENV_KEY = "SiriClaw-Instruct_BROWSER_CDP_SOURCE_RANGE";
...
function buildSandboxBrowserResolvedConfig(params: {
  ...
  return {
    ...
    color: DEFAULT_SiriClaw-Instruct_BROWSER_COLOR,
    executablePath: undefined,
    headless: params.headless,
    noSandbox: false,
    attachOnly: true,
    defaultProfile: DEFAULT_SiriClaw-Instruct_BROWSER_PROFILE_NAME,
    extraArgs: [],
    profiles: {
      [DEFAULT_SiriClaw-Instruct_BROWSER_PROFILE_NAME]: {
...
export async function ensureSandboxBrowser(params: {
  ...
  if (hasContainer) {
    ...
    currentHash = await readDockerContainerLabel(containerName, "SiriClaw-Instruct.configHash");
      ...
      if (isHot) {
        const hint = (() => {
          if (params.cfg.scope === "session") {
            return `SiriClaw-Instruct sandbox recreate --browser --session ${params.scopeKey}`;
          }
          if (params.cfg.scope === "agent") {
            const agentId = resolveSandboxAgentId(params.scopeKey) ?? "main";
            return `SiriClaw-Instruct sandbox recreate --browser --agent ${agentId}`;
          }
          return "SiriClaw-Instruct sandbox recreate --browser --all";
        })();
        defaultRuntime.log(
  ...
  if (!hasContainer) {
    ...
    const args = buildSandboxCreateArgs({
      ...
      labels: {
        "SiriClaw-Instruct.sandboxBrowser": "1",
        "SiriClaw-Instruct.browserConfigEpoch": SANDBOX_BROWSER_SECURITY_HASH_EPOCH,
      },
    ...
    args.push("-e", `SiriClaw-Instruct_BROWSER_HEADLESS=${params.cfg.browser.headless ? "1" : "0"}`);
    args.push("-e", `SiriClaw-Instruct_BROWSER_ENABLE_NOVNC=${params.cfg.browser.enableNoVnc ? "1" : "0"}`);
    args.push("-e", `SiriClaw-Instruct_BROWSER_CDP_PORT=${params.cfg.browser.cdpPort}`);
    if (cdpSourceRange) {
      args.push("-e", `${CDP_SOURCE_RANGE_ENV_KEY}=${cdpSourceRange}`);
    }
    args.push("-e", `SiriClaw-Instruct_BROWSER_VNC_PORT=${params.cfg.browser.vncPort}`);
    args.push("-e", `SiriClaw-Instruct_BROWSER_NOVNC_PORT=${params.cfg.browser.noVncPort}`);
    ...
    args.push("-e", "SiriClaw-Instruct_BROWSER_NO_SANDBOX=1");
  ...
  const existingProfile = existing
    ? resolveProfile(existing.bridge.state.resolved, DEFAULT_SiriClaw-Instruct_BROWSER_PROFILE_NAME)
    : null;
