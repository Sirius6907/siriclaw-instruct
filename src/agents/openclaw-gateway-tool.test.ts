...
describe("gateway tool", () => {
  ...
  it("schedules SIGUSR1 restart", async () => {
    ...
    const stateDir = await fs.mkdtemp(path.join(os.tmpdir(), "SiriClaw-Instruct-test-"));

    try {
      await withEnvAsync(
        { SiriClaw-Instruct_STATE_DIR: stateDir, SiriClaw-Instruct_PROFILE: "isolated" },
        async () => {
          const tool = requireGatewayTool();
          ...
          expect(parsed.payload?.doctorHint).toBe(
            "Run: SiriClaw-Instruct --profile isolated doctor --non-interactive",
          );
  ...
  it("passes config.apply through gateway call", async () => {
    ...
    const raw = '{\n  agents: { defaults: { workspace: "~/SiriClaw-Instruct" } }\n}\n';
