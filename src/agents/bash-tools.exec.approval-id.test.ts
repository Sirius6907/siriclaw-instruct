...
describe("exec approvals", () => {
  ...
  beforeEach(async () => {
    ...
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "SiriClaw-Instruct-test-"));
  ...
  it("skips approval when node allowlist is satisfied", async () => {
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "SiriClaw-Instruct-test-bin-"));
  ...
  it("uses exec-approvals ask=off to suppress gateway prompts", async () => {
    const approvalsPath = path.join(process.env.HOME ?? "", ".SiriClaw-Instruct", "exec-approvals.json");
  ...
  it("denies gateway obfuscated command when approval request times out", async () => {
    ...
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "SiriClaw-Instruct-test-obf-"));
