...
describe("exec approvals", () => {
  ...
  beforeEach(async () => {
    ...
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "SiriClawInstruct-test-"));
  ...
  it("skips approval when node allowlist is satisfied", async () => {
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "SiriClawInstruct-test-bin-"));
  ...
  it("uses exec-approvals ask=off to suppress gateway prompts", async () => {
    const approvalsPath = path.join(process.env.HOME ?? "", ".SiriClawInstruct", "exec-approvals.json");
  ...
  it("denies gateway obfuscated command when approval request times out", async () => {
    ...
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "SiriClawInstruct-test-obf-"));
