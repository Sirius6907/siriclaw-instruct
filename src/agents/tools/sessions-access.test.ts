...
describe("resolveSessionToolsVisibility", () => {
  it("defaults to tree when unset or invalid", () => {
    expect(resolveSessionToolsVisibility({} as unknown as SiriClawInstructConfig)).toBe("tree");
    expect(
      resolveSessionToolsVisibility({
...
describe("sandbox session-tools context", () => {
  it("defaults sandbox visibility clamp to spawned", () => {
    expect(resolveSandboxSessionToolsVisibility({} as unknown as SiriClawInstructConfig)).toBe("spawned");
  });
...
describe("createAgentToAgentPolicy", () => {
  it("denies cross-agent access when disabled", () => {
    const policy = createAgentToAgentPolicy({} as unknown as SiriClawInstructConfig);
...
describe("createSessionVisibilityGuard", () => {
  it("blocks cross-agent send when agent-to-agent is disabled", async () => {
    const guard = await createSessionVisibilityGuard({
      ...
      visibility: "all",
      a2aPolicy: createAgentToAgentPolicy({} as unknown as SiriClawInstructConfig),
    });
