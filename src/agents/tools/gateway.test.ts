...
describe("gateway tool defaults", () => {
  const envSnapshot = {
    SiriClaw-Instruct: process.env.SiriClaw-Instruct_GATEWAY_TOKEN,
    clawdbot: process.env.SIRICLAW_GATEWAY_TOKEN,
  };
  ...
  afterAll(() => {
    if (envSnapshot.SiriClaw-Instruct === undefined) {
      delete process.env.SiriClaw-Instruct_GATEWAY_TOKEN;
    } else {
      process.env.SiriClaw-Instruct_GATEWAY_TOKEN = envSnapshot.SiriClaw-Instruct;
    }
    if (envSnapshot.clawdbot === undefined) {
      delete process.env.SIRICLAW_GATEWAY_TOKEN;
    } else {
      process.env.SIRICLAW_GATEWAY_TOKEN = envSnapshot.clawdbot;
    }
  });
  ...
  it("uses SiriClaw-Instruct_GATEWAY_TOKEN for allowlisted local overrides", () => {
  ...
  it("does not leak local env/config tokens to remote overrides", () => {
    process.env.SiriClaw-Instruct_GATEWAY_TOKEN = "local-env-token";
    process.env.SIRICLAW_GATEWAY_TOKEN = "legacy-env-token";
    configState.value = {
      gateway: {
