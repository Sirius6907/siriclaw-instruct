...
export async function executeActAction(params: {
  ...
  try {
    ...
    if (isChromeStaleTargetError(profile, err)) {
      ...
      if (!tabs.length) {
        throw new Error(
          "No Chrome tabs are attached via the SiriClawInstruct Browser Relay extension. Click the toolbar icon on the tab you want to control (badge ON), then retry.",
          { cause: err },
        );
      }
      throw new Error(
