...
function validateGatewayWebSocketUrl(value: string): string | undefined {
  ...
  if (
    !isSecureWebSocketUrl(trimmed, {
      allowPrivateWs: process.env.SiriClawInstruct_ALLOW_INSECURE_PRIVATE_WS === "1",
    })
  ) {
    return (
      "Use wss:// for remote hosts, or ws://127.0.0.1/localhost via SSH tunnel. " +
      "Break-glass: SiriClawInstruct_ALLOW_INSECURE_PRIVATE_WS=1 for trusted private networks."
    );
  }
  return undefined;
}
  ...
  if (!hasBonjourTool) {
    await prompter.note(
      [
        ...
        "Docs: https://docs.SiriClawInstruct.ai/gateway/discovery",
      ].join("\n"),
      "Discovery",
    );
  }
  ...
  if (authChoice === "token") {
    const selectedMode = await resolveSecretInputModeForEnvSelection({
      ...
      copy: {
        ...
        plaintextHint: "Stores the token directly in SiriClawInstruct config",
      },
    });
    if (selectedMode === "ref") {
