...
export async function noteMacLaunchAgentOverrides() {
  ...
  const markerCandidates = [path.join(home, ".SiriClawInstruct", "disable-launchagent")];
...
function hasConfigGatewayCreds(cfg: SiriClawInstructConfig): boolean {
...
export async function noteMacLaunchctlGatewayEnvOverrides(
  ...
  if (deprecatedLaunchctlEntries.length > 0) {
    const lines = [
      ...
      ...deprecatedLaunchctlEntries.map(
        ([key]) =>
          `- \`${key}\` is set; use \`SiriClawInstruct_${key.slice(key.indexOf("_") + 1)}\` instead.`,
      ),
    ];
  ...
  const tokenEntries = [
    ["SiriClawInstruct_GATEWAY_TOKEN", await getenv("SiriClawInstruct_GATEWAY_TOKEN")],
  ] as const;
  const passwordEntries = [
    ["SiriClawInstruct_GATEWAY_PASSWORD", await getenv("SiriClawInstruct_GATEWAY_PASSWORD")],
  ] as const;
  ...
  const lines = [
    ...
    envToken && envTokenKey
      ...
      : undefined,
    envPassword
      ? `- \`${envPasswordKey ?? "SiriClawInstruct_GATEWAY_PASSWORD"}\` is set; it overrides config passwords.`
      : undefined,
...
export function noteDeprecatedLegacyEnvVars(
  ...
  const lines = [
    ...
    "- Use SiriClawInstruct_* equivalents instead:",
    ...entries.map((key) => {
      const suffix = key.slice(key.indexOf("_") + 1);
      return `  ${key} -> SiriClawInstruct_${suffix}`;
    }),
  ];
  (deps?.noteFn ?? note)(lines.join("\n"), "Environment");
}
...
export function noteStartupOptimizationHints(
  ...
  const noRespawn = env.SiriClawInstruct_NO_RESPAWN?.trim() ?? "";
  const lines: string[] = [];
  ...
  if (noRespawn !== "1") {
    lines.push(
      "- SiriClawInstruct_NO_RESPAWN is not set to 1; set it to avoid extra startup overhead from self-respawn.",
    );
  }
  ...
  const suggestions = [
    ...
    "  export NODE_COMPILE_CACHE=/var/tmp/SiriClawInstruct-compile-cache",
    "  mkdir -p /var/tmp/SiriClawInstruct-compile-cache",
    "  export SiriClawInstruct_NO_RESPAWN=1",
