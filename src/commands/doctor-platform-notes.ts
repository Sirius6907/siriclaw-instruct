...
export async function noteMacLaunchAgentOverrides() {
  ...
  const markerCandidates = [path.join(home, ".SiriClaw-Instruct", "disable-launchagent")];
...
function hasConfigGatewayCreds(cfg: SiriClaw-InstructConfig): boolean {
...
export async function noteMacLaunchctlGatewayEnvOverrides(
  ...
  const deprecatedLaunchctlEntries = [
    ["SIRICLAW_GATEWAY_TOKEN", await getenv("SIRICLAW_GATEWAY_TOKEN")],
    ["SIRICLAW_GATEWAY_PASSWORD", await getenv("SIRICLAW_GATEWAY_PASSWORD")],
  ...
  if (deprecatedLaunchctlEntries.length > 0) {
    const lines = [
      ...
      ...deprecatedLaunchctlEntries.map(
        ([key]) =>
          `- \`${key}\` is set; use \`SiriClaw-Instruct_${key.slice(key.indexOf("_") + 1)}\` instead.`,
      ),
    ];
  ...
  const tokenEntries = [
    ["SiriClaw-Instruct_GATEWAY_TOKEN", await getenv("SiriClaw-Instruct_GATEWAY_TOKEN")],
  ] as const;
  const passwordEntries = [
    ["SiriClaw-Instruct_GATEWAY_PASSWORD", await getenv("SiriClaw-Instruct_GATEWAY_PASSWORD")],
  ] as const;
  ...
  const lines = [
    ...
    envToken && envTokenKey
      ...
      : undefined,
    envPassword
      ? `- \`${envPasswordKey ?? "SiriClaw-Instruct_GATEWAY_PASSWORD"}\` is set; it overrides config passwords.`
      : undefined,
...
export function noteDeprecatedLegacyEnvVars(
  ...
  const entries = Object.entries(env)
    .filter(([key, value]) => key.startsWith("SIRICLAW_") && value?.trim())
    .map(([key]) => key);
  if (entries.length === 0) {
    return;
  }
  ...
  const lines = [
    ...
    "- Use SiriClaw-Instruct_* equivalents instead:",
    ...entries.map((key) => {
      const suffix = key.slice(key.indexOf("_") + 1);
      return `  ${key} -> SiriClaw-Instruct_${suffix}`;
    }),
  ];
  (deps?.noteFn ?? note)(lines.join("\n"), "Environment");
}
...
export function noteStartupOptimizationHints(
  ...
  const noRespawn = env.SiriClaw-Instruct_NO_RESPAWN?.trim() ?? "";
  const lines: string[] = [];
  ...
  if (noRespawn !== "1") {
    lines.push(
      "- SiriClaw-Instruct_NO_RESPAWN is not set to 1; set it to avoid extra startup overhead from self-respawn.",
    );
  }
