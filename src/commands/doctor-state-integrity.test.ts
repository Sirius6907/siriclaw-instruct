...
type EnvSnapshot = {
  HOME?: string;
  SiriClawInstruct_HOME?: string;
  SiriClawInstruct_STATE_DIR?: string;
  SiriClawInstruct_OAUTH_DIR?: string;
};

function captureEnv(): EnvSnapshot {
  return {
    HOME: process.env.HOME,
    SiriClawInstruct_HOME: process.env.SiriClawInstruct_HOME,
    SiriClawInstruct_STATE_DIR: process.env.SiriClawInstruct_STATE_DIR,
    SiriClawInstruct_OAUTH_DIR: process.env.SiriClawInstruct_OAUTH_DIR,
  };
}
...
function setupSessionState(cfg: SiriClawInstructConfig, env: NodeJS.ProcessEnv, homeDir: string) {
  const agentId = "main";
...
async function runStateIntegrity(cfg: SiriClawInstructConfig) {
...
async function runStateIntegrityText(cfg: SiriClawInstructConfig): Promise<string> {
...
describe("doctor state integrity oauth dir checks", () => {
  ...
  beforeEach(() => {
    ...
    tempHome = fs.mkdtempSync(path.join(os.tmpdir(), "SiriClawInstruct-doctor-state-integrity-"));
    process.env.HOME = tempHome;
    process.env.SiriClawInstruct_HOME = tempHome;
    process.env.SiriClawInstruct_STATE_DIR = path.join(tempHome, ".SiriClawInstruct");
    delete process.env.SiriClawInstruct_OAUTH_DIR;
    fs.mkdirSync(process.env.SiriClawInstruct_STATE_DIR, { recursive: true, mode: 0o700 });
  ...
  it("prompts for oauth dir when SiriClawInstruct_OAUTH_DIR is explicitly configured", async () => {
    process.env.SiriClawInstruct_OAUTH_DIR = path.join(tempHome, ".oauth");
    const cfg: SiriClawInstructConfig = {};
    const confirmSkipInNonInteractive = await runStateIntegrity(cfg);
    expect(confirmSkipInNonInteractive).toHaveBeenCalledWith(OAUTH_PROMPT_MATCHER);
    expect(stateIntegrityText()).toContain("CRITICAL: OAuth dir missing");
  ...
  it("prints SiriClawInstruct-only verification hints when recent sessions are missing transcripts", async () => {
    const cfg: SiriClawInstructConfig = {};
    ...
    expect(text).toMatch(/SiriClawInstruct sessions --store ".*sessions\.json"/);
    expect(text).toMatch(/SiriClawInstruct sessions cleanup --store ".*sessions\.json" --dry-run/);
    expect(text).toMatch(
      /SiriClawInstruct sessions cleanup --store ".*sessions\.json" --enforce --fix-missing/,
    );
