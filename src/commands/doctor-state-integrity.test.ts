...
type EnvSnapshot = {
  HOME?: string;
  SiriClaw-Instruct_HOME?: string;
  SiriClaw-Instruct_STATE_DIR?: string;
  SiriClaw-Instruct_OAUTH_DIR?: string;
};

function captureEnv(): EnvSnapshot {
  return {
    HOME: process.env.HOME,
    SiriClaw-Instruct_HOME: process.env.SiriClaw-Instruct_HOME,
    SiriClaw-Instruct_STATE_DIR: process.env.SiriClaw-Instruct_STATE_DIR,
    SiriClaw-Instruct_OAUTH_DIR: process.env.SiriClaw-Instruct_OAUTH_DIR,
  };
}
...
function setupSessionState(cfg: SiriClaw-InstructConfig, env: NodeJS.ProcessEnv, homeDir: string) {
  const agentId = "main";
...
async function runStateIntegrity(cfg: SiriClaw-InstructConfig) {
...
async function runStateIntegrityText(cfg: SiriClaw-InstructConfig): Promise<string> {
...
describe("doctor state integrity oauth dir checks", () => {
  ...
  beforeEach(() => {
    ...
    tempHome = fs.mkdtempSync(path.join(os.tmpdir(), "SiriClaw-Instruct-doctor-state-integrity-"));
    process.env.HOME = tempHome;
    process.env.SiriClaw-Instruct_HOME = tempHome;
    ...
    fs.mkdirSync(process.env.SiriClaw-Instruct_STATE_DIR, { recursive: true, mode: 0o700 });
  ...
  it("prompts for oauth dir when SiriClaw-Instruct_OAUTH_DIR is explicitly configured", async () => {
    process.env.SiriClaw-Instruct_OAUTH_DIR = path.join(tempHome, ".oauth");
    const cfg: SiriClaw-InstructConfig = {};
    const confirmSkipInNonInteractive = await runStateIntegrity(cfg);
    expect(confirmSkipInNonInteractive).toHaveBeenCalledWith(OAUTH_PROMPT_MATCHER);
    expect(stateIntegrityText()).toContain("CRITICAL: OAuth dir missing");
  ...
  it("prints SiriClaw-Instruct-only verification hints when recent sessions are missing transcripts", async () => {
    const cfg: SiriClaw-InstructConfig = {};
    ...
    expect(text).toMatch(/SiriClaw-Instruct sessions --store ".*sessions\.json"/);
    expect(text).toMatch(/SiriClaw-Instruct sessions cleanup --store ".*sessions\.json" --dry-run/);
    expect(text).toMatch(
      /SiriClaw-Instruct sessions cleanup --store ".*sessions\.json" --enforce --fix-missing/,
    );
