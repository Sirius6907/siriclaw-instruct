...
describe("detectLinuxSdBackedStateDir", () => {
  it("detects state dir on mmc-backed mount", () => {
    ...
    const result = detectLinuxSdBackedStateDir("/home/pi/.SiriClaw-Instruct", {
      platform: "linux",
      mountInfo,
    });

    expect(result).toEqual({
      path: "/home/pi/.SiriClaw-Instruct",
      mountPoint: "/",
      fsType: "ext4",
  ...
  it("returns null for non-mmc devices", () => {
    ...
    const result = detectLinuxSdBackedStateDir("/home/user/.SiriClaw-Instruct", {
      platform: "linux",
      mountInfo,
    });
  ...
  it("resolves /dev/disk aliases to mmc devices", () => {
    ...
    expect(result).toEqual({
      path: "/home/user/.SiriClaw-Instruct",
      mountPoint: "/",
      fsType: "ext4",
  ...
  it("uses resolved state path to select mount", () => {
    ...
    const result = detectLinuxSdBackedStateDir("/tmp/SiriClaw-Instruct-state", {
      platform: "linux",
      mountInfo,
      resolveRealPath: () => "/mnt/slow/SiriClaw-Instruct/.SiriClaw-Instruct",
    });

    expect(result).toEqual({
      path: "/mnt/slow/SiriClaw-Instruct/.SiriClaw-Instruct",
      mountPoint: "/mnt/slow",
      fsType: "ext4",
  ...
  it("returns null outside linux", () => {
    ...
    const result = detectLinuxSdBackedStateDir(path.join("/Users", "tester", ".SiriClaw-Instruct"), {
      platform: "darwin",
      mountInfo,
    });
  ...
  it("escapes decoded mountinfo control characters in warning output", () => {
    ...
    const stateDir = `${mountRoot}/.SiriClaw-Instruct`;
