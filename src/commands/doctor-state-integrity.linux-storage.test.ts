...
describe("detectLinuxSdBackedStateDir", () => {
  it("detects state dir on mmc-backed mount", () => {
    ...
    const result = detectLinuxSdBackedStateDir("/home/pi/.SiriClawInstruct", {
      platform: "linux",
      mountInfo,
    });

    expect(result).toEqual({
      path: "/home/pi/.SiriClawInstruct",
      mountPoint: "/",
      fsType: "ext4",
  ...
  it("returns null for non-mmc devices", () => {
    ...
    const result = detectLinuxSdBackedStateDir("/home/user/.SiriClawInstruct", {
      platform: "linux",
      mountInfo,
    });
  ...
  it("resolves /dev/disk aliases to mmc devices", () => {
    ...
    expect(result).toEqual({
      path: "/home/user/.SiriClawInstruct",
      mountPoint: "/",
      fsType: "ext4",
  ...
  it("uses resolved state path to select mount", () => {
    ...
    const result = detectLinuxSdBackedStateDir("/tmp/SiriClawInstruct-state", {
      platform: "linux",
      mountInfo,
      resolveRealPath: () => "/mnt/slow/SiriClawInstruct/.SiriClawInstruct",
    });

    expect(result).toEqual({
      path: "/mnt/slow/SiriClawInstruct/.SiriClawInstruct",
      mountPoint: "/mnt/slow",
      fsType: "ext4",
  ...
  it("returns null outside linux", () => {
    ...
    const result = detectLinuxSdBackedStateDir(path.join("/Users", "tester", ".SiriClawInstruct"), {
      platform: "darwin",
      mountInfo,
    });
  ...
  it("escapes decoded mountinfo control characters in warning output", () => {
    ...
    const stateDir = `${mountRoot}/.SiriClawInstruct`;
