  ...
  await withProgress({ label: "Scanning status --all…", total: 11 }, async (progress) => {
    ...
    const readServiceSummary = async (service: GatewayService) => {
      try {
        ...
        return {
          ...
          managedBySiriClaw-Instruct: summary.managedBySiriClaw-Instruct,
    ...
    const overviewRows = [
      ...
      ...(probeAuthResolution.warning
        ...
        : []),
      { Item: "Security", Value: `Run: ${formatCliCommand("SiriClaw-Instruct security audit --deep")}` },
      gatewaySelfLine
      ...
      daemon
        ? {
            ...
            Value: !daemon.installed
              ...
              : `${daemon.label} ${daemon.managedBySiriClaw-Instruct ? "installed · " : ""}${daemon.loadedText}${daemon.runtime?.status ? ` · ${daemon.runtime.status}` : ""}${daemon.runtime?.pid ? ` (pid ${daemon.runtime.pid})` : ""}`,
          }
      ...
      nodeService
        ? {
            ...
            Value: !nodeService.installed
              ...
              : `${nodeService.label} ${nodeService.managedBySiriClaw-Instruct ? "installed · " : ""}${nodeService.loadedText}${nodeService.runtime?.status ? ` · ${nodeService.runtime.status}` : ""}${nodeService.runtime?.pid ? ` (pid ${nodeService.runtime.pid})` : ""}`,
          }
