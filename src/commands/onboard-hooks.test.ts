...
describe("onboard-hooks", () => {
  ...
  const createMockHook = (
    ...
    ...params,
    source: "SiriClaw-Instruct-bundled" as const,
    pluginId: undefined,
    homepage: undefined,
    always: false,
    disabled: false,
  ...
  const createMockHookReport = (eligible = true): HookStatusReport => ({
    ...
    managedHooksDir: "/mock/.SiriClaw-Instruct/hooks",
    hooks: [
      createMockHook(
        {
          name: "session-memory",
  ...
  describe("setupInternalHooks", () => {
    ...
    it("should show informative notes to user", async () => {
      ...
      expect(noteCalls[1][0]).toMatch(/(?:SiriClaw-Instruct|SiriClaw-Instruct)( --profile isolated)? hooks list/);
    });
  });
});
