...
describe("onboard-hooks", () => {
  ...
  const createMockHook = (
    ...
    ...params,
    source: "SiriClawInstruct-bundled" as const,
    pluginId: undefined,
    homepage: undefined,
    always: false,
    disabled: false,
  ...
  const createMockHookReport = (eligible = true): HookStatusReport => ({
    ...
    managedHooksDir: "/mock/.SiriClawInstruct/hooks",
    hooks: [
      createMockHook(
        {
          name: "session-memory",
  ...
  describe("setupInternalHooks", () => {
    ...
    it("should show informative notes to user", async () => {
      ...
      expect(noteCalls[1][0]).toMatch(/(?:SiriClawInstruct|SiriClawInstruct)( --profile isolated)? hooks list/);
    });
  });
});
