 ...
 * Runs as part of `SiriClawInstruct doctor` — config-only, no network calls.
 */
  ...
  if (resolved.provider !== "auto") {
    if (resolved.provider === "local") {
        ...
        if (opts?.gatewayMemoryProbe?.checked && !opts.gatewayMemoryProbe.ready) {
          ...
          note(
            [
              ...
              "",
              `Verify: ${formatCliCommand("SiriClawInstruct memory status --deep")}`,
            ]
              .filter(Boolean)
              .join("\n"),
            "Memory search",
      ...
      note(
        [
          ...
          `- Switch to a remote provider: ${formatCliCommand("SiriClawInstruct config set agents.defaults.memorySearch.provider openai")}`,
          "",
          `Verify: ${formatCliCommand("SiriClawInstruct memory status --deep")}`,
        ].join("\n"),
        "Memory search",
    ...
    note(
      [
        ...
        `- Configure credentials: ${formatCliCommand("SiriClawInstruct configure --section model")}`,
        `- To disable: ${formatCliCommand("SiriClawInstruct config set agents.defaults.memorySearch.enabled false")}`,
        "",
        `Verify: ${formatCliCommand("SiriClawInstruct memory status --deep")}`,
      ].join("\n"),
      "Memory search",
