 ...
 * Runs as part of `SiriClaw-Instruct doctor` — config-only, no network calls.
 */
  ...
  if (resolved.provider !== "auto") {
    if (resolved.provider === "local") {
        ...
        if (opts?.gatewayMemoryProbe?.checked && !opts.gatewayMemoryProbe.ready) {
          ...
          note(
            [
              ...
              "",
              `Verify: ${formatCliCommand("SiriClaw-Instruct memory status --deep")}`,
            ]
              .filter(Boolean)
              .join("\n"),
            "Memory search",
      ...
      note(
        [
          ...
          `- Switch to a remote provider: ${formatCliCommand("SiriClaw-Instruct config set agents.defaults.memorySearch.provider openai")}`,
          "",
          `Verify: ${formatCliCommand("SiriClaw-Instruct memory status --deep")}`,
        ].join("\n"),
        "Memory search",
    ...
    note(
      [
        ...
        `- Configure credentials: ${formatCliCommand("SiriClaw-Instruct configure --section model")}`,
        `- To disable: ${formatCliCommand("SiriClaw-Instruct config set agents.defaults.memorySearch.enabled false")}`,
        "",
        `Verify: ${formatCliCommand("SiriClaw-Instruct memory status --deep")}`,
      ].join("\n"),
      "Memory search",
