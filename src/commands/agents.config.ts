...
function resolveAgentName(cfg: SiriClaw-InstructConfig, agentId: string) {
...
function resolveAgentModel(cfg: SiriClaw-InstructConfig, agentId: string) {
  const entry = listAgentEntries(cfg).find(
    (agent) => normalizeAgentId(agent.id) === normalizeAgentId(agentId),
  );
  if (entry?.model) {
...
export function buildAgentSummaries(cfg: SiriClaw-InstructConfig): AgentSummary[] {
