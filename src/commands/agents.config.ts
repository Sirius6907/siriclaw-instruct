...
function resolveAgentName(cfg: SiriClawInstructConfig, agentId: string) {
...
function resolveAgentModel(cfg: SiriClawInstructConfig, agentId: string) {
  const entry = listAgentEntries(cfg).find(
    (agent) => normalizeAgentId(agent.id) === normalizeAgentId(agentId),
  );
  if (entry?.model) {
...
export function buildAgentSummaries(cfg: SiriClawInstructConfig): AgentSummary[] {
