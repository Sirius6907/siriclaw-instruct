...
export async function statusCommand(
  ...
  const daemonValue = (() => {
    ...
    const installedPrefix = daemon.managedBySiriClawInstruct ? "installed · " : "";
  ...
  const nodeDaemonValue = (() => {
    ...
    const installedPrefix = nodeDaemon.managedBySiriClawInstruct ? "installed · " : "";
  ...
  runtime.log(theme.heading("SiriClawInstruct status"));
  runtime.log("");
  ...
  if (pairingRecovery) {
    ...
    if (pairingRecovery.requestId) {
      runtime.log(
        theme.muted(
          `Recovery: ${formatCliCommand(`SiriClawInstruct devices approve ${pairingRecovery.requestId}`)}`,
        ),
      );
    }
    runtime.log(theme.muted(`Fallback: ${formatCliCommand("SiriClawInstruct devices approve --latest")}`));
    runtime.log(theme.muted(`Inspect: ${formatCliCommand("SiriClawInstruct devices list")}`));
  }
  ...
  runtime.log(theme.muted(`Full report: ${formatCliCommand("SiriClawInstruct security audit")}`));
  runtime.log(theme.muted(`Deep probe: ${formatCliCommand("SiriClawInstruct security audit --deep")}`));
  ...
  runtime.log("");
  runtime.log("FAQ: https://docs.SiriClawInstruct.ai/faq");
  runtime.log("Troubleshooting: https://docs.SiriClawInstruct.ai/troubleshooting");
  runtime.log("");
  ...
  runtime.log(`  Need to share?      ${formatCliCommand("SiriClawInstruct status --all")}`);
  runtime.log(`  Need to debug live? ${formatCliCommand("SiriClawInstruct logs --follow")}`);
  if (gatewayReachable) {
    runtime.log(`  Need to test channels? ${formatCliCommand("SiriClawInstruct status --deep")}`);
  } else {
    runtime.log(`  Fix reachability first: ${formatCliCommand("SiriClawInstruct gateway probe")}`);
  }
}
