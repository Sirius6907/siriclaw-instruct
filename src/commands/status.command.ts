...
export async function statusCommand(
  ...
  const daemonValue = (() => {
    ...
    const installedPrefix = daemon.managedBySiriClaw-Instruct ? "installed · " : "";
  ...
  const nodeDaemonValue = (() => {
    ...
    const installedPrefix = nodeDaemon.managedBySiriClaw-Instruct ? "installed · " : "";
  ...
  runtime.log(theme.heading("SiriClaw-Instruct status"));
  runtime.log("");
  ...
  if (pairingRecovery) {
    ...
    if (pairingRecovery.requestId) {
      runtime.log(
        theme.muted(
          `Recovery: ${formatCliCommand(`SiriClaw-Instruct devices approve ${pairingRecovery.requestId}`)}`,
        ),
      );
    }
    runtime.log(theme.muted(`Fallback: ${formatCliCommand("SiriClaw-Instruct devices approve --latest")}`));
    runtime.log(theme.muted(`Inspect: ${formatCliCommand("SiriClaw-Instruct devices list")}`));
  }
  ...
  runtime.log(theme.muted(`Full report: ${formatCliCommand("SiriClaw-Instruct security audit")}`));
  runtime.log(theme.muted(`Deep probe: ${formatCliCommand("SiriClaw-Instruct security audit --deep")}`));
  ...
  runtime.log("");
  runtime.log("FAQ: https://docs.SiriClaw-Instruct.ai/faq");
  runtime.log("Troubleshooting: https://docs.SiriClaw-Instruct.ai/troubleshooting");
  runtime.log("");
  ...
  runtime.log(`  Need to share?      ${formatCliCommand("SiriClaw-Instruct status --all")}`);
  runtime.log(`  Need to debug live? ${formatCliCommand("SiriClaw-Instruct logs --follow")}`);
  if (gatewayReachable) {
    runtime.log(`  Need to test channels? ${formatCliCommand("SiriClaw-Instruct status --deep")}`);
  } else {
    runtime.log(`  Fix reachability first: ${formatCliCommand("SiriClaw-Instruct gateway probe")}`);
  }
}
