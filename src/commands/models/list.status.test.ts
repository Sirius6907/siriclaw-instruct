...
const mocks = vi.hoisted(() => {
  ...
  return {
    store,
    resolveSiriClawInstructAgentDir: vi.fn().mockReturnValue("/tmp/SiriClawInstruct-agent"),
    resolveAgentDir: vi.fn().mockReturnValue("/tmp/SiriClawInstruct-agent"),
...
vi.mock("../../agents/agent-paths.js", () => ({
  resolveSiriClawInstructAgentDir: mocks.resolveSiriClawInstructAgentDir,
}));
...
async function withAgentScopeOverrides<T>(
  ...
  try {
    ...
    if (originalAgentDir) {
      ...
      mocks.resolveAgentDir.mockReturnValue("/tmp/SiriClawInstruct-agent");
    }
  }
}
...
describe("modelsStatusCommand auth overview", () => {
  it("includes masked auth sources in JSON output", async () => {
    ...
    expect(mocks.resolveSiriClawInstructAgentDir).toHaveBeenCalled();
    expect(payload.defaultModel).toBe("anthropic/claude-opus-4-5");
    expect(payload.auth.storePath).toBe("/tmp/SiriClawInstruct-agent/auth-profiles.json");
    ...
    await withAgentScopeOverrides(
      {
        ...
        agentDir: "/tmp/SiriClawInstruct-agent-custom",
      },
      async () => {
        ...
        expect(payload.agentDir).toBe("/tmp/SiriClawInstruct-agent-custom");
