...
const mocks = vi.hoisted(() => {
  ...
  return {
    store,
    resolveSiriClaw-InstructAgentDir: vi.fn().mockReturnValue("/tmp/SiriClaw-Instruct-agent"),
    resolveAgentDir: vi.fn().mockReturnValue("/tmp/SiriClaw-Instruct-agent"),
...
vi.mock("../../agents/agent-paths.js", () => ({
  resolveSiriClaw-InstructAgentDir: mocks.resolveSiriClaw-InstructAgentDir,
}));
...
async function withAgentScopeOverrides<T>(
  ...
  try {
    ...
    if (originalAgentDir) {
      ...
      mocks.resolveAgentDir.mockReturnValue("/tmp/SiriClaw-Instruct-agent");
    }
  }
}
...
describe("modelsStatusCommand auth overview", () => {
  it("includes masked auth sources in JSON output", async () => {
    ...
    expect(mocks.resolveSiriClaw-InstructAgentDir).toHaveBeenCalled();
    expect(payload.defaultModel).toBe("anthropic/claude-opus-4-5");
    expect(payload.auth.storePath).toBe("/tmp/SiriClaw-Instruct-agent/auth-profiles.json");
    ...
    await withAgentScopeOverrides(
      {
        ...
        agentDir: "/tmp/SiriClaw-Instruct-agent-custom",
      },
      async () => {
        ...
        expect(payload.agentDir).toBe("/tmp/SiriClaw-Instruct-agent-custom");
