...
type AgentDefaults = NonNullable<NonNullable<SiriClaw-InstructConfig["agents"]>["defaults"]>;
...
const readUsageFromSessionLog = (
  ...
  | undefined => {
  // Transcripts are stored at the session file path (fallback: ~/.SiriClaw-Instruct/sessions/<SessionId>.jsonl)
  if (!sessionId) {
    return undefined;
  }
  let logPath: string;
...
export function buildStatusMessage(args: StatusArgs): string {
  ...
  const versionLine = `🦞 SiriClaw-Instruct ${VERSION}${commit ? ` (${commit})` : ""}`;
...
export function buildHelpMessage(cfg?: SiriClaw-InstructConfig): string {
  const lines = ["ℹ️ Help", ""];
