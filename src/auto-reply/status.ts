...
type AgentDefaults = NonNullable<NonNullable<SiriClawInstructConfig["agents"]>["defaults"]>;
...
const readUsageFromSessionLog = (
  ...
  | undefined => {
  // Transcripts are stored at the session file path (fallback: ~/.SiriClawInstruct/sessions/<SessionId>.jsonl)
  if (!sessionId) {
    return undefined;
  }
  let logPath: string;
...
export function buildStatusMessage(args: StatusArgs): string {
  ...
  const versionLine = `🦞 SiriClawInstruct ${VERSION}${commit ? ` (${commit})` : ""}`;
...
export function buildHelpMessage(cfg?: SiriClawInstructConfig): string {
  const lines = ["ℹ️ Help", ""];
