...
describe("buildInboundMetaSystemPrompt", () => {
  it("includes session-stable routing fields", () => {
    ...
    expect(payload["schema"]).toBe("SiriClaw-Instruct.inbound_meta.v1");
...
describe("buildInboundUserContextPrefix", () => {
  it("omits conversation label block for direct chats", () => {
    const text = buildInboundUserContextPrefix({
      ChatType: "direct",
      ConversationLabel: "SiriClaw-Instruct-tui",
    } as TemplateContext);
  ...
  it("does not treat group chats as direct based on sender id", () => {
    const text = buildInboundUserContextPrefix({
      ChatType: "group",
      SenderId: "SiriClaw-Instruct-control-ui",
      MessageSid: "123",
    ...
    expect(conversationInfo["sender_id"]).toBe("SiriClaw-Instruct-control-ui");
