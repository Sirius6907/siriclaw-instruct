...
beforeAll(async () => {
  testWorkspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "SiriClaw-Instruct-commands-"));
...
function buildParams(commandBody: string, cfg: SiriClaw-InstructConfig, ctxOverrides?: Partial<MsgContext>) {
...
describe("handleCommands gating", () => {
  it("blocks gated commands when disabled or not elevated-allowlisted", async () => {
    const cases = typedCases<{
      name: string;
      commandBody: string;
      makeCfg: () => SiriClaw-InstructConfig;
...
describe("/compact command", () => {
  ...
  it("routes manual compaction with explicit trigger and context metadata", async () => {
    const cfg = {
      commands: { text: true },
      channels: { whatsapp: { allowFrom: ["*"] } },
      session: { store: "/tmp/SiriClaw-Instruct-session-store.json" },
    } as SiriClaw-InstructConfig;
    const params = buildParams("/compact: focus on decisions", cfg, {
      From: "+15550001",
      To: "+15550002",
    });
    const agentDir = "/tmp/SiriClaw-Instruct-agent-compact";
...
describe("handleCommands /allowlist", () => {
  ...
  it("removes DM allowlist entries from canonical allowFrom and deletes legacy dm.allowFrom", async () => {
    ...
    for (const testCase of cases) {
      ...
      const written = writeConfigFileMock.mock.calls.at(-1)?.[0] as SiriClaw-InstructConfig;
...
describe("handleCommands ACP-bound /new and /reset", () => {
  ...
  const buildDiscordBoundConfig = (): SiriClaw-InstructConfig =>
    ({
      commands: { text: true },
      bindings: [
        {
