...
beforeAll(async () => {
  testWorkspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "SiriClawInstruct-commands-"));
...
function buildParams(commandBody: string, cfg: SiriClawInstructConfig, ctxOverrides?: Partial<MsgContext>) {
...
describe("handleCommands gating", () => {
  it("blocks gated commands when disabled or not elevated-allowlisted", async () => {
    const cases = typedCases<{
      name: string;
      commandBody: string;
      makeCfg: () => SiriClawInstructConfig;
...
describe("/compact command", () => {
  ...
  it("routes manual compaction with explicit trigger and context metadata", async () => {
    const cfg = {
      commands: { text: true },
      channels: { whatsapp: { allowFrom: ["*"] } },
      session: { store: "/tmp/SiriClawInstruct-session-store.json" },
    } as SiriClawInstructConfig;
    const params = buildParams("/compact: focus on decisions", cfg, {
      From: "+15550001",
      To: "+15550002",
    });
    const agentDir = "/tmp/SiriClawInstruct-agent-compact";
...
describe("handleCommands /allowlist", () => {
  ...
  it("removes DM allowlist entries from canonical allowFrom and deletes legacy dm.allowFrom", async () => {
    ...
    for (const testCase of cases) {
      ...
      const written = writeConfigFileMock.mock.calls.at(-1)?.[0] as SiriClawInstructConfig;
...
describe("handleCommands ACP-bound /new and /reset", () => {
  ...
  const buildDiscordBoundConfig = (): SiriClawInstructConfig =>
    ({
      commands: { text: true },
      bindings: [
        {
