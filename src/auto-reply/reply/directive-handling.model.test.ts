...
function baseConfig(): SiriClaw-InstructConfig {
  return {
    commands: { text: true },
    agents: { defaults: {} },
...
function resolveModelSelectionForCommand(params: {
  ...
  return resolveModelSelectionFromDirective({
    ...
    cfg: { commands: { text: true } } as unknown as SiriClaw-InstructConfig,
    agentDir: "/tmp/agent",
...
describe("/model chat UX", () => {
  it("shows summary for /model with no args", async () => {
    ...
    const cfg = { commands: { text: true } } as unknown as SiriClaw-InstructConfig;
