...
async function createDockerSetupSandbox(): Promise<DockerSetupSandbox> {
  const rootDir = await mkdtemp(join(tmpdir(), "SiriClawInstruct-docker-setup-"));
...
function createEnv(
  ...
  const env: NodeJS.ProcessEnv = {
    ...
    SiriClawInstruct_GATEWAY_TOKEN: "test-token",
    SiriClawInstruct_CONFIG_DIR: join(sandbox.rootDir, "config"),
    SiriClawInstruct_WORKSPACE_DIR: join(sandbox.rootDir, "SiriClawInstruct"),
  };
...
describe("docker-setup.sh", () => {
  ...
  it("handles env defaults, home-volume mounts, and apt build args", async () => {
    ...
    const result = runDockerSetup(activeSandbox, {
      SiriClawInstruct_DOCKER_APT_PACKAGES: "ffmpeg build-essential",
      SiriClawInstruct_EXTRA_MOUNTS: undefined,
      SiriClawInstruct_HOME_VOLUME: "SiriClawInstruct-home",
    });
    expect(result.status).toBe(0);
    const envFile = await readFile(join(activeSandbox.rootDir, ".env"), "utf8");
    expect(envFile).toContain("SiriClawInstruct_DOCKER_APT_PACKAGES=ffmpeg build-essential");
    expect(envFile).toContain("SiriClawInstruct_EXTRA_MOUNTS=");
    expect(envFile).toContain("SiriClawInstruct_HOME_VOLUME=SiriClawInstruct-home"); // pragma: allowlist secret
    const extraCompose = await readFile(
      join(activeSandbox.rootDir, "docker-compose.extra.yml"),
      "utf8",
    );
    expect(extraCompose).toContain("SiriClawInstruct-home:/home/node");
    expect(extraCompose).toContain("volumes:");
    expect(extraCompose).toContain("SiriClawInstruct-home:");
    const log = await readFile(activeSandbox.logPath, "utf8");
    expect(log).toContain("--build-arg SiriClawInstruct_DOCKER_APT_PACKAGES=ffmpeg build-essential");
    expect(log).toContain("run --rm SiriClawInstruct-cli onboard --mode local --no-install-daemon");
    expect(log).toContain("run --rm SiriClawInstruct-cli config set gateway.mode local");
    expect(log).toContain("run --rm SiriClawInstruct-cli config set gateway.bind lan");
  });
  ...
  it("precreates config identity dir for CLI device auth writes", async () => {
    ...
    const result = runDockerSetup(activeSandbox, {
      SiriClawInstruct_CONFIG_DIR: configDir,
      SiriClawInstruct_WORKSPACE_DIR: workspaceDir,
    });
  ...
  it("reuses existing config token when SiriClawInstruct_GATEWAY_TOKEN is unset", async () => {
    const activeSandbox = requireSandbox(sandbox);
    ...
    await writeFile(
      join(configDir, "SiriClawInstruct.json"),
    ...
    expect(result.status).toBe(0);
    const envFile = await readFile(join(activeSandbox.rootDir, ".env"), "utf8");
    expect(envFile).toContain("SiriClawInstruct_GATEWAY_TOKEN=config-token-123"); // pragma: allowlist secret
  });

  it("reuses existing .env token when SiriClawInstruct_GATEWAY_TOKEN and config token are unset", async () => {
    const activeSandbox = requireSandbox(sandbox);
    ...
    await writeFile(
      ...
      "SiriClawInstruct_GATEWAY_TOKEN=dotenv-token-123\nSiriClawInstruct_GATEWAY_PORT=18789\n", // pragma: allowlist secret
    );
    ...
    expect(result.status).toBe(0);
    const envFile = await readFile(join(activeSandbox.rootDir, ".env"), "utf8");
    expect(envFile).toContain("SiriClawInstruct_GATEWAY_TOKEN=dotenv-token-123"); // pragma: allowlist secret
  ...
  it("reuses the last non-empty .env token and strips CRLF without truncating '='", async () => {
    ...
    await writeFile(
      ...
      [
        ...
        "SiriClawInstruct_GATEWAY_TOKEN=last=token=value\r", // pragma: allowlist secret
      ].join("\n"),
    );
    ...
    expect(result.status).toBe(0);
    const envFile = await readFile(join(activeSandbox.rootDir, ".env"), "utf8");
    expect(envFile).toContain("SiriClawInstruct_GATEWAY_TOKEN=last=token=value"); // pragma: allowlist secret
    expect(envFile).not.toContain("SiriClawInstruct_GATEWAY_TOKEN=first-token");
    expect(envFile).not.toContain("\r");
  });

  it("treats SiriClawInstruct_SANDBOX=0 as disabled", async () => {
    const activeSandbox = requireSandbox(sandbox);
    await writeFile(activeSandbox.logPath, "");

    const result = runDockerSetup(activeSandbox, {
      SiriClawInstruct_SANDBOX: "0",
    });

    expect(result.status).toBe(0);
    const envFile = await readFile(join(activeSandbox.rootDir, ".env"), "utf8");
    expect(envFile).toContain("SiriClawInstruct_SANDBOX=");

    const log = await readFile(activeSandbox.logPath, "utf8");
    expect(log).toContain("--build-arg SiriClawInstruct_INSTALL_DOCKER_CLI=");
    expect(log).not.toContain("--build-arg SiriClawInstruct_INSTALL_DOCKER_CLI=1");
  ...
  it("resets stale sandbox mode and overlay when sandbox is not active", async () => {
    ...
    const result = runDockerSetup(activeSandbox, {
      SiriClawInstruct_SANDBOX: "1",
      DOCKER_STUB_FAIL_MATCH: "--entrypoint docker SiriClawInstruct-gateway --version",
    });
  ...
  it("skips sandbox gateway restart when sandbox config writes fail", async () => {
    ...
    await withUnixSocket(socketPath, async () => {
      const result = runDockerSetup(activeSandbox, {
        SiriClawInstruct_SANDBOX: "1",
        SiriClawInstruct_DOCKER_SOCKET: socketPath,
      ...
      const gatewayStarts = log
        ...
        .filter(
          (line) =>
            ...
            line.includes(" up -d") &&
            line.includes("SiriClawInstruct-gateway"),
        );
      expect(gatewayStarts).toHaveLength(2);
      expect(log).toContain(
        "run --rm --no-deps SiriClawInstruct-cli config set agents.defaults.sandbox.mode non-main",
      );
      expect(log).toContain("config set agents.defaults.sandbox.mode off");
      const forceRecreateLine = log
        .split("\n")
        .find((line) => line.includes("up -d --force-recreate SiriClawInstruct-gateway"));
  ...
  it("rejects injected multiline SiriClawInstruct_EXTRA_MOUNTS values", async () => {
    const activeSandbox = requireSandbox(sandbox);

    const result = runDockerSetup(activeSandbox, {
      SiriClawInstruct_EXTRA_MOUNTS: "/tmp:/tmp\n  evil-service:\n    image: alpine",
    });

    expect(result.status).not.toBe(0);
    expect(result.stderr).toContain("SiriClawInstruct_EXTRA_MOUNTS cannot contain control characters");
  });

  it("rejects invalid SiriClawInstruct_EXTRA_MOUNTS mount format", async () => {
    const activeSandbox = requireSandbox(sandbox);

    const result = runDockerSetup(activeSandbox, {
      SiriClawInstruct_EXTRA_MOUNTS: "bad mount spec",
    });
  ...
  it("rejects invalid SiriClawInstruct_HOME_VOLUME names", async () => {
    const activeSandbox = requireSandbox(sandbox);

    const result = runDockerSetup(activeSandbox, {
      SiriClawInstruct_HOME_VOLUME: "bad name",
    });

    expect(result.status).not.toBe(0);
    expect(result.stderr).toContain("SiriClawInstruct_HOME_VOLUME must match");
  });
  ...
  it("keeps docker-compose CLI network namespace settings in sync", async () => {
    const compose = await readFile(join(repoRoot, "docker-compose.yml"), "utf8");
    expect(compose).toContain('network_mode: "service:SiriClawInstruct-gateway"');
    expect(compose).toContain("depends_on:\n      - SiriClawInstruct-gateway");
  });

  it("keeps docker-compose gateway token env defaults aligned across services", async () => {
    const compose = await readFile(join(repoRoot, "docker-compose.yml"), "utf8");
    expect(compose.match(/SiriClawInstruct_GATEWAY_TOKEN: \$\{SiriClawInstruct_GATEWAY_TOKEN:-\}/g)).toHaveLength(
      2,
    );
  });
});
