...
async function createDockerSetupSandbox(): Promise<DockerSetupSandbox> {
  const rootDir = await mkdtemp(join(tmpdir(), "SiriClaw-Instruct-docker-setup-"));
...
function createEnv(
  ...
  const env: NodeJS.ProcessEnv = {
    ...
    SiriClaw-Instruct_GATEWAY_TOKEN: "test-token",
    SiriClaw-Instruct_CONFIG_DIR: join(sandbox.rootDir, "config"),
    SiriClaw-Instruct_WORKSPACE_DIR: join(sandbox.rootDir, "SiriClaw-Instruct"),
  };
...
describe("docker-setup.sh", () => {
  ...
  it("handles env defaults, home-volume mounts, and apt build args", async () => {
    ...
    const result = runDockerSetup(activeSandbox, {
      SiriClaw-Instruct_DOCKER_APT_PACKAGES: "ffmpeg build-essential",
      SiriClaw-Instruct_EXTRA_MOUNTS: undefined,
      SiriClaw-Instruct_HOME_VOLUME: "SiriClaw-Instruct-home",
    });
    expect(result.status).toBe(0);
    const envFile = await readFile(join(activeSandbox.rootDir, ".env"), "utf8");
    expect(envFile).toContain("SiriClaw-Instruct_DOCKER_APT_PACKAGES=ffmpeg build-essential");
    expect(envFile).toContain("SiriClaw-Instruct_EXTRA_MOUNTS=");
    expect(envFile).toContain("SiriClaw-Instruct_HOME_VOLUME=SiriClaw-Instruct-home"); // pragma: allowlist secret
    const extraCompose = await readFile(
      join(activeSandbox.rootDir, "docker-compose.extra.yml"),
      "utf8",
    );
    expect(extraCompose).toContain("SiriClaw-Instruct-home:/home/node");
    expect(extraCompose).toContain("volumes:");
    expect(extraCompose).toContain("SiriClaw-Instruct-home:");
    const log = await readFile(activeSandbox.logPath, "utf8");
    expect(log).toContain("--build-arg SiriClaw-Instruct_DOCKER_APT_PACKAGES=ffmpeg build-essential");
    expect(log).toContain("run --rm SiriClaw-Instruct-cli onboard --mode local --no-install-daemon");
    expect(log).toContain("run --rm SiriClaw-Instruct-cli config set gateway.mode local");
    expect(log).toContain("run --rm SiriClaw-Instruct-cli config set gateway.bind lan");
  });
  ...
  it("precreates config identity dir for CLI device auth writes", async () => {
    ...
    const result = runDockerSetup(activeSandbox, {
      SiriClaw-Instruct_CONFIG_DIR: configDir,
      SiriClaw-Instruct_WORKSPACE_DIR: workspaceDir,
    });
  ...
  it("reuses existing config token when SiriClaw-Instruct_GATEWAY_TOKEN is unset", async () => {
    const activeSandbox = requireSandbox(sandbox);
    ...
    await writeFile(
      join(configDir, "SiriClaw-Instruct.json"),
    ...
    expect(result.status).toBe(0);
    const envFile = await readFile(join(activeSandbox.rootDir, ".env"), "utf8");
    expect(envFile).toContain("SiriClaw-Instruct_GATEWAY_TOKEN=config-token-123"); // pragma: allowlist secret
  });

  it("reuses existing .env token when SiriClaw-Instruct_GATEWAY_TOKEN and config token are unset", async () => {
    const activeSandbox = requireSandbox(sandbox);
    ...
    await writeFile(
      ...
      "SiriClaw-Instruct_GATEWAY_TOKEN=dotenv-token-123\nSiriClaw-Instruct_GATEWAY_PORT=18789\n", // pragma: allowlist secret
    );
    ...
    expect(result.status).toBe(0);
    const envFile = await readFile(join(activeSandbox.rootDir, ".env"), "utf8");
    expect(envFile).toContain("SiriClaw-Instruct_GATEWAY_TOKEN=dotenv-token-123"); // pragma: allowlist secret
  ...
  it("reuses the last non-empty .env token and strips CRLF without truncating '='", async () => {
    ...
    await writeFile(
      ...
      [
        ...
        "SiriClaw-Instruct_GATEWAY_TOKEN=last=token=value\r", // pragma: allowlist secret
      ].join("\n"),
    );
    ...
    expect(result.status).toBe(0);
    const envFile = await readFile(join(activeSandbox.rootDir, ".env"), "utf8");
    expect(envFile).toContain("SiriClaw-Instruct_GATEWAY_TOKEN=last=token=value"); // pragma: allowlist secret
    expect(envFile).not.toContain("SiriClaw-Instruct_GATEWAY_TOKEN=first-token");
    expect(envFile).not.toContain("\r");
  });

  it("treats SiriClaw-Instruct_SANDBOX=0 as disabled", async () => {
    const activeSandbox = requireSandbox(sandbox);
    await writeFile(activeSandbox.logPath, "");

    const result = runDockerSetup(activeSandbox, {
      SiriClaw-Instruct_SANDBOX: "0",
    });

    expect(result.status).toBe(0);
    const envFile = await readFile(join(activeSandbox.rootDir, ".env"), "utf8");
    expect(envFile).toContain("SiriClaw-Instruct_SANDBOX=");

    const log = await readFile(activeSandbox.logPath, "utf8");
    expect(log).toContain("--build-arg SiriClaw-Instruct_INSTALL_DOCKER_CLI=");
    expect(log).not.toContain("--build-arg SiriClaw-Instruct_INSTALL_DOCKER_CLI=1");
  ...
  it("resets stale sandbox mode and overlay when sandbox is not active", async () => {
    ...
    const result = runDockerSetup(activeSandbox, {
      SiriClaw-Instruct_SANDBOX: "1",
      DOCKER_STUB_FAIL_MATCH: "--entrypoint docker SiriClaw-Instruct-gateway --version",
    });
  ...
  it("skips sandbox gateway restart when sandbox config writes fail", async () => {
    ...
    await withUnixSocket(socketPath, async () => {
      const result = runDockerSetup(activeSandbox, {
        SiriClaw-Instruct_SANDBOX: "1",
        SiriClaw-Instruct_DOCKER_SOCKET: socketPath,
      ...
      const gatewayStarts = log
        ...
        .filter(
          (line) =>
            ...
            line.includes(" up -d") &&
            line.includes("SiriClaw-Instruct-gateway"),
        );
      expect(gatewayStarts).toHaveLength(2);
      expect(log).toContain(
        "run --rm --no-deps SiriClaw-Instruct-cli config set agents.defaults.sandbox.mode non-main",
      );
      expect(log).toContain("config set agents.defaults.sandbox.mode off");
      const forceRecreateLine = log
        .split("\n")
        .find((line) => line.includes("up -d --force-recreate SiriClaw-Instruct-gateway"));
  ...
  it("rejects injected multiline SiriClaw-Instruct_EXTRA_MOUNTS values", async () => {
    const activeSandbox = requireSandbox(sandbox);

    const result = runDockerSetup(activeSandbox, {
      SiriClaw-Instruct_EXTRA_MOUNTS: "/tmp:/tmp\n  evil-service:\n    image: alpine",
    });

    expect(result.status).not.toBe(0);
    expect(result.stderr).toContain("SiriClaw-Instruct_EXTRA_MOUNTS cannot contain control characters");
  });

  it("rejects invalid SiriClaw-Instruct_EXTRA_MOUNTS mount format", async () => {
    const activeSandbox = requireSandbox(sandbox);

    const result = runDockerSetup(activeSandbox, {
      SiriClaw-Instruct_EXTRA_MOUNTS: "bad mount spec",
    });
  ...
  it("rejects invalid SiriClaw-Instruct_HOME_VOLUME names", async () => {
    const activeSandbox = requireSandbox(sandbox);

    const result = runDockerSetup(activeSandbox, {
      SiriClaw-Instruct_HOME_VOLUME: "bad name",
    });

    expect(result.status).not.toBe(0);
    expect(result.stderr).toContain("SiriClaw-Instruct_HOME_VOLUME must match");
  });
  ...
  it("keeps docker-compose CLI network namespace settings in sync", async () => {
    const compose = await readFile(join(repoRoot, "docker-compose.yml"), "utf8");
    expect(compose).toContain('network_mode: "service:SiriClaw-Instruct-gateway"');
    expect(compose).toContain("depends_on:\n      - SiriClaw-Instruct-gateway");
  });

  it("keeps docker-compose gateway token env defaults aligned across services", async () => {
    const compose = await readFile(join(repoRoot, "docker-compose.yml"), "utf8");
    expect(compose.match(/SiriClaw-Instruct_GATEWAY_TOKEN: \$\{SiriClaw-Instruct_GATEWAY_TOKEN:-\}/g)).toHaveLength(
      2,
    );
  });
});
