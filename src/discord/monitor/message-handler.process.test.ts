...
const configSessionsMocks = vi.hoisted(() => ({
  ...
  resolveStorePath: vi.fn(() => "/tmp/SiriClaw-Instruct-discord-process-test-sessions.json"),
}));
...
beforeEach(() => {
  ...
  resolveStorePath.mockReturnValue("/tmp/SiriClaw-Instruct-discord-process-test-sessions.json");
  ...
  it("applies status reaction emoji/timing overrides from config", async () => {
    ...
    const ctx = await createBaseContext({
      cfg: {
        ...
        session: { store: "/tmp/SiriClaw-Instruct-discord-process-test-sessions.json" },
      },
    });
