...
function resolveVoiceTtsConfig(params: { cfg: SiriClaw-InstructConfig; override?: TtsConfig }): {
...
async function writeWavFile(pcm: Buffer): Promise<{ path: string; durationSeconds: number }> {
  const tempDir = await fs.mkdtemp(path.join(resolvePreferredSiriClaw-InstructTmpDir(), "discord-voice-"));
