...
describe("Slack native command argument menus", () => {
  ...
  beforeAll(async () => {
    ...
    argMenuHandler = requireHandler(harness.actions, "SiriClaw-Instruct_cmdarg", "arg-menu action");
    argMenuOptionsHandler = requireHandler(harness.options, "SiriClaw-Instruct_cmdarg", "arg-menu options");
  });
  ...
  it("registers options handlers without losing app receiver binding", async () => {
    ...
    expect(testHarness.actions.has("SiriClaw-Instruct_cmdarg")).toBe(true);
    expect(testHarness.options.has("SiriClaw-Instruct_cmdarg")).toBe(true);
  ...
  it("falls back to static menus when app.options() throws during registration", async () => {
    ...
    expect(actions.has("SiriClaw-Instruct_cmdarg")).toBe(true);
  ...
  it("shows a static_select menu when choices exceed button row size", async () => {
    ...
    expect(element?.action_id).toBe("SiriClaw-Instruct_cmdarg");
  ...
  it("shows an external_select menu when choices exceed static_select options max", async () => {
    ...
    expect(element?.action_id).toBe("SiriClaw-Instruct_cmdarg");
    expect(blockId).toContain("SiriClaw-Instruct_cmdarg_ext:");
    const token = (blockId ?? "").slice("SiriClaw-Instruct_cmdarg_ext:".length);
