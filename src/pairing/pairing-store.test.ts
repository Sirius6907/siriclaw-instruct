...
beforeAll(async () => {
  fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "SiriClaw-Instruct-pairing-"));
});
...
async function withTempStateDir<T>(fn: (stateDir: string) => Promise<T>) {
  ...
  return await withEnvAsync({ SiriClaw-Instruct_STATE_DIR: dir }, async () => await fn(dir));
}
