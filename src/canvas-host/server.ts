...
<title>SiriClaw-Instruct Canvas</title>
<style>
...
<div class="wrap">
  <div class="card">
    <div class="title">
      <h1>SiriClaw-Instruct Canvas</h1>
...
(() => {
  ...
  const hasIOS = () =>
    !!(
      ...
      window.webkit.messageHandlers.SiriClaw-InstructCanvasA2UIAction
    );
  const hasAndroid = () =>
    !!(
      (window.SiriClaw-InstructCanvasA2UIAction &&
        typeof window.SiriClaw-InstructCanvasA2UIAction.postMessage === "function")
    );
  const hasHelper = () => typeof window.SiriClaw-InstructSendUserAction === "function";
  statusEl.innerHTML =
    "Bridge: " +
  ...
  window.addEventListener("SiriClaw-Instruct:a2ui-action-status", onStatus);

  function send(name, sourceComponentId) {
    if (!hasHelper()) {
      log("No action bridge found. Ensure you're viewing this on an iOS/Android SiriClaw-Instruct node canvas.");
      return;
    }
    const sendUserAction =
      typeof window.SiriClaw-InstructSendUserAction === "function"
        ? window.SiriClaw-InstructSendUserAction
        : undefined;
...
function isDisabledByEnv() {
  if (isTruthyEnvValue(process.env.SiriClaw-Instruct_SKIP_CANVAS_HOST)) {
    return true;
  }
  if (isTruthyEnvValue(process.env.SiriClaw-Instruct_SKIP_CANVAS_HOST)) {
    return true;
    ...
    try {
      ...
      if (!opened) {
        if (urlPath === "/" || urlPath.endsWith("/")) {
          ...
          res.end(
            `<!doctype html><meta charset="utf-8" /><title>SiriClaw-Instruct Canvas</title><pre>Missing file.\nCreate ${rootDir}/index.html</pre>`,
          );
          return true;
        }
        res.statusCode = 404;
