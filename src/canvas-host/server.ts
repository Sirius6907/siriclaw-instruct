...
<title>SiriClawInstruct Canvas</title>
<style>
...
(() => {
  ...
  const hasIOS = () =>
    !!(
      ...
      window.webkit.messageHandlers.SiriClawInstructCanvasA2UIAction
    );
  const hasAndroid = () =>
    !!(
      (window.SiriClawInstructCanvasA2UIAction &&
        typeof window.SiriClawInstructCanvasA2UIAction.postMessage === "function")
    );
  const hasHelper = () => typeof window.SiriClawInstructSendUserAction === "function";
  statusEl.innerHTML =
    "Bridge: " +
  ...
  window.addEventListener("SiriClawInstruct:a2ui-action-status", onStatus);

  function send(name, sourceComponentId) {
    if (!hasHelper()) {
      log("No action bridge found. Ensure you're viewing this on an iOS/Android SiriClawInstruct node canvas.");
      return;
    }
    const sendUserAction =
      typeof window.SiriClawInstructSendUserAction === "function"
        ? window.SiriClawInstructSendUserAction
        : undefined;
...
function isDisabledByEnv() {
  if (isTruthyEnvValue(process.env.SiriClawInstruct_SKIP_CANVAS_HOST)) {
    return true;
  }
  if (isTruthyEnvValue(process.env.SiriClawInstruct_SKIP_CANVAS_HOST)) {
    return true;
    ...
    try {
      ...
      if (!opened) {
        if (urlPath === "/" || urlPath.endsWith("/")) {
          ...
          res.end(
            `<!doctype html><meta charset="utf-8" /><title>SiriClawInstruct Canvas</title><pre>Missing file.\nCreate ${rootDir}/index.html</pre>`,
          );
          return true;
        }
        res.statusCode = 404;
