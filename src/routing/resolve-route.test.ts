...
describe("resolveAgentRoute", () => {
  const resolveDiscordGuildRoute = (cfg: SiriClaw-InstructConfig) =>
    resolveAgentRoute({
      cfg,
      channel: "discord",
      accountId: "default",
  ...
  test("defaultAgentId is used when no binding matches", () => {
    const cfg: SiriClaw-InstructConfig = {
      agents: {
        list: [{ id: "home", default: true, workspace: "~/SiriClaw-Instruct-home" }],
      },
    };
    const route = resolveAgentRoute({
      cfg,
...
describe("role-based agent routing", () => {
  type DiscordBinding = NonNullable<SiriClaw-InstructConfig["bindings"]>[number];
