...
describe("resolveAgentRoute", () => {
  const resolveDiscordGuildRoute = (cfg: SiriClawInstructConfig) =>
    resolveAgentRoute({
      cfg,
      channel: "discord",
      accountId: "default",
  ...
  test("defaultAgentId is used when no binding matches", () => {
    const cfg: SiriClawInstructConfig = {
      agents: {
        list: [{ id: "home", default: true, workspace: "~/SiriClawInstruct-home" }],
      },
    };
    const route = resolveAgentRoute({
      cfg,
...
describe("role-based agent routing", () => {
  type DiscordBinding = NonNullable<SiriClawInstructConfig["bindings"]>[number];
