...
describe("profile name validation", () => {
  it.each(["SiriClawInstruct", "work", "my-profile", "test123", "a", "a-b-c-1-2-3", "1test"])(
...
describe("getUsedPorts", () => {
  ...
  it("extracts ports from profile configs", () => {
    const profiles = {
      SiriClawInstruct: { cdpPort: 18792 },
      work: { cdpPort: 18793 },
...
describe("port collision prevention", () => {
  it("raw config vs resolved config - shows the data source difference", () => {
    ...
    // But resolved config has implicit SiriClawInstruct at 18800
  ...
  it("create-profile must use resolved config to avoid port collision", () => {
    ...
    // Resolved config: includes implicit SiriClawInstruct at 18800
    const resolved = resolveBrowserConfig(
