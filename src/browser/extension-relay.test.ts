...
describe("chrome extension relay server", () => {
  ...
  beforeEach(() => {
    envSnapshot = captureEnv([
      ...
      "SiriClaw-Instruct_EXTENSION_RELAY_RECONNECT_GRACE_MS",
      "SiriClaw-Instruct_EXTENSION_RELAY_COMMAND_RECONNECT_WAIT_MS",
    ]);
    process.env.SiriClaw-Instruct_GATEWAY_TOKEN = TEST_GATEWAY_TOKEN;
    delete process.env.SiriClaw-Instruct_EXTENSION_RELAY_RECONNECT_GRACE_MS;
    delete process.env.SiriClaw-Instruct_EXTENSION_RELAY_COMMAND_RECONNECT_WAIT_MS;
  });
  ...
  it("uses relay-scoped token only for known relay ports", async () => {
    ...
    expect(Object.keys(headers)).toContain("x-SiriClaw-Instruct-relay-token");
    expect(headers["x-SiriClaw-Instruct-relay-token"]).not.toBe(TEST_GATEWAY_TOKEN);
  });
  ...
  it("allows CORS preflight from chrome-extension origins", async () => {
    ...
    const res = await fetch(`${sharedUrl}/json/version`, {
      ...
      headers: {
        ...
        "Access-Control-Request-Headers": "x-SiriClaw-Instruct-relay-token",
      },
    });
    ...
    expect(res.headers.get("access-control-allow-headers") ?? "").toContain(
      "x-SiriClaw-Instruct-relay-token",
    );
  });
  ...
  it("closes CDP clients after reconnect grace when extension stays disconnected", async () => {
    process.env.SiriClaw-Instruct_EXTENSION_RELAY_RECONNECT_GRACE_MS = "150";
  ...
  it("stops advertising websocket endpoint after reconnect grace expires", async () => {
    process.env.SiriClaw-Instruct_EXTENSION_RELAY_RECONNECT_GRACE_MS = "120";
  ...
  it("accepts /json endpoints with relay token query param", async () => {
    ...
    const token = relayAuthHeaders(sharedUrl)["x-SiriClaw-Instruct-relay-token"];
    expect(token).toBeTruthy();
  ...
  it("accepts raw gateway token for relay auth compatibility", async () => {
    ...
    const versionRes = await fetch(`${sharedUrl}/json/version`, {
      headers: { "x-SiriClaw-Instruct-relay-token": TEST_GATEWAY_TOKEN },
    });
    expect(versionRes.status).toBe(200);
  ...
  it(
    ...
    async () => {
      process.env.SiriClaw-Instruct_EXTENSION_RELAY_RECONNECT_GRACE_MS = "200";
  ...
  it(
    ...
    async () => {
      process.env.SiriClaw-Instruct_EXTENSION_RELAY_RECONNECT_GRACE_MS = "2000";
  ...
  it("does not swallow EADDRINUSE when occupied port is not an SiriClaw-Instruct relay", async () => {
    const port = await getFreePort();
