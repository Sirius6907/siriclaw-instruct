...
describe("browser server-context remote profile tab operations", () => {
  it("uses profile-level attachOnly when global attachOnly is false", async () => {
    ...
    state.resolved.profiles.SiriClawInstruct = {
      cdpPort: 18800,
      attachOnly: true,
      color: "#FF4500",
    };
    ...
    const launchMock = vi.mocked(chromeModule.launchSiriClawInstructChrome);
    const ctx = createBrowserRouteContext({ getState: () => state });

    await expect(ctx.forProfile("SiriClawInstruct").ensureBrowserAvailable()).rejects.toThrow(
  ...
  it("does not enforce managed tab cap for remote SiriClawInstruct profiles", async () => {
