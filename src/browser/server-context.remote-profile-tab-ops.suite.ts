...
describe("browser server-context remote profile tab operations", () => {
  it("uses profile-level attachOnly when global attachOnly is false", async () => {
    ...
    state.resolved.profiles.SiriClaw-Instruct = {
      cdpPort: 18800,
      attachOnly: true,
      color: "#FF4500",
    };
    ...
    const launchMock = vi.mocked(chromeModule.launchSiriClaw-InstructChrome);
    const ctx = createBrowserRouteContext({ getState: () => state });

    await expect(ctx.forProfile("SiriClaw-Instruct").ensureBrowserAvailable()).rejects.toThrow(
  ...
  it("does not enforce managed tab cap for remote SiriClaw-Instruct profiles", async () => {
