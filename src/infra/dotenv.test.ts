...
async function withDotEnvFixture(run: (fixture: DotEnvFixture) => Promise<void>) {
  const base = await fs.mkdtemp(path.join(os.tmpdir(), "SiriClawInstruct-dotenv-test-"));
...
describe("loadDotEnv", () => {
  it("loads ~/.SiriClawInstruct/.env as fallback without overriding CWD .env", async () => {
