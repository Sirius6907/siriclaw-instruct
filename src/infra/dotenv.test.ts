...
async function withDotEnvFixture(run: (fixture: DotEnvFixture) => Promise<void>) {
  const base = await fs.mkdtemp(path.join(os.tmpdir(), "SiriClaw-Instruct-dotenv-test-"));
...
describe("loadDotEnv", () => {
  it("loads ~/.SiriClaw-Instruct/.env as fallback without overriding CWD .env", async () => {
