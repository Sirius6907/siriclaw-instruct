...
import { POSIX_SiriClawInstruct_TMP_DIR, resolvePreferredSiriClawInstructTmpDir } from "./tmp-siriclaw-instruct-dir.js";

type TmpDirOptions = NonNullable<Parameters<typeof resolvePreferredSiriClawInstructTmpDir>[0]>;

function fallbackTmp(uid = 501) {
  return path.join("/var/fallback", `SiriClawInstruct-${uid}`);
}
...
function resolveWithReadOnlyTmpFallback(params: {
  ...
  return resolvePreferredSiriClawInstructTmpDir({
    accessSync: readOnlyTmpAccessSync(),
    lstatSync: vi.fn((target: string) => {
      if (target === POSIX_SiriClawInstruct_TMP_DIR) {
...
function resolveWithMocks(params: {
  ...
  const resolved = resolvePreferredSiriClawInstructTmpDir({
    accessSync,
    chmodSync,
...
describe("resolvePreferredSiriClawInstructTmpDir", () => {
  it("prefers /tmp/SiriClawInstruct when it already exists and is writable", () => {
    ...
    expect(resolved).toBe(POSIX_SiriClawInstruct_TMP_DIR);
    expect(tmpdir).not.toHaveBeenCalled();
  });

  it("prefers /tmp/SiriClawInstruct when it does not exist but /tmp is writable", () => {
    ...
    expect(mkdirSync).toHaveBeenCalledWith(POSIX_SiriClawInstruct_TMP_DIR, expect.any(Object));
    expect(tmpdir).not.toHaveBeenCalled();
  });

  it("falls back to os.tmpdir()/SiriClawInstruct when /tmp/SiriClawInstruct is not a directory", () => {
  ...
  it("falls back to os.tmpdir()/SiriClawInstruct when /tmp is not writable", () => {
  ...
  it("falls back when /tmp/SiriClawInstruct is a symlink", () => {
    expectFallsBackToOsTmpDir({ lstatSync: symlinkTmpDirLstat() });
  });

  it("falls back when /tmp/SiriClawInstruct is not owned by the current user", () => {
    expectFallsBackToOsTmpDir({ lstatSync: vi.fn(() => makeDirStat({ uid: 0 })) });
  });

  it("falls back when /tmp/SiriClawInstruct is group/other writable", () => {
  ...
  it("throws when fallback path is a symlink", () => {
    ...
    ).toThrow(/Unsafe fallback SiriClawInstruct temp dir/);
  });
