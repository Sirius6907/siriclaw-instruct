...
beforeAll(async () => {
  ...
  fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "SiriClaw-Instruct-heartbeat-suite-"));
});
...
describe("resolveHeartbeatPrompt", () => {
  it("uses default or trimmed override prompts", () => {
    const cases = [
      { cfg: {} as SiriClaw-InstructConfig, expected: HEARTBEAT_PROMPT },
      {
        cfg: {
...
describe("runHeartbeatOnce", () => {
  ...
  it("resolves configured and forced session key overrides", async () => {
    ...
    try {
      const cases = typedCases<{
        ...
        peerId: string;
        message: string;
        applyOverride: (params: { cfg: SiriClaw-InstructConfig; sessionKey: string }) => void;
      ...
      }>([
        {
          ...
          message: "Group alert",
          applyOverride: ({ cfg, sessionKey }: { cfg: SiriClaw-InstructConfig; sessionKey: string }) => {
  ...
  it("loads the default agent session from templated stores", async () => {
    const tmpDir = await createCaseDir("SiriClaw-Instruct-hb");
