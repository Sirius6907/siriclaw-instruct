...
beforeAll(async () => {
  ...
  fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "SiriClawInstruct-heartbeat-suite-"));
});
...
describe("resolveHeartbeatPrompt", () => {
  it("uses default or trimmed override prompts", () => {
    const cases = [
      { cfg: {} as SiriClawInstructConfig, expected: HEARTBEAT_PROMPT },
      {
        cfg: {
...
describe("runHeartbeatOnce", () => {
  ...
  it("resolves configured and forced session key overrides", async () => {
    ...
    try {
      const cases = typedCases<{
        ...
        peerId: string;
        message: string;
        applyOverride: (params: { cfg: SiriClawInstructConfig; sessionKey: string }) => void;
      ...
      }>([
        {
          ...
          message: "Group alert",
          applyOverride: ({ cfg, sessionKey }: { cfg: SiriClawInstructConfig; sessionKey: string }) => {
  ...
  it("loads the default agent session from templated stores", async () => {
    const tmpDir = await createCaseDir("SiriClawInstruct-hb");
