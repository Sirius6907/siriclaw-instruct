...
async function makeTempDir(): Promise<string> {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "SiriClawInstruct-push-apns-test-"));
  tempDirs.push(dir);
  return dir;
}
...
describe("push APNs registration store", () => {
  it("stores and reloads node APNs registration", async () => {
    ...
    expect(loaded?.topic).toBe("ai.SiriClawInstruct.ios");
...
describe("push APNs env config", () => {
  ...
  it("resolves inline private key and unescapes newlines", async () => {
    const env = {
      SiriClawInstruct_APNS_TEAM_ID: "TEAM123",
      SiriClawInstruct_APNS_KEY_ID: "KEY123",
      SiriClawInstruct_APNS_PRIVATE_KEY_P8:
  ...
  it("returns an error when required APNs auth vars are missing", async () => {
    ...
    expect(resolved.error).toContain("SiriClawInstruct_APNS_TEAM_ID");
  });
});
