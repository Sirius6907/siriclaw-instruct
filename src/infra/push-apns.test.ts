...
async function makeTempDir(): Promise<string> {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "SiriClaw-Instruct-push-apns-test-"));
  tempDirs.push(dir);
  return dir;
}
...
describe("push APNs registration store", () => {
  it("stores and reloads node APNs registration", async () => {
    ...
    const saved = await registerApnsToken({
      ...
      topic: "ai.SiriClaw-Instruct.ios",
      environment: "sandbox",
      baseDir,
    });
    ...
    expect(loaded?.topic).toBe("ai.SiriClaw-Instruct.ios");
...
describe("push APNs env config", () => {
  ...
  it("resolves inline private key and unescapes newlines", async () => {
    const env = {
      SiriClaw-Instruct_APNS_TEAM_ID: "TEAM123",
      SiriClaw-Instruct_APNS_KEY_ID: "KEY123",
      SiriClaw-Instruct_APNS_PRIVATE_KEY_P8:
  ...
  it("returns an error when required APNs auth vars are missing", async () => {
    ...
    expect(resolved.error).toContain("SiriClaw-Instruct_APNS_TEAM_ID");
  });
});
