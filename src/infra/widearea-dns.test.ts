...
describe("wide-area DNS-SD zone rendering", () => {
  it("renders a zone with gateway PTR/SRV/TXT records", () => {
    const txt = renderWideAreaGatewayZoneText({
      ...
      serial: 2025121701,
      gatewayPort: 18789,
      displayName: "Mac Studio (SiriClaw-Instruct)",
    ...
    expect(txt).toContain(`$ORIGIN SiriClaw-Instruct.internal.`);
    ...
    expect(txt).toContain(`_SiriClaw-Instruct-gw._tcp IN PTR studio-london._SiriClaw-Instruct-gw._tcp`);
    expect(txt).toContain(`studio-london._SiriClaw-Instruct-gw._tcp IN SRV 0 0 18789 studio-london`);
    expect(txt).toContain(`displayName=Mac Studio (SiriClaw-Instruct)`);
    expect(txt).toContain(`gatewayPort=18789`);
    expect(txt).toContain(`sshPort=22`);
    expect(txt).toContain(`cliPath=/opt/homebrew/bin/SiriClaw-Instruct`);
  });
