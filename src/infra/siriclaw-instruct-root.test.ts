...
const VITEST_FS_BASE = path.join(path.parse(process.cwd()).root, "__SiriClawInstruct_vitest__");
const FIXTURE_BASE = path.join(VITEST_FS_BASE, "SiriClawInstruct-root");
...
describe("resolveSiriClawInstructPackageRoot", () => {
  let resolveSiriClawInstructPackageRoot: typeof import("./SiriClawInstruct-root.js").resolveSiriClawInstructPackageRoot;
  let resolveSiriClawInstructPackageRootSync: typeof import("./SiriClawInstruct-root.js").resolveSiriClawInstructPackageRootSync;

  beforeAll(async () => {
    ({ resolveSiriClawInstructPackageRoot, resolveSiriClawInstructPackageRootSync } =
      await import("./SiriClawInstruct-root.js"));
  });
  ...
  it("resolves package root from .bin argv1", async () => {
    ...
    const argv1 = path.join(project, "node_modules", ".bin", "SiriClawInstruct");
    const pkgRoot = path.join(project, "node_modules", "SiriClawInstruct");
    setFile(path.join(pkgRoot, "package.json"), JSON.stringify({ name: "SiriClawInstruct" }));

    expect(resolveSiriClawInstructPackageRootSync({ argv1 })).toBe(pkgRoot);
  });

  it("resolves package root via symlinked argv1", async () => {
    const project = fx("symlink-scenario");
    const bin = path.join(project, "bin", "SiriClawInstruct");
    const realPkg = path.join(project, "real-pkg");
    state.realpaths.set(abs(bin), abs(path.join(realPkg, "SiriClawInstruct.mjs")));
    setFile(path.join(realPkg, "package.json"), JSON.stringify({ name: "SiriClawInstruct" }));

    expect(resolveSiriClawInstructPackageRootSync({ argv1: bin })).toBe(realPkg);
  });
  ...
  it("prefers moduleUrl candidates", async () => {
    ...
    expect(resolveSiriClawInstructPackageRootSync({ moduleUrl })).toBe(pkgRoot);
  });

  it("returns null for non-SiriClawInstruct package roots", async () => {
    const pkgRoot = fx("not-SiriClawInstruct");
    setFile(path.join(pkgRoot, "package.json"), JSON.stringify({ name: "not-SiriClawInstruct" }));

    expect(resolveSiriClawInstructPackageRootSync({ cwd: pkgRoot })).toBeNull();
  });
  ...
  it("async resolver matches sync behavior", async () => {
    ...
    await expect(resolveSiriClawInstructPackageRoot({ cwd: pkgRoot })).resolves.toBe(pkgRoot);
  });

  it("async resolver returns null when no package roots exist", async () => {
    await expect(resolveSiriClawInstructPackageRoot({ cwd: fx("missing") })).resolves.toBeNull();
  });
});
