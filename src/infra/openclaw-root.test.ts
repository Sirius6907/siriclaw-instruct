...
const VITEST_FS_BASE = path.join(path.parse(process.cwd()).root, "__SiriClaw-Instruct_vitest__");
const FIXTURE_BASE = path.join(VITEST_FS_BASE, "SiriClaw-Instruct-root");
...
describe("resolveSiriClaw-InstructPackageRoot", () => {
  let resolveSiriClaw-InstructPackageRoot: typeof import("./SiriClaw-Instruct-root.js").resolveSiriClaw-InstructPackageRoot;
  let resolveSiriClaw-InstructPackageRootSync: typeof import("./SiriClaw-Instruct-root.js").resolveSiriClaw-InstructPackageRootSync;

  beforeAll(async () => {
    ({ resolveSiriClaw-InstructPackageRoot, resolveSiriClaw-InstructPackageRootSync } =
      await import("./SiriClaw-Instruct-root.js"));
  });
  ...
  it("resolves package root from .bin argv1", async () => {
    ...
    const argv1 = path.join(project, "node_modules", ".bin", "SiriClaw-Instruct");
    const pkgRoot = path.join(project, "node_modules", "SiriClaw-Instruct");
    setFile(path.join(pkgRoot, "package.json"), JSON.stringify({ name: "SiriClaw-Instruct" }));

    expect(resolveSiriClaw-InstructPackageRootSync({ argv1 })).toBe(pkgRoot);
  });

  it("resolves package root via symlinked argv1", async () => {
    const project = fx("symlink-scenario");
    const bin = path.join(project, "bin", "SiriClaw-Instruct");
    const realPkg = path.join(project, "real-pkg");
    state.realpaths.set(abs(bin), abs(path.join(realPkg, "SiriClaw-Instruct.mjs")));
    setFile(path.join(realPkg, "package.json"), JSON.stringify({ name: "SiriClaw-Instruct" }));

    expect(resolveSiriClaw-InstructPackageRootSync({ argv1: bin })).toBe(realPkg);
  });
  ...
  it("prefers moduleUrl candidates", async () => {
    ...
    expect(resolveSiriClaw-InstructPackageRootSync({ moduleUrl })).toBe(pkgRoot);
  });

  it("returns null for non-SiriClaw-Instruct package roots", async () => {
    const pkgRoot = fx("not-SiriClaw-Instruct");
    setFile(path.join(pkgRoot, "package.json"), JSON.stringify({ name: "not-SiriClaw-Instruct" }));

    expect(resolveSiriClaw-InstructPackageRootSync({ cwd: pkgRoot })).toBeNull();
  });
  ...
  it("async resolver matches sync behavior", async () => {
    ...
    await expect(resolveSiriClaw-InstructPackageRoot({ cwd: pkgRoot })).resolves.toBe(pkgRoot);
  });

  it("async resolver returns null when no package roots exist", async () => {
    await expect(resolveSiriClaw-InstructPackageRoot({ cwd: fx("missing") })).resolves.toBeNull();
  });
});
