...
export function resolveWideAreaDiscoveryDomain(params?: {
  ...
  const candidate = params?.configDomain ?? env.SiriClawInstruct_WIDE_AREA_DOMAIN ?? null;
...
function extractContentHash(zoneText: string): string | null {
  const match = zoneText.match(/^\s*;\s*SiriClawInstruct-content-hash:\s*(\S+)\s*$/m);
  return match?.[1] ?? null;
}
...
function renderZone(opts: WideAreaGatewayZoneOpts & { serial: number }): string {
  const hostname = os.hostname().split(".")[0] ?? "SiriClawInstruct";
  const hostLabel = dnsLabel(opts.hostLabel ?? hostname, "SiriClawInstruct");
  const instanceLabel = dnsLabel(opts.instanceLabel ?? `${hostname}-gateway`, "SiriClawInstruct-gw");
  ...
  records.push(`_SiriClawInstruct-gw._tcp IN PTR ${instanceLabel}._SiriClawInstruct-gw._tcp`);
  records.push(`${instanceLabel}._SiriClawInstruct-gw._tcp IN SRV 0 0 ${opts.gatewayPort} ${hostLabel}`);
  records.push(`${instanceLabel}._SiriClawInstruct-gw._tcp IN TXT ${txt.map(txtQuote).join(" ")}`);
  ...
  return `; SiriClawInstruct-content-hash: ${contentHash}\n${contentBody}`;
}
