...
export function resolveWideAreaDiscoveryDomain(params?: {
  ...
  const candidate = params?.configDomain ?? env.SiriClaw-Instruct_WIDE_AREA_DOMAIN ?? null;
...
function extractContentHash(zoneText: string): string | null {
  const match = zoneText.match(/^\s*;\s*SiriClaw-Instruct-content-hash:\s*(\S+)\s*$/m);
  return match?.[1] ?? null;
}
...
function renderZone(opts: WideAreaGatewayZoneOpts & { serial: number }): string {
  const hostname = os.hostname().split(".")[0] ?? "SiriClaw-Instruct";
  const hostLabel = dnsLabel(opts.hostLabel ?? hostname, "SiriClaw-Instruct");
  const instanceLabel = dnsLabel(opts.instanceLabel ?? `${hostname}-gateway`, "SiriClaw-Instruct-gw");
  ...
  records.push(`_SiriClaw-Instruct-gw._tcp IN PTR ${instanceLabel}._SiriClaw-Instruct-gw._tcp`);
  records.push(`${instanceLabel}._SiriClaw-Instruct-gw._tcp IN SRV 0 0 ${opts.gatewayPort} ${hostLabel}`);
  records.push(`${instanceLabel}._SiriClaw-Instruct-gw._tcp IN TXT ${txt.map(txtQuote).join(" ")}`);
  ...
  return `; SiriClaw-Instruct-content-hash: ${contentHash}\n${contentBody}`;
}
