...
import { POSIX_SiriClaw-Instruct_TMP_DIR, resolvePreferredSiriClaw-InstructTmpDir } from "./tmp-SiriClaw-Instruct-dir.js";

type TmpDirOptions = NonNullable<Parameters<typeof resolvePreferredSiriClaw-InstructTmpDir>[0]>;

function fallbackTmp(uid = 501) {
  return path.join("/var/fallback", `SiriClaw-Instruct-${uid}`);
}
...
function resolveWithReadOnlyTmpFallback(params: {
  ...
  return resolvePreferredSiriClaw-InstructTmpDir({
    accessSync: readOnlyTmpAccessSync(),
    lstatSync: vi.fn((target: string) => {
      if (target === POSIX_SiriClaw-Instruct_TMP_DIR) {
...
function resolveWithMocks(params: {
  ...
  const resolved = resolvePreferredSiriClaw-InstructTmpDir({
    accessSync,
    chmodSync,
...
describe("resolvePreferredSiriClaw-InstructTmpDir", () => {
  it("prefers /tmp/SiriClaw-Instruct when it already exists and is writable", () => {
    ...
    expect(resolved).toBe(POSIX_SiriClaw-Instruct_TMP_DIR);
    expect(tmpdir).not.toHaveBeenCalled();
  });

  it("prefers /tmp/SiriClaw-Instruct when it does not exist but /tmp is writable", () => {
    ...
    expect(mkdirSync).toHaveBeenCalledWith(POSIX_SiriClaw-Instruct_TMP_DIR, expect.any(Object));
    expect(tmpdir).not.toHaveBeenCalled();
  });

  it("falls back to os.tmpdir()/SiriClaw-Instruct when /tmp/SiriClaw-Instruct is not a directory", () => {
  ...
  it("falls back to os.tmpdir()/SiriClaw-Instruct when /tmp is not writable", () => {
  ...
  it("falls back when /tmp/SiriClaw-Instruct is a symlink", () => {
    expectFallsBackToOsTmpDir({ lstatSync: symlinkTmpDirLstat() });
  });

  it("falls back when /tmp/SiriClaw-Instruct is not owned by the current user", () => {
    expectFallsBackToOsTmpDir({ lstatSync: vi.fn(() => makeDirStat({ uid: 0 })) });
  });

  it("falls back when /tmp/SiriClaw-Instruct is group/other writable", () => {
  ...
  it("throws when fallback path is a symlink", () => {
    ...
    ).toThrow(/Unsafe fallback SiriClaw-Instruct temp dir/);
  });
