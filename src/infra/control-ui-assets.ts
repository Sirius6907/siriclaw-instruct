...
import { resolveSiriClaw-InstructPackageRoot, resolveSiriClaw-InstructPackageRootSync } from "./SiriClaw-Instruct-root.js";
...
export async function resolveControlUiDistIndexPath(
  ...
  const packageRoot = await resolveSiriClaw-InstructPackageRoot({ argv1: normalized, moduleUrl });
  if (packageRoot) {
    return path.join(packageRoot, "dist", "control-ui", "index.html");
  }

  // Fallback: traverse up and find package.json with name "SiriClaw-Instruct" + dist/control-ui/index.html
  ...
  for (let i = 0; i < 8; i++) {
    ...
    if (fs.existsSync(pkgJsonPath)) {
      try {
        ...
        if (parsed.name === "SiriClaw-Instruct") {
...
export function resolveControlUiRootSync(opts: ControlUiRootResolveOptions = {}): string | null {
  ...
  const packageRoot = resolveSiriClaw-InstructPackageRootSync({
    argv1,
    moduleUrl: opts.moduleUrl,
    cwd,
  });
  ...
  if (argv1Dir) {
    // SiriClaw-Instruct.mjs or dist/<bundle>.js
