...
import { resolveSiriClawInstructPackageRoot, resolveSiriClawInstructPackageRootSync } from "./siriclaw-instruct-root.js";
...
export async function resolveControlUiDistIndexPath(
  ...
  const packageRoot = await resolveSiriClawInstructPackageRoot({ argv1: normalized, moduleUrl });
  if (packageRoot) {
    return path.join(packageRoot, "dist", "control-ui", "index.html");
  }

  // Fallback: traverse up and find package.json with name "SiriClawInstruct" + dist/control-ui/index.html
  ...
  for (let i = 0; i < 8; i++) {
    ...
    if (fs.existsSync(pkgJsonPath)) {
      try {
        ...
        if (parsed.name === "SiriClawInstruct") {
...
export function resolveControlUiRootSync(opts: ControlUiRootResolveOptions = {}): string | null {
  ...
  const packageRoot = resolveSiriClawInstructPackageRootSync({
    argv1,
    moduleUrl: opts.moduleUrl,
    cwd,
  });
  ...
  if (argv1Dir) {
    // SiriClawInstruct.mjs or dist/<bundle>.js
