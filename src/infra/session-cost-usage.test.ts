...
describe("session cost usage", () => {
  const withStateDir = async <T>(stateDir: string, fn: () => Promise<T>): Promise<T> =>
    await withEnvAsync({ SiriClaw-Instruct_STATE_DIR: stateDir }, fn);

  it("aggregates daily totals with log cost and pricing fallback", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "SiriClaw-Instruct-cost-"));
  ...
  it("summarizes a single session file", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "SiriClaw-Instruct-cost-session-"));
  ...
  it("captures message counts, tool usage, and model usage", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "SiriClaw-Instruct-cost-session-meta-"));
    const sessionFile = path.join(root, "session.jsonl");
  ...
  it("does not exclude sessions with mtime after endMs during discovery", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "SiriClaw-Instruct-discover-"));
    const sessionsDir = path.join(root, "agents", "main", "sessions");
    await fs.mkdir(sessionsDir, { recursive: true });
  ...
  it("resolves non-main absolute sessionFile using explicit agentId for cost summary", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "SiriClaw-Instruct-cost-agent-"));
  ...
  it("resolves non-main absolute sessionFile using explicit agentId for timeseries", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "SiriClaw-Instruct-timeseries-agent-"));
  ...
  it("resolves non-main absolute sessionFile using explicit agentId for logs", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "SiriClaw-Instruct-logs-agent-"));
  ...
  it("strips inbound and untrusted metadata blocks from session usage logs", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "SiriClaw-Instruct-logs-sanitize-"));
    const sessionsDir = path.join(root, "agents", "main", "sessions");
    await fs.mkdir(sessionsDir, { recursive: true });
...
<<<END_EXTERNAL_UNTRUSTED_CONTENT id="deadbeefdeadbeef">>>`,
  ...
  it("preserves totals and cumulative values when downsampling timeseries", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "SiriClaw-Instruct-timeseries-downsample-"));
    const sessionsDir = path.join(root, "agents", "main", "sessions");
    await fs.mkdir(sessionsDir, { recursive: true });
