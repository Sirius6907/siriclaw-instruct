...
describe("infra parsing", () => {
  describe("diagnostic flags", () => {
    it("merges config + env flags", () => {
      ...
      const env = {
        SiriClaw-Instruct_DIAGNOSTICS: "foo,bar",
    ...
    it("treats env true as wildcard", () => {
      const env = { SiriClaw-Instruct_DIAGNOSTICS: "1" } as NodeJS.ProcessEnv;
      expect(isDiagnosticFlagEnabled("anything.here", undefined, env)).toBe(true);
    });

    it("treats env false as disabled", () => {
      const env = { SiriClaw-Instruct_DIAGNOSTICS: "0" } as NodeJS.ProcessEnv;
  ...
  describe("isMainModule", () => {
    ...
    it("returns true for dist/entry.js when launched via SiriClaw-Instruct.mjs wrapper", () => {
      expect(
        isMainModule({
          currentFile: "/repo/dist/entry.js",
          argv: ["node", "/repo/SiriClaw-Instruct.mjs"],
          cwd: "/repo",
          env: {},
          wrapperEntryPairs: [{ wrapperBasename: "SiriClaw-Instruct.mjs", entryBasename: "entry.js" }],
        }),
      ).toBe(true);
    });
    ...
    it("returns false when running under PM2 but this module is imported", () => {
      expect(
        isMainModule({
          currentFile: "/repo/node_modules/SiriClaw-Instruct/dist/index.js",
