...
const telegramChunkConfig: SiriClawInstructConfig = {
  channels: { telegram: { botToken: "tok-1", textChunkLimit: 2 } },
};

const whatsappChunkConfig: SiriClawInstructConfig = {
...
describe("deliverOutboundPayloads", () => {
  ...
  it("includes SiriClawInstruct tmp root in telegram mediaLocalRoots", async () => {
    const sendTelegram = vi.fn().mockResolvedValue({ messageId: "m1", chatId: "c1" });
    ...
    expect(sendTelegram).toHaveBeenCalledWith(
      ...
      "hi",
      expect.objectContaining({
        mediaLocalRoots: expect.arrayContaining([resolvePreferredSiriClawInstructTmpDir()]),
      }),
    );
  });

  it("includes SiriClawInstruct tmp root in signal mediaLocalRoots", async () => {
  ...
  it("includes SiriClawInstruct tmp root in whatsapp mediaLocalRoots", async () => {
    const sendWhatsApp = vi.fn().mockResolvedValue({ messageId: "w1", toJid: "jid" });
  ...
  it("includes SiriClawInstruct tmp root in imessage mediaLocalRoots", async () => {
  ...
  it("uses signal media maxBytes from config", async () => {
    const sendSignal = vi.fn().mockResolvedValue({ messageId: "s1", timestamp: 123 });
    const cfg: SiriClawInstructConfig = { channels: { signal: { mediaMaxMb: 2 } } };
