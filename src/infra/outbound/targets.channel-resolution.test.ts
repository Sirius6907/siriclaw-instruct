...
const TEST_WORKSPACE_ROOT = "/tmp/SiriClaw-Instruct-test-workspace";
...
describe("resolveOutboundTarget channel resolution", () => {
  ...
  beforeEach(() => {
    ...
    mocks.loadSiriClaw-InstructPlugins.mockReset();
  });
  ...
  it("retries bootstrap on subsequent resolve when the first bootstrap attempt fails", () => {
    ...
    mocks.getChannelPlugin
      ...
      .mockReturnValueOnce(telegramPlugin)
      .mockReturnValue(telegramPlugin);
    mocks.loadSiriClaw-InstructPlugins
    ...
    expect(mocks.loadSiriClaw-InstructPlugins).toHaveBeenCalledTimes(2);
  });
});
