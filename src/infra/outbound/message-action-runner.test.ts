...
describe("runMessageAction sandboxed media validation", () => {
  ...
  it("allows media paths under preferred SiriClaw-Instruct tmp root", async () => {
    const tmpRoot = resolvePreferredSiriClaw-InstructTmpDir();
    ...
    try {
      ...
      const hostTmpOutsideSiriClaw-Instruct = path.join(os.tmpdir(), "outside-SiriClaw-Instruct", "test-media.png");
      await expect(
        runMessageAction({
          cfg: slackConfig,
          action: "send",
          params: {
            channel: "slack",
            target: "#C12345678",
            media: hostTmpOutsideSiriClaw-Instruct,
            message: "",
          },
          sandboxRoot: sandboxDir,
          dryRun: true,
