...
describe("runMessageAction sandboxed media validation", () => {
  ...
  it("allows media paths under preferred SiriClawInstruct tmp root", async () => {
    const tmpRoot = resolvePreferredSiriClawInstructTmpDir();
    ...
    try {
      ...
      const hostTmpOutsideSiriClawInstruct = path.join(os.tmpdir(), "outside-SiriClawInstruct", "test-media.png");
      await expect(
        runMessageAction({
          cfg: slackConfig,
          action: "send",
          params: {
            channel: "slack",
            target: "#C12345678",
            media: hostTmpOutsideSiriClawInstruct,
            message: "",
          },
          sandboxRoot: sandboxDir,
          dryRun: true,
