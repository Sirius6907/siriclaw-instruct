...
function summarizeGroupPolicy(cfg: SiriClawInstructConfig): {
  open: number;
  allowlist: number;
  other: number;
} {
...
function isGatewayRemotelyExposed(cfg: SiriClawInstructConfig): boolean {
...
function collectModels(cfg: SiriClawInstructConfig): ModelRef[] {
  const out: ModelRef[] = [];
  addModel(
    out,
...
function listKnownNodeCommands(cfg: SiriClawInstructConfig): Set<string> {
  const baseCfg: SiriClawInstructConfig = {
    ...cfg,
    gateway: {
      ...cfg.gateway,
      nodes: {
...
function hasWebSearchKey(cfg: SiriClawInstructConfig, env: NodeJS.ProcessEnv): boolean {
...
function isWebSearchEnabled(cfg: SiriClawInstructConfig, env: NodeJS.ProcessEnv): boolean {
...
function isWebFetchEnabled(cfg: SiriClawInstructConfig): boolean {
...
function isBrowserEnabled(cfg: SiriClawInstructConfig): boolean {
  try {
...
function listGroupPolicyOpen(cfg: SiriClawInstructConfig): string[] {
  const out: string[] = [];
  const channels = cfg.channels as Record<string, unknown> | undefined;
  if (!channels || typeof channels !== "object") {
    return out;
...
function listPotentialMultiUserSignals(cfg: SiriClawInstructConfig): string[] {
  const out = new Set<string>();
  const channels = cfg.channels as Record<string, unknown> | undefined;
  if (!channels || typeof channels !== "object") {
    return [];
...
function collectRiskyToolExposureContexts(cfg: SiriClawInstructConfig): {
...
export function collectAttackSurfaceSummaryFindings(cfg: SiriClawInstructConfig): SecurityAuditFinding[] {
...
export function collectSyncedFolderFindings(params: {
  ...
  if (isProbablySyncedPath(params.stateDir) || isProbablySyncedPath(params.configPath)) {
    findings.push({
      ...
      remediation: `Keep SiriClawInstruct_STATE_DIR on a local-only volume and re-run "${formatCliCommand("SiriClawInstruct security audit --fix")}".`,
    });
  }
  return findings;
}

export function collectSecretsInConfigFindings(cfg: SiriClawInstructConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
  const password =
  ...
  if (password && !looksLikeEnvRef(password)) {
    findings.push({
      ...
      remediation:
        "Prefer SiriClawInstruct_GATEWAY_PASSWORD (env) and remove gateway.auth.password from disk.",
    });
  }
...
export function collectHooksHardeningFindings(
  ...
  const SiriClawInstructGatewayToken =
    typeof env.SiriClawInstruct_GATEWAY_TOKEN === "string" && env.SiriClawInstruct_GATEWAY_TOKEN.trim()
      ? env.SiriClawInstruct_GATEWAY_TOKEN.trim()
      : null;
  const gatewayToken =
...
export function collectGatewayHttpSessionKeyOverrideFindings(
  ...
  findings.push({
    ...
    detail:
      `${enabledEndpoints.join(", ")} accept x-SiriClawInstruct-session-key for per-request session routing. ` +
...
export function collectSandboxDockerNoopFindings(cfg: SiriClawInstructConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
...
export function collectSandboxDangerousConfigFindings(cfg: SiriClawInstructConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
  const agents = Array.isArray(cfg.agents?.list) ? cfg.agents.list : [];
  ...
  if (browserExposurePaths.length > 0) {
    findings.push({
      ...
      remediation:
        "Set sandbox.browser.network to a dedicated bridge network (recommended default: SiriClawInstruct-sandbox-browser), " +
...
export function collectNodeDenyCommandPatternFindings(cfg: SiriClawInstructConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
...
export function collectMinimalProfileOverrideFindings(cfg: SiriClawInstructConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
...
export function collectModelHygieneFindings(cfg: SiriClawInstructConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
...
export function collectExposureMatrixFindings(cfg: SiriClawInstructConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
...
export function collectLikelyMultiUserSetupFindings(cfg: SiriClawInstructConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
  ...
  findings.push({
    ...
    detail:
      ...
      `\n${impactLine}\n${riskyContextsDetail}\n` +
      "SiriClawInstruct's default security model is personal-assistant (one trusted operator boundary), not hostile multi-tenant isolation on one shared gateway.",
    remediation:
