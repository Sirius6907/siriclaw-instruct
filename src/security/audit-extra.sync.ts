...
function summarizeGroupPolicy(cfg: SiriClaw-InstructConfig): {
  open: number;
  allowlist: number;
  other: number;
} {
...
function isGatewayRemotelyExposed(cfg: SiriClaw-InstructConfig): boolean {
...
function collectModels(cfg: SiriClaw-InstructConfig): ModelRef[] {
  const out: ModelRef[] = [];
  addModel(
    out,
...
function listKnownNodeCommands(cfg: SiriClaw-InstructConfig): Set<string> {
  const baseCfg: SiriClaw-InstructConfig = {
    ...cfg,
    gateway: {
      ...cfg.gateway,
      nodes: {
...
function hasWebSearchKey(cfg: SiriClaw-InstructConfig, env: NodeJS.ProcessEnv): boolean {
...
function isWebSearchEnabled(cfg: SiriClaw-InstructConfig, env: NodeJS.ProcessEnv): boolean {
...
function isWebFetchEnabled(cfg: SiriClaw-InstructConfig): boolean {
...
function isBrowserEnabled(cfg: SiriClaw-InstructConfig): boolean {
  try {
...
function listGroupPolicyOpen(cfg: SiriClaw-InstructConfig): string[] {
  const out: string[] = [];
  const channels = cfg.channels as Record<string, unknown> | undefined;
  if (!channels || typeof channels !== "object") {
    return out;
...
function listPotentialMultiUserSignals(cfg: SiriClaw-InstructConfig): string[] {
  const out = new Set<string>();
  const channels = cfg.channels as Record<string, unknown> | undefined;
  if (!channels || typeof channels !== "object") {
    return [];
...
function collectRiskyToolExposureContexts(cfg: SiriClaw-InstructConfig): {
...
export function collectAttackSurfaceSummaryFindings(cfg: SiriClaw-InstructConfig): SecurityAuditFinding[] {
...
export function collectSyncedFolderFindings(params: {
  ...
  if (isProbablySyncedPath(params.stateDir) || isProbablySyncedPath(params.configPath)) {
    findings.push({
      ...
      remediation: `Keep SiriClaw-Instruct_STATE_DIR on a local-only volume and re-run "${formatCliCommand("SiriClaw-Instruct security audit --fix")}".`,
    });
  }
  return findings;
}

export function collectSecretsInConfigFindings(cfg: SiriClaw-InstructConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
  const password =
  ...
  if (password && !looksLikeEnvRef(password)) {
    findings.push({
      ...
      remediation:
        "Prefer SiriClaw-Instruct_GATEWAY_PASSWORD (env) and remove gateway.auth.password from disk.",
    });
  }
...
export function collectHooksHardeningFindings(
  ...
  const SiriClaw-InstructGatewayToken =
    typeof env.SiriClaw-Instruct_GATEWAY_TOKEN === "string" && env.SiriClaw-Instruct_GATEWAY_TOKEN.trim()
      ? env.SiriClaw-Instruct_GATEWAY_TOKEN.trim()
      : null;
  const gatewayToken =
...
export function collectGatewayHttpSessionKeyOverrideFindings(
  ...
  findings.push({
    ...
    detail:
      `${enabledEndpoints.join(", ")} accept x-SiriClaw-Instruct-session-key for per-request session routing. ` +
...
export function collectSandboxDockerNoopFindings(cfg: SiriClaw-InstructConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
...
export function collectSandboxDangerousConfigFindings(cfg: SiriClaw-InstructConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
  const agents = Array.isArray(cfg.agents?.list) ? cfg.agents.list : [];
  ...
  if (browserExposurePaths.length > 0) {
    findings.push({
      ...
      remediation:
        "Set sandbox.browser.network to a dedicated bridge network (recommended default: SiriClaw-Instruct-sandbox-browser), " +
...
export function collectNodeDenyCommandPatternFindings(cfg: SiriClaw-InstructConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
...
export function collectMinimalProfileOverrideFindings(cfg: SiriClaw-InstructConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
...
export function collectModelHygieneFindings(cfg: SiriClaw-InstructConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
...
export function collectExposureMatrixFindings(cfg: SiriClaw-InstructConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
...
export function collectLikelyMultiUserSetupFindings(cfg: SiriClaw-InstructConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
  ...
  findings.push({
    ...
    detail:
      ...
      `\n${impactLine}\n${riskyContextsDetail}\n` +
      "SiriClaw-Instruct's default security model is personal-assistant (one trusted operator boundary), not hostile multi-tenant isolation on one shared gateway.",
    remediation:
