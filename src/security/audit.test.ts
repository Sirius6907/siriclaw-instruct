...
function stubChannelPlugin(params: {
  ...
  label: string;
  resolveAccount: (cfg: SiriClaw-InstructConfig, accountId: string | null | undefined) => unknown;
  inspectAccount?: (cfg: SiriClaw-InstructConfig, accountId: string | null | undefined) => unknown;
  listAccountIds?: (cfg: SiriClaw-InstructConfig) => string[];
  isConfigured?: (account: unknown, cfg: SiriClaw-InstructConfig) => boolean;
  isEnabled?: (account: unknown, cfg: SiriClaw-InstructConfig) => boolean;
}): ChannelPlugin {
  return {
    id: params.id,
    meta: {
...
describe("security audit", () => {
  ...
  const withChannelSecurityStateDir = async (fn: (tmp: string) => Promise<void>) => {
    ...
    await withEnvAsync({ SiriClaw-Instruct_STATE_DIR: sharedChannelSecurityStateDir }, () =>
  ...
  const createSharedCodeSafetyFixture = async () => {
    ...
    await fs.writeFile(
      ...
      JSON.stringify({
        name: "evil-plugin",
        SiriClaw-Instruct: { extensions: [".hidden/index.js"] },
      }),
    );
    await fs.writeFile(
...
`,
  ...
  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "SiriClaw-Instruct-security-audit-"));
  ...
  it("flags non-loopback bind without auth as critical", async () => {
    ...
    const prevPassword = process.env.SiriClaw-Instruct_GATEWAY_PASSWORD;
    ...
    try {
      ...
      if (prevPassword === undefined) {
        ...
        process.env.SiriClaw-Instruct_GATEWAY_PASSWORD = prevPassword;
      }
    }
  });
  ...
  it("warns for risky safeBinTrustedDirs entries", async () => {
    const riskyGlobalTrustedDirs =
      process.platform === "win32"
        ...
        : ["/usr/local/bin", "/tmp/SiriClaw-Instruct-safe-bins"];
    const cfg: SiriClaw-InstructConfig = {
      tools: {
        exec: {
  ...
  it("warns when sandbox browser containers have missing or stale hash labels", async () => {
    ...
    const execDockerRawFn = (async (args: string[]) => {
      if (args[0] === "ps") {
        return {
          stdout: Buffer.from("SiriClaw-Instruct-sbx-browser-old\nSiriClaw-Instruct-sbx-browser-missing-hash\n"),
          stderr: Buffer.alloc(0),
          code: 0,
        };
      }
      if (args[0] === "inspect" && args.at(-1) === "SiriClaw-Instruct-sbx-browser-old") {
        return {
      ...
      if (args[0] === "inspect" && args.at(-1) === "SiriClaw-Instruct-sbx-browser-missing-hash") {
        return {
    ...
    expect(staleEpoch?.detail).toContain("SiriClaw-Instruct-sbx-browser-old");
  });
  ...
  it("flags sandbox browser containers with non-loopback published ports", async () => {
    ...
    const execDockerRawFn = (async (args: string[]) => {
      if (args[0] === "ps") {
        return {
          stdout: Buffer.from("SiriClaw-Instruct-sbx-browser-exposed\n"),
          stderr: Buffer.alloc(0),
          code: 0,
        };
      }
      if (args[0] === "inspect" && args.at(-1) === "SiriClaw-Instruct-sbx-browser-exposed") {
        return {
      ...
      if (args[0] === "port" && args.at(-1) === "SiriClaw-Instruct-sbx-browser-exposed") {
        return {
  ...
  it("uses symlink target permissions for config checks", async () => {
    ...
    const targetConfigPath = path.join(tmp, "managed-SiriClaw-Instruct.json");
  ...
  it("flags wildcard Control UI origins by exposure level", async () => {
    const loopbackCfg: SiriClaw-InstructConfig = {
      gateway: {
        bind: "loopback",
        controlUi: { allowedOrigins: ["*"] },
      },
    };
    const exposedCfg: SiriClaw-InstructConfig = {
      gateway: {
        bind: "lan",
        auth: { mode: "token", token: "very-long-browser-token-0123456789" },
        controlUi: { allowedOrigins: ["*"] },
  ...
  it("scores X-Real-IP fallback risk by gateway exposure", async () => {
    const trustedProxyCfg = (trustedProxies: string[]): SiriClaw-InstructConfig => ({
      gateway: {
        bind: "loopback",
  ...
  it("warns when multiple DM senders share the main session", async () => {
    const cfg: SiriClaw-InstructConfig = { session: { dmScope: "main" } };
  ...
  it("keeps channel security findings when SecretRef credentials are configured but unavailable", async () => {
    await withChannelSecurityStateDir(async () => {
      const sourceConfig: SiriClaw-InstructConfig = {
        channels: {
          discord: {
            enabled: true,
      ...
      const resolvedConfig: SiriClaw-InstructConfig = {
        channels: {
          discord: {
            enabled: true,
            groupPolicy: "allowlist",
  ...
  it("warns when Discord allowlists contain name-based entries", async () => {
    await withChannelSecurityStateDir(async (tmp) => {
      ...
      expect(finding?.detail).toContain(
        "~/.SiriClaw-Instruct/credentials/discord-allowFrom.json:team.owner",
      );
  ...
  it("adds probe_failed warnings for deep probe failure modes", async () => {
    const cfg: SiriClaw-InstructConfig = { gateway: { mode: "local" } };
    const cases: Array<{
      name: string;
  ...
  it("flags hooks token reuse of the gateway env token as critical", async () => {
    const prevToken = process.env.SiriClaw-Instruct_GATEWAY_TOKEN;
    process.env.SiriClaw-Instruct_GATEWAY_TOKEN = "shared-gateway-token-1234567890";
    const cfg: SiriClaw-InstructConfig = {
  ...
  it("scores hooks request sessionKey override by gateway exposure", async () => {
    ...
    } satisfies NonNullable<SiriClaw-InstructConfig["hooks"]>;
    const cases: Array<{
      name: string;
      cfg: SiriClaw-InstructConfig;
      expectedSeverity: "warn" | "critical";
  ...
  it("warns when state/config look like a synced folder", async () => {
    ...
    const res = await audit(cfg, {
      stateDir: "/Users/test/Dropbox/.SiriClaw-Instruct",
      configPath: "/Users/test/Dropbox/.SiriClaw-Instruct/SiriClaw-Instruct.json",
    });
  ...
  it("flags group/world-readable config include files", async () => {
    ...
    const cfg: SiriClaw-InstructConfig = { logging: { redactSensitive: "off" } };
    const user = "DESKTOP-TEST\\Tester";
  ...
  it("warns on unpinned npm install specs and missing integrity metadata", async () => {
    const cfg: SiriClaw-InstructConfig = {
      plugins: {
        installs: {
          "voice-call": {
            source: "npm",
            spec: "@SiriClaw-Instruct/voice-call",
          },
        },
      },
      hooks: {
        internal: {
          installs: {
            "test-hooks": {
              source: "npm",
              spec: "@SiriClaw-Instruct/test-hooks",
            },
          },
        },
      },
    ...
    const res = await runSecurityAudit({
      ...
      configPath: path.join(sharedInstallMetadataStateDir, "SiriClaw-Instruct.json"),
      execDockerRawFn: execDockerRawUnavailable,
    });
    ...
    const cfg: SiriClaw-InstructConfig = {
      plugins: {
        installs: {
          "voice-call": {
            source: "npm",
            spec: "@SiriClaw-Instruct/voice-call@1.2.3",
      ...
      hooks: {
        internal: {
          installs: {
            "test-hooks": {
              source: "npm",
              spec: "@SiriClaw-Instruct/test-hooks@1.2.3",
  ...
  it("warns when install records drift from installed package versions", async () => {
    ...
    await fs.writeFile(
      path.join(pluginDir, "package.json"),
      JSON.stringify({ name: "@SiriClaw-Instruct/voice-call", version: "9.9.9" }),
      "utf-8",
    );
    await fs.writeFile(
      path.join(hookDir, "package.json"),
      JSON.stringify({ name: "@SiriClaw-Instruct/test-hooks", version: "8.8.8" }),
      "utf-8",
    );
  ...
  it("flags plugin extension entry path traversal in deep audit", async () => {
    ...
    await fs.writeFile(
      ...
      JSON.stringify({
        name: "escape-plugin",
        SiriClaw-Instruct: { extensions: ["../outside.js"] },
      }),
    );
    ...
    try {
      ...
      await fs.writeFile(
        ...
        JSON.stringify({
          ...
          SiriClaw-Instruct: { extensions: ["index.js"] },
        }),
      );
      await fs.writeFile(path.join(pluginDir, "index.js"), "export {};");
  ...
  describe("maybeProbeGateway auth selection", () => {
    ...
    const makeProbeEnv = (env?: { token?: string; password?: string }) => {
      ...
      if (env?.token !== undefined) {
        probeEnv.SiriClaw-Instruct_GATEWAY_TOKEN = env.token;
      }
      if (env?.password !== undefined) {
        probeEnv.SiriClaw-Instruct_GATEWAY_PASSWORD = env.password;
      }
      return probeEnv;
    };
