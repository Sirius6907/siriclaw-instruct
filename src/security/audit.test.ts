...
function stubChannelPlugin(params: {
  ...
  label: string;
  resolveAccount: (cfg: SiriClawInstructConfig, accountId: string | null | undefined) => unknown;
  inspectAccount?: (cfg: SiriClawInstructConfig, accountId: string | null | undefined) => unknown;
  listAccountIds?: (cfg: SiriClawInstructConfig) => string[];
  isConfigured?: (account: unknown, cfg: SiriClawInstructConfig) => boolean;
  isEnabled?: (account: unknown, cfg: SiriClawInstructConfig) => boolean;
}): ChannelPlugin {
  return {
    id: params.id,
    meta: {
...
describe("security audit", () => {
  ...
  const withChannelSecurityStateDir = async (fn: (tmp: string) => Promise<void>) => {
    ...
    await withEnvAsync({ SiriClawInstruct_STATE_DIR: sharedChannelSecurityStateDir }, () =>
  ...
  const createSharedCodeSafetyFixture = async () => {
    ...
    await fs.writeFile(
      ...
      JSON.stringify({
        name: "evil-plugin",
        SiriClawInstruct: { extensions: [".hidden/index.js"] },
      }),
    );
    await fs.writeFile(
...
`,
  ...
  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "SiriClawInstruct-security-audit-"));
  ...
  it("flags non-loopback bind without auth as critical", async () => {
    ...
    const prevPassword = process.env.SiriClawInstruct_GATEWAY_PASSWORD;
    ...
    try {
      ...
      if (prevPassword === undefined) {
        ...
        process.env.SiriClawInstruct_GATEWAY_PASSWORD = prevPassword;
      }
    }
  });
  ...
  it("warns for risky safeBinTrustedDirs entries", async () => {
    const riskyGlobalTrustedDirs =
      process.platform === "win32"
        ...
        : ["/usr/local/bin", "/tmp/SiriClawInstruct-safe-bins"];
    const cfg: SiriClawInstructConfig = {
      tools: {
        exec: {
  ...
  it("warns when sandbox browser containers have missing or stale hash labels", async () => {
    ...
    const execDockerRawFn = (async (args: string[]) => {
      if (args[0] === "ps") {
        return {
          stdout: Buffer.from("SiriClawInstruct-sbx-browser-old\nSiriClawInstruct-sbx-browser-missing-hash\n"),
          stderr: Buffer.alloc(0),
          code: 0,
        };
      }
      if (args[0] === "inspect" && args.at(-1) === "SiriClawInstruct-sbx-browser-old") {
        return {
      ...
      if (args[0] === "inspect" && args.at(-1) === "SiriClawInstruct-sbx-browser-missing-hash") {
        return {
    ...
    expect(staleEpoch?.detail).toContain("SiriClawInstruct-sbx-browser-old");
  });
  ...
  it("flags sandbox browser containers with non-loopback published ports", async () => {
    ...
    const execDockerRawFn = (async (args: string[]) => {
      if (args[0] === "ps") {
        return {
          stdout: Buffer.from("SiriClawInstruct-sbx-browser-exposed\n"),
          stderr: Buffer.alloc(0),
          code: 0,
        };
      }
      if (args[0] === "inspect" && args.at(-1) === "SiriClawInstruct-sbx-browser-exposed") {
        return {
      ...
      if (args[0] === "port" && args.at(-1) === "SiriClawInstruct-sbx-browser-exposed") {
        return {
  ...
  it("uses symlink target permissions for config checks", async () => {
    ...
    const targetConfigPath = path.join(tmp, "managed-SiriClawInstruct.json");
  ...
  it("flags wildcard Control UI origins by exposure level", async () => {
    const loopbackCfg: SiriClawInstructConfig = {
      gateway: {
        bind: "loopback",
        controlUi: { allowedOrigins: ["*"] },
      },
    };
    const exposedCfg: SiriClawInstructConfig = {
      gateway: {
        bind: "lan",
        auth: { mode: "token", token: "very-long-browser-token-0123456789" },
        controlUi: { allowedOrigins: ["*"] },
  ...
  it("scores X-Real-IP fallback risk by gateway exposure", async () => {
    const trustedProxyCfg = (trustedProxies: string[]): SiriClawInstructConfig => ({
      gateway: {
        bind: "loopback",
  ...
  it("warns when multiple DM senders share the main session", async () => {
    const cfg: SiriClawInstructConfig = { session: { dmScope: "main" } };
  ...
  it("keeps channel security findings when SecretRef credentials are configured but unavailable", async () => {
    await withChannelSecurityStateDir(async () => {
      const sourceConfig: SiriClawInstructConfig = {
        channels: {
          discord: {
            enabled: true,
      ...
      const resolvedConfig: SiriClawInstructConfig = {
        channels: {
          discord: {
            enabled: true,
            groupPolicy: "allowlist",
  ...
  it("warns when Discord allowlists contain name-based entries", async () => {
    await withChannelSecurityStateDir(async (tmp) => {
      ...
      expect(finding?.detail).toContain(
        "~/.SiriClawInstruct/credentials/discord-allowFrom.json:team.owner",
      );
  ...
  it("adds probe_failed warnings for deep probe failure modes", async () => {
    const cfg: SiriClawInstructConfig = { gateway: { mode: "local" } };
    const cases: Array<{
      name: string;
  ...
  it("flags hooks token reuse of the gateway env token as critical", async () => {
    const prevToken = process.env.SiriClawInstruct_GATEWAY_TOKEN;
    process.env.SiriClawInstruct_GATEWAY_TOKEN = "shared-gateway-token-1234567890";
    const cfg: SiriClawInstructConfig = {
  ...
  it("scores hooks request sessionKey override by gateway exposure", async () => {
    ...
    } satisfies NonNullable<SiriClawInstructConfig["hooks"]>;
    const cases: Array<{
      name: string;
      cfg: SiriClawInstructConfig;
      expectedSeverity: "warn" | "critical";
  ...
  it("warns when state/config look like a synced folder", async () => {
    ...
    const res = await audit(cfg, {
      stateDir: "/Users/test/Dropbox/.SiriClawInstruct",
      configPath: "/Users/test/Dropbox/.SiriClawInstruct/SiriClawInstruct.json",
    });
  ...
  it("flags group/world-readable config include files", async () => {
    ...
    const cfg: SiriClawInstructConfig = { logging: { redactSensitive: "off" } };
    const user = "DESKTOP-TEST\\Tester";
  ...
  it("warns on unpinned npm install specs and missing integrity metadata", async () => {
    const cfg: SiriClawInstructConfig = {
      plugins: {
        installs: {
          "voice-call": {
            source: "npm",
            spec: "@SiriClawInstruct/voice-call",
          },
        },
      },
      hooks: {
        internal: {
          installs: {
            "test-hooks": {
              source: "npm",
              spec: "@SiriClawInstruct/test-hooks",
            },
          },
        },
      },
    ...
    const res = await runSecurityAudit({
      ...
      configPath: path.join(sharedInstallMetadataStateDir, "SiriClawInstruct.json"),
      execDockerRawFn: execDockerRawUnavailable,
    });
    ...
    const cfg: SiriClawInstructConfig = {
      plugins: {
        installs: {
          "voice-call": {
            source: "npm",
            spec: "@SiriClawInstruct/voice-call@1.2.3",
      ...
      hooks: {
        internal: {
          installs: {
            "test-hooks": {
              source: "npm",
              spec: "@SiriClawInstruct/test-hooks@1.2.3",
  ...
  it("warns when install records drift from installed package versions", async () => {
    ...
    await fs.writeFile(
      path.join(pluginDir, "package.json"),
      JSON.stringify({ name: "@SiriClawInstruct/voice-call", version: "9.9.9" }),
      "utf-8",
    );
    await fs.writeFile(
      path.join(hookDir, "package.json"),
      JSON.stringify({ name: "@SiriClawInstruct/test-hooks", version: "8.8.8" }),
      "utf-8",
    );
  ...
  it("flags plugin extension entry path traversal in deep audit", async () => {
    ...
    await fs.writeFile(
      ...
      JSON.stringify({
        name: "escape-plugin",
        SiriClawInstruct: { extensions: ["../outside.js"] },
      }),
    );
    ...
    try {
      ...
      await fs.writeFile(
        ...
        JSON.stringify({
          ...
          SiriClawInstruct: { extensions: ["index.js"] },
        }),
      );
      await fs.writeFile(path.join(pluginDir, "index.js"), "export {};");
  ...
  describe("maybeProbeGateway auth selection", () => {
    ...
    const makeProbeEnv = (env?: { token?: string; password?: string }) => {
      ...
      if (env?.token !== undefined) {
        probeEnv.SiriClawInstruct_GATEWAY_TOKEN = env.token;
      }
      if (env?.password !== undefined) {
        probeEnv.SiriClawInstruct_GATEWAY_PASSWORD = env.password;
      }
      return probeEnv;
    };
