...
function isWhatsAppConfigured(cfg: SiriClaw-InstructConfig): boolean {
...
function isGenericChannelConfigured(cfg: SiriClaw-InstructConfig, channelId: string): boolean {
  const entry = resolveChannelConfig(cfg, channelId);
  return recordHasKeys(entry);
}
...
function collectModelRefs(cfg: SiriClaw-InstructConfig): string[] {
  const refs: string[] = [];
...
function isProviderConfigured(cfg: SiriClaw-InstructConfig, providerId: string): boolean {
...
function collectCandidateChannelIds(cfg: SiriClaw-InstructConfig): string[] {
...
function isPluginExplicitlyDisabled(cfg: SiriClaw-InstructConfig, pluginId: string): boolean {
...
function isPluginDenied(cfg: SiriClaw-InstructConfig, pluginId: string): boolean {
...
function registerPluginEntry(cfg: SiriClaw-InstructConfig, pluginId: string): SiriClaw-InstructConfig {
  const builtInChannelId = normalizeChatChannelId(pluginId);
  if (builtInChannelId) {
    const channels = cfg.channels as Record<string, unknown> | undefined;
