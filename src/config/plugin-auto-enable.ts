...
function isWhatsAppConfigured(cfg: SiriClawInstructConfig): boolean {
...
function isGenericChannelConfigured(cfg: SiriClawInstructConfig, channelId: string): boolean {
  const entry = resolveChannelConfig(cfg, channelId);
  return recordHasKeys(entry);
}
...
function collectModelRefs(cfg: SiriClawInstructConfig): string[] {
  const refs: string[] = [];
...
function isProviderConfigured(cfg: SiriClawInstructConfig, providerId: string): boolean {
...
function collectCandidateChannelIds(cfg: SiriClawInstructConfig): string[] {
...
function isPluginExplicitlyDisabled(cfg: SiriClawInstructConfig, pluginId: string): boolean {
...
function isPluginDenied(cfg: SiriClawInstructConfig, pluginId: string): boolean {
...
function registerPluginEntry(cfg: SiriClawInstructConfig, pluginId: string): SiriClawInstructConfig {
  const builtInChannelId = normalizeChatChannelId(pluginId);
  if (builtInChannelId) {
    const channels = cfg.channels as Record<string, unknown> | undefined;
