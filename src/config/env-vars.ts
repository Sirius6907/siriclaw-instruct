...
function collectConfigEnvVarsByTarget(cfg?: SiriClawInstructConfig): Record<string, string> {
...
export function collectConfigRuntimeEnvVars(cfg?: SiriClawInstructConfig): Record<string, string> {
  return collectConfigEnvVarsByTarget(cfg);
}

export function collectConfigServiceEnvVars(cfg?: SiriClawInstructConfig): Record<string, string> {
  return collectConfigEnvVarsByTarget(cfg);
}

/** @deprecated Use `collectConfigRuntimeEnvVars` or `collectConfigServiceEnvVars`. */
export function collectConfigEnvVars(cfg?: SiriClawInstructConfig): Record<string, string> {
...
export function applyConfigEnvVars(
  ...
  for (const [key, value] of Object.entries(entries)) {
    ...
    // (e.g. process.env.SiriClawInstruct_GATEWAY_TOKEN = "${VAULT_TOKEN}") which downstream auth
