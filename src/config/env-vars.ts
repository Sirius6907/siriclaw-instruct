...
function collectConfigEnvVarsByTarget(cfg?: SiriClaw-InstructConfig): Record<string, string> {
...
export function collectConfigRuntimeEnvVars(cfg?: SiriClaw-InstructConfig): Record<string, string> {
  return collectConfigEnvVarsByTarget(cfg);
}

export function collectConfigServiceEnvVars(cfg?: SiriClaw-InstructConfig): Record<string, string> {
  return collectConfigEnvVarsByTarget(cfg);
}

/** @deprecated Use `collectConfigRuntimeEnvVars` or `collectConfigServiceEnvVars`. */
export function collectConfigEnvVars(cfg?: SiriClaw-InstructConfig): Record<string, string> {
...
export function applyConfigEnvVars(
  ...
  for (const [key, value] of Object.entries(entries)) {
    ...
    // (e.g. process.env.SiriClaw-Instruct_GATEWAY_TOKEN = "${VAULT_TOKEN}") which downstream auth
