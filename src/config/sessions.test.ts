...
describe("sessions", () => {
  ...
  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "SiriClaw-Instruct-sessions-suite-"));
  });
  ...
  const withStateDir = <T>(stateDir: string, fn: () => T): T =>
    withEnv({ SiriClaw-Instruct_STATE_DIR: stateDir }, fn);
  ...
  it("builds discord display name with guild+channel slugs", () => {
    expect(
      buildGroupDisplayName({
        ...
        space: "friends-of-SiriClaw-Instruct",
        id: "123",
        key: "discord:group:123",
      }),
    ).toBe("discord:friends-of-SiriClaw-Instruct#general");
  });
  ...
  it("derives session transcripts dir from SiriClaw-Instruct_STATE_DIR", () => {
    const dir = resolveSessionTranscriptsDir(
      { SiriClaw-Instruct_STATE_DIR: "/custom/state" } as NodeJS.ProcessEnv,
      () => "/home/ignored",
    );
  ...
  it("resolves cross-agent paths when SiriClaw-Instruct_STATE_DIR differs from stored paths", () => {
