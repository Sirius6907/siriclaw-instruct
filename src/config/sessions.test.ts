...
describe("sessions", () => {
  ...
  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "SiriClawInstruct-sessions-suite-"));
  });
  ...
  const withStateDir = <T>(stateDir: string, fn: () => T): T =>
    withEnv({ SiriClawInstruct_STATE_DIR: stateDir }, fn);
  ...
  it("builds discord display name with guild+channel slugs", () => {
    expect(
      buildGroupDisplayName({
        ...
        space: "friends-of-SiriClawInstruct",
        id: "123",
        key: "discord:group:123",
      }),
    ).toBe("discord:friends-of-SiriClawInstruct#general");
  });
  ...
  it("derives session transcripts dir from SiriClawInstruct_STATE_DIR", () => {
    const dir = resolveSessionTranscriptsDir(
      { SiriClawInstruct_STATE_DIR: "/custom/state" } as NodeJS.ProcessEnv,
      () => "/home/ignored",
    );
  ...
  it("resolves cross-agent paths when SiriClawInstruct_STATE_DIR differs from stored paths", () => {
