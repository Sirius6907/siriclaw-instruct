...
describe("multi-agent agentDir validation", () => {
  it("rejects shared agents.list agentDir", async () => {
    const shared = path.join(tmpdir(), "SiriClawInstruct-shared-agentdir");
    ...
    await withTempHomeConfig(
      {
        agents: {
          list: [
            { id: "a", agentDir: "~/.SiriClawInstruct/agents/shared/agent" },
            { id: "b", agentDir: "~/.SiriClawInstruct/agents/shared/agent" },
          ],
        },
