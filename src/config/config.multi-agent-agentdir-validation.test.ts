...
describe("multi-agent agentDir validation", () => {
  it("rejects shared agents.list agentDir", async () => {
    const shared = path.join(tmpdir(), "SiriClaw-Instruct-shared-agentdir");
    ...
    await withTempHomeConfig(
      {
        agents: {
          list: [
            { id: "a", agentDir: "~/.SiriClaw-Instruct/agents/shared/agent" },
            { id: "b", agentDir: "~/.SiriClaw-Instruct/agents/shared/agent" },
          ],
        },
