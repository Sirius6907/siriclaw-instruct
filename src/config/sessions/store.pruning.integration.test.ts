...
// Keep integration tests deterministic: never read a real SiriClaw-Instruct.json.
vi.mock("../config.js", () => ({
...
describe("Integration: saveSessionStore with pruning", () => {
  ...
  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "SiriClaw-Instruct-pruning-integ-"));
  });
  ...
  beforeEach(async () => {
    ...
    savedCacheTtl = process.env.SiriClaw-Instruct_SESSION_CACHE_TTL_MS;
    process.env.SiriClaw-Instruct_SESSION_CACHE_TTL_MS = "0";
  ...
  afterEach(() => {
    ...
    if (savedCacheTtl === undefined) {
      delete process.env.SiriClaw-Instruct_SESSION_CACHE_TTL_MS;
    } else {
      process.env.SiriClaw-Instruct_SESSION_CACHE_TTL_MS = savedCacheTtl;
    }
  });
  ...
  it("does not archive external transcript paths when capping entries", async () => {
    ...
    const now = Date.now();
    const externalDir = await fs.mkdtemp(path.join(os.tmpdir(), "SiriClaw-Instruct-external-cap-"));
  ...
  it("never deletes transcripts outside the agent sessions directory during budget cleanup", async () => {
    ...
    const now = Date.now();
    const externalDir = await fs.mkdtemp(path.join(os.tmpdir(), "SiriClaw-Instruct-external-session-"));
    const externalTranscript = path.join(externalDir, "outside.jsonl");
