...
describe("configureGatewayForOnboarding", () => {
  ...
  it("prefers SiriClawInstruct_GATEWAY_TOKEN during quickstart token setup", async () => {
    const prevToken = process.env.SiriClawInstruct_GATEWAY_TOKEN;
    process.env.SiriClawInstruct_GATEWAY_TOKEN = "token-from-env";
    mocks.randomToken.mockReturnValue("generated-token");
  ...
  it("honors secretInputMode=ref for gateway password prompts", async () => {
    ...
    process.env.SiriClawInstruct_GATEWAY_PASSWORD = "gateway-secret"; // pragma: allowlist secret
    try {
      const prompter = createPrompter({
        selectQueue: ["loopback", "password", "off", "env"],
        textQueue: ["18789", "SiriClawInstruct_GATEWAY_PASSWORD"],
      });
      const runtime = createRuntime();
