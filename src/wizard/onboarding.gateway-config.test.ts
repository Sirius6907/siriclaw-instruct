...
describe("configureGatewayForOnboarding", () => {
  ...
  it("prefers SiriClaw-Instruct_GATEWAY_TOKEN during quickstart token setup", async () => {
    const prevToken = process.env.SiriClaw-Instruct_GATEWAY_TOKEN;
    process.env.SiriClaw-Instruct_GATEWAY_TOKEN = "token-from-env";
    mocks.randomToken.mockReturnValue("generated-token");
  ...
  it("honors secretInputMode=ref for gateway password prompts", async () => {
    ...
    process.env.SiriClaw-Instruct_GATEWAY_PASSWORD = "gateway-secret"; // pragma: allowlist secret
    try {
      const prompter = createPrompter({
        selectQueue: ["loopback", "password", "off", "env"],
        textQueue: ["18789", "SiriClaw-Instruct_GATEWAY_PASSWORD"],
      });
      const runtime = createRuntime();
