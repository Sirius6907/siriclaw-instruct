...
describe("promptSingleChannelSecretInput", () => {
  ...
  it("returns ref + resolved value when external env ref is selected", async () => {
    process.env.SiriClawInstruct_TEST_TOKEN = "secret-token";
    const prompter = {
      select: vi.fn().mockResolvedValueOnce("ref").mockResolvedValueOnce("env"),
      confirm: vi.fn(async () => false),
      text: vi.fn(async () => "SiriClawInstruct_TEST_TOKEN"),
      note: vi.fn(async () => undefined),
    };
    ...
    const result = await promptSingleChannelSecretInput({
      ...
      keepPrompt: "keep",
      inputPrompt: "token",
      preferredEnvVar: "SiriClawInstruct_TEST_TOKEN",
    });
    ...
    expect(result).toEqual({
      ...
      value: {
        source: "env",
        provider: "default",
        id: "SiriClawInstruct_TEST_TOKEN",
      },
