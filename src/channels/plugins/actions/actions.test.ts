...
function telegramCfg(): SiriClawInstructConfig {
  return { channels: { telegram: { botToken: "tok" } } } as SiriClawInstructConfig;
}
...
async function runTelegramAction(
  ...
  options?: { cfg?: SiriClawInstructConfig; accountId?: string },
) {
...
async function runSignalAction(
  ...
  const cfg =
    options?.cfg ?? ({ channels: { signal: { account: "+15550001111" } } } as SiriClawInstructConfig);
...
function slackHarness() {
  const cfg = { channels: { slack: { botToken: "tok" } } } as SiriClawInstructConfig;
...
function createSignalAccountOverrideCfg(): SiriClawInstructConfig {
  return {
    channels: {
      signal: {
...
describe("discord message actions", () => {
  it("lists channel and upload actions by default", async () => {
    const cfg = { channels: { discord: { token: "d0" } } } as SiriClawInstructConfig;
  ...
  it("lists moderation when at least one account enables it", () => {
    ...
    for (const channelConfig of cases) {
      const cfg = channelConfig as unknown as SiriClawInstructConfig;
      const actions = discordMessageActions.listActions?.({ cfg }) ?? [];
  ...
  it("maps reaction targets into signal sendReaction calls", async () => {
    const cases = [
      ...
      },
      {
        ...
        cfg: { channels: { signal: { account: "+15550001111" } } } as SiriClawInstructConfig,
        accountId: undefined,
        params: {
