...
function telegramCfg(): SiriClaw-InstructConfig {
  return { channels: { telegram: { botToken: "tok" } } } as SiriClaw-InstructConfig;
}
...
async function runTelegramAction(
  ...
  options?: { cfg?: SiriClaw-InstructConfig; accountId?: string },
) {
...
async function runSignalAction(
  ...
  const cfg =
    options?.cfg ?? ({ channels: { signal: { account: "+15550001111" } } } as SiriClaw-InstructConfig);
...
function slackHarness() {
  const cfg = { channels: { slack: { botToken: "tok" } } } as SiriClaw-InstructConfig;
...
function createSignalAccountOverrideCfg(): SiriClaw-InstructConfig {
  return {
    channels: {
      signal: {
...
describe("discord message actions", () => {
  it("lists channel and upload actions by default", async () => {
    const cfg = { channels: { discord: { token: "d0" } } } as SiriClaw-InstructConfig;
  ...
  it("lists moderation when at least one account enables it", () => {
    ...
    for (const channelConfig of cases) {
      const cfg = channelConfig as unknown as SiriClaw-InstructConfig;
      const actions = discordMessageActions.listActions?.({ cfg }) ?? [];
  ...
  it("maps reaction targets into signal sendReaction calls", async () => {
    const cases = [
      ...
      },
      {
        ...
        cfg: { channels: { signal: { account: "+15550001111" } } } as SiriClaw-InstructConfig,
        accountId: undefined,
        params: {
