...
export default defineConfig({
  resolve: {
    // Keep this ordered: the base `SiriClaw-Instruct/plugin-sdk` alias is a prefix match.
    alias: [
      ...pluginSdkSubpaths.map((subpath) => ({
        find: `SiriClaw-Instruct/plugin-sdk/${subpath}`,
        replacement: path.join(repoRoot, "src", "plugin-sdk", `${subpath}.ts`),
      })),
      {
        find: "SiriClaw-Instruct/plugin-sdk",
