...
- Jobs persist under `~/.SiriClaw-Instruct/cron/` so restarts don’t lose schedules.
...
SiriClaw-Instruct cron list
SiriClaw-Instruct cron run <job-id>
SiriClaw-Instruct cron runs --id <job-id>
```
...
Cron jobs are persisted on the Gateway host at `~/.SiriClaw-Instruct/cron/jobs.json` by default.
The Gateway loads the file into memory and writes it back on changes, so manual edits
are only safe when the Gateway is stopped. Prefer `SiriClaw-Instruct cron add/edit` or the cron
...
To reduce top-of-hour load spikes across many gateways, SiriClaw-Instruct applies a
...
If `delivery` is omitted for isolated jobs, SiriClaw-Instruct defaults to `announce`.
...
- CLI equivalents: `SiriClaw-Instruct cron add --light-context ...` and `SiriClaw-Instruct cron edit --light-context`.
...
- Job store: `~/.SiriClaw-Instruct/cron/jobs.json` (Gateway-managed JSON).
- Run history: `~/.SiriClaw-Instruct/cron/runs/<jobId>.jsonl` (JSONL, auto-pruned by size and line count).
- Isolated cron run sessions in `sessions.json` are pruned by `cron.sessionRetention` (default `24h`; set `false` to disable).
- Override store path: `cron.store` in config.

## Retry policy

When a job fails, SiriClaw-Instruct classifies errors as **transient** (retryable) or **permanent** (disable immediately).
...
```json5
{
  cron: {
    ...
    store: "~/.SiriClaw-Instruct/cron/jobs.json",
...
- `SiriClaw-Instruct_SKIP_CRON=1` (env)
...
- For removed run sessions no longer referenced by the session store, SiriClaw-Instruct archives transcript files and purges old deleted archives on the same retention window.
...
- review growth periodically with `SiriClaw-Instruct cron runs` and adjust retention before logs become large
...
SiriClaw-Instruct cron add --name "Ops sweep" --cron "0 6 * * *" --session isolated --message "Check ops queue" --agent ops

# Switch or clear the agent on an existing job
SiriClaw-Instruct cron edit <jobId> --agent ops
SiriClaw-Instruct cron edit <jobId> --clear-agent
```

Manual run (force is the default, use `--due` to only run when due):

```bash
SiriClaw-Instruct cron run <jobId>
SiriClaw-Instruct cron run <jobId> --due
```

Edit an existing job (patch fields):

```bash
SiriClaw-Instruct cron edit <jobId> \
...
```bash
SiriClaw-Instruct cron edit <jobId> --exact
```
...
```bash
SiriClaw-Instruct system event --mode now --text "Next heartbeat: check battery."
```
...
- `cron.run` (force or due), `cron.runs`
  For immediate system events without a job, use [`SiriClaw-Instruct system event`](/cli/system).

## Troubleshooting

### “Nothing runs”

- Check cron is enabled: `cron.enabled` and `SiriClaw-Instruct_SKIP_CRON`.
- Check the Gateway is running continuously (cron runs inside the Gateway process).
- For `cron` schedules: confirm timezone (`--tz`) vs the host timezone.

### A recurring job keeps delaying after failures

- SiriClaw-Instruct applies exponential retry backoff for recurring jobs after consecutive errors:
