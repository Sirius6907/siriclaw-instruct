...
read_when:
  ...
  - Wiring external systems into SiriClaw-Instruct
title: "Webhooks"
---
...
- `x-SiriClaw-Instruct-token: <token>`
...
```json5
{
  hooks: {
    enabled: true,
    token: "${SiriClaw-Instruct_HOOKS_TOKEN}",
...
- `hooks.transformsDir` + `transform.module` loads a JS/TS module for custom logic.
  - `hooks.transformsDir` (if set) must stay within the transforms root under your SiriClaw-Instruct config directory (typically `~/.SiriClaw-Instruct/hooks/transforms`).
...
- `SiriClaw-Instruct webhooks gmail setup` writes `hooks.gmail` config for `SiriClaw-Instruct webhooks gmail run`.
...
curl -X POST http://127.0.0.1:18789/hooks/agent \
  -H 'x-SiriClaw-Instruct-token: SECRET' \
  -H 'Content-Type: application/json' \
