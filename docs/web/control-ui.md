...
- optional prefix: set `gateway.controlUi.basePath` (e.g. `/SiriClaw-Instruct`)
...
If the page fails to load, start the Gateway first: `SiriClaw-Instruct gateway`.
...
you revoke it with `SiriClaw-Instruct devices revoke --device <id> --role <role>`. See
...
- Config: view/edit `~/.SiriClaw-Instruct/SiriClaw-Instruct.json` (`config.get`, `config.set`)
...
- Stop:
  ...
  - Type `/stop` (or standalone abort phrases like `stop`, `stop action`, `stop run`, `stop SiriClaw-Instruct`, `please stop`) to abort out-of-band
...
(`tailscale-user-login`) when `gateway.auth.allowTailscale` is `true`. SiriClaw-Instruct
...
SiriClaw-Instruct **blocks** Control UI connections without device identity.
