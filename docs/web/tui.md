...
```bash
SiriClaw-Instruct tui --url ws://<host>:<port> --token <gateway-token>
```
...
- Turn delivery on:
  ...
  - or start with `SiriClaw-Instruct tui --deliver`
...
- Local shell commands receive `SiriClaw-Instruct_SHELL=tui-local` in their environment.
...
- Check the Gateway logs: `SiriClaw-Instruct logs --follow`.
- Confirm the agent can run: `SiriClaw-Instruct status` and `SiriClaw-Instruct models status`.
...
- No agents in picker: check `SiriClaw-Instruct agents list` and your routing config.
