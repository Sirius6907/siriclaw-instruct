...
read_when:
  - You want to understand what “context” means in SiriClaw-Instruct
...
“Context” is **everything SiriClaw-Instruct sends to the model for a run**. It is bounded by the model’s **context window** (token limit).

Beginner mental model:

- **System prompt** (SiriClaw-Instruct-built): rules, tools, skills list, time/runtime, and injected workspace files.
...
## How SiriClaw-Instruct builds the system prompt

The system prompt is **SiriClaw-Instruct-owned** and rebuilt each run. It includes:
...
By default, SiriClaw-Instruct injects a fixed set of workspace files (if present):
...
By default, SiriClaw-Instruct uses the built-in `legacy` context engine for assembly and
compaction. If you install a plugin that provides `kind: "context-engine"` and
select it with `plugins.slots.contextEngine`, SiriClaw-Instruct delegates context
