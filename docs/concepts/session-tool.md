...
- When given a `sessionId`, SiriClaw-Instruct resolves it to the corresponding session key (missing ids error).
...
- After the primary run completes, SiriClaw-Instruct runs a **reply-back loop**:
  - Round 2+ alternates between requester and target agents.
  - Reply exactly `REPLY_SKIP` to stop the ping‑pong.
  - Max turns is `session.agentToAgent.maxPingPongTurns` (0–5, default 5).
- Once the loop ends, SiriClaw-Instruct runs the **agent‑to‑agent announce step** (target agent only):
...
- `attachments?` (optional array of inline files; subagent runtime only, ACP rejects). Each entry: `{ name, content, encoding?: "utf8" | "base64", mimeType? }`. Files are materialized into the child workspace at `.SiriClaw-Instruct/attachments/<uuid>/`. Returns a receipt with sha256 per file.
...
- After completion, SiriClaw-Instruct runs a sub-agent **announce step** and posts the result to the requester chat channel.
...
- When a session is sandboxed and `sessionToolsVisibility="spawned"`, SiriClaw-Instruct clamps visibility to `tree` even if you set `tools.sessions.visibility="all"`.
