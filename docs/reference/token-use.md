---
summary: "How SiriClaw-Instruct builds prompt context and reports token usage + costs"
read_when:
...
SiriClaw-Instruct tracks **tokens**, not characters. Tokens are model-specific, but most
OpenAI-style models average ~4 characters per token for English text.

## How the system prompt is built

SiriClaw-Instruct assembles its own system prompt on every run. It includes:
...
For images, SiriClaw-Instruct downscales transcript/tool image payloads before provider calls.
...
- `/usage cost` → shows a local cost summary from SiriClaw-Instruct session logs.

Other surfaces:

- **TUI/Web TUI:** `/status` + `/usage` are supported.
- **CLI:** `SiriClaw-Instruct status --usage` and `SiriClaw-Instruct channels list` show
...
`cacheWrite`. If pricing is missing, SiriClaw-Instruct shows tokens only. OAuth tokens
never show dollar cost.

## Cache TTL and pruning impact

Provider prompt caching only applies within the cache TTL window. SiriClaw-Instruct can
...
Anthropic's 1M context window is currently beta-gated. SiriClaw-Instruct can inject the
...
SiriClaw-Instruct skips the `context-1m-*` beta header because Anthropic currently
