...
This document explains how SiriClaw-Instruct manages sessions end-to-end:
...
SiriClaw-Instruct is designed around a single **Gateway process** that owns session state.
...
SiriClaw-Instruct persists sessions in two layers:
...
- Transcripts: `~/.SiriClaw-Instruct/agents/<agentId>/sessions/<sessionId>.jsonl`
  - Telegram topic sessions: `.../<sessionId>-topic-<threadId>.jsonl`

SiriClaw-Instruct resolves these via `src/config/sessions.ts`.
...
In `mode: "warn"`, SiriClaw-Instruct reports potential evictions but does not mutate the store/files.
...
- `cron.runLog.maxBytes` + `cron.runLog.keepLines` prune `~/.SiriClaw-Instruct/cron/runs/<jobId>.jsonl` files (defaults: `2_000_000` bytes and `2000` lines).
...
SiriClaw-Instruct intentionally does **not** “fix up” transcripts; the Gateway uses `SessionManager` to read/write them.
...
These are Pi runtime semantics (SiriClaw-Instruct consumes the events, but Pi decides when to compact).
...
SiriClaw-Instruct also enforces a safety floor for embedded runs:

- If `compaction.reserveTokens < reserveTokensFloor`, SiriClaw-Instruct bumps it.
- Default floor is `20000` tokens.
- Set `agents.defaults.compaction.reserveTokensFloor: 0` to disable the floor.
- If it’s already higher, SiriClaw-Instruct leaves it alone.
...
- `SiriClaw-Instruct status` (CLI)
- `SiriClaw-Instruct sessions` / `sessions --json`
...
SiriClaw-Instruct supports “silent” turns for background tasks where the user should not see intermediate output.

Convention:

- The assistant starts its output with `NO_REPLY` to indicate “do not deliver a reply to the user”.
- SiriClaw-Instruct strips/suppresses this in the delivery layer.

As of `2026.1.10`, SiriClaw-Instruct also suppresses **draft/typing streaming** when a partial chunk begins with `NO_REPLY`, so silent operations don’t leak partial output mid-turn.
...
SiriClaw-Instruct uses the **pre-threshold flush** approach:
...
Pi also exposes a `session_before_compact` hook in the extension API, but SiriClaw-Instruct’s
...
- Store vs transcript mismatch? Confirm the Gateway host and the store path from `SiriClaw-Instruct status`.
