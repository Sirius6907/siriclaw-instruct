...
SiriClaw-Instruct integrates external CLIs via JSON-RPC. Two patterns are used today.
...
- SiriClaw-Instruct owns lifecycle when `channels.signal.autoStart=true`.
...
- SiriClaw-Instruct spawns `imsg rpc` as a child process (legacy iMessage integration).
