...
This is the full reference for the `SiriClaw-Instruct onboard` CLI wizard.
...
<Steps>
  <Step title="Existing config detection">
    - If `~/.SiriClaw-Instruct/SiriClaw-Instruct.json` exists, choose **Keep / Modify / Reset**.
    - Re-running the wizard does **not** wipe anything unless you explicitly choose **Reset**
      (or pass `--reset`).
    - CLI `--reset` defaults to `config+creds+sessions`; use `--reset-scope full`
      to also remove workspace.
    - If the config is invalid or contains legacy keys, the wizard stops and asks
      you to run `SiriClaw-Instruct doctor` before continuing.
  ...
  <Step title="Model/Auth">
    ...
    - OAuth credentials live in `~/.SiriClaw-Instruct/credentials/oauth.json`; auth profiles live in `~/.SiriClaw-Instruct/agents/<agentId>/agent/auth-profiles.json` (API keys + OAuth).
    - More detail: [/concepts/oauth](/concepts/oauth)
    <Note>
    Headless/server tip: complete OAuth on a machine with a browser, then copy
    `~/.SiriClaw-Instruct/credentials/oauth.json` (or `$SiriClaw-Instruct_STATE_DIR/credentials/oauth.json`) to the
    gateway host.
    </Note>
  </Step>
  <Step title="Workspace">
    - Default `~/.SiriClaw-Instruct/workspace` (configurable).
  ...
  <Step title="Channels">
    ...
    - DM security: default is pairing. First DM sends a code; approve via `SiriClaw-Instruct pairing approve <channel> <code>` or use allowlists.
  </Step>
  <Step title="Web search">
    ...
    - Configure later: `SiriClaw-Instruct configure --section web`.
  </Step>
  ...
  <Step title="Health check">
    - Starts the Gateway (if needed) and runs `SiriClaw-Instruct health`.
    - Tip: `SiriClaw-Instruct status --deep` adds gateway health probes to status output (requires a reachable gateway).
  </Step>
...
```bash
export SiriClaw-Instruct_GATEWAY_TOKEN="your-token"
SiriClaw-Instruct onboard --non-interactive \
  --mode local \
  --auth-choice skip \
  --gateway-auth token \
  --gateway-token-ref-env SiriClaw-Instruct_GATEWAY_TOKEN
```
...
- Stores it under `~/.SiriClaw-Instruct/tools/signal-cli/<version>/`.
...
Typical fields in `~/.SiriClaw-Instruct/SiriClaw-Instruct.json`:
...
`SiriClaw-Instruct agents add` writes `agents.list[]` and optional `bindings`.

WhatsApp credentials go under `~/.SiriClaw-Instruct/credentials/whatsapp/<accountId>/`.
Sessions are stored under `~/.SiriClaw-Instruct/agents/<agentId>/sessions/`.
