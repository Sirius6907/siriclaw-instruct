...
- With Anthropic API-key auth profiles, SiriClaw-Instruct seeds `cacheRetention: "short"` for Anthropic model refs when unset.
...
For `openrouter/anthropic/*` model refs, SiriClaw-Instruct injects Anthropic `cache_control` on system/developer prompt blocks to improve prompt-cache reuse.
...
SiriClaw-Instruct exposes dedicated cache-trace diagnostics for embedded agent runs.
...
diagnostics:
  cacheTrace:
    enabled: true
    filePath: "~/.SiriClaw-Instruct/logs/cache-trace.jsonl" # optional
...
- `filePath`: `$SiriClaw-Instruct_STATE_DIR/logs/cache-trace.jsonl`
...
- `SiriClaw-Instruct_CACHE_TRACE=1` enables cache tracing.
- `SiriClaw-Instruct_CACHE_TRACE_FILE=/path/to/cache-trace.jsonl` overrides output path.
- `SiriClaw-Instruct_CACHE_TRACE_MESSAGES=0|1` toggles full message payload capture.
- `SiriClaw-Instruct_CACHE_TRACE_PROMPT=0|1` toggles prompt text capture.
- `SiriClaw-Instruct_CACHE_TRACE_SYSTEM=0|1` toggles system prompt capture.
