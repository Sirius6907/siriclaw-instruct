...
SiriClaw-Instruct features that can generate provider usage or paid API calls.
...
- `SiriClaw-Instruct status --usage` and `SiriClaw-Instruct channels list` show provider **usage windows**
...
SiriClaw-Instruct can pick up credentials from:
...
- `SiriClaw-Instruct status --usage`
- `SiriClaw-Instruct models status --json`
...
`SiriClaw-Instruct models scan` can probe OpenRouter models and uses `OPENROUTER_API_KEY` when
probing is enabled.
