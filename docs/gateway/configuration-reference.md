...
description: "Complete field-by-field reference for ~/.SiriClaw-Instruct/SiriClaw-Instruct.json"
summary: "Complete reference for every SiriClaw-Instruct config key, defaults, and channel settings"
read_when:
...
Every field available in `~/.SiriClaw-Instruct/SiriClaw-Instruct.json`. For a task-oriented overview, see [Configuration](/gateway/configuration).

Config format is **JSON5** (comments + trailing commas allowed). All fields are optional — SiriClaw-Instruct uses safe defaults when omitted.
...
- Legacy single-account Baileys auth dir is migrated by `SiriClaw-Instruct doctor` into `whatsapp/default`.
...
- In multi-account setups (2+ account ids), set an explicit default (`channels.telegram.defaultAccount` or `channels.telegram.accounts.default`) to avoid fallback routing; `SiriClaw-Instruct doctor` warns when this is missing or invalid.
...
```json5
{
  channels: {
    discord: {
      ...
      dm: { enabled: true, groupEnabled: false, groupChannels: ["SiriClaw-Instruct-dm"] },
      guilds: {
        "123456789012345678": {
...
- SiriClaw-Instruct additionally attempts voice receive recovery by leaving/rejoining a voice session after repeated decrypt failures.
...
Mattermost ships as a plugin: `SiriClaw-Instruct plugins install @SiriClaw-Instruct/mattermost`.
...
- `commands.callbackUrl` must resolve to the SiriClaw-Instruct gateway endpoint and be reachable from the Mattermost server.
...
SiriClaw-Instruct spawns `imsg rpc` (JSON-RPC over stdio). No daemon or port required.
...
- If you add a non-default account via `SiriClaw-Instruct channels add` (or channel onboarding) while still on a single-account top-level channel config, SiriClaw-Instruct moves account-scoped top-level single-account values into `channels.<channel>.accounts.default` first so the original account keeps working.
- Existing channel-only bindings (no `accountId`) keep matching the default account; account-scoped bindings remain optional.
- `SiriClaw-Instruct doctor --fix` also repairs mixed shapes by moving account-scoped top-level single-account values into `accounts.default` when named accounts exist but `default` is missing.
...
```json5
{
  ...
  agents: {
    list: [
      {
        id: "main",
        groupChat: { mentionPatterns: ["reisponde", "@SiriClaw-Instruct"] },
      },
    ],
  },
}
...
- `config: true` enables `/config` (reads/writes `SiriClaw-Instruct.json`). For gateway `chat.send` clients, persistent `/config set|unset` writes also require `operator.admin`; read-only `/config show` stays available to normal write-scoped operator clients.
...
Default: `~/.SiriClaw-Instruct/workspace`.
...
Optional repository root shown in the system prompt's Runtime line. If unset, SiriClaw-Instruct auto-detects by walking upward from the workspace.
...
- `model.primary`: format `provider/model` (e.g. `anthropic/claude-opus-4-6`). If you omit the provider, SiriClaw-Instruct assumes `anthropic` (deprecated).
...
- `none`: per-scope sandbox workspace under `~/.SiriClaw-Instruct/sandboxes`
...
**Sandboxed browser** (`sandbox.browser.enabled`): Chromium + CDP in a container. noVNC URL injected into system prompt. Does not require `browser.enabled` in `SiriClaw-Instruct.json`.
noVNC observer access uses VNC auth by default and SiriClaw-Instruct emits a short-lived token URL (instead of exposing the password in the shared URL).

- `allowHostControl: false` (default) blocks sandboxed sessions from targeting the host browser.
- `network` defaults to `SiriClaw-Instruct-sandbox-browser` (dedicated bridge network). Set to `bridge` only when you explicitly want global bridge connectivity.
...
- Launch defaults are defined in `scripts/sandbox-browser-entrypoint.sh` and tuned for container hosts:
  ...
  - `--disable-3d-apis`, `--disable-software-rasterizer`, and `--disable-gpu` are
    ...
    `SiriClaw-Instruct_BROWSER_DISABLE_GRAPHICS_FLAGS=0` if WebGL/3D usage requires it.
  - `SiriClaw-Instruct_BROWSER_DISABLE_EXTENSIONS=0` re-enables extensions if your workflow
    depends on them.
  - `--renderer-process-limit=2` can be changed with
    `SiriClaw-Instruct_BROWSER_RENDERER_PROCESS_LIMIT=<N>`; set `0` to use Chromium's
    ...
    list: [
      {
        ...
        default: true,
        name: "Main Agent",
        workspace: "~/.SiriClaw-Instruct/workspace",
        agentDir: "~/.SiriClaw-Instruct/agents/main/agent",
        ...
        runtime: {
          ...
          acp: {
            ...
            backend: "acpx",
            mode: "persistent",
            cwd: "/workspace/SiriClaw-Instruct",
          },
        },
...
For `type: "acp"` entries, SiriClaw-Instruct resolves by exact conversation identity (`match.channel` + account + `match.peer.id`) and does not use the route binding tier order above.
...
- **`parentForkMaxTokens`**: max parent-session `totalTokens` allowed when creating a forked thread session (default `100000`).
  - If parent `totalTokens` is above this value, SiriClaw-Instruct starts a fresh thread session instead of inheriting parent transcript history.
...
- When `openai.baseUrl` points to a non-OpenAI endpoint, SiriClaw-Instruct treats it as an OpenAI-compatible TTS server and relaxes model/voice validation.
...
| `group:nodes`      | `nodes`                                                                                  |
| `group:SiriClaw-Instruct`   | All built-in tools (excludes provider plugins)                                           |
...
- Files are materialized into the child workspace at `.SiriClaw-Instruct/attachments/<uuid>/` with a `.manifest.json`.
...
SiriClaw-Instruct uses the pi-coding-agent model catalog. Add custom providers via `models.providers` in config or `~/.SiriClaw-Instruct/agents/<agentId>/agent/models.json`.
...
- Override agent config root with `SiriClaw-Instruct_AGENT_DIR` (or `PI_CODING_AGENT_DIR`).
...
- `models.providers.*.models.*.compat.supportsDeveloperRole`: optional compatibility hint. For `api: "openai-completions"` with a non-empty non-native `baseUrl` (host not `api.openai.com`), SiriClaw-Instruct forces this to `false` at runtime. Empty/omitted `baseUrl` keeps default OpenAI behavior.
...
Set `OPENCODE_API_KEY` (or `OPENCODE_ZEN_API_KEY`). Shortcut: `SiriClaw-Instruct onboard --auth-choice opencode-zen`.
...
Set `ZAI_API_KEY`. `z.ai/*` and `z-ai/*` are accepted aliases. Shortcut: `SiriClaw-Instruct onboard --auth-choice zai-api-key`.
...
For the China endpoint: `baseUrl: "https://api.moonshot.cn/v1"` or `SiriClaw-Instruct onboard --auth-choice moonshot-api-key-cn`.
...
Anthropic-compatible, built-in provider. Shortcut: `SiriClaw-Instruct onboard --auth-choice kimi-code-api-key`.
...
Base URL should omit `/v1` (Anthropic client appends it). Shortcut: `SiriClaw-Instruct onboard --auth-choice synthetic-api-key`.
...
Set `MINIMAX_API_KEY`. Shortcut: `SiriClaw-Instruct onboard --auth-choice minimax-api`.
...
- Loaded from `~/.SiriClaw-Instruct/extensions`, `<workspace>/.SiriClaw-Instruct/extensions`, plus `plugins.load.paths`.
...
- `plugins.installs`: CLI-managed install metadata used by `SiriClaw-Instruct plugins update`.
...
```json5
{
  gateway: {
    ...
    auth: {
      ...
      token: "your-token",
      // password: "your-password", // or SiriClaw-Instruct_GATEWAY_PASSWORD
...
- `port`: single multiplexed port for WS + HTTP. Precedence: `--port` > `SiriClaw-Instruct_GATEWAY_PORT` > `gateway.port` > `18789`.
...
- `SiriClaw-Instruct_ALLOW_INSECURE_PRIVATE_WS=1`: client-side break-glass override that allows plaintext `ws://` to trusted private-network IPs; default remains loopback-only for plaintext.
...
Convenience flags: `--dev` (uses `~/.SiriClaw-Instruct-dev` + port `19001`), `--profile <name>` (uses `~/.SiriClaw-Instruct-<name>`).
...
```json5
{
  hooks: {
    ...
    presets: ["gmail"],
    transformsDir: "~/.SiriClaw-Instruct/hooks/transforms",
    mappings: [
      {
        match: { path: "gmail" },
        action: "agent",
...
Auth: `Authorization: Bearer <token>` or `x-SiriClaw-Instruct-token: <token>`.
...
- Gateway auto-starts `gog gmail watch serve` on boot when configured. Set `SiriClaw-Instruct_SKIP_GMAIL_WATCHER=1` to disable.
...
```json5
{
  canvasHost: {
    ...
    liveReload: true,
    // enabled: false, // or SiriClaw-Instruct_SKIP_CANVAS_HOST=1
  },
}
```

- Serves agent-editable HTML/CSS/JS and A2UI over HTTP under the Gateway port:
  - `http://<gateway-host>:<gateway.port>/__SiriClaw-Instruct__/canvas/`
  - `http://<gateway-host>:<gateway.port>/__SiriClaw-Instruct__/a2ui/`
...
- Also serves A2UI at `/__SiriClaw-Instruct__/a2ui/`.
...
- Hostname defaults to `SiriClaw-Instruct`. Override with `SiriClaw-Instruct_MDNS_HOSTNAME`.
...
Writes a unicast DNS-SD zone under `~/.SiriClaw-Instruct/dns/`. For cross-network discovery, pair with a DNS server (CoreDNS recommended) + Tailscale split DNS.

Setup: `SiriClaw-Instruct dns setup --apply`.
...
- `.env` files: CWD `.env` + `~/.SiriClaw-Instruct/.env` (neither overrides existing vars).
...
{
  gateway: {
    auth: { token: "${SiriClaw-Instruct_GATEWAY_TOKEN}" },
  },
}
```
...
- `secrets apply` targets supported `SiriClaw-Instruct.json` credential paths.
...
```json5
{
  secrets: {
    providers: {
      ...
      vault: {
        source: "exec",
        command: "/usr/local/bin/SiriClaw-Instruct-vault-resolver",
...
- Legacy OAuth imports from `~/.SiriClaw-Instruct/credentials/oauth.json`.
...
- Default log file: `/tmp/SiriClaw-Instruct/SiriClaw-Instruct-YYYY-MM-DD.log`.
...
- `cli.banner.taglineMode` controls banner tagline style:
  ...
  - `"default"`: fixed neutral tagline (`All your chats, one SiriClaw-Instruct.`).
  - `"off"`: no tagline text (banner title/version still shown).
- To hide the entire banner (not just taglines), set env `SiriClaw-Instruct_HIDE_BANNER=1`.
...
Current builds no longer include the TCP bridge. Nodes connect over the Gateway WebSocket. `bridge.*` keys are no longer part of the config schema (validation fails until removed; `SiriClaw-Instruct doctor --fix` can strip unknown keys).
...
- Paths: resolved relative to the including file, but must stay inside the top-level config directory (`dirname` of `SiriClaw-Instruct.json`). Absolute/`../` forms are allowed only when they still resolve inside that boundary.
