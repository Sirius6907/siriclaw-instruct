...
SiriClaw-Instruct has two distinct problems that look similar on the surface:

1. **Operator remote control**: the macOS menu bar app controlling a gateway running elsewhere.
2. **Node pairing**: iOS/Android (and future nodes) finding a gateway and pairing securely.

The design goal is to keep all network discovery/advertising in the **Node Gateway** (`SiriClaw-Instruct gateway`) and keep clients (mac app, iOS) as consumers.
...
- Service types:
  - `_SiriClaw-Instruct-gw._tcp` (gateway transport beacon)
- TXT keys (non-secret):
  - `role=gateway`
  ...
  - `cliPath=<path>` (optional; absolute path to a runnable `SiriClaw-Instruct` entrypoint or binary)
...
- `SiriClaw-Instruct_DISABLE_BONJOUR=1` disables advertising.
- `gateway.bind` in `~/.SiriClaw-Instruct/SiriClaw-Instruct.json` controls the Gateway bind mode.
- `SiriClaw-Instruct_SSH_PORT` overrides the SSH port advertised in TXT (defaults to 22).
- `SiriClaw-Instruct_TAILNET_DNS` publishes a `tailnetDns` hint (MagicDNS).
- `SiriClaw-Instruct_CLI_PATH` overrides the advertised CLI path.
