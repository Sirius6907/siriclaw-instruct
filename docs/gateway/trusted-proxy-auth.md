...
read_when:
  - Running SiriClaw-Instruct behind an identity-aware proxy
  - Setting up Pomerium, Caddy, or nginx with OAuth in front of SiriClaw-Instruct
...
- You run SiriClaw-Instruct behind an **identity-aware proxy** (Pomerium, Caddy + OAuth, nginx + oauth2-proxy, Traefik + forward auth)
...
3. SiriClaw-Instruct checks that the request came from a **trusted proxy IP** (configured in `gateway.trustedProxies`)
4. SiriClaw-Instruct extracts the user identity from the configured header
...
- SiriClaw-Instruct can stay on loopback HTTP behind the proxy.
...
If SiriClaw-Instruct itself serves HTTPS directly (no TLS-terminating proxy), set:
...
```yaml
routes:
  - from: https://SiriClaw-Instruct.example.com
    to: http://SiriClaw-Instruct-gateway:18789
    policy:
      - allow:
          or:
            - email:
...
```
SiriClaw-Instruct.example.com {
    authenticate with oauth2_provider
    authorize with policy1

    reverse_proxy SiriClaw-Instruct:18789 {
...
location / {
    ...
    proxy_pass http://SiriClaw-Instruct:18789;
...
`SiriClaw-Instruct security audit` will flag trusted-proxy auth with a **critical** severity finding. This is intentional — it's a reminder that you're delegating security to your proxy setup.
...
3. Update SiriClaw-Instruct config with trusted-proxy auth
4. Restart the Gateway
5. Test WebSocket connections from the Control UI
6. Run `SiriClaw-Instruct security audit` and review findings
