...
SiriClaw-Instruct supports additive SecretRefs so supported credentials do not need to be stored as plaintext in configuration.
...
- `gateway.auth.token` SecretRef is inactive for startup auth resolution when `SiriClaw-Instruct_GATEWAY_TOKEN` (or `SIRICLAW_GATEWAY_TOKEN`) is set, because env token input wins for that runtime.
...
When onboarding runs in interactive mode and you choose SecretRef storage, SiriClaw-Instruct runs preflight validation before saving:
...
- Set `allowSymlinkCommand: true` to allow symlink command paths (for example Homebrew shims). SiriClaw-Instruct validates the resolved target path.
  ...
  secrets: {
    providers: {
      onepassword_openai: {
        ...
        args: ["read", "op://Personal/SiriClaw-Instruct QA API Key/password"],
        passEnv: ["HOME"],
        jsonOnly: false,
      },
    },
  ...
  secrets: {
    providers: {
      vault_openai: {
        ...
        allowSymlinkCommand: true, // required for Homebrew symlinked binaries
        trustedDirs: ["/opt/homebrew"],
        args: ["kv", "get", "-field=OPENAI_API_KEY", "secret/SiriClaw-Instruct"],
...
- `REF_SHADOWED` (audit finding when `auth-profiles.json` credentials take precedence over `SiriClaw-Instruct.json` refs)
...
When reload-time activation fails after a healthy state, SiriClaw-Instruct enters degraded secrets state.
...
- Strict command paths (for example `SiriClaw-Instruct memory` remote-memory paths and `SiriClaw-Instruct qr --remote`) read from the active snapshot and fail fast when a required SecretRef is unavailable.
- Read-only command paths (for example `SiriClaw-Instruct status`, `SiriClaw-Instruct status --all`, `SiriClaw-Instruct channels status`, `SiriClaw-Instruct channels resolve`, and read-only doctor/config repair flows) also prefer the active snapshot, but degrade instead of aborting when a targeted SecretRef is unavailable in that command path.
...
- Snapshot refresh after backend secret rotation is handled by `SiriClaw-Instruct secrets reload`.
...
```bash
SiriClaw-Instruct secrets audit --check
SiriClaw-Instruct secrets configure
SiriClaw-Instruct secrets audit --check
```
...
- plaintext values at rest (`SiriClaw-Instruct.json`, `auth-profiles.json`, `.env`, and generated `agents/*/agent/models.json`)
- plaintext sensitive provider header residues in generated `models.json` entries
- unresolved refs
- precedence shadowing (`auth-profiles.json` taking priority over `SiriClaw-Instruct.json` refs)
...
- lets you select supported secret-bearing fields in `SiriClaw-Instruct.json` plus `auth-profiles.json` for one agent scope
...
- `SiriClaw-Instruct secrets configure --providers-only`
- `SiriClaw-Instruct secrets configure --skip-provider-setup`
- `SiriClaw-Instruct secrets configure --agent <id>`
...
```bash
SiriClaw-Instruct secrets apply --from /tmp/SiriClaw-Instruct-secrets-plan.json
SiriClaw-Instruct secrets apply --from /tmp/SiriClaw-Instruct-secrets-plan.json --dry-run
```
...
SiriClaw-Instruct intentionally does not write rollback backups containing historical plaintext secret values.
