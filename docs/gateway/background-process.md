...
SiriClaw-Instruct runs shell commands through the `exec` tool and keeps long‑running tasks in memory. The `process` tool manages those background sessions.
...
- Spawned exec commands receive `SiriClaw-Instruct_SHELL=exec` for context-aware shell/profile rules.
...
- `SiriClaw-Instruct_BASH_PENDING_MAX_OUTPUT_CHARS`: pending stdout/stderr cap per stream (chars)
