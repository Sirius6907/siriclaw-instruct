...
- During heartbeat runs, SiriClaw-Instruct treats `HEARTBEAT_OK` as an ack when it appears
...
- `session`: optional session key for heartbeat runs.
  ...
  - Explicit session key (copy from `SiriClaw-Instruct sessions --json` or the [sessions CLI](/cli/sessions)).
...
If **all three** are false, SiriClaw-Instruct skips the heartbeat run entirely (no model call).
...
```bash
SiriClaw-Instruct system event --text "Check for urgent follow-ups" --mode now
```
