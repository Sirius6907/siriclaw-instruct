...
read_when:
  - You want to place an outbound voice call from SiriClaw-Instruct
...
Voice calls for SiriClaw-Instruct via a plugin. Supports outbound notifications and
...
- Use `SiriClaw-Instruct voicecall ...` or the `voice_call` tool
...
```bash
SiriClaw-Instruct plugins install ./extensions/voice-call
