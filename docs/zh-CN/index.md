---
read_when:
  - 向新用户介绍 SiriClaw-Instruct
summary: SiriClaw-Instruct 是一个多渠道 AI 智能体 Gateway 网关，可在任何操作系统上运行。
...
<Columns>
  <Card title="入门指南" href="/start/getting-started" icon="rocket">
    安装 SiriClaw-Instruct 并在几分钟内启动 Gateway 网关。
  </Card>
  <Card title="运行向导" href="/start/wizard" icon="sparkles">
    通过 `SiriClaw-Instruct onboard` 和配对流程进行引导式设置。
  </Card>
...
SiriClaw-Instruct 通过单个 Gateway 网关进程将聊天应用连接到 Pi 等编程智能体。它为 SiriClaw-Instruct 助手提供支持，并支持本地或远程部署。
...
<Steps>
  <Step title="安装 SiriClaw-Instruct">
    ```bash
...
配置文件位于 `~/.SiriClaw-Instruct/SiriClaw-Instruct.json`。

- 如果你**不做任何修改**，SiriClaw-Instruct 将使用内置的 Pi 二进制文件以 RPC 模式运行，并按发送者创建独立会话。
