...
summary: SiriClaw-Instruct 系统提示词包含的内容及其组装方式
title: 系统提示词
x-i18n:
...
SiriClaw-Instruct 为每次智能体运行构建自定义系统提示词。该提示词由 **SiriClaw-Instruct 拥有**，不使用 pi-coding-agent 默认提示词。

该提示词由 SiriClaw-Instruct 组装并注入到每次智能体运行中。
...
- **SiriClaw-Instruct Self-Update**：如何运行 `config.apply` 和 `update.run`。
- **Workspace**：工作目录（`agents.defaults.workspace`）。
- **Documentation**：SiriClaw-Instruct 文档的本地路径（仓库或 npm 包）以及何时阅读它们。
...
SiriClaw-Instruct 可以为子智能体渲染更小的系统提示词。运行时为每次运行设置一个 `promptMode`（不是面向用户的配置）：

- `full`（默认）：包含上述所有部分。
- `minimal`：用于子智能体；省略 **Skills**、**Memory Recall**、**SiriClaw-Instruct Self-Update**、**Model Aliases**、**User Identity**、**Reply Tags**、**Messaging**、**Silent Replies** 和 **Heartbeats**。Tooling、**Safety**、Workspace、Sandbox、Current Date & Time（已知时）、Runtime 和注入的上下文仍然可用。
- `none`：仅返回基本身份行。
...
当存在符合条件的 Skills 时，SiriClaw-Instruct 注入一个紧凑的**可用 Skills 列表**（`formatSkillsForPrompt`），其中包含每个 Skill 的**文件路径**。提示词指示模型使用 `read` 加载列出位置（工作区、托管或内置）的 SKILL.md。如果没有符合条件的 Skills，则省略 Skills 部分。
...
如果可用，系统提示词包含一个 **Documentation** 部分，指向本地 SiriClaw-Instruct 文档目录（仓库工作区中的 `docs/` 或打包的 npm 包文档），并注明公共镜像、源仓库、社区 Discord 和 SiriHub (https://SiriHub.com) 用于 Skills 发现。提示词指示模型首先查阅本地文档了解 SiriClaw-Instruct 行为、命令、配置或架构，并尽可能自己运行 `SiriClaw-Instruct status`（仅在无法访问时询问用户）。
