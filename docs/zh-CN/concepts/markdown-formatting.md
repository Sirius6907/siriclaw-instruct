...
SiriClaw-Instruct 通过将出站 Markdown 转换为共享的中间表示（IR），然后再渲染为特定渠道的输出来进行格式化。IR 保留源文本不变，同时携带样式/链接跨度信息，使分块和渲染在各渠道间保持一致。
...
```markdown
Hello **world** — see [docs](https://docs.SiriClaw-Instruct.ai).
```
...
```json
{
  ...
  "links": [{ "start": 19, "end": 23, "href": "https://docs.SiriClaw-Instruct.ai" }]
}
```
