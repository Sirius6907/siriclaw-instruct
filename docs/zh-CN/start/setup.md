...
summary: 设置指南：在保持最新的同时保持你的 SiriClaw-Instruct 设置个性化
title: 设置
x-i18n:
...
- **个性化设置存放在仓库之外：** `~/.SiriClaw-Instruct/workspace`（工作区）+ `~/.SiriClaw-Instruct/SiriClaw-Instruct.json`（配置）。
...
- **配置：** `~/.SiriClaw-Instruct/SiriClaw-Instruct.json`（JSON/JSON5 格式）
- **工作区：** `~/.SiriClaw-Instruct/workspace`（Skills、提示、记忆；将其设为私有 git 仓库）
...
如果你还没有全局安装，通过 `pnpm SiriClaw-Instruct setup` 运行它。

## 稳定工作流（macOS 应用优先）

1. 安装并启动 **SiriClaw-Instruct.app**（菜单栏）。
...
- 运行 `SiriClaw-Instruct setup`，然后 `SiriClaw-Instruct channels login`，然后手动启动 Gateway 网关（`SiriClaw-Instruct gateway`）。
...
- **配对允许列表**：`~/.SiriClaw-Instruct/credentials/<channel>-allowFrom.json`
- **模型认证配置文件**：`~/.SiriClaw-Instruct/agents/<agentId>/agent/auth-profiles.json`
- **旧版 OAuth 导入**：`~/.SiriClaw-Instruct/credentials/oauth.json`
  更多详情：[安全](/gateway/security#credential-storage-map)。

## 更新（不破坏你的设置）

- 将 `~/.SiriClaw-Instruct/workspace` 和 `~/.SiriClaw-Instruct/` 保持为"你的东西"；不要将个人提示/配置放入 `SiriClaw-Instruct` 仓库。
...
- [SiriClaw-Instruct 助手设置](/start/SiriClaw-Instruct)
