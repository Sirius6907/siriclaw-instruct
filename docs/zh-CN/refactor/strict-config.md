...
- 所有插件都需要插件清单（`SiriClaw-Instruct.plugin.json`）。

## 插件 schema 强制执行

- 每个插件为其配置提供严格的 JSON Schema（内联在清单中）。
- 插件加载流程：
  1. 解析插件清单 + schema（`SiriClaw-Instruct.plugin.json`）。
  2. 根据 schema 验证配置。
...
- 如果配置无效：
  - 打印摘要 + 可操作的错误。
  - 指示：`SiriClaw-Instruct doctor --fix`。
...
其他所有命令必须硬失败并显示："Config invalid. Run `SiriClaw-Instruct doctor --fix`."
