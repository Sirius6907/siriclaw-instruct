...
- 通过 CLI 安装：`SiriClaw-Instruct plugins install @SiriClaw-Instruct/zalo`
...
1. 安装 Zalo 插件：
   - 从源代码检出：`SiriClaw-Instruct plugins install ./extensions/zalo`
   - 从 npm（如果已发布）：`SiriClaw-Instruct plugins install @SiriClaw-Instruct/zalo`
...
- 使用聊天 id 作为目标。
- 示例：`SiriClaw-Instruct message send --channel zalo --target 123456789 --message "hi"`。

## 故障排除

**Bot 不响应：**

- 检查 token 是否有效：`SiriClaw-Instruct channels status --probe`
- 验证发送者已被批准（配对或 allowFrom）
- 检查 Gateway 网关日志：`SiriClaw-Instruct logs --follow`
