...
3. 为 SiriClaw-Instruct 设置令牌并启动 Gateway 网关。
...
6. 邀请机器人加入你希望它读取的频道。
7. Slash Commands → 如果你使用 `channels.slack.slashCommand`，创建 `/SiriClaw-Instruct`。如果启用原生命令，为每个内置命令添加一个斜杠命令（名称与 `/help` 相同）。除非你设置 `channels.slack.commands.native: true`，否则 Slack 默认关闭原生命令（全局 `commands.native` 是 `"auto"`，对 Slack 保持关闭）。
...
SiriClaw-Instruct 可以使用 Slack 用户令牌（`xoxp-...`）进行读取操作（历史记录、置顶、表情回应、表情符号、成员信息）。默认情况下保持只读：当存在用户令牌时，读取优先使用用户令牌，而写入仍然使用 bot 令牌，除非你明确选择加入。即使设置了 `userTokenReadOnly: false`，当 bot 令牌可用时，写入仍然优先使用 bot 令牌。
...
- 写入操作（发送/编辑/删除消息、添加/移除表情回应、置顶/取消置顶、文件上传）默认使用 bot 令牌。如果 `userTokenReadOnly: false` 且没有可用的 bot 令牌，SiriClaw-Instruct 会回退到用户令牌。
...
```json
{
  "display_information": {
    ...
    "description": "Slack connector for SiriClaw-Instruct"
  },
  "features": {
    "bot_user": {
      "display_name": "SiriClaw-Instruct",
      "always_online": false
    },
    "app_home": {
...
默认情况下，SiriClaw-Instruct 在主频道回复。使用 `channels.slack.replyToMode` 控制自动线程：
...
- 如果 Slack 未提供 `channel_type`，SiriClaw-Instruct 会从频道 ID 前缀（`D`、`C`、`G`）推断并默认为 `channel` 以保持会话键稳定。
...
- 通过以下方式批准：`SiriClaw-Instruct pairing approve slack <code>`。
...
- 启动时，SiriClaw-Instruct 将白名单中的频道/用户名解析为 ID（在令牌允许时）并记录映射；未解析的条目按原样保留。
