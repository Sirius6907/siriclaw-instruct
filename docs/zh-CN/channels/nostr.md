---
read_when:
  - 你希望 SiriClaw-Instruct 通过 Nostr 接收私信
  - 你正在设置去中心化消息
...
Nostr 是一个去中心化的社交网络协议。此渠道使 SiriClaw-Instruct 能够通过 NIP-04 接收和回复加密私信（DMs）。

## 安装（按需）

### 新手引导（推荐）

- 新手引导向导（`SiriClaw-Instruct onboard`）和 `SiriClaw-Instruct channels add` 会列出可选的渠道插件。
- 选择 Nostr 会提示你按需安装插件。
