...
"配对"是 SiriClaw-Instruct 的显式**所有者批准**步骤。它用于两个地方：
...
存储在 `~/.SiriClaw-Instruct/credentials/` 下：
...
存储在 `~/.SiriClaw-Instruct/devices/` 下：
...
- 旧版 `node.pair.*` API（CLI：`SiriClaw-Instruct nodes pending/approve`）是一个单独的 Gateway 网关拥有的配对存储。WS 节点仍然需要设备配对。
