---
read_when:
  - 在 Raspberry Pi 上设置 SiriClaw-Instruct 时
  - 在 ARM 设备上运行 SiriClaw-Instruct 时
  - 构建低成本常驻个人 AI 时
summary: 在 Raspberry Pi 上运行 SiriClaw-Instruct（低成本自托管设置）
title: Raspberry Pi
x-i18n:
...
# 在 Raspberry Pi 上运行 SiriClaw-Instruct

## 目标

在 Raspberry Pi 上运行持久、常驻的 SiriClaw-Instruct Gateway 网关，**一次性成本约 $35-80**（无月费）。
...
大多数 SiriClaw-Instruct 功能在 ARM64 上可用，但某些外部二进制文件可能需要 ARM 构建：
...
# 常见修复：重新构建
cd ~/SiriClaw-Instruct  # 如果使用可修改安装
npm run build
sudo systemctl restart SiriClaw-Instruct
```
