...
- `~/Library/Application Support/SiriClaw-Instruct/canvas/<session>/...`

Canvas 面板通过**自定义 URL 方案**提供这些文件：

- `SiriClaw-Instruct-canvas://<session>/<path>`

示例：

- `SiriClaw-Instruct-canvas://main/` → `<canvasRoot>/main/index.html`
- `SiriClaw-Instruct-canvas://main/assets/app.css` → `<canvasRoot>/main/assets/app.css`
- `SiriClaw-Instruct-canvas://main/widgets/todo/` → `<canvasRoot>/main/widgets/todo/index.html`
...
```bash
SiriClaw-Instruct nodes canvas present --node <id>
SiriClaw-Instruct nodes canvas navigate --node <id> --url "/"
SiriClaw-Instruct nodes canvas eval --node <id> --js "document.title"
SiriClaw-Instruct nodes canvas snapshot --node <id>
```
...
```
http://<gateway-host>:18793/__SiriClaw-Instruct__/a2ui/
```
...
SiriClaw-Instruct nodes canvas a2ui push --jsonl /tmp/a2ui-v0.8.jsonl --node <id>
```

快速测试：

```bash
SiriClaw-Instruct nodes canvas a2ui push --node <id> --text "Hello from A2UI"
```
...
```js
window.location.href = "SiriClaw-Instruct://agent?message=Review%20this%20design";
```
