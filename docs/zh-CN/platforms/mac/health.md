...
- 应用每约 60 秒和按需时通过 `ShellExecutor` 运行 `SiriClaw-Instruct health --json`。探测加载凭证并报告状态，不发送消息。
- 分别缓存上次成功的快照和上次错误以避免闪烁；显示每个的时间戳。

## 有疑问时

- 你仍然可以使用 [Gateway 网关健康](/gateway/health) 中的 CLI 流程（`SiriClaw-Instruct status`、`SiriClaw-Instruct status --deep`、`SiriClaw-Instruct health --json`），并在 `/tmp/SiriClaw-Instruct/SiriClaw-Instruct-*.log` 中跟踪 `web-heartbeat` / `web-reconnect`。
