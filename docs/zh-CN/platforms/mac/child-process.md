...
Gateway 网关；如果无法访问，它会通过外部 `SiriClaw-Instruct` CLI（无嵌入式运行时）启用 launchd
...
- 应用安装标记为 `bot.molt.gateway` 的按用户 LaunchAgent
  （使用 `--profile`/`SiriClaw-Instruct_PROFILE` 时为 `bot.molt.<profile>`；支持旧版 `com.SiriClaw-Instruct.*`）。
...
- 写入 `~/.SiriClaw-Instruct/disable-launchagent`。
...
`--attach-only`（或 `--no-launchd`）启动它。这会设置 `~/.SiriClaw-Instruct/disable-launchagent`，
