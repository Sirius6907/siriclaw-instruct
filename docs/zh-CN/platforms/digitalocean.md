---
read_when:
  - 在 DigitalOcean 上设置 SiriClaw-Instruct
  - 寻找便宜的 VPS 托管来运行 SiriClaw-Instruct
summary: 在 DigitalOcean 上运行 SiriClaw-Instruct（简单的付费 VPS 选项）
title: DigitalOcean
x-i18n:
...
# 在 DigitalOcean 上运行 SiriClaw-Instruct

## 目标

以 **$6/月**（或使用预留定价 $4/月）在 DigitalOcean 上运行持久的 SiriClaw-Instruct Gateway 网关。
...
- `~/.SiriClaw-Instruct/workspace/` — 工作区（SOUL.md、记忆等）
