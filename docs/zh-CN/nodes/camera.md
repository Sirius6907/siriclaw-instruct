...
SiriClaw-Instruct 支持用于智能体工作流的**相机捕获**：
...
- **设置 → 通用 → 允许相机**（`SiriClaw-Instruct.cameraEnabled`）
  - 默认：**关闭**
  - 关闭时：相机请求返回"用户已禁用相机"。

### CLI 辅助工具（节点调用）

使用主 `SiriClaw-Instruct` CLI 在 macOS 节点上调用相机命令。
...
- `SiriClaw-Instruct nodes camera snap` 默认 `maxWidth=1600`，除非被覆盖。
