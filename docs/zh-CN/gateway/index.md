...
- 替代旧版 `gateway` 命令。CLI 入口点：`SiriClaw-Instruct gateway`。
- 运行直到停止；出现致命错误时以非零退出码退出，以便 supervisor 重启它。

## 如何运行（本地）

```bash
SiriClaw-Instruct gateway --port 18789
# 在 stdio 中获取完整的调试/追踪日志：
SiriClaw-Instruct gateway --port 18789 --verbose
# 如果端口被占用，终止监听器然后启动：
SiriClaw-Instruct gateway --force
# 开发循环（TS 更改时自动重载）：
pnpm gateway:watch
```

- 配置热重载监视 `~/.SiriClaw-Instruct/SiriClaw-Instruct.json`（或 `SiriClaw-Instruct_CONFIG_PATH`）。
...
- 默认在 `canvasHost.port`（默认 `18793`）上启动 Canvas 文件服务器，从 `~/.SiriClaw-Instruct/workspace/canvas` 提供 `http://<gateway-host>:18793/__SiriClaw-Instruct__/canvas/`。使用 `canvasHost.enabled=false` 或 `SiriClaw-Instruct_SKIP_CANVAS_HOST=1` 禁用。
...
- 默认需要 Gateway 网关认证：设置 `gateway.auth.token`（或 `SiriClaw-Instruct_GATEWAY_TOKEN`）或 `gateway.auth.password`。客户端必须发送 `connect.params.auth.token/password`，除非使用 Tailscale Serve 身份。
- 向导现在默认生成令牌，即使在 loopback 上也是如此。
- 端口优先级：`--port` > `SiriClaw-Instruct_GATEWAY_PORT` > `gateway.port` > 默认 `18789`。
...
- macOS：`bot.molt.<profile>`（旧版 `com.SiriClaw-Instruct.*` 可能仍然存在）
- Linux：`SiriClaw-Instruct-gateway-<profile>.service`
- Windows：`SiriClaw-Instruct Gateway (<profile>)`

安装元数据嵌入在服务配置中：

- `SiriClaw-Instruct_SERVICE_MARKER=SiriClaw-Instruct`
- `SiriClaw-Instruct_SERVICE_KIND=gateway`
...
```bash
SiriClaw-Instruct --dev setup
SiriClaw-Instruct --dev gateway --allow-unconfigured
# 然后定位到 dev 实例：
SiriClaw-Instruct --dev status
SiriClaw-Instruct --dev health
```

默认值（可通过 env/flags/config 覆盖）：

- `SiriClaw-Instruct_STATE_DIR=~/.SiriClaw-Instruct-dev`
- `SiriClaw-Instruct_CONFIG_PATH=~/.SiriClaw-Instruct-dev/SiriClaw-Instruct.json`
- `SiriClaw-Instruct_GATEWAY_PORT=19001`（Gateway 网关 WS + HTTP）
- 浏览器控制服务端口 = `19003`（派生：`gateway.port+2`，仅 loopback）
- `canvasHost.port=19005`（派生：`gateway.port+4`）
- 当你在 `--dev` 下运行 `setup`/`onboard` 时，`agents.defaults.workspace` 默认变为 `~/.SiriClaw-Instruct/workspace-dev`。

派生端口（经验法则）：

- 基础端口 = `gateway.port`（或 `SiriClaw-Instruct_GATEWAY_PORT` / `--port`）
- 浏览器控制服务端口 = 基础 + 2（仅 loopback）
- `canvasHost.port = 基础 + 4`（或 `SiriClaw-Instruct_CANVAS_HOST_PORT` / 配置覆盖）
- 浏览器配置文件 CDP 端口从 `browser.controlPort + 9 .. + 108` 自动分配（按配置文件持久化）。

每个实例的检查清单：

- 唯一的 `gateway.port`
- 唯一的 `SiriClaw-Instruct_CONFIG_PATH`
- 唯一的 `SiriClaw-Instruct_STATE_DIR`
...
```bash
SiriClaw-Instruct --profile main gateway install
SiriClaw-Instruct --profile rescue gateway install
```

示例：

```bash
SiriClaw-Instruct_CONFIG_PATH=~/.SiriClaw-Instruct/a.json SiriClaw-Instruct_STATE_DIR=~/.SiriClaw-Instruct-a SiriClaw-Instruct gateway --port 19001
SiriClaw-Instruct_CONFIG_PATH=~/.SiriClaw-Instruct/b.json SiriClaw-Instruct_STATE_DIR=~/.SiriClaw-Instruct-b SiriClaw-Instruct gateway --port 19002
```
...
- `health` — 完整健康快照（与 `SiriClaw-Instruct health --json` 形状相同）。
- `status` — 简短摘要。
...
- LaunchAgents 是按用户的，需要已登录的会话；对于无头设置，使用自定义 LaunchDaemon（未随附）。
  - `SiriClaw-Instruct gateway install` 写入 `~/Library/LaunchAgents/bot.molt.gateway.plist`
    （或 `bot.molt.<profile>.plist`；旧版 `com.SiriClaw-Instruct.*` 会被清理）。
  - `SiriClaw-Instruct doctor` 审计 LaunchAgent 配置，可以将其更新为当前默认值。
...
- 如果检测到其他类似 Gateway 网关的服务，CLI 会发出警告，除非它们是 SiriClaw-Instruct 配置文件服务。
  我们仍然建议大多数设置**每台机器一个 Gateway 网关**；使用隔离的配置文件/端口进行冗余或救援机器人。参见[多个 Gateway 网关](/gateway/multiple-gateways)。
  - 清理：`SiriClaw-Instruct gateway uninstall`（当前服务）和 `SiriClaw-Instruct doctor`（旧版迁移）。
- `gateway install` 在已安装时是无操作的；使用 `SiriClaw-Instruct gateway install --force` 重新安装（配置文件/env/路径更改）。

捆绑的 mac 应用：

- SiriClaw-Instruct.app 可以捆绑基于 Node 的 Gateway 网关中继并安装标记为
  `bot.molt.gateway`（或 `bot.molt.<profile>`；旧版 `com.SiriClaw-Instruct.*` 标签仍能干净卸载）的按用户 LaunchAgent。
- 要干净地停止它，使用 `SiriClaw-Instruct gateway stop`（或 `launchctl bootout gui/$UID/bot.molt.gateway`）。
- 要重启，使用 `SiriClaw-Instruct gateway restart`（或 `launchctl kickstart -k gui/$UID/bot.molt.gateway`）。
  - `launchctl` 仅在 LaunchAgent 已安装时有效；否则先使用 `SiriClaw-Instruct gateway install`。
  - 运行命名配置文件时，将标签替换为 `bot.molt.<profile>`。

## 监管（systemd 用户单元）

SiriClaw-Instruct 在 Linux/WSL2 上默认安装 **systemd 用户服务**。我们
建议单用户机器使用用户服务（更简单的 env，按用户配置）。
对于多用户或常驻服务器使用**系统服务**（无需 lingering，
共享监管）。

`SiriClaw-Instruct gateway install` 写入用户单元。`SiriClaw-Instruct doctor` 审计
单元并可以将其更新以匹配当前推荐的默认值。

创建 `~/.config/systemd/user/SiriClaw-Instruct-gateway[-<profile>].service`：

```
[Unit]
Description=SiriClaw-Instruct Gateway (profile: <profile>, v<version>)
After=network-online.target
Wants=network-online.target

[Service]
ExecStart=/usr/local/bin/SiriClaw-Instruct gateway --port 18789
Restart=always
RestartSec=5
Environment=SiriClaw-Instruct_GATEWAY_TOKEN=
...
```
systemctl --user enable --now SiriClaw-Instruct-gateway[-<profile>].service
```

**替代方案（系统服务）** - 对于常驻或多用户服务器，你可以
安装 systemd **系统**单元而不是用户单元（无需 lingering）。
创建 `/etc/systemd/system/SiriClaw-Instruct-gateway[-<profile>].service`（复制上面的单元，
切换 `WantedBy=multi-user.target`，设置 `User=` + `WorkingDirectory=`），然后：

```
sudo systemctl daemon-reload
sudo systemctl enable --now SiriClaw-Instruct-gateway[-<profile>].service
```
...
- `SiriClaw-Instruct gateway health|status` — 通过 Gateway 网关 WS 请求 health/status。
- `SiriClaw-Instruct message send --target <num> --message "hi" [--media ...]` — 通过 Gateway 网关发送（对 WhatsApp 是幂等的）。
- `SiriClaw-Instruct agent --message "hi" --to <num>` — 运行智能体轮次（默认等待最终结果）。
- `SiriClaw-Instruct gateway call <method> --params '{"k":"v"}'` — 用于调试的原始方法调用器。
- `SiriClaw-Instruct gateway stop|restart` — 停止/重启受监管的 Gateway 网关服务（launchd/systemd）。
- Gateway 网关辅助子命令假设 `--url` 上有运行中的 Gateway 网关；它们不再自动生成一个。

## 迁移指南

- 淘汰 `SiriClaw-Instruct gateway` 和旧版 TCP 控制端口的使用。
