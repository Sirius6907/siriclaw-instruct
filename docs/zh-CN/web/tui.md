...
- 开启投递：
  - `/deliver on`
  - 或设置面板
  - 或使用 `SiriClaw-Instruct tui --deliver` 启动
...
- 确认智能体可以运行：`SiriClaw-Instruct status` 和 `SiriClaw-Instruct models status`。
...
- 选择器中没有智能体：检查 `SiriClaw-Instruct agents list` 和你的路由配置。
