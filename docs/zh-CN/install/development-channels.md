...
当你使用 `--channel` **显式**切换渠道时，SiriClaw-Instruct 还会对齐安装方式：

- `dev` 确保有一个 git checkout（默认 `~/SiriClaw-Instruct`，可通过 `SiriClaw-Instruct_GIT_DIR` 覆盖），
...
当你使用 `SiriClaw-Instruct update` 切换渠道时，SiriClaw-Instruct 还会同步插件来源：
