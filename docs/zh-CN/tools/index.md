...
read_when:
  - 添加或修改智能体工具
  - 停用或更改 `SiriClaw-Instruct-*` Skills
summary: SiriClaw-Instruct 的智能体工具接口（browser、canvas、nodes、message、cron），替代旧版 `SiriClaw-Instruct-*` Skills
title: 工具
x-i18n:
...
# 工具（SiriClaw-Instruct）

SiriClaw-Instruct 为 browser、canvas、nodes 和 cron 暴露**一流的智能体工具**。
这些工具取代了旧的 `SiriClaw-Instruct-*` Skills：工具是类型化的，无需调用 shell，
智能体应该直接依赖它们。

## 禁用工具

你可以通过 `SiriClaw-Instruct.json` 中的 `tools.allow` / `tools.deny` 全局允许/拒绝工具
...
- 匹配不区分大小写。
- 支持 `*` 通配符（`"*"` 表示所有工具）。
- 如果 `tools.allow` 仅引用未知或未加载的插件工具名称，SiriClaw-Instruct 会记录警告并忽略允许列表，以确保核心工具保持可用。
...
- `group:nodes`：`nodes`
- `group:SiriClaw-Instruct`：所有内置 SiriClaw-Instruct 工具（不包括提供商插件）
...
- `host=node` 可以针对 macOS 配套应用或无头节点主机（`SiriClaw-Instruct node run`）。
...
- 需要 Brave API 密钥（推荐：`SiriClaw-Instruct configure --section web`，或设置 `BRAVE_API_KEY`）。
...
控制专用的 SiriClaw-Instruct 管理的浏览器。
...
- 快速冒烟测试：`SiriClaw-Instruct nodes canvas a2ui push --node <id> --text "Hello from A2UI"`。
...
- `restart`（授权 + 发送 `SIGUSR1` 进行进程内重启；`SiriClaw-Instruct gateway` 就地重启）
