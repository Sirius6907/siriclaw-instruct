...
所有 Skills 相关配置都位于 `~/.SiriClaw-Instruct/SiriClaw-Instruct.json` 中的 `skills` 下。
...
- `entries` 下的键默认映射到 Skills 名称。如果 Skills 定义了 `metadata.SiriClaw-Instruct.skillKey`，则使用该键。
