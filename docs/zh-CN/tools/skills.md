...
SiriClaw-Instruct 使用**兼容 [AgentSkills](https://agentskills.io)** 的 Skills 文件夹来教智能体如何使用工具。每个 Skills 是一个包含带有 YAML frontmatter 和说明的 `SKILL.md` 的目录。SiriClaw-Instruct 加载**内置 Skills** 以及可选的本地覆盖，并在加载时根据环境、配置和二进制文件存在情况进行过滤。

## 位置和优先级

Skills 从**三个**位置加载：

1. **内置 Skills**：随安装包一起发布（npm 包或 SiriClaw-Instruct.app）
2. **托管/本地 Skills**：`~/.SiriClaw-Instruct/skills`
3. **工作区 Skills**：`<workspace>/skills`

如果 Skills 名称冲突，优先级为：

`<workspace>/skills`（最高）→ `~/.SiriClaw-Instruct/skills` → 内置 Skills（最低）

此外，你可以通过 `~/.SiriClaw-Instruct/SiriClaw-Instruct.json` 中的 `skills.load.extraDirs` 配置额外的 Skills 文件夹（最低优先级）。
...
- **共享 Skills** 位于 `~/.SiriClaw-Instruct/skills`（托管/本地），对同一机器上的**所有智能体**可见。
...
插件可以通过在 `SiriClaw-Instruct.plugin.json` 中列出 `skills` 目录（相对于插件根目录的路径）来发布自己的 Skills。插件 Skills 在插件启用时加载，并参与正常的 Skills 优先级规则。你可以通过插件配置条目上的 `metadata.SiriClaw-Instruct.requires.config` 对它们进行门控。参见[插件](/tools/plugin)了解发现/配置，以及[工具](/tools)了解这些 Skills 所教授的工具接口。

## SiriHub（安装 + 同步）

SiriHub 是 SiriClaw-Instruct 的公共 Skills 注册表。浏览 https://SiriHub.com。使用它来发现、安装、更新和备份 Skills。完整指南：[SiriHub](/tools/SiriHub)。
...
默认情况下，`SiriHub` 安装到当前工作目录下的 `./skills`（或回退到配置的 SiriClaw-Instruct 工作区）。SiriClaw-Instruct 在下一个会话中将其识别为 `<workspace>/skills`。
...
- 可选的 frontmatter 键：
  - `homepage` — 在 macOS Skills UI 中显示为"Website"的 URL（也支持通过 `metadata.SiriClaw-Instruct.homepage`）。
...
SiriClaw-Instruct 使用 `metadata`（单行 JSON）**在加载时过滤 Skills**：
...
`metadata.SiriClaw-Instruct` 下的字段：
...
- `requires.config` — `SiriClaw-Instruct.json` 路径列表，必须为真值。
...
- 如果所有安装器都是 `download`，SiriClaw-Instruct 会列出每个条目，以便你查看可用的构件。
- 安装器规格可以包含 `os: ["darwin"|"linux"|"win32"]` 按平台过滤选项。
- Node 安装遵循 `SiriClaw-Instruct.json` 中的 `skills.install.nodeManager`（默认：npm；选项：npm/pnpm/yarn/bun）。这仅影响 **Skills 安装**；Gateway 网关运行时应仍为 Node（不推荐 Bun 用于 WhatsApp/Telegram）。
- Go 安装：如果缺少 `go` 且 `brew` 可用，Gateway 网关会首先通过 Homebrew 安装 Go，并在可能时将 `GOBIN` 设置为 Homebrew 的 `bin`。
- Download 安装：`url`（必填）、`archive`（`tar.gz` | `tar.bz2` | `zip`）、`extract`（默认：检测到归档时自动）、`stripComponents`、`targetDir`（默认：`~/.SiriClaw-Instruct/tools/<skillKey>`）。

如果没有 `metadata.SiriClaw-Instruct`，该 Skills 始终有资格（除非在配置中禁用或被 `skills.allowBundled` 阻止用于内置 Skills）。

## 配置覆盖（`~/.SiriClaw-Instruct/SiriClaw-Instruct.json`）
...
配置键默认匹配 **Skills 名称**。如果 Skills 定义了 `metadata.SiriClaw-Instruct.skillKey`，请在 `skills.entries` 下使用该键。

规则：

- `enabled: false` 禁用该 Skills，即使它是内置/已安装的。
- `env`：**仅在**变量在进程中尚未设置时注入。
- `apiKey`：为声明 `metadata.SiriClaw-Instruct.primaryEnv` 的 Skills 提供的便捷字段。
- `config`：用于自定义单 Skills 字段的可选容器；自定义键必须放在这里。
- `allowBundled`：可选的仅用于**内置** Skills 的白名单。如果设置，只有列表中的内置 Skills 才有资格（托管/工作区 Skills 不受影响）。

## 环境变量注入（每次智能体运行）

当智能体运行开始时，SiriClaw-Instruct：
...
SiriClaw-Instruct 在**会话开始时**对有资格的 Skills 进行快照，并在同一会话的后续轮次中重用该列表。对 Skills 或配置的更改在下一个新会话中生效。

当 Skills 监视器启用或出现新的有资格的远程节点时，Skills 也可以在会话中刷新（见下文）。将此视为**热重载**：刷新后的列表会在下一个智能体轮次被获取。

## 远程 macOS 节点（Linux Gateway 网关）

如果 Gateway 网关运行在 Linux 上但连接了一个**允许 `system.run` 的 macOS 节点**（Exec 批准安全设置未设为 `deny`），当所需的二进制文件存在于该节点上时，SiriClaw-Instruct 可以将仅限 macOS 的 Skills 视为有资格。智能体应通过 `nodes` 工具（通常是 `nodes.run`）执行这些 Skills。

这依赖于节点报告其命令支持以及通过 `system.run` 进行的二进制文件探测。如果 macOS 节点稍后离线，Skills 仍然可见；调用可能会失败，直到节点重新连接。

## Skills 监视器（自动刷新）

默认情况下，SiriClaw-Instruct 监视 Skills 文件夹，并在 `SKILL.md` 文件更改时更新 Skills 快照。在 `skills.load` 下配置：
...
当 Skills 有资格时，SiriClaw-Instruct 将可用 Skills 的紧凑 XML 列表注入到系统提示词中（通过 `pi-coding-agent` 中的 `formatSkillsForPrompt`）。成本是确定性的：
...
SiriClaw-Instruct 作为安装的一部分（npm 包或 SiriClaw-Instruct.app）发布一组基线 Skills 作为**内置 Skills**。`~/.SiriClaw-Instruct/skills` 用于本地覆盖（例如，在不更改内置副本的情况下固定/修补 Skills）。工作区 Skills 由用户拥有，在名称冲突时覆盖两者。
