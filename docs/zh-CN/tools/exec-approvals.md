...
- **gateway 主机** → gateway 机器上的 `SiriClaw-Instruct` 进程
...
如果节点尚未通告执行审批，请直接编辑其本地的 `~/.SiriClaw-Instruct/exec-approvals.json`。

CLI：`SiriClaw-Instruct approvals` 支持 gateway 或 node 编辑（参见 [Approvals CLI](/cli/approvals)）。
