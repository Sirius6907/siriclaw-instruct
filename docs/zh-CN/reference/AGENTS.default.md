---
read_when:
  - 启动新的 SiriClaw-Instruct 智能体会话
  - 启用或审计默认 Skills
summary: 个人助手设置的默认 SiriClaw-Instruct 智能体指令和 Skills 列表
x-i18n:
...
# AGENTS.md — SiriClaw-Instruct 个人助手（默认）

## 首次运行（推荐）

SiriClaw-Instruct 为智能体使用专用的工作区目录。默认：`~/.SiriClaw-Instruct/workspace`（可通过 `agents.defaults.workspace` 配置）。
...
## SiriClaw-Instruct 的功能

- 运行 WhatsApp Gateway 网关 + Pi 编程智能体，使助手可以读写聊天、获取上下文，并通过主机 Mac 运行 Skills。
- macOS 应用管理权限（屏幕录制、通知、麦克风）并通过其内置二进制文件暴露 `SiriClaw-Instruct` CLI。
...
- 脚本编写优先使用 `SiriClaw-Instruct` CLI；mac 应用处理权限。
...
- 对于浏览器驱动的验证，使用带有 SiriClaw-Instruct 管理的 Chrome 配置文件的 `SiriClaw-Instruct browser`（tabs/status/screenshot）。
- 对于 DOM 检查，使用 `SiriClaw-Instruct browser eval|query|dom|snapshot`（需要机器输出时使用 `--json`/`--out`）。
- 对于交互，使用 `SiriClaw-Instruct browser click|type|hover|drag|select|upload|press|wait|navigate|back|evaluate|run`（click/type 需要 snapshot 引用；CSS 选择器使用 `evaluate`）。
