...
summary: "`SiriClaw-Instruct node` 的 CLI 参考（无头节点主机）"
title: node
x-i18n:
...
使用 `SiriClaw-Instruct node run` 运行前台节点主机（无服务）。
...
- `SiriClaw-Instruct approvals --node <id|name|ip>`（从 Gateway 网关编辑）
