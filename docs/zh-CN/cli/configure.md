...
summary: "`SiriClaw-Instruct configure` 的 CLI 参考（交互式配置提示）"
title: configure
x-i18n:
...
提示：不带子命令的 `SiriClaw-Instruct config` 会打开相同的向导。使用 `SiriClaw-Instruct config get|set|unset` 进行非交互式编辑。
...
```bash
SiriClaw-Instruct configure
SiriClaw-Instruct configure --section models --section channels
```
