...
read_when:
  ...
  - 你想设置默认工作区路径
summary: "`SiriClaw-Instruct setup` 的 CLI 参考（初始化配置 + 工作区）"
title: setup
x-i18n:
...
初始化 `~/.SiriClaw-Instruct/SiriClaw-Instruct.json` 和智能体工作区。
...
```bash
SiriClaw-Instruct setup
SiriClaw-Instruct setup --workspace ~/.SiriClaw-Instruct/workspace
```

通过 setup 运行向导：

```bash
SiriClaw-Instruct setup --wizard
```
