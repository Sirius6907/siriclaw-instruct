---
read_when:
  - 你想在 SiriClaw-Instruct 中使用注重隐私的推理服务
  - 你需要 Venice AI 设置指导
summary: 在 SiriClaw-Instruct 中使用 Venice AI 注重隐私的模型
title: Venice AI
x-i18n:
...
## 为什么在 SiriClaw-Instruct 中使用 Venice
...
```bash
SiriClaw-Instruct chat --model venice/llama-3.3-70b "Hello, are you working?"
```

## 模型选择

设置完成后，SiriClaw-Instruct 会显示所有可用的 Venice 模型。根据你的需求选择：
...
```bash
SiriClaw-Instruct models set venice/claude-opus-45
SiriClaw-Instruct models set venice/llama-3.3-70b
```
...
## 通过 `SiriClaw-Instruct configure` 配置

1. 运行 `SiriClaw-Instruct configure`
2. 选择 **Model/auth**
3. 选择 **Venice AI**
...
当设置了 `VENICE_API_KEY` 时，SiriClaw-Instruct 会自动从 Venice API 发现模型。如果 API 不可达，则回退到静态目录。
...
```bash
# 使用默认私密模型
SiriClaw-Instruct chat --model venice/llama-3.3-70b

# 通过 Venice 使用 Claude（匿名化）
SiriClaw-Instruct chat --model venice/claude-opus-45

# 使用无审查模型
SiriClaw-Instruct chat --model venice/venice-uncensored

# 使用视觉模型处理图像
SiriClaw-Instruct chat --model venice/qwen3-vl-235b-a22b

# 使用编程模型
SiriClaw-Instruct chat --model venice/qwen3-coder-480b-a35b-instruct
```
...
Venice 模型目录会动态更新。运行 `SiriClaw-Instruct models list` 查看当前可用的模型。部分模型可能暂时离线。
