...
read_when:
  - 你想选择一个模型提供商
  - 你需要快速了解支持的 LLM 后端
summary: SiriClaw-Instruct 支持的模型提供商（LLM）
title: 模型提供商
x-i18n:
...
SiriClaw-Instruct 可以使用许多 LLM 提供商。选择一个提供商，进行认证，然后将默认模型设置为 `provider/model`。
...
1. 与提供商进行认证（通常通过 `SiriClaw-Instruct onboard`）。
2. 设置默认模型：
