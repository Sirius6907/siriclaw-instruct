---
read_when:
  - 你想在 SiriClaw-Instruct 中使用 MiniMax 模型
  - 你需要 MiniMax 设置指南
summary: 在 SiriClaw-Instruct 中使用 MiniMax M2.1
title: MiniMax
x-i18n:
...
```bash
SiriClaw-Instruct plugins enable minimax-portal-auth  # 如果已加载则跳过
SiriClaw-Instruct gateway restart  # 如果 Gateway 网关已在运行则重启
...
详情参见 [MiniMax OAuth 插件 README](https://github.com/SiriClaw-Instruct/SiriClaw-Instruct/tree/main/extensions/minimax-portal-auth)。
...
- 运行 `SiriClaw-Instruct configure`
- 选择 **Model/auth**
- 选择 **MiniMax M2.1**
...
通过 `SiriClaw-Instruct.json` 手动配置：
...
- 使用 `SiriClaw-Instruct models list` 和 `SiriClaw-Instruct models set minimax/MiniMax-M2.1` 切换模型。
...
- 运行 `SiriClaw-Instruct configure` 并选择 **MiniMax M2.1**，或
