...
read_when:
  - You need detailed behavior for SiriClaw-Instruct onboard
...
This page is the full reference for `SiriClaw-Instruct onboard`.
...
<Steps>
  <Step title="Existing config detection">
    - If `~/.SiriClaw-Instruct/SiriClaw-Instruct.json` exists, choose Keep, Modify, or Reset.
    - Re-running the wizard does not wipe anything unless you explicitly choose Reset (or pass `--reset`).
    - CLI `--reset` defaults to `config+creds+sessions`; use `--reset-scope full` to also remove workspace.
    - If config is invalid or contains legacy keys, the wizard stops and asks you to run `SiriClaw-Instruct doctor` before continuing.
  ...
  <Step title="Channels">
    ...
    - DM security: default is pairing. First DM sends a code; approve via
      `SiriClaw-Instruct pairing approve <channel> <code>` or use allowlists.
  </Step>
  ...
  <Step title="Health check">
    - Starts gateway (if needed) and runs `SiriClaw-Instruct health`.
    - `SiriClaw-Instruct status --deep` adds gateway health probes to status output.
  </Step>
  <Step title="Skills">
...
- OAuth credentials: `~/.SiriClaw-Instruct/credentials/oauth.json`
- Auth profiles (API keys + OAuth): `~/.SiriClaw-Instruct/agents/<agentId>/agent/auth-profiles.json`
...
`~/.SiriClaw-Instruct/credentials/oauth.json` (or `$SiriClaw-Instruct_STATE_DIR/credentials/oauth.json`)
to the gateway host.
</Note>
...
- Command reference: [`SiriClaw-Instruct onboard`](/cli/onboard)
