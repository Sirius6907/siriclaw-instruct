...
The onboarding wizard is the **recommended** way to set up SiriClaw-Instruct on macOS,
...
`SiriClaw-Instruct dashboard` and chat in the browser. Docs: [Dashboard](/web/dashboard).
</Info>
...
`SiriClaw-Instruct configure --section web`. Docs: [Web tools](/tools/web).
</Tip>
...
2. **Workspace** — Location for agent files (default `~/.SiriClaw-Instruct/workspace`). Seeds bootstrap files.
...
If the config is invalid or contains legacy keys, the wizard asks you to run `SiriClaw-Instruct doctor` first.
</Note>
...
Use `SiriClaw-Instruct agents add <name>` to create a separate agent with its own workspace,
...
- Default workspaces follow `~/.SiriClaw-Instruct/workspace-<agentId>`.
...
- CLI command reference: [`SiriClaw-Instruct onboard`](/cli/onboard)
