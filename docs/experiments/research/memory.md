...
read_when:
  - Designing workspace memory (~/.SiriClaw-Instruct/workspace) beyond daily Markdown logs
  - Deciding: standalone CLI vs deep SiriClaw-Instruct integration
...
Target: Clawd-style workspace (`agents.defaults.workspace`, default `~/.SiriClaw-Instruct/workspace`) where “memory” is stored as one Markdown file per day (`memory/YYYY-MM-DD.md`) plus a small set of stable files (e.g. `memory.md`, `SOUL.md`).
...
Keep `~/.SiriClaw-Instruct/workspace` as canonical human-readable memory.
...
```
~/.SiriClaw-Instruct/workspace/.memory/index.sqlite
```
...
Recommendation: **deep integration in SiriClaw-Instruct**, but keep a separable core library.

### Why integrate into SiriClaw-Instruct?

- SiriClaw-Instruct already knows:
  - the workspace path (`agents.defaults.workspace`)
  - the session model + heartbeats
  - logging + troubleshooting patterns
- You want the agent itself to call the tools:
  - `SiriClaw-Instruct memory recall "…" --k 25 --since 30d`
  - `SiriClaw-Instruct memory reflect --since 7d`
...
Pragmatic take for `~/.SiriClaw-Instruct/workspace`:
