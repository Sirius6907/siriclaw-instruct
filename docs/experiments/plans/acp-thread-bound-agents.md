...
This plan defines how SiriClaw-Instruct should support ACP coding agents in thread-capable channels (Discord first) with production-level lifecycle and recovery.
...
- SiriClaw-Instruct core owns ACP control plane concerns
...
SiriClaw-Instruct should not reimplement ACP transport internals in core.
SiriClaw-Instruct should not rely on a pure plugin-only interception path for routing.

## North-star architecture (holy grail)

Treat ACP as a first-class control plane in SiriClaw-Instruct, with pluggable runtime adapters.
...
- move ACP control-plane state to a dedicated SQLite store (WAL mode) under SiriClaw-Instruct state dir
...
Core (must be in SiriClaw-Instruct core):
...
- ACP event stream is normalized to SiriClaw-Instruct reply chunks
...
For the first production backend (`extensions/acpx`), SiriClaw-Instruct and acpx are
...
- encoded payload fields:
  - `name` (acpx named session; uses SiriClaw-Instruct `sessionKey`)
...
- SiriClaw-Instruct consumes ndjson events from `acpx --format json --json-strict`
