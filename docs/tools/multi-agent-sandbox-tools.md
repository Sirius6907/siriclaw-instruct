...
For debugging “why is this blocked?”, see [Sandbox vs Tool Policy vs Elevated](/gateway/sandbox-vs-tool-policy-vs-elevated) and `SiriClaw-Instruct sandbox explain`.
...
Legacy `agent.*` configs are migrated by `SiriClaw-Instruct doctor`; prefer `agents.defaults` + `agents.list` going forward.
...
2. **Verify sandbox containers:**
   ...
   ```exec
   docker ps --filter "name=SiriClaw-Instruct-sbx-"
   ```
...
4. **Monitor logs:**
   ...
   ```exec
   tail -f "${SiriClaw-Instruct_STATE_DIR:-$HOME/.SiriClaw-Instruct}/logs/gateway.log" | grep -E "routing|sandbox|tools"
   ```
