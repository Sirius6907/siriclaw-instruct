...
- `gateway`/`node` approvals are controlled by `~/.SiriClaw-Instruct/exec-approvals.json`.
...
- SiriClaw-Instruct sets `SiriClaw-Instruct_SHELL=exec` in the spawned command environment (including PTY and sandbox execution) so shell/profile rules can detect exec-tool context.
...
- `host=sandbox`: runs `sh -lc` (login shell) inside the container, so `/etc/profile` may reset `PATH`.
  SiriClaw-Instruct prepends `env.PATH` after profile sourcing via an internal env var (no shell interpolation);
...
`SiriClaw-Instruct security audit` warns when interpreter/runtime `safeBins` entries are missing explicit profiles, and `SiriClaw-Instruct doctor --fix` can scaffold missing custom `safeBinProfiles` entries.
