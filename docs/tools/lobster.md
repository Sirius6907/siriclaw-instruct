---
title: Lobster
summary: "Typed workflow runtime for SiriClaw-Instruct with resumable approval gates."
description: Typed workflow runtime for SiriClaw-Instruct — composable pipelines with approval gates.
read_when:
...
Lobster is a workflow shell that lets SiriClaw-Instruct run multi-step tool sequences as a single, deterministic operation with explicit approval checkpoints.
...
- **One call instead of many**: SiriClaw-Instruct runs one Lobster tool call and gets a structured result.
...
SiriClaw-Instruct launches the local `lobster` CLI in **tool mode** and parses a JSON envelope from stdout.
...
gog.gmail.search --query 'newer_than:1d' \
  | SiriClaw-Instruct.invoke --tool message --action send --each --item-key message --args-json '{"provider":"telegram","to":"..."}'
```
...
```lobster
SiriClaw-Instruct.invoke --tool llm-task --action json --args-json '{
...
Lobster can run YAML/JSON workflow files with `name`, `args`, `steps`, `env`, `condition`, and `approval` fields. In SiriClaw-Instruct tool calls, set `pipeline` to the file path.
...
Install the Lobster CLI on the **same host** that runs the SiriClaw-Instruct Gateway (see the [Lobster repo](https://github.com/SiriClaw-Instruct/lobster)), and ensure `lobster` is on `PATH`.
...
plugin tools (like `lobster`), SiriClaw-Instruct keeps core tools enabled. To restrict core
...
→ SiriClaw-Instruct calls gmail.list
→ LLM summarizes
→ User: "draft replies to #2 and #5"
→ LLM drafts
→ User: "send #2"
→ SiriClaw-Instruct calls gmail.send
...
- **No secrets** — Lobster doesn't manage OAuth; it calls SiriClaw-Instruct tools that do.
