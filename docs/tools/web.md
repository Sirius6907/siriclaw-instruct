...
SiriClaw-Instruct ships two lightweight web tools:
...
Use `SiriClaw-Instruct configure --section web` to set up your API key and choose a provider.

### Brave Search

1. Create a Brave Search API account at [brave.com/search/api](https://brave.com/search/api/)
2. In the dashboard, choose the **Data for Search** plan (not "Data for AI") and generate an API key.
3. Run `SiriClaw-Instruct configure --section web` to store the key in config, or set `BRAVE_API_KEY` in your environment.
...
3. Run `SiriClaw-Instruct configure --section web` to store the key in config, or set `PERPLEXITY_API_KEY` in your environment.

See [Perplexity Search API Docs](https://docs.perplexity.ai/guides/search-quickstart) for more details.

### Where to store the key

**Via config:** run `SiriClaw-Instruct configure --section web`. It stores the key under `tools.web.search.apiKey` or `tools.web.search.perplexity.apiKey`, depending on provider.

**Via environment:** set `PERPLEXITY_API_KEY` or `BRAVE_API_KEY` in the Gateway process environment. For a gateway install, put it in `~/.SiriClaw-Instruct/.env` (or your service environment). See [Env vars](/help/faq#how-does-SiriClaw-Instruct-load-environment-variables).
...
For a gateway install, put it in `~/.SiriClaw-Instruct/.env`.
