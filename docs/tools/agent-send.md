---
summary: "Direct `SiriClaw-Instruct agent` CLI runs (with optional delivery)"
read_when:
  - Adding or modifying the agent CLI entrypoint
title: "Agent Send"
---

# `SiriClaw-Instruct agent` (direct agent runs)

`SiriClaw-Instruct agent` runs a single agent turn without needing an inbound chat message.
...
- Optional delivery back to a channel with `--deliver` + `--channel` (target formats match `SiriClaw-Instruct message --target`).
...
```bash
SiriClaw-Instruct agent --to +15555550123 --message "status update"
...
SiriClaw-Instruct agent --to +15555550123 --message "Summon reply" --deliver
