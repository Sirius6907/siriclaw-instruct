...
- **gateway host** → `SiriClaw-Instruct` process on the gateway machine
...
```json
{
  "version": 1,
  "socket": {
    "path": "~/.SiriClaw-Instruct/exec-approvals.sock",
...
- allowlist entries live in host-local `~/.SiriClaw-Instruct/exec-approvals.json` under `agents.<id>.allowlist` (or via Control UI / `SiriClaw-Instruct approvals allowlist ...`).
- `SiriClaw-Instruct security audit` warns with `tools.exec.safe_bins_interpreter_unprofiled` when interpreter/runtime bins appear in `safeBins` without explicit profiles.
- `SiriClaw-Instruct doctor --fix` can scaffold missing custom `safeBinProfiles.<bin>` entries as `{}` (review and tighten afterward). Interpreter/runtime bins are not auto-scaffolded.
...
`~/.SiriClaw-Instruct/exec-approvals.json` directly.

CLI: `SiriClaw-Instruct approvals` supports gateway or node editing (see [Approvals CLI](/cli/approvals)).
