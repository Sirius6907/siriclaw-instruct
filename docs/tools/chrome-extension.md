---
summary: "Chrome extension: let SiriClaw-Instruct drive your existing Chrome tab"
read_when:
...
The SiriClaw-Instruct Chrome extension lets the agent control your **existing Chrome tabs** (your normal Chrome window) instead of launching a separate SiriClaw-Instruct-managed Chrome profile.
...
SiriClaw-Instruct then controls the attached tab through the normal `browser` tool surface (selecting the right profile).
...
The extension ships inside the SiriClaw-Instruct release (npm package) as static files. There is no separate “build” step.

After upgrading SiriClaw-Instruct:

- Re-run `SiriClaw-Instruct browser extension install` to refresh the installed files under your SiriClaw-Instruct state directory.
- Chrome → `chrome://extensions` → click “Reload” on the extension.

## Use it (set gateway token once)

SiriClaw-Instruct ships with a built-in browser profile named `chrome` that targets the extension relay on the default port.

Before first attach, open extension Options and set:

- `Port` (default `18792`)
- `Gateway token` (must match `gateway.auth.token` / `SiriClaw-Instruct_GATEWAY_TOKEN`)
...
- Open the tab you want SiriClaw-Instruct to control.
...
- `ON`: attached; SiriClaw-Instruct can drive that tab.
...
Debugging: `SiriClaw-Instruct sandbox explain`
...
`SiriClaw-Instruct browser extension path` prints the **installed** on-disk directory containing the extension files.

The CLI intentionally does **not** print a `node_modules` path. Always run `SiriClaw-Instruct browser extension install` first to copy the extension to a stable location under your SiriClaw-Instruct state directory.
...
- **This is not isolated** like the dedicated SiriClaw-Instruct-managed profile.
