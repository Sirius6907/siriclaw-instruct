---
summary: "Use OpenAI via API keys or Codex subscription in SiriClaw-Instruct"
read_when:
  - You want to use OpenAI models in SiriClaw-Instruct
...
OpenAI explicitly supports subscription OAuth usage in external tools/workflows like SiriClaw-Instruct.
...
```bash
SiriClaw-Instruct onboard --auth-choice openai-api-key
# or non-interactive
SiriClaw-Instruct onboard --openai-api-key "$OPENAI_API_KEY"
```
...
OpenAI API usage. SiriClaw-Instruct forwards both through the `openai/*` Responses path.
...
SiriClaw-Instruct onboard --auth-choice openai-codex

# Or run OAuth directly
SiriClaw-Instruct models auth login --provider openai-codex
```
...
OpenAI's current Codex docs list `gpt-5.4` as the current Codex model. SiriClaw-Instruct
maps that to `openai-codex/gpt-5.4` for ChatGPT/Codex OAuth usage.

### Transport default

SiriClaw-Instruct uses `pi-ai` for model streaming. For both `openai/*` and
...
For `openai/*` (Responses API), SiriClaw-Instruct also enables WebSocket warm-up by
...
OpenAI docs describe warm-up as optional. SiriClaw-Instruct enables it by default for
...
SiriClaw-Instruct, set `agents.defaults.models["openai/<model>"].params.serviceTier` to
...
`baseUrl` on `api.openai.com`), SiriClaw-Instruct now auto-enables OpenAI server-side
