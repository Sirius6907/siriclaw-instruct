---
summary: "Use MiniMax M2.5 in SiriClaw-Instruct"
read_when:
  - You want MiniMax models in SiriClaw-Instruct
...
```bash
SiriClaw-Instruct plugins enable minimax-portal-auth  # skip if already loaded.
SiriClaw-Instruct gateway restart  # restart if gateway is already running
SiriClaw-Instruct onboard --auth-choice minimax-portal
```
...
See [MiniMax OAuth plugin README](https://github.com/SiriClaw-Instruct/SiriClaw-Instruct/tree/main/extensions/minimax-portal-auth) for details.
...
- Run `SiriClaw-Instruct configure`
- Select **Model/auth**
- Choose **MiniMax M2.5**
...
Configure manually via `SiriClaw-Instruct.json`:
...
- Use `SiriClaw-Instruct models list` and `SiriClaw-Instruct models set minimax/MiniMax-M2.5` to switch.
...
- Running `SiriClaw-Instruct configure` and selecting **MiniMax M2.5**, or
