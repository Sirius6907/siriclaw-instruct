---
summary: "Use Venice AI privacy-focused models in SiriClaw-Instruct"
read_when:
  - You want privacy-focused inference in SiriClaw-Instruct
...
## Why Venice in SiriClaw-Instruct
...
```bash
SiriClaw-Instruct onboard --auth-choice venice-api-key
```
...
```bash
SiriClaw-Instruct agent --model venice/kimi-k2-5 --message "Hello, are you working?"
```

## Model Selection

After setup, SiriClaw-Instruct shows all available Venice models. Pick based on your needs:
...
```bash
SiriClaw-Instruct models set venice/kimi-k2-5
SiriClaw-Instruct models set venice/claude-opus-4-6
```

List all available models:

```bash
SiriClaw-Instruct models list | grep venice
```

## Configure via `SiriClaw-Instruct configure`

1. Run `SiriClaw-Instruct configure`
2. Select **Model/auth**
3. Choose **Venice AI**
...
SiriClaw-Instruct automatically discovers models from the Venice API when `VENICE_API_KEY` is set. If the API is unreachable, it falls back to a static catalog.
...
SiriClaw-Instruct agent --model venice/kimi-k2-5 --message "Quick health check"

# Use Claude Opus via Venice (anonymized)
SiriClaw-Instruct agent --model venice/claude-opus-4-6 --message "Summarize this task"

# Use uncensored model
SiriClaw-Instruct agent --model venice/venice-uncensored --message "Draft options"

# Use vision model with image
SiriClaw-Instruct agent --model venice/qwen3-vl-235b-a22b --message "Review attached image"

# Use coding model
SiriClaw-Instruct agent --model venice/qwen3-coder-480b-a35b-instruct --message "Refactor this function"
```
...
The Venice model catalog updates dynamically. Run `SiriClaw-Instruct models list` to see currently available models. Some models may be temporarily offline.
