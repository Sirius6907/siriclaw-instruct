...
SiriClaw-Instruct supports Perplexity Search API as a `web_search` provider.
...
**Via config:** run `SiriClaw-Instruct configure --section web`. It stores the key in
`~/.SiriClaw-Instruct/SiriClaw-Instruct.json` under `tools.web.search.perplexity.apiKey`.

**Via environment:** set `PERPLEXITY_API_KEY` in the Gateway process
environment. For a gateway install, put it in `~/.SiriClaw-Instruct/.env` (or your
service environment). See [Env vars](/help/faq#how-does-SiriClaw-Instruct-load-environment-variables).
