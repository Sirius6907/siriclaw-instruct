...
- Publish as `SiriClaw-Instruct/plugin-sdk` (or export from core under `SiriClaw-Instruct/plugin-sdk`).
- Semver with explicit stability guarantees.

### 2) Plugin Runtime (execution surface, injected)

Scope: everything that touches core runtime behavior.
Accessed via `SiriClaw-InstructPluginApi.runtime` so plugins never import `src/**`.
...
export type PluginRuntime = {
  channel: {
    text: {
      ...
      resolveTextChunkLimit(cfg: SiriClaw-InstructConfig, channel: string, accountId?: string): number;
      hasControlCommand(text: string, cfg: SiriClaw-InstructConfig): boolean;
    };
    reply: {
    ...
    mentions: {
      buildMentionRegexes(cfg: SiriClaw-InstructConfig, agentId?: string): RegExp[];
    ...
    debounce: {
      ...
      resolveInboundDebounceMs(cfg: SiriClaw-InstructConfig, channel: string): number;
    };
    commands: {
  ...
  state: {
    resolveStateDir(cfg: SiriClaw-InstructConfig): string;
  };
};
```
...
- Introduce `SiriClaw-Instruct/plugin-sdk`.
- Add `api.runtime` to `SiriClaw-InstructPluginApi` with the surface above.
...
- Plugins declare a required runtime range (e.g., `SiriClaw-InstructRuntime: ">=2026.2.0"`).
