...
- Plugin manifests (`SiriClaw-Instruct.plugin.json`) are required for all plugins.

## Plugin schema enforcement

- Each plugin provides a strict JSON Schema for its config (inline in the manifest).
- Plugin load flow:
  1. Resolve plugin manifest + schema (`SiriClaw-Instruct.plugin.json`).
...
- If config invalid:
  ...
  - Instruct: `SiriClaw-Instruct doctor --fix`.
...
- `SiriClaw-Instruct help`
- `SiriClaw-Instruct status`
- `SiriClaw-Instruct gateway status`

Everything else must hard-fail with: “Config invalid. Run `SiriClaw-Instruct doctor --fix`.”
