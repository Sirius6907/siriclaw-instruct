...
- **Ask default:** `on-miss`.
- **Approvals store:** `~/.SiriClaw-Instruct/exec-approvals.json` (JSON, no legacy migration).
- **Runner:** headless system service; UI app hosts a Unix socket for approvals.
- **Node identity:** use existing `nodeId`.
- **Socket auth:** Unix socket + token (cross-platform); split later if needed.
- **Node host state:** `~/.SiriClaw-Instruct/node.json` (node id + pairing token).
...
Path: `~/.SiriClaw-Instruct/exec-approvals.json`
...
- Unix socket at `~/.SiriClaw-Instruct/exec-approvals.sock` (0600).
