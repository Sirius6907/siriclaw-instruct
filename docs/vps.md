---
summary: "VPS hosting hub for SiriClaw-Instruct (Oracle/Fly/Hetzner/GCP/exe.dev)"
read_when:
...
```bash
grep -q 'NODE_COMPILE_CACHE=/var/tmp/SiriClaw-Instruct-compile-cache' ~/.bashrc || cat >> ~/.bashrc <<'EOF'
...
- `SiriClaw-Instruct_NO_RESPAWN=1` avoids extra startup overhead from a self-respawn path.
...
- Add service env for stable startup path:
  - `SiriClaw-Instruct_NO_RESPAWN=1`
  - `NODE_COMPILE_CACHE=/var/tmp/SiriClaw-Instruct-compile-cache`
