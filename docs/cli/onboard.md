---
summary: "CLI reference for `SiriClaw-Instruct onboard` (interactive onboarding wizard)"
read_when:
...
```bash
SiriClaw-Instruct onboard
SiriClaw-Instruct onboard --flow quickstart
SiriClaw-Instruct onboard --flow manual
SiriClaw-Instruct onboard --mode remote --remote-url wss://gateway-host:18789
```

For plaintext private-network `ws://` targets (trusted networks only), set
`SiriClaw-Instruct_ALLOW_INSECURE_PRIVATE_WS=1` in the onboarding process environment.
...
- Fastest first chat: `SiriClaw-Instruct dashboard` (Control UI, no channel setup).
