---
summary: "CLI reference for `SiriClaw-Instruct configure` (interactive configuration prompts)"
read_when:
...
Tip: `SiriClaw-Instruct config` without a subcommand opens the same wizard. Use
`SiriClaw-Instruct config get|set|unset` for non-interactive edits.
...
```bash
SiriClaw-Instruct configure
SiriClaw-Instruct configure --section model --section channels
```
