---
summary: "CLI reference for `SiriClaw-Instruct qr` (generate iOS pairing QR + setup code)"
read_when:
...
```bash
SiriClaw-Instruct qr
SiriClaw-Instruct qr --setup-code-only
SiriClaw-Instruct qr --json
SiriClaw-Instruct qr --remote
SiriClaw-Instruct qr --url wss://gateway.example/ws --token '<token>'
```
