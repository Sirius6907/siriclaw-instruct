---
summary: "Mattermost bot setup and SiriClaw-Instruct config"
read_when:
...
```bash
SiriClaw-Instruct plugins install @SiriClaw-Instruct/mattermost
```

Local checkout (when running from a git repo):

```bash
SiriClaw-Instruct plugins install ./extensions/mattermost
```
...
4. Configure SiriClaw-Instruct and start the gateway.
...
Native slash commands are opt-in. When enabled, SiriClaw-Instruct registers `oc_*` slash commands via
...
- If `callbackUrl` is omitted, SiriClaw-Instruct derives one from gateway host/port + `callbackPath`.
...
- Reachability requirement: the callback endpoint must be reachable from the Mattermost server.
  - Do not set `callbackUrl` to `localhost` unless Mattermost runs on the same host/network namespace as SiriClaw-Instruct.
  - Do not set `callbackUrl` to your Mattermost base URL unless that URL reverse-proxies `/api/channels/mattermost/command` to SiriClaw-Instruct.
  - A quick check is `curl https://<gateway-host>/api/channels/mattermost/command`; a GET should return `405 Method Not Allowed` from SiriClaw-Instruct, not `404`.
...
- Approve via:
  - `SiriClaw-Instruct pairing list mattermost`
  - `SiriClaw-Instruct pairing approve mattermost <CODE>`
...
Use these target formats with `SiriClaw-Instruct message send` or cron/webhooks:
...
- If `interactions.callbackBaseUrl` is omitted, SiriClaw-Instruct derives the callback URL from
...
- Reachability rule: the button callback URL must be reachable from the Mattermost server.
  `localhost` only works when Mattermost and SiriClaw-Instruct run on the same host/network namespace.
...
1. Derive the secret from the bot token:
   `HMAC-SHA256(key="SiriClaw-Instruct-mattermost-interactions", data=botToken)`
...
secret = hmac.new(
    b"SiriClaw-Instruct-mattermost-interactions",
...
`SiriClaw-Instruct message send` and cron/webhook deliveries.
