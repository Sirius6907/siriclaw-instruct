...
Matrix is an open, decentralized messaging protocol. SiriClaw-Instruct connects as a Matrix **user**
...
```bash
SiriClaw-Instruct plugins install @SiriClaw-Instruct/matrix
```

Local checkout (when running from a git repo):

```bash
SiriClaw-Instruct plugins install ./extensions/matrix
```
...
1. Install the Matrix plugin:
   - From npm: `SiriClaw-Instruct plugins install @SiriClaw-Instruct/matrix`
   - From a local checkout: `SiriClaw-Instruct plugins install ./extensions/matrix`
...
3. Get an access token for the bot account:
   ...
   - Or set `channels.matrix.userId` + `channels.matrix.password`: SiriClaw-Instruct calls the same
     login endpoint, stores the access token in `~/.SiriClaw-Instruct/credentials/matrix/credentials.json`,
...
- On first connection, SiriClaw-Instruct requests device verification from your other sessions.
- Verify the device in another Matrix client (Element, etc.) to enable key sharing.
- If the crypto module cannot be loaded, E2EE is disabled and encrypted rooms will not decrypt;
  SiriClaw-Instruct logs a warning.
...
`~/.SiriClaw-Instruct/matrix/accounts/<account>/<homeserver>__<user>/<token-hash>/crypto/`
...
- Approve via:
  - `SiriClaw-Instruct pairing list matrix`
  - `SiriClaw-Instruct pairing approve matrix <CODE>`
...
- On startup, SiriClaw-Instruct resolves room/user names in allowlists to IDs and logs the mapping; unresolved entries are ignored for allowlist matching.
