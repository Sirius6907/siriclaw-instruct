...
```bash
SiriClaw-Instruct plugins install @SiriClaw-Instruct/nextcloud-talk
```

Local checkout (when running from a git repo):

```bash
SiriClaw-Instruct plugins install ./extensions/nextcloud-talk
```
...
2. On your Nextcloud server, create a bot:
   ...
   ```bash
   ./occ talk:bot:install "SiriClaw-Instruct" "<shared-secret>" "<webhook-url>" --feature reaction
   ```
...
- Approve via:
  - `SiriClaw-Instruct pairing list nextcloud-talk`
  - `SiriClaw-Instruct pairing approve nextcloud-talk <CODE>`
