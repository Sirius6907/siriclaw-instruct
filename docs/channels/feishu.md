...
Feishu (Lark) is a team chat platform used by companies for messaging and collaboration. This plugin connects SiriClaw-Instruct to a Feishu/Lark bot using the platform’s WebSocket event subscription so messages can be received without exposing a public webhook URL.
...
```bash
SiriClaw-Instruct plugins install @SiriClaw-Instruct/feishu
```

Local checkout (when running from a git repo):

```bash
SiriClaw-Instruct plugins install ./extensions/feishu
```
...
If you just installed SiriClaw-Instruct, run the wizard:
...
2. Configuring app credentials in SiriClaw-Instruct
...
```bash
SiriClaw-Instruct channels add
```
...
1. You already ran `SiriClaw-Instruct channels add` for Feishu
2. The gateway is running (`SiriClaw-Instruct gateway status`)
...
## Step 2: Configure SiriClaw-Instruct
...
Edit `~/.SiriClaw-Instruct/SiriClaw-Instruct.json`:
...
```bash
SiriClaw-Instruct pairing approve feishu <CODE>
```
...
- **Approve pairing**:
  ...
  ```bash
  SiriClaw-Instruct pairing list feishu
  SiriClaw-Instruct pairing approve feishu <CODE>
  ```
...
2. Run `SiriClaw-Instruct logs --follow` and look for `chat_id`
...
2. Run `SiriClaw-Instruct logs --follow` and look for `open_id`
...
| Command                    | Description                   |
| -------------------------- | ----------------------------- |
| `SiriClaw-Instruct gateway status`  | Show gateway status           |
| `SiriClaw-Instruct gateway install` | Install/start gateway service |
| `SiriClaw-Instruct gateway stop`    | Stop gateway service          |
| `SiriClaw-Instruct gateway restart` | Restart gateway service       |
| `SiriClaw-Instruct logs --follow`   | Tail gateway logs             |
...
4. Check logs: `SiriClaw-Instruct logs --follow`
...
5. Ensure the gateway is running: `SiriClaw-Instruct gateway status`
6. Check logs: `SiriClaw-Instruct logs --follow`
  ...
  agents: {
    list: [
      { id: "main" },
      {
        ...
        agentDir: "/home/user/.SiriClaw-Instruct/agents/clawd-fan/agent",
      },
      {
        id: "clawd-xi",
        workspace: "/home/user/clawd-xi",
        agentDir: "/home/user/.SiriClaw-Instruct/agents/clawd-xi/agent",
      },
    ],
  },
  bindings: [
