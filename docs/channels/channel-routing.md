...
SiriClaw-Instruct routes replies **back to the channel where a message came from**. The
...
SiriClaw-Instruct infers a pinned owner from `allowFrom` when all of these are true:
...
In that mismatch case, SiriClaw-Instruct still records inbound session metadata, but it
...
Broadcast groups let you run **multiple agents** for the same peer **when SiriClaw-Instruct would normally reply** (for example: in WhatsApp groups, after mention/activation gating).
...
{
  agents: {
    list: [{ id: "support", name: "Support", workspace: "~/.SiriClaw-Instruct/workspace-support" }],
  },
  bindings: [
...
Session stores live under the state directory (default `~/.SiriClaw-Instruct`):

- `~/.SiriClaw-Instruct/agents/<agentId>/sessions/sessions.json`
