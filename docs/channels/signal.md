...
- SiriClaw-Instruct installed on your server (Linux flow below tested on Ubuntu 24).
...
3. Choose one setup path:
   - **Path A (QR link):** `signal-cli link -n "SiriClaw-Instruct"` and scan with Signal.
   - **Path B (SMS register):** register a dedicated number with captcha + SMS verification.
4. Configure SiriClaw-Instruct and restart the gateway.
5. Send a first DM and approve pairing (`SiriClaw-Instruct pairing approve signal <CODE>`).
...
2. Link a bot account:
   - `signal-cli link -n "SiriClaw-Instruct"` then scan the QR in Signal.
...
4. Configure SiriClaw-Instruct, restart gateway, verify channel:
...
5. Pair your DM sender:
   ...
   - Approve code on the server: `SiriClaw-Instruct pairing approve signal <PAIRING_CODE>`.
...
If you want to manage `signal-cli` yourself (slow JVM cold starts, container init, or shared CPUs), run the daemon separately and point SiriClaw-Instruct at it:
...
This skips auto-spawn and the startup wait inside SiriClaw-Instruct. For slow starts when auto-spawning, set `channels.signal.startupTimeoutMs`.
...
- Approve via:
  - `SiriClaw-Instruct pairing list signal`
  - `SiriClaw-Instruct pairing approve signal <CODE>`
...
- **Typing indicators**: SiriClaw-Instruct sends typing signals via `signal-cli sendTyping` and refreshes them while a reply is running.
- **Read receipts**: when `channels.signal.sendReadReceipts` is true, SiriClaw-Instruct forwards read receipts for allowed DMs.
...
- Config validation errors after edits: run `SiriClaw-Instruct doctor --fix`.
...
SiriClaw-Instruct pairing list signal
pgrep -af signal-cli
grep -i "signal" "/tmp/SiriClaw-Instruct/SiriClaw-Instruct-$(date +%Y-%m-%d).log" | tail -20
```
