...
read_when:
  - Setting up Twitch chat integration for SiriClaw-Instruct
title: "Twitch"
---

# Twitch (plugin)

Twitch chat support via IRC connection. SiriClaw-Instruct connects as a Twitch user (bot account) to receive and send messages in channels.
...
```bash
SiriClaw-Instruct plugins install @SiriClaw-Instruct/twitch
```

Local checkout (when running from a git repo):

```bash
SiriClaw-Instruct plugins install ./extensions/twitch
```
...
4. Configure the token:
   - Env: `SiriClaw-Instruct_TWITCH_ACCESS_TOKEN=...` (default account only)
...
```json5
{
  channels: {
    twitch: {
      enabled: true,
      username: "SiriClaw-Instruct", // Bot's Twitch account
      accessToken: "oauth:abc123...", // OAuth Access Token (or use SiriClaw-Instruct_TWITCH_ACCESS_TOKEN env var)
...
```bash
SiriClaw-Instruct_TWITCH_ACCESS_TOKEN=oauth:abc123...
```
