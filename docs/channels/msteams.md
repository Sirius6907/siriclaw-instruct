...
```bash
SiriClaw-Instruct plugins install @SiriClaw-Instruct/msteams
```

Local checkout (when running from a git repo):

```bash
SiriClaw-Instruct plugins install ./extensions/msteams
```

If you choose Teams during configure/onboarding and a git checkout is detected,
SiriClaw-Instruct will offer the local install path automatically.
...
3. Configure SiriClaw-Instruct with those credentials.
...
- Talk to SiriClaw-Instruct via Teams DMs, group chats, or channels.
...
- On startup, SiriClaw-Instruct resolves team/channel and user allowlist names to IDs (when Graph permissions allow)
...
5. Configure `msteams` in `~/.SiriClaw-Instruct/SiriClaw-Instruct.json` (or env vars) and start the gateway.
6. The gateway listens for Bot Framework webhook traffic on `/api/messages` by default.

## Azure Bot Setup (Prerequisites)

Before configuring SiriClaw-Instruct, you need to create an Azure Bot resource.
...
2. Fill in the **Basics** tab:
   ...
   | Field              | Value                                                    |
   | ------------------ | -------------------------------------------------------- |
   | **Bot handle**     | Your bot name, e.g., `SiriClaw-Instruct-msteams` (must be unique) |
...
1. **Install the Microsoft Teams plugin**
   - From npm: `SiriClaw-Instruct plugins install @SiriClaw-Instruct/msteams`
   - From a local checkout: `SiriClaw-Instruct plugins install ./extensions/msteams`
...
```json
{
  ...
  "description": { "short": "SiriClaw-Instruct in Teams", "full": "SiriClaw-Instruct in Teams" },
...
SiriClaw-Instruct handles this by returning quickly and sending replies proactively, but very slow responses may still cause issues.
...
By default, SiriClaw-Instruct only downloads media from Microsoft/Teams hostnames. Override with `channels.msteams.mediaAllowHosts` (use `["*"]` to allow any host).
...
Uploaded files are stored in a `/SiriClaw-InstructShared/` folder in the configured SharePoint site's default document library.

## Polls (Adaptive Cards)

SiriClaw-Instruct sends Teams polls as Adaptive Cards (there is no native Teams poll API).

- CLI: `SiriClaw-Instruct message poll --channel msteams --target conversation:<id> ...`
- Votes are recorded by the gateway in `~/.SiriClaw-Instruct/msteams-polls.json`.
...
```bash
SiriClaw-Instruct message send --channel msteams \
...
SiriClaw-Instruct message send --channel msteams --target "user:40a1a0ed-..." --message "Hello"

# Send to a user by display name (triggers Graph API lookup)
SiriClaw-Instruct message send --channel msteams --target "user:John Smith" --message "Hello"

# Send to a group chat or channel
SiriClaw-Instruct message send --channel msteams --target "conversation:19:abc...@thread.tacv2" --message "Hello"

# Send an Adaptive Card to a conversation
SiriClaw-Instruct message send --channel msteams --target "conversation:19:abc...@thread.tacv2" \
  --card '{"type":"AdaptiveCard","version":"1.5","body":[{"type":"TextBlock","text":"Hello"}]}'
```
