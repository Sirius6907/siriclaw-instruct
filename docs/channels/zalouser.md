...
read_when:
  - Setting up Zalo Personal for SiriClaw-Instruct
...
Status: experimental. This integration automates a **personal Zalo account** via native `zca-js` inside SiriClaw-Instruct.
...
- Install via CLI: `SiriClaw-Instruct plugins install @SiriClaw-Instruct/zalouser`
- Or from a source checkout: `SiriClaw-Instruct plugins install ./extensions/zalouser`
...
2. Login (QR, on the Gateway machine):
   - `SiriClaw-Instruct channels login --channel zalouser`
...
```bash
SiriClaw-Instruct directory self --channel zalouser
SiriClaw-Instruct directory peers list --channel zalouser --query "name"
SiriClaw-Instruct directory groups list --channel zalouser --query "work"
```
...
- `SiriClaw-Instruct pairing list zalouser`
- `SiriClaw-Instruct pairing approve zalouser <code>`
...
- On startup, SiriClaw-Instruct resolves group/user names in allowlists to IDs and logs the mapping; unresolved entries are kept as typed.
...
- When a group message is skipped because mention is required, SiriClaw-Instruct stores it as pending group history and includes it on the next processed group message.
...
Accounts map to `zalouser` profiles in SiriClaw-Instruct state. Example:
...
- SiriClaw-Instruct sends a typing event before dispatching a reply (best-effort).
...
- For inbound messages that include event metadata, SiriClaw-Instruct sends delivered + seen acknowledgements (best-effort).
...
- Re-login: `SiriClaw-Instruct channels logout --channel zalouser && SiriClaw-Instruct channels login --channel zalouser`
...
- The channel now runs fully in SiriClaw-Instruct without external CLI binaries.
