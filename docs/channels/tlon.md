...
Tlon is a decentralized messenger built on Urbit. SiriClaw-Instruct connects to your Urbit ship and can
...
```bash
SiriClaw-Instruct plugins install @SiriClaw-Instruct/tlon
```

Local checkout (when running from a git repo):

```bash
SiriClaw-Instruct plugins install ./extensions/tlon
```
...
By default, SiriClaw-Instruct blocks private/internal hostnames and IP ranges for SSRF protection.
...
Use these with `SiriClaw-Instruct message send` or cron delivery:
...
- Thread replies: if the inbound message is in a thread, SiriClaw-Instruct replies in-thread.
