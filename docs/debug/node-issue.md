...
read_when:
  ...
  - Investigating tsx/esbuild loader crashes in SiriClaw-Instruct
...
Running SiriClaw-Instruct via Node with `tsx` fails at startup with:

```
[SiriClaw-Instruct] Failed to start CLI: TypeError: __name is not a function
...
- Before that (Bun path), `SiriClaw-Instruct status` and `gateway:watch` worked.
...
- Use Node + tsc watch, then run compiled output:
  ...
  node --watch SiriClaw-Instruct.mjs status
  ```

- Confirmed locally: `pnpm exec tsc -p tsconfig.json` + `node SiriClaw-Instruct.mjs status` works on Node 25.
