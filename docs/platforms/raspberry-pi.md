---
summary: "SiriClaw-Instruct on Raspberry Pi (budget self-hosted setup)"
read_when:
  - Setting up SiriClaw-Instruct on a Raspberry Pi
  - Running SiriClaw-Instruct on ARM devices
  - Building a cheap always-on personal AI
title: "Raspberry Pi"
---

# SiriClaw-Instruct on Raspberry Pi

## Goal

Run a persistent, always-on SiriClaw-Instruct Gateway on a Raspberry Pi for **~$35-80** one-time cost (no monthly fees).
...
# View logs
journalctl -u SiriClaw-Instruct -f
```
...
# Update config
SiriClaw-Instruct config set gateway.bind tailnet
...
```bash
grep -q 'NODE_COMPILE_CACHE=/var/tmp/SiriClaw-Instruct-compile-cache' ~/.bashrc || cat >> ~/.bashrc <<'EOF' # pragma: allowlist secret
export NODE_COMPILE_CACHE=/var/tmp/SiriClaw-Instruct-compile-cache
mkdir -p /var/tmp/SiriClaw-Instruct-compile-cache
export SiriClaw-Instruct_NO_RESPAWN=1
EOF
source ~/.bashrc
```
...
- `SiriClaw-Instruct_NO_RESPAWN=1` avoids extra startup cost from CLI self-respawn.
- First run warms the cache; later runs benefit most.

### systemd startup tuning (optional)

If this Pi is mostly running SiriClaw-Instruct, add a service drop-in to reduce restart
jitter and keep startup env stable:

```bash
sudo systemctl edit SiriClaw-Instruct
```

```ini
[Service]
Environment=SiriClaw-Instruct_NO_RESPAWN=1
Environment=NODE_COMPILE_CACHE=/var/tmp/SiriClaw-Instruct-compile-cache
Restart=always
RestartSec=2
TimeoutStartSec=90
```
...
If possible, keep SiriClaw-Instruct state/cache on SSD-backed storage to avoid SD-card
...
Most SiriClaw-Instruct features work on ARM64, but some external binaries may need ARM builds:
...
sudo systemctl is-enabled SiriClaw-Instruct

# Enable if not
sudo systemctl enable SiriClaw-Instruct

# Start on boot
sudo systemctl start SiriClaw-Instruct
```
...
```bash
# Check logs
journalctl -u SiriClaw-Instruct --no-pager -n 100

# Common fix: rebuild
cd ~/SiriClaw-Instruct  # if using hackable install
npm run build
sudo systemctl restart SiriClaw-Instruct
```
