...
- Same path: run the app from a fixed location (for SiriClaw-Instruct, `dist/SiriClaw-Instruct.app`).
...
```bash
sudo tccutil reset Accessibility ai.SiriClaw-Instruct.mac
sudo tccutil reset ScreenCapture ai.SiriClaw-Instruct.mac
...
Workaround: move files into the SiriClaw-Instruct workspace (`~/.SiriClaw-Instruct/workspace`) if you want to avoid per-folder grants.
