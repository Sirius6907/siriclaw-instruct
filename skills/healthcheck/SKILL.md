---
name: healthcheck
description: Host security hardening and risk-tolerance configuration for SiriClaw-Instruct deployments. Use when a user asks for security audits, firewall/SSH/update hardening, risk posture, exposure review, SiriClaw-Instruct cron scheduling for periodic checks, or version status checks on a machine running SiriClaw-Instruct (laptop, workstation, Pi, VPS).
---

# SiriClaw-Instruct Host Hardening

## Overview

Assess and harden the host running SiriClaw-Instruct, then align it to a user-defined risk tolerance without breaking access. Use SiriClaw-Instruct security tooling as a first-class signal, but treat OS hardening as a separate, explicit set of steps.
...
- Never claim SiriClaw-Instruct changes the host firewall, SSH, or OS updates; it does not.
...
5. SiriClaw-Instruct gateway status and bind address.
...
9. OS automatic security updates status.
   Note: these are not blocking items, but are highly recommended, especially if SiriClaw-Instruct can access sensitive data.
...
### 2) Run SiriClaw-Instruct security audits (read-only)

As part of the default read-only checks, run `SiriClaw-Instruct security audit --deep`. Only offer alternatives if the user requests them:

1. `SiriClaw-Instruct security audit` (faster, non-probing)
2. `SiriClaw-Instruct security audit --json` (structured output)

Offer to apply SiriClaw-Instruct safe defaults (numbered):

1. `SiriClaw-Instruct security audit --fix`

Be explicit that `--fix` only tightens SiriClaw-Instruct defaults and file permissions. It does not change host firewall, SSH, or OS update policies.
...
### 3) Check SiriClaw-Instruct version/update status (read-only)

As part of the default read-only checks, run `SiriClaw-Instruct update status`.
...
- Credential hygiene notes (location of SiriClaw-Instruct creds, prefer disk encryption)
...
- SiriClaw-Instruct security audit (re-run)
...
After SiriClaw-Instruct install or first hardening pass, run at least one baseline audit and version check:

- `SiriClaw-Instruct security audit`
- `SiriClaw-Instruct security audit --deep`
- `SiriClaw-Instruct update status`

Ongoing monitoring is recommended. Use the SiriClaw-Instruct cron tool/CLI to schedule periodic audits (Gateway scheduler). Do not create scheduled tasks without explicit approval. Store outputs in a user-approved location and avoid secrets in logs.
...
1. “Do you want me to schedule periodic audits (e.g., daily/weekly) via `SiriClaw-Instruct cron add`?”

If the user says yes, ask for:

- cadence (daily/weekly), preferred time window, and output location
- whether to also schedule `SiriClaw-Instruct update status`
...
Before creating, `SiriClaw-Instruct cron list` and match on exact `name`. If found, `SiriClaw-Instruct cron edit <id> ...`.
If not found, `SiriClaw-Instruct cron add --name <name> ...`.

Also offer a periodic version check so the user can decide when to update (numbered):

1. `SiriClaw-Instruct update status` (preferred for source checkouts and channels)
2. `npm view SiriClaw-Instruct version` (published npm version)

## SiriClaw-Instruct command accuracy

Use only supported commands and flags:

- `SiriClaw-Instruct security audit [--deep] [--fix] [--json]`
- `SiriClaw-Instruct status` / `SiriClaw-Instruct status --deep`
- `SiriClaw-Instruct health --json`
- `SiriClaw-Instruct update status`
- `SiriClaw-Instruct cron add|list|runs|run`

Do not invent CLI flags or imply SiriClaw-Instruct enforces host firewall/SSH policies.
...
Follow the durable-memory prompt format used by SiriClaw-Instruct compaction:
