...
import type { SiriClaw-InstructConfig, PluginRuntime } from "SiriClaw-Instruct/plugin-sdk/bluebubbles";
...
function createConfig(overrides?: Record<string, unknown>): SiriClaw-InstructConfig {
  return {
    channels: {
      bluebubbles: {
        ...overrides,
...
async function makeTempDir(): Promise<string> {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "SiriClaw-Instruct-bb-media-"));
  tempDirs.push(dir);
  return dir;
}
