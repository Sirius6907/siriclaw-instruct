# Google Gemini CLI Auth (SiriClaw-Instruct plugin)
...
```bash
SiriClaw-Instruct plugins enable google-gemini-cli-auth
```
...
```bash
SiriClaw-Instruct models auth login --provider google-gemini-cli --set-default
```
...
- `SiriClaw-Instruct_GEMINI_OAUTH_CLIENT_ID` / `GEMINI_CLI_OAUTH_CLIENT_ID`
- `SiriClaw-Instruct_GEMINI_OAUTH_CLIENT_SECRET` / `GEMINI_CLI_OAUTH_CLIENT_SECRET`
