...
import type { SiriClaw-InstructPluginApi, SiriClaw-InstructPluginToolContext } from "SiriClaw-Instruct/plugin-sdk/lobster";
...
function fakeApi(overrides: Partial<SiriClaw-InstructPluginApi> = {}): SiriClaw-InstructPluginApi {
  return {
    id: "lobster",
    name: "lobster",
    source: "test",
...
function fakeCtx(overrides: Partial<SiriClaw-InstructPluginToolContext> = {}): SiriClaw-InstructPluginToolContext {
  return {
    config: {},
    workspaceDir: "/tmp",
    agentDir: "/tmp",
...
describe("lobster plugin tool", () => {
  ...
  beforeAll(async () => {
    ...
    tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "SiriClaw-Instruct-lobster-plugin-"));
  });
  ...
  it("can be gated off in sandboxed contexts", async () => {
    const api = fakeApi();
    const factoryTool = (ctx: SiriClaw-InstructPluginToolContext) => {
      if (ctx.sandboxed) {
        return null;
      }
