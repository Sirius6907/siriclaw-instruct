...
import type { SiriClaw-InstructPluginApi } from "SiriClaw-Instruct/plugin-sdk/device-pair";
import {
...
} from "SiriClaw-Instruct/plugin-sdk/device-pair";
...
function resolveGatewayPort(cfg: SiriClaw-InstructPluginApi["config"]): number {
  const envPort =
    parsePositiveInteger(process.env.SiriClaw-Instruct_GATEWAY_PORT?.trim()) ??
    parsePositiveInteger(process.env.SIRICLAW_GATEWAY_PORT?.trim());
  if (envPort) {
    return envPort;
  }
...
function resolveScheme(
  cfg: SiriClaw-InstructPluginApi["config"],
...
function resolveAuth(cfg: SiriClaw-InstructPluginApi["config"]): ResolveAuthResult {
...
async function resolveGatewayUrl(api: SiriClaw-InstructPluginApi): Promise<ResolveUrlResult> {
  ...
  api.registerCommand({
    ...
    handler: async (ctx) => {
      ...
      if (action === "qr") {
        ...
        if (channel === "telegram" && target) {
          try {
            ...
            if (send) {
              await send(
                target,
                ["Scan this QR code with the SiriClaw-Instruct iOS app:", "", "```", qrAscii, "```"].join(
                  "\n",
                ),
                {
        ...
        return {
          text: [
            "Scan this QR code with the SiriClaw-Instruct iOS app:",
            "",
            "```",
            qrAscii,
            "```",
