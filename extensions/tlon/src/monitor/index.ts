import type { RuntimeEnv, ReplyPayload, SiriClaw-InstructConfig } from "SiriClaw-Instruct/plugin-sdk/tlon";
import { createLoggerBackedRuntime, createReplyPrefixOptions } from "SiriClaw-Instruct/plugin-sdk/tlon";
...
export async function monitorTlonProvider(opts: MonitorTlonOpts = {}): Promise<void> {
  ...
  const cfg = core.config.loadConfig() as SiriClaw-InstructConfig;
      ...
      if (senders.size > 0 && !senders.has(senderShip)) {
        ...
        runtime.log?.(
          `[tlon] ⚠️ SECURITY: Multiple users sharing DM session. ` +
            `Configure "session.dmScope: per-channel-peer" in SiriClaw-Instruct config.`,
        );
        ...
        if (!sharedSessionWarningSent && effectiveOwnerShip) {
          ...
          const warningMsg =
            ...
            `Fix: Add to your SiriClaw-Instruct config:\n` +
            `session:\n  dmScope: "per-channel-peer"\n\n` +
            `Docs: https://docs.SiriClaw-Instruct.ai/concepts/session#secure-dm-mode`;
