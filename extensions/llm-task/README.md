...
`SiriClaw-Instruct.invoke --each`) without adding new SiriClaw-Instruct code per workflow.
...
This extension depends on SiriClaw-Instruct internal modules (the embedded agent runner).
It is intended to ship as a **bundled** SiriClaw-Instruct extension (like `lobster`) and
be enabled via `plugins.entries` + tool allowlists.

It is **not** currently designed to be copied into
`~/.SiriClaw-Instruct/extensions` as a standalone plugin directory.
