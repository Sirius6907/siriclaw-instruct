...
import Ajv from "ajv";
import { resolvePreferredSiriClaw-InstructTmpDir } from "SiriClaw-Instruct/plugin-sdk/llm-task";
// NOTE: This extension is intended to be bundled with SiriClaw-Instruct.
// When running from source (tests/dev), SiriClaw-Instruct internals live under src/.
// When running from a built install, internals live under dist/ (no src/ tree).
// So we resolve internal imports dynamically with src-first, dist-fallback.
import type { SiriClaw-InstructPluginApi } from "SiriClaw-Instruct/plugin-sdk/llm-task";
...
export function createLlmTaskTool(api: SiriClaw-InstructPluginApi) {
  return {
    name: "llm-task",
    label: "LLM Task",
    description:
      "Run a generic JSON-only LLM task and return schema-validated JSON. Designed for orchestration from Lobster workflows via SiriClaw-Instruct.invoke.",
    ...
    async execute(_id: string, params: Record<string, unknown>) {
      ...
      try {
        tmpDir = await fs.mkdtemp(
          path.join(resolvePreferredSiriClaw-InstructTmpDir(), "SiriClaw-Instruct-llm-task-"),
        );
