...
function resolveSiriClaw-InstructRoot(): string {
  if (coreRootCache) {
    return coreRootCache;
  }
  const override = process.env.SiriClaw-Instruct_ROOT?.trim();
  if (override) {
  ...
  for (const start of candidates) {
    for (const name of ["SiriClaw-Instruct"]) {
  ...
  throw new Error("Unable to resolve core root. Set SiriClaw-Instruct_ROOT to the package root.");
}
...
async function importCoreExtensionAPI(): Promise<{
  ...
  const distPath = path.join(resolveSiriClaw-InstructRoot(), "dist", "extensionAPI.js");
