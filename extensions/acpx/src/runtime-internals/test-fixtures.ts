...
const SiriClaw-InstructShell = process.env.SiriClaw-Instruct_SHELL || "";
...
async function ensureMockCliScriptPath(): Promise<string> {
  ...
  sharedMockCliScriptPath = (async () => {
    const dir = await mkdtemp(
      path.join(resolvePreferredSiriClaw-InstructTmpDir(), "SiriClaw-Instruct-acpx-runtime-test-"),
    );
    tempDirs.push(dir);
