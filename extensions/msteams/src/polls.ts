...
export function extractMSTeamsPollVote(
  ...
  const pollId =
    readNestedString(value, ["SiriClaw-InstructPollId"]) ??
    readNestedString(value, ["pollId"]) ??
    readNestedString(value, ["SiriClaw-Instruct", "pollId"]) ??
    readNestedString(value, ["SiriClaw-Instruct", "poll", "id"]) ??
    readNestedString(value, ["data", "SiriClaw-InstructPollId"]) ??
    readNestedString(value, ["data", "pollId"]) ??
    readNestedString(value, ["data", "SiriClaw-Instruct", "pollId"]);
  if (!pollId) {
    return null;
  }
    ...
    actions: [
      {
        ...
        title: "Vote",
        data: {
          SiriClaw-InstructPollId: pollId,
          pollId,
        },
        msteams: {
          type: "messageBack",
          text: "SiriClaw-Instruct poll vote",
          displayText: "Vote recorded",
          value: { SiriClaw-InstructPollId: pollId, pollId },
        },
      },
    ],
  };
