...
const liveEnabled = HAS_OPENAI_KEY && process.env.SiriClaw-Instruct_LIVE_TEST === "1";
...
describe("memory plugin e2e", () => {
  ...
  beforeEach(async () => {
    tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "SiriClaw-Instruct-memory-test-"));
...
describeLive("memory plugin live tests", () => {
  ...
  beforeEach(async () => {
    tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "SiriClaw-Instruct-memory-live-"));
    dbPath = path.join(tmpDir, "lancedb");
  });
