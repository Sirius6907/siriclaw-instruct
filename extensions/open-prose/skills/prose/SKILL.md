...
metadata: { "SiriClaw-Instruct": { "emoji": "🪶", "homepage": "https://www.prose.md" } }
---
...
- **Task tool** in the upstream spec == SiriClaw-Instruct `sessions_spawn`
- **File I/O** == SiriClaw-Instruct `read`/`write`
- **Remote fetch** == SiriClaw-Instruct `web_fetch` (or `exec` with curl when POST is required)
