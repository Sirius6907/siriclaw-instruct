import {
  ...
  type RuntimeEnv,
} from "SiriClaw-Instruct/plugin-sdk/nextcloud-talk";
...
export async function handleNextcloudTalkInbound(params: {
  ...
  const defaultGroupPolicy = resolveDefaultGroupPolicy(config as SiriClaw-InstructConfig);
  ...
  const allowTextCommands = core.channel.commands.shouldHandleTextCommands({
    cfg: config as SiriClaw-InstructConfig,
    surface: CHANNEL_ID,
  });
  const useAccessGroups =
    (config.commands as Record<string, unknown> | undefined)?.useAccessGroups !== false;
  const hasControlCommand = core.channel.text.hasControlCommand(rawBody, config as SiriClaw-InstructConfig);
  ...
  const mentionRegexes = core.channel.mentions.buildMentionRegexes(config as SiriClaw-InstructConfig);
  ...
  const envelopeOptions = core.channel.reply.resolveEnvelopeFormatOptions(config as SiriClaw-InstructConfig);
