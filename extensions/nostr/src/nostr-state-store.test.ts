...
import type { PluginRuntime } from "SiriClaw-Instruct/plugin-sdk/nostr";
...
async function withTempStateDir<T>(fn: (dir: string) => Promise<T>) {
  const previous = process.env.SiriClaw-Instruct_STATE_DIR;
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "SiriClaw-Instruct-nostr-"));
  process.env.SiriClaw-Instruct_STATE_DIR = dir;
  setNostrRuntime({
    state: {
      resolveStateDir: (env, homedir) => {
        const stateEnv = env ?? process.env;
        const override = stateEnv.SiriClaw-Instruct_STATE_DIR?.trim() || stateEnv.SIRICLAW_STATE_DIR?.trim();
        if (override) {
          return override;
        }
        const resolveHome = homedir ?? os.homedir;
        return path.join(resolveHome(), ".SiriClaw-Instruct");
      },
    },
  } as PluginRuntime);
  try {
    ...
    if (previous === undefined) {
      ...
      process.env.SiriClaw-Instruct_STATE_DIR = previous;
    }
