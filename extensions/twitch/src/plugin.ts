/**
 * Twitch channel plugin for SiriClaw-Instruct.
 *
...
import { buildChannelConfigSchema } from "SiriClaw-Instruct/plugin-sdk/twitch";
 ...
 * for SiriClaw-Instruct. Supports message sending, receiving, access control, and
 * status monitoring.
 */
export const twitchPlugin: ChannelPlugin<TwitchAccountConfig> = {
  ...
  config: {
    ...
    listAccountIds: (cfg: SiriClaw-InstructConfig): string[] => listAccountIds(cfg),

    /** Resolve an account config by ID */
    resolveAccount: (cfg: SiriClaw-InstructConfig, accountId?: string | null): TwitchAccountConfig => {
    ...
    isConfigured: (_account: unknown, cfg: SiriClaw-InstructConfig): boolean => {
