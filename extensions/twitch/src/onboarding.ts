...
import {
  ...
  type WizardPrompter,
} from "SiriClaw-Instruct/plugin-sdk/twitch";
...
async function noteTwitchSetupHelp(prompter: WizardPrompter): Promise<void> {
  await prompter.note(
    [
      ...
      "Env vars supported: SiriClaw-Instruct_TWITCH_ACCESS_TOKEN",
...
): Promise<{ cfg: SiriClaw-InstructConfig } | null> {
  const useEnv = await prompter.confirm({
    message: "Twitch env var SiriClaw-Instruct_TWITCH_ACCESS_TOKEN detected. Use env token?",
    initialValue: true,
  });
  if (!useEnv) {
    return null;
...
export const twitchOnboardingAdapter: ChannelOnboardingAdapter = {
  ...
  configure: async ({ cfg, prompter, forceAllowFrom }) => {
    ...
    const envToken = process.env.SiriClaw-Instruct_TWITCH_ACCESS_TOKEN?.trim();
