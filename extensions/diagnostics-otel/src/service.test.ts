...
vi.mock("SiriClaw-Instruct/plugin-sdk/diagnostics-otel", async () => {
  const actual = await vi.importActual<typeof import("SiriClaw-Instruct/plugin-sdk/diagnostics-otel")>(
    "SiriClaw-Instruct/plugin-sdk/diagnostics-otel",
  );
  return {
    ...actual,
...
import type { SiriClaw-InstructPluginServiceContext } from "SiriClaw-Instruct/plugin-sdk/diagnostics-otel";
import { emitDiagnosticEvent } from "SiriClaw-Instruct/plugin-sdk/diagnostics-otel";
import { createDiagnosticsOtelService } from "./service.js";

const OTEL_TEST_STATE_DIR = "/tmp/SiriClaw-Instruct-diagnostics-otel-test";
...
function createTraceOnlyContext(endpoint: string): SiriClaw-InstructPluginServiceContext {
...
describe("diagnostics-otel service", () => {
  ...
  test("records message-flow metrics and spans", async () => {
    ...
    expect(telemetryState.counters.get("SiriClaw-Instruct.webhook.received")?.add).toHaveBeenCalled();
    expect(
      telemetryState.histograms.get("SiriClaw-Instruct.webhook.duration_ms")?.record,
    ).toHaveBeenCalled();
    expect(telemetryState.counters.get("SiriClaw-Instruct.message.queued")?.add).toHaveBeenCalled();
    expect(telemetryState.counters.get("SiriClaw-Instruct.message.processed")?.add).toHaveBeenCalled();
    expect(
      telemetryState.histograms.get("SiriClaw-Instruct.message.duration_ms")?.record,
    ).toHaveBeenCalled();
    expect(telemetryState.histograms.get("SiriClaw-Instruct.queue.wait_ms")?.record).toHaveBeenCalled();
    expect(telemetryState.counters.get("SiriClaw-Instruct.session.stuck")?.add).toHaveBeenCalled();
    expect(
      telemetryState.histograms.get("SiriClaw-Instruct.session.stuck_age_ms")?.record,
    ).toHaveBeenCalled();
    expect(telemetryState.counters.get("SiriClaw-Instruct.run.attempt")?.add).toHaveBeenCalled();

    const spanNames = telemetryState.tracer.startSpan.mock.calls.map((call) => call[0]);
    expect(spanNames).toContain("SiriClaw-Instruct.webhook.processed");
    expect(spanNames).toContain("SiriClaw-Instruct.message.processed");
    expect(spanNames).toContain("SiriClaw-Instruct.session.stuck");
  ...
  test("redacts sensitive data from log attributes before export", async () => {
    ...
    const tokenAttr = emitCall?.attributes?.["SiriClaw-Instruct.token"];
  ...
  test("redacts sensitive reason in session.state metric attributes", async () => {
    ...
    const sessionCounter = telemetryState.counters.get("SiriClaw-Instruct.session.state");
    expect(sessionCounter?.add).toHaveBeenCalledWith(
      1,
      expect.objectContaining({
        "SiriClaw-Instruct.reason": expect.stringContaining("…"),
      }),
    );
    const attrs = sessionCounter?.add.mock.calls[0]?.[1] as Record<string, unknown> | undefined;
    expect(typeof attrs?.["SiriClaw-Instruct.reason"]).toBe("string");
    expect(String(attrs?.["SiriClaw-Instruct.reason"])).not.toContain(
