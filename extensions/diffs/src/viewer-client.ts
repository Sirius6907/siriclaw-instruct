...
function getCards(): Array<{ host: HTMLElement; payload: DiffViewerPayload }> {
  ...
  for (const card of document.querySelectorAll<HTMLElement>(".oc-diff-card")) {
    const host = card.querySelector<HTMLElement>("[data-SiriClaw-Instruct-diff-host]");
    const payloadNode = card.querySelector<HTMLScriptElement>("[data-SiriClaw-Instruct-diff-payload]");
    if (!host || !payloadNode) {
      continue;
    }
...
async function main(): Promise<void> {
  try {
    await hydrateViewer();
    document.documentElement.dataset.SiriClaw-InstructDiffsReady = "true";
  } catch (error) {
    document.documentElement.dataset.SiriClaw-InstructDiffsError = "true";
