...
function renderDiffCard(payload: DiffViewerPayload): string {
  return `<section class="oc-diff-card">
    <diffs-container class="oc-diff-host" data-SiriClaw-Instruct-diff-host>
      <template shadowrootmode="open">${payload.prerenderedHTML}</template>
    </diffs-container>
    <script type="application/json" data-SiriClaw-Instruct-diff-payload>${escapeJsonScript(payload)}</script>
  </section>`;
}
...
function buildHtmlDocument(params: {
  ...
  return `<!doctype html>
<html lang="en"${params.runtimeMode === "image" ? ' data-SiriClaw-Instruct-diffs-ready="true"' : ""}>
  <head>
    <meta charset="utf-8" />
    ...
    <style>
      * {
      ...
      [data-SiriClaw-Instruct-diff-root] {
        display: grid;
        gap: 18px;
      }
  ...
  <body data-theme="${params.theme}">
    <main class="oc-frame" data-render-mode="${params.runtimeMode}">
      <div data-SiriClaw-Instruct-diff-root>
        ${params.bodyHtml}
      </div>
    </main>
