# @SiriClaw-Instruct/diffs

Read-only diff viewer plugin for **SiriClaw-Instruct** agents.
...
Set plugin-wide defaults in `~/.SiriClaw-Instruct/SiriClaw-Instruct.json`:
...
Before:
SiriClaw-Instruct supports plugins.

After:
SiriClaw-Instruct supports plugins and hosted diff views.
```
...
- Artifacts are ephemeral and stored in the plugin temp subfolder (`$TMPDIR/SiriClaw-Instruct-diffs`).
