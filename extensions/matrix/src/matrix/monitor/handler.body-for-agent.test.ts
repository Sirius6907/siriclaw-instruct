...
import type { PluginRuntime, RuntimeEnv, RuntimeLogger } from "SiriClaw-Instruct/plugin-sdk/matrix";
  ...
  it("stores sender-labeled BodyForAgent for group thread messages", async () => {
    ...
    const core = {
      channel: {
        ...
        session: {
          resolveStorePath: vi.fn().mockReturnValue("/tmp/SiriClaw-Instruct-test-session.json"),
