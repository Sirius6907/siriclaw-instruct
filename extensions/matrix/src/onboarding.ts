import type { DmPolicy } from "SiriClaw-Instruct/plugin-sdk/matrix";
import {
  ...
  type WizardPrompter,
} from "SiriClaw-Instruct/plugin-sdk/matrix";
  ...
  configure: async ({ cfg, runtime, prompter, forceAllowFrom }) => {
    ...
    const deviceName = String(
      await prompter.text({
        ...
        initialValue: existing.deviceName ?? "SiriClaw-Instruct Gateway",
      }),
    ).trim();
