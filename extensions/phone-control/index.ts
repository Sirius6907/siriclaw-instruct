...
import type { SiriClaw-InstructPluginApi, SiriClaw-InstructPluginService } from "SiriClaw-Instruct/plugin-sdk/phone-control";
...
function normalizeDenyList(cfg: SiriClaw-InstructPluginApi["config"]): string[] {
  return uniqSorted([...(cfg.gateway?.nodes?.denyCommands ?? [])]);
}

function normalizeAllowList(cfg: SiriClaw-InstructPluginApi["config"]): string[] {
...
): SiriClaw-InstructPluginApi["config"] {
  return {
    ...cfg,
    gateway: {
      ...cfg.gateway,
...
async function disarmNow(params: {
  api: SiriClaw-InstructPluginApi;
  stateDir: string;
  statePath: string;
  reason: string;
...
export default function register(api: SiriClaw-InstructPluginApi) {
  ...
  const timerService: SiriClaw-InstructPluginService = {
