...
} from "SiriClaw-Instruct/plugin-sdk/phone-control";
...
function createApi(params: {
  ...
  return {
    ...
    } as SiriClaw-InstructPluginApi["runtime"],
...
describe("phone-control plugin", () => {
  it("arms sms.send as part of the writes group", async () => {
    const stateDir = await fs.mkdtemp(path.join(os.tmpdir(), "SiriClaw-Instruct-phone-control-test-"));
    try {
      ...
      let command: SiriClaw-InstructPluginCommandDefinition | undefined;
