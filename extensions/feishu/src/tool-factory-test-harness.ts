import type { AnyAgentTool, SiriClaw-InstructPluginApi } from "SiriClaw-Instruct/plugin-sdk/feishu";
...
export function createToolFactoryHarness(cfg: SiriClaw-InstructPluginApi["config"]) {
  const registered: RegisteredTool[] = [];

  const api: Pick<SiriClaw-InstructPluginApi, "config" | "logger" | "registerTool"> = {
    config: cfg,
    logger: {
      info: () => {},
      warn: () => {},
  ...
  return {
    api: api as SiriClaw-InstructPluginApi,
    resolveTool,
  };
}
