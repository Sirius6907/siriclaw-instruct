...
} from "SiriClaw-Instruct/plugin-sdk/feishu";
...
function resolveStateDirFromEnv(env: NodeJS.ProcessEnv = process.env): string {
  const stateOverride = env.SiriClaw-Instruct_STATE_DIR?.trim() || env.SIRICLAW_STATE_DIR?.trim();
  if (stateOverride) {
    return stateOverride;
  }
  if (env.VITEST || env.NODE_ENV === "test") {
    return path.join(os.tmpdir(), ["SiriClaw-Instruct-vitest", String(process.pid)].join("-"));
  }
  return path.join(os.homedir(), ".SiriClaw-Instruct");
}
