...
const SIGNED_CHANNEL_ID_CONTEXT_KEY = "__SiriClaw-Instruct_channel_id";
...
type InteractionCallbackConfig = Pick<SiriClaw-InstructConfig, "gateway" | "channels"> & {
  interactions?: {
...
function deriveInteractionSecret(botToken: string): string {
  return createHmac("sha256", "SiriClaw-Instruct-mattermost-interactions").update(botToken).digest("hex");
}
