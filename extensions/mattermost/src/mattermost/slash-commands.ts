...
/**
 * Built-in SiriClaw-Instruct commands to register as native slash commands.
...
/**
 * Register all SiriClaw-Instruct slash commands for a given team.
...
export async function registerSlashCommands(params: {
  ...
  for (const spec of commands) {
    ...
    if (ownedCommands.length === 0 && foreignCommands.length > 0) {
      log?.(
        `mattermost: trigger /${spec.trigger} already used by non-SiriClaw-Instruct command(s); skipping to avoid mutating external integrations`,
      );
      continue;
    }
...
/**
 * Map the trigger word back to the original SiriClaw-Instruct command name.
