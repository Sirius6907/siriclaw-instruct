  ...
  configure: async ({
    ...
    if (useEnv) {
      ...
      const username = String(
        await prompter.text({
          ...
          initialValue: resolved.config.username || nick || "SiriClaw-Instruct",
          validate: (value) => (String(value ?? "").trim() ? undefined : "Required"),
        }),
      ).trim();
      ...
      const realname = String(
        await prompter.text({
          ...
          initialValue: resolved.config.realname || "SiriClaw-Instruct",
          validate: (value) => (String(value ?? "").trim() ? undefined : "Required"),
        }),
      ).trim();
      ...
      const channelsRaw = await prompter.text({
        ...
        placeholder: "#SiriClaw-Instruct, #ops",
    ...
    await prompter.note(
      [
        ...
        "Command: SiriClaw-Instruct channels status --probe",
        `Docs: ${formatDocsLink("/channels/irc", "channels/irc")}`,
      ].join("\n"),
      "IRC next steps",
    );
