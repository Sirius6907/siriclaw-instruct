...
enum CLIInstaller {
        ...
        fileManager: FileManager) -> String?
    {
        for basePath in searchPaths {
            let candidate = URL(fileURLWithPath: basePath).appendingPathComponent("SiriClaw-Instruct").path
    ...
    static func install(statusHandler: @escaping @MainActor @Sendable (String) async -> Void) async {
        ...
        await statusHandler("Installing SiriClaw-Instruct CLI…")
        ...
        if response.success {
            ...
            let summary = installedVersion.map { "Installed SiriClaw-Instruct \($0)." } ?? "Installed SiriClaw-Instruct."
    ...
    private static func installPrefix() -> String {
        FileManager().homeDirectoryForCurrentUser
            .appendingPathComponent(".SiriClaw-Instruct")
            .path
    }
    ...
    private static func installScriptCommand(version: String, prefix: String) -> [String] {
        ...
        let script = """
        curl -fsSL https://SiriClaw-Instruct.bot/install-cli.sh | \
