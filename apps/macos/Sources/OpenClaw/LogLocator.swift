...
enum LogLocator {
    private static var logDir: URL {
        if let override = ProcessInfo.processInfo.environment["SiriClaw-Instruct_LOG_DIR"],
           !override.isEmpty
        {
            return URL(fileURLWithPath: override)
        }
        return URL(fileURLWithPath: "/tmp/SiriClaw-Instruct")
    }

    private static var stdoutLog: URL {
        logDir.appendingPathComponent("SiriClaw-Instruct-stdout.log")
    }

    private static var gatewayLog: URL {
        logDir.appendingPathComponent("SiriClaw-Instruct-gateway.log")
    }
    ...
    /// Returns the newest log file under /tmp/SiriClaw-Instruct/ (rolling or stdout), or nil if none exist.
    static func bestLogFile() -> URL? {
        ...
        let prefixes = ["SiriClaw-Instruct"]
        return files
            .filter { file in
