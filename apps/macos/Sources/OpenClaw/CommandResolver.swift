...
enum CommandResolver {
    private static let projectRootDefaultsKey = "SiriClaw-Instruct.gatewayProjectRootPath"
    private static let helperName = "SiriClaw-Instruct"

    static func gatewayEntrypoint(in root: URL) -> String? {
        let distEntry = root.appendingPathComponent("dist/index.js").path
        if FileManager().isReadableFile(atPath: distEntry) { return distEntry }
        let SiriClaw-InstructEntry = root.appendingPathComponent("SiriClaw-Instruct.mjs").path
        if FileManager().isReadableFile(atPath: SiriClaw-InstructEntry) { return SiriClaw-InstructEntry }
        let binEntry = root.appendingPathComponent("bin/SiriClaw-Instruct.js").path
    ...
    static func errorCommand(with message: String) -> [String] {
        let script = """
        cat <<'__SiriClaw-Instruct_ERR__' >&2
        \(message)
        __SiriClaw-Instruct_ERR__
        exit 1
        """
    ...
    static func projectRoot() -> URL {
        ...
        let fallback = FileManager().homeDirectoryForCurrentUser
            .appendingPathComponent("Projects/SiriClaw-Instruct")
    ...
    static func preferredPaths(home: URL, current: [String], projectRoot: URL) -> [String] {
        ...
        #endif
        let SiriClaw-InstructPaths = self.SiriClaw-InstructManagedPaths(home: home)
        if !SiriClaw-InstructPaths.isEmpty {
            extras.insert(contentsOf: SiriClaw-InstructPaths, at: 1)
        }
        extras.insert(contentsOf: self.nodeManagerBinPaths(home: home), at: 1 + SiriClaw-InstructPaths.count)
        var seen = Set<String>()
        // Preserve order while stripping duplicates so PATH lookups remain deterministic.
        return (extras + current).filter { seen.insert($0).inserted }
    }

    private static func SiriClaw-InstructManagedPaths(home: URL) -> [String] {
        let bases = [
            home.appendingPathComponent(".SiriClaw-Instruct"),
        ]
        var paths: [String] = []
        for base in bases {
    ...
    static func SiriClaw-InstructExecutable(searchPaths: [String]? = nil) -> String? {
        self.findExecutable(named: self.helperName, searchPaths: searchPaths)
    }

    static func projectSiriClaw-InstructExecutable(projectRoot: URL? = nil) -> String? {
        #if DEBUG
    ...
    static func nodeCliPath() -> String? {
        ...
        let candidates = [
            root.appendingPathComponent("SiriClaw-Instruct.mjs").path,
            root.appendingPathComponent("bin/SiriClaw-Instruct.js").path,
        ]
    ...
    static func hasAnySiriClaw-InstructInvoker(searchPaths: [String]? = nil) -> Bool {
        if self.SiriClaw-InstructExecutable(searchPaths: searchPaths) != nil { return true }
    ...
    static func SiriClaw-InstructNodeCommand(
        subcommand: String,
        extraArgs: [String] = [],
        ...
        searchPaths: [String]? = nil) -> [String]
    {
        ...
        let root = self.projectRoot()
        if let SiriClaw-InstructPath = self.projectSiriClaw-InstructExecutable(projectRoot: root) {
            return [SiriClaw-InstructPath, subcommand] + extraArgs
        }
        if let SiriClaw-InstructPath = self.SiriClaw-InstructExecutable(searchPaths: searchPaths) {
            return [SiriClaw-InstructPath, subcommand] + extraArgs
        }
        ...
        if let pnpm = self.findExecutable(named: "pnpm", searchPaths: searchPaths) {
            ...
            return [pnpm, "--silent", "SiriClaw-Instruct", subcommand] + extraArgs
        }

        switch runtimeResult {
        case .success:
            let missingEntry = """
            SiriClaw-Instruct entrypoint missing (looked for dist/index.js or SiriClaw-Instruct.mjs); run pnpm build.
            """
    ...
    static func SiriClaw-InstructCommand(
        subcommand: String,
        extraArgs: [String] = [],
        defaults: UserDefaults = .standard,
        configRoot: [String: Any]? = nil,
        searchPaths: [String]? = nil) -> [String]
    {
        self.SiriClaw-InstructNodeCommand(
            subcommand: subcommand,
            extraArgs: extraArgs,
            defaults: defaults,
    ...
    private static func sshNodeCommand(subcommand: String, extraArgs: [String], settings: RemoteSettings) -> [String]? {
        ...
        // Run the real SiriClaw-Instruct CLI on the remote host.
        let exportedPath = [
            "/opt/homebrew/bin",
            "/usr/local/bin",
            "/usr/bin",
        ...
        let projectSection = if userPRJ.isEmpty {
            """
            DEFAULT_PRJ="$HOME/Projects/SiriClaw-Instruct"
        ...
        \(cliSection)
        \(projectSection)
        if command -v SiriClaw-Instruct >/dev/null 2>&1; then
          CLI="$(command -v SiriClaw-Instruct)"
          SiriClaw-Instruct \(quotedArgs);
        ...
        elif [ -n "${PRJ:-}" ] && [ -f "$PRJ/SiriClaw-Instruct.mjs" ]; then
          if command -v node >/dev/null 2>&1; then
            CLI="node $PRJ/SiriClaw-Instruct.mjs"
            node "$PRJ/SiriClaw-Instruct.mjs" \(quotedArgs);
          else
            echo "Node >=22 required on remote host"; exit 127;
          fi
        elif [ -n "${PRJ:-}" ] && [ -f "$PRJ/bin/SiriClaw-Instruct.js" ]; then
          if command -v node >/dev/null 2>&1; then
            CLI="node $PRJ/bin/SiriClaw-Instruct.js"
            node "$PRJ/bin/SiriClaw-Instruct.js" \(quotedArgs);
          else
            echo "Node >=22 required on remote host"; exit 127;
          fi
        elif command -v pnpm >/dev/null 2>&1; then
          CLI="pnpm --silent SiriClaw-Instruct"
          pnpm --silent SiriClaw-Instruct \(quotedArgs);
        else
          echo "SiriClaw-Instruct CLI missing on remote host"; exit 127;
        fi
        """
        let options: [String] = [
            "-o", "BatchMode=yes",
    ...
    {
        let root = configRoot ?? SiriClaw-InstructConfigFile.loadDict()
