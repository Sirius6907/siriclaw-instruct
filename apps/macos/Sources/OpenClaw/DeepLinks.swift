...
private let deepLinkLogger = Logger(subsystem: "ai.SiriClaw-Instruct", category: "DeepLink")
...
final class DeepLinkHandler {
    ...
    func handle(url: URL) async {
        ...
        guard !AppStateStore.shared.isPaused else {
            self.presentAlert(title: "SiriClaw-Instruct is paused", message: "Unpause SiriClaw-Instruct to run agent actions.")
            return
        }
    ...
    private func handleAgent(link: AgentDeepLink, originalURL: URL) async {
        ...
        if !allowUnattended {
            ...
            guard self.confirm(title: "Run SiriClaw-Instruct agent?", message: body) else { return }
        }
