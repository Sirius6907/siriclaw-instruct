...
enum RuntimeLocator {
    private static let logger = Logger(subsystem: "ai.SiriClaw-Instruct", category: "runtime")
    ...
    static func describeFailure(_ error: RuntimeResolutionError) -> String {
        ...
        case let .notFound(searchPaths):
            [
                "SiriClaw-Instruct needs Node >=22.0.0 but found no runtime.",
        ...
        case let .unsupported(kind, found, required, path, searchPaths):
            [
                ...
                "PATH searched: \(searchPaths.joined(separator: ":"))",
                "Upgrade Node and rerun SiriClaw-Instruct.",
            ].joined(separator: "\n")
