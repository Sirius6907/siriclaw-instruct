...
final class RemotePortTunnel {
    private static let logger = Logger(subsystem: "ai.SiriClaw-Instruct", category: "remote.tunnel")
    ...
    private static func resolveRemotePortOverride(for sshHost: String) -> Int? {
        ...
        let sshKey = SiriClaw-InstructConfigFile.hostKey(sshHost)
        let urlKey = SiriClaw-InstructConfigFile.hostKey(host)
    ...
    private static func findPort(preferred: UInt16?, allowRandom: Bool) async throws -> UInt16 {
        ...
        return try await withCheckedThrowingContinuation { cont in
            let queue = DispatchQueue(label: "ai.SiriClaw-Instruct.remote.tunnel.port", qos: .utility)
            do {
