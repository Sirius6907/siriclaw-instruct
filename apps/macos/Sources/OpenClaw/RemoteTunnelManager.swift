...
actor RemoteTunnelManager {
    ...
    private let logger = Logger(subsystem: "ai.SiriClaw-Instruct", category: "remote-tunnel")
    ...
    func controlTunnelPortIfRunning() async -> UInt16? {
        ...
        // If a previous SiriClaw-Instruct run already has an SSH listener on the expected port (common after restarts),
