...
private let webChatSwiftLogger = Logger(subsystem: "ai.SiriClaw-Instruct", category: "WebChatSwiftUI")
...
struct MacGatewayChatTransport: SiriClaw-InstructChatTransport {
    ...
    func listSessions(limit: Int?) async throws -> SiriClaw-InstructChatSessionsListResponse {
        ...
        let data = try await GatewayConnection.shared.request(
            ...
            params: params,
            timeoutMs: 15000)
        return try JSONDecoder().decode(SiriClaw-InstructChatSessionsListResponse.self, from: data)
    }
    ...
    static func mapPushToTransportEvent(_ push: GatewayPush) -> SiriClaw-InstructChatTransportEvent? {
        switch push {
        case let .snapshot(hello):
            let ok = (try? JSONDecoder().decode(
                SiriClaw-InstructGatewayHealthOK.self,
        ...
        case let .event(evt):
            ...
            case "chat":
                ...
                guard let chat = try? JSONDecoder().decode(
                    SiriClaw-InstructChatEventPayload.self,
            ...
            case "agent":
                ...
                guard let agent = try? JSONDecoder().decode(
                    SiriClaw-InstructAgentEventPayload.self,
                    from: JSONEncoder().encode(payload))
                else {
                    return nil
                }
...
final class WebChatSwiftUIWindowController {
    ...
    private let hosting: NSHostingController<SiriClaw-InstructChatView>
    ...
    init(sessionKey: String, presentation: WebChatPresentation, transport: any SiriClaw-InstructChatTransport) {
        self.sessionKey = sessionKey
        self.presentation = presentation
        let vm = SiriClaw-InstructChatViewModel(sessionKey: sessionKey, transport: transport)
        let accent = Self.color(fromHex: AppStateStore.shared.seamColorHex)
        self.hosting = NSHostingController(rootView: SiriClaw-InstructChatView(
            viewModel: vm,
        ...
        contentViewController: NSViewController) -> NSWindow
    {
        ...
        case .window:
            let window = NSWindow(
                ...
                backing: .buffered,
                defer: false)
            window.title = "SiriClaw-Instruct Chat"
    ...
    private static func makeContentController(
        for presentation: WebChatPresentation,
        hosting: NSHostingController<SiriClaw-InstructChatView>) -> NSViewController
    {
