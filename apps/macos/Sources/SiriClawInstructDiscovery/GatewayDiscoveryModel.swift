...
public final class GatewayDiscoveryModel {
    ...
    private let logger = Logger(subsystem: "ai.SiriClaw-Instruct", category: "gateway-discovery")
    ...
    public func start() {
        ...
        for domain in SiriClaw-InstructBonjour.gatewayServiceDomains {
            let browser = GatewayDiscoveryBrowserSupport.makeBrowser(
                serviceType: SiriClaw-InstructBonjour.gatewayServiceType,
                domain: domain,
                queueLabelPrefix: "ai.SiriClaw-Instruct.macos.gateway-discovery",
    ...
    public func refreshWideAreaFallbackNow(timeoutSeconds: TimeInterval = 5.0) {
        guard let domain = SiriClaw-InstructBonjour.wideAreaGatewayServiceDomain else { return }
    ...
    private func updateGateways(for domain: String) {
        ...
        if let wideAreaDomain = SiriClaw-InstructBonjour.wideAreaGatewayServiceDomain,
    ...
    private var hasUsableWideAreaResults: Bool {
        guard let domain = SiriClaw-InstructBonjour.wideAreaGatewayServiceDomain else { return false }
    ...
    private nonisolated static func prettifyInstanceName(_ decodedName: String) -> String {
        ...
        let stripped = normalized.replacingOccurrences(of: " (SiriClaw-Instruct)", with: "")
