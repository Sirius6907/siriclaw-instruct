...
struct ControlAgentEvent: Codable, Identifiable {
    ...
    let stream: String
    let ts: Double
    let data: [String: SiriClaw-InstructProtocol.AnyCodable]
    let summary: String?
}
...
final class ControlChannel {
    ...
    private let logger = Logger(subsystem: "ai.SiriClaw-Instruct", category: "control")
    ...
    {
        do {
            let rawParams = params?.reduce(into: [String: SiriClaw-InstructKit.AnyCodable]()) {
                $0[$1.key] = SiriClaw-InstructKit.AnyCodable($1.value.base)
            }
    ...
    private static func bridgeToProtocolArgs(
        _ value: SiriClaw-InstructProtocol.AnyCodable?) -> [String: SiriClaw-InstructProtocol.AnyCodable]?
    {
        guard let value else { return nil }
        if let dict = value.value as? [String: SiriClaw-InstructProtocol.AnyCodable] {
            return dict
        }
        if let dict = value.value as? [String: SiriClaw-InstructKit.AnyCodable],
           let data = try? JSONEncoder().encode(dict),
           let decoded = try? JSONDecoder().decode([String: SiriClaw-InstructProtocol.AnyCodable].self, from: data)
        {
            return decoded
        }
...
extension Notification.Name {
    static let controlHeartbeat = Notification.Name("SiriClaw-Instruct.control.heartbeat")
    static let controlAgentEvent = Notification.Name("SiriClaw-Instruct.control.agent")
}
