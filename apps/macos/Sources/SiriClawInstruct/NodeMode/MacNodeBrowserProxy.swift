...
actor MacNodeBrowserProxy {
    ...
    private struct RequestParams: Decodable {
        let method: String?
        let path: String?
        let query: [String: SiriClaw-InstructProtocol.AnyCodable]?
        let body: SiriClaw-InstructProtocol.AnyCodable?
        let timeoutMs: Int?
        let profile: String?
    }
    ...
    private static func makeRequest(params: RequestParams, endpoint: Endpoint) throws -> URLRequest {
        ...
        if let token = endpoint.token?.trimmingCharacters(in: .whitespacesAndNewlines), !token.isEmpty {
            request.setValue("Bearer \(token)", forHTTPHeaderField: "Authorization")
                  ...
                  !password.isEmpty
        {
            request.setValue(password, forHTTPHeaderField: "x-SiriClaw-Instruct-password")
        }
