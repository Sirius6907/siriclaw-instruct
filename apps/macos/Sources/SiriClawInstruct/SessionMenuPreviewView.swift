...
enum SessionMenuPreviewLoader {
    private static let logger = Logger(subsystem: "ai.SiriClaw-Instruct", category: "SessionPreview")
    ...
    private static func requestPreview(
        keys: [String],
        maxItems: Int) async throws -> SiriClaw-InstructSessionsPreviewPayload
    {
    ...
    private static func snapshot(
        from entry: SiriClaw-InstructSessionPreviewEntry,
    ...
    private static func cache(payload: SiriClaw-InstructSessionsPreviewPayload, maxItems: Int) async {
    ...
    private static func previewItems(
        from payload: SiriClaw-InstructChatHistoryPayload,
        maxItems: Int) -> [SessionPreviewItem]
    {
        let boundedItems = self.normalizeMaxItems(maxItems)
        let raw: [SiriClaw-InstructKit.AnyCodable] = payload.messages ?? []
    ...
    private static func decodeMessages(_ raw: [SiriClaw-InstructKit.AnyCodable]) -> [SiriClaw-InstructChatMessage] {
        raw.compactMap { item in
            guard let data = try? JSONEncoder().encode(item) else { return nil }
            return try? JSONDecoder().decode(SiriClaw-InstructChatMessage.self, from: data)
        }
    }
    ...
    private static func previewText(for message: SiriClaw-InstructChatMessage) -> String? {
    ...
    private static func isToolCall(_ message: SiriClaw-InstructChatMessage) -> Bool {
    ...
    private static func toolNames(for message: SiriClaw-InstructChatMessage) -> [String] {
        var names: [String] = []
    ...
    private static func mediaSummary(for message: SiriClaw-InstructChatMessage) -> String? {
