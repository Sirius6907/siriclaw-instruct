...
enum SiriClaw-InstructLogging {
    private static let labelSeparator = "::"

    private static let didBootstrap: Void = {
        LoggingSystem.bootstrap { label in
            let (subsystem, category) = Self.parseLabel(label)
            let osHandler = SiriClaw-InstructOSLogHandler(subsystem: subsystem, category: category)
            let fileHandler = SiriClaw-InstructFileLogHandler(label: label)
    ...
    static func parseLabel(_ label: String) -> (String, String) {
        guard let range = label.range(of: labelSeparator) else {
            return ("ai.SiriClaw-Instruct", label)
        }
...
extension Logging.Logger {
    init(subsystem: String, category: String) {
        SiriClaw-InstructLogging.bootstrapIfNeeded()
        let label = SiriClaw-InstructLogging.makeLabel(subsystem: subsystem, category: category)
...
struct SiriClaw-InstructOSLogHandler: AppLogLevelBackedHandler {
...
struct SiriClaw-InstructFileLogHandler: AppLogLevelBackedHandler {
    let label: String
    var metadata: Logger.Metadata = [:]
        ...
        line: UInt)
    {
        ...
        let (subsystem, category) = SiriClaw-InstructLogging.parseLabel(self.label)
