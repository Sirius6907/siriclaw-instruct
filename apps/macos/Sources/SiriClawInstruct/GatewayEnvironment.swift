...
enum GatewayEnvironment {
    private static let logger = Logger(subsystem: "ai.SiriClaw-Instruct", category: "gateway.env")
    private static let supportedBindModes: Set<String> = ["loopback", "tailnet", "lan", "auto"]

    static func gatewayPort() -> Int {
        if let raw = ProcessInfo.processInfo.environment["SiriClaw-Instruct_GATEWAY_PORT"] {
            let trimmed = raw.trimmingCharacters(in: .whitespacesAndNewlines)
            if let parsed = Int(trimmed), parsed > 0 { return parsed }
        }
        if let configPort = SiriClaw-InstructConfigFile.gatewayPort(), configPort > 0 {
            return configPort
        }
    ...
    static func check() -> GatewayEnvironmentStatus {
        ...
        case let .success(runtime):
            let gatewayBin = CommandResolver.SiriClaw-InstructExecutable()
            ...
            if gatewayBin == nil, projectEntrypoint == nil {
                return GatewayEnvironmentStatus(
                    ...
                    gatewayVersion: nil,
                    requiredGateway: expectedString,
                    message: "SiriClaw-Instruct CLI not found in PATH; install the CLI.")
            }
    ...
    private static func preferredGatewayBind() -> String? {
        ...
        if let env = ProcessInfo.processInfo.environment["SiriClaw-Instruct_GATEWAY_BIND"] {
    ...
    static func installGlobal(versionString: String?, statusHandler: @escaping @Sendable (String) -> Void) async {
        ...
        let (label, cmd): (String, [String]) =
            if let npm {
                ("npm", [npm, "install", "-g", "SiriClaw-Instruct@\(target)"])
            } else if let pnpm {
                ("pnpm", [pnpm, "add", "-g", "SiriClaw-Instruct@\(target)"])
            } else if let bun {
                ("bun", [bun, "add", "-g", "SiriClaw-Instruct@\(target)"])
            } else {
                ("npm", ["npm", "install", "-g", "SiriClaw-Instruct@\(target)"])
            }

        statusHandler("Installing SiriClaw-Instruct@\(target) via \(label)…")
        ...
        if response.success {
            statusHandler("Installed SiriClaw-Instruct@\(target)")
        } else {
