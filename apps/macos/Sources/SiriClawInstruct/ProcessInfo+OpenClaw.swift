...
extension ProcessInfo {
    ...
    /// Nix deployments may write defaults into a stable suite (`ai.SiriClaw-Instruct.mac`) even if the shipped
    ...
    {
        if environment["SiriClaw-Instruct_NIX_MODE"] == "1" { return true }
        if standard.bool(forKey: "SiriClaw-Instruct.nixMode") { return true }
        ...
        if isAppBundle, let stableSuite, stableSuite.bool(forKey: "SiriClaw-Instruct.nixMode") { return true }
