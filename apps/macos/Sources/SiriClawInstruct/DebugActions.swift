...
enum DebugActions {
    private static let verboseDefaultsKey = "SiriClaw-Instruct.debug.verboseMain"
    private static let sessionMenuLimit = 12
    private static let onboardingSeenKey = "SiriClaw-Instruct.onboardingSeen"
    ...
    static func openConfigFolder() {
        let url = SiriClaw-InstructPaths.stateDirURL
        NSWorkspace.shared.activateFileViewerSelecting([url])
    }
    ...
    static func sendTestNotification() async {
        _ = await NotificationManager().send(title: "SiriClaw-Instruct", body: "Test notification", sound: nil)
    }
    ...
    private static func resolveSessionStorePath() -> String {
        ...
        let configURL = SiriClaw-InstructPaths.configURL
        guard
