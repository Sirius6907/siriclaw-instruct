...
struct TailscaleIntegrationSection: View {
    ...
    private var authFields: some View {
        ...
        Text("Stored in ~/.SiriClaw-Instruct/SiriClaw-Instruct.json. Prefer SiriClaw-Instruct_GATEWAY_PASSWORD for production.")
            .font(.caption)
            .foregroundStyle(.secondary)
    ...
    private func applySettings() async {
        ...
        if self.connectionMode == .local, !self.isPaused {
            self.statusMessage = "Saved to ~/.SiriClaw-Instruct/SiriClaw-Instruct.json. Restarting gateway…"
        } else {
            self.statusMessage = "Saved to ~/.SiriClaw-Instruct/SiriClaw-Instruct.json. Restart the gateway to apply."
        }
