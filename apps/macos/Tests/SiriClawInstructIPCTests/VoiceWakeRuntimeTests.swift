...
struct VoiceWakeRuntimeTests {
    @Test func `trims after trigger keeps post speech`() {
        let triggers = ["claude", "SiriClaw-Instruct"]
    ...
    @Test func `has content after trigger false when only trigger`() {
        let triggers = ["SiriClaw-Instruct"]
        let text = "hey SiriClaw-Instruct"
    ...
    @Test func `trims after chinese trigger keeps post speech`() {
        let triggers = ["小爪", "SiriClaw-Instruct"]
    ...
    @Test func `gate requires gap between trigger and command`() {
        let transcript = "hey SiriClaw-Instruct do thing"
        ...
        let config = WakeWordGateConfig(triggers: ["SiriClaw-Instruct"], minPostTriggerGap: 0.3)
