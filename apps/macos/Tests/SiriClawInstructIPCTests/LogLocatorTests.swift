...
struct LogLocatorTests {
    @Test func `launchd gateway log path ensures tmp dir exists`() {
        ...
        let logDir = baseDir.appendingPathComponent("SiriClaw-Instruct-tests-\(UUID().uuidString)")

        setenv("SiriClaw-Instruct_LOG_DIR", logDir.path, 1)
        defer {
            unsetenv("SiriClaw-Instruct_LOG_DIR")
