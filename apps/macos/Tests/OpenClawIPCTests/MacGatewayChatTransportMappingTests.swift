...
struct MacGatewayChatTransportMappingTests {
    @Test func `snapshot maps to health`() {
        let snapshot = Snapshot(
            presence: [],
            health: SiriClaw-InstructProtocol.AnyCodable(["ok": SiriClaw-InstructProtocol.AnyCodable(false)]),
    ...
    @Test func `health event maps to health`() {
        let frame = EventFrame(
            type: "event",
            event: "health",
            payload: SiriClaw-InstructProtocol.AnyCodable(["ok": SiriClaw-InstructProtocol.AnyCodable(true)]),
            seq: 1,
            stateversion: nil)
    ...
    @Test func `chat event maps to chat`() {
        let payload = SiriClaw-InstructProtocol.AnyCodable([
            "runId": SiriClaw-InstructProtocol.AnyCodable("run-1"),
            "sessionKey": SiriClaw-InstructProtocol.AnyCodable("main"),
            "state": SiriClaw-InstructProtocol.AnyCodable("final"),
        ])
    ...
    @Test func `unknown event maps to nil`() {
        let frame = EventFrame(
            type: "event",
            event: "unknown",
            payload: SiriClaw-InstructProtocol.AnyCodable(["a": SiriClaw-InstructProtocol.AnyCodable(1)]),
            seq: 1,
            stateversion: nil)
        let mapped = MacGatewayChatTransport.mapPushToTransportEvent(.event(frame))
        #expect(mapped == nil)
