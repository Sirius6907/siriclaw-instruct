...
@Suite(.serialized) struct VoiceWakeGlobalSettingsSyncTests {
    private func voiceWakeChangedEvent(payload: SiriClaw-InstructProtocol.AnyCodable) -> EventFrame {
        EventFrame(
            type: "event",
    ...
    @Test func `applies voice wake changed event to app state`() async {
        ...
        let evt = self.voiceWakeChangedEvent(payload: SiriClaw-InstructProtocol.AnyCodable(["triggers": [
            "SiriClaw-Instruct",
            "computer",
        ]]))
        ...
        #expect(updated == ["SiriClaw-Instruct", "computer"])
    ...
    @Test func `ignores voice wake changed event with invalid payload`() async {
        let previous = await applyTriggersAndCapturePrevious(["before"])
        let evt = self.voiceWakeChangedEvent(payload: SiriClaw-InstructProtocol.AnyCodable(["unexpected": 123]))
