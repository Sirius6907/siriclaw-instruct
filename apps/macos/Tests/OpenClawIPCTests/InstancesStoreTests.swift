...
struct InstancesStoreTests {
    ...
    func `presence event payload decodes via JSON encoder`() {
        ...
        let entry: [String: SiriClaw-InstructProtocol.AnyCodable] = [
            "host": .init("gw"),
            "ip": .init("10.0.0.1"),
            "version": .init("2.0.0"),
            "mode": .init("gateway"),
        ...
        let payloadMap: [String: SiriClaw-InstructProtocol.AnyCodable] = [
            "presence": .init([SiriClaw-InstructProtocol.AnyCodable(entry)]),
        ]
        let payload = SiriClaw-InstructProtocol.AnyCodable(payloadMap)
