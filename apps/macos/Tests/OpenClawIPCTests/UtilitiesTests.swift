...
@Suite(.serialized) struct UtilitiesTests {
    ...
    @Test func `gateway entrypoint prefers dist over bin`() throws {
        ...
        let bin = tmp.appendingPathComponent("bin/SiriClaw-Instruct.js")
    ...
    @Test func `log locator picks newest log file`() throws {
        let fm = FileManager()
        let dir = URL(fileURLWithPath: "/tmp/SiriClaw-Instruct", isDirectory: true)
        try? fm.createDirectory(at: dir, withIntermediateDirectories: true)

        let older = dir.appendingPathComponent("SiriClaw-Instruct-old-\(UUID().uuidString).log")
        let newer = dir.appendingPathComponent("SiriClaw-Instruct-new-\(UUID().uuidString).log")
