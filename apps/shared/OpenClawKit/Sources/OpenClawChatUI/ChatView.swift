...
@MainActor
public struct SiriClaw-InstructChatView: View {
    public enum Style {
        case standard
        case onboarding
    }

    @State private var viewModel: SiriClaw-InstructChatViewModel
    ...
    public init(
        viewModel: SiriClaw-InstructChatViewModel,
    ...
    public var body: some View {
        ZStack {
            if self.style == .standard {
                SiriClaw-InstructChatTheme.background
                    .ignoresSafeArea()
            }
    ...
    private var visibleMessages: [SiriClaw-InstructChatMessage] {
        let base: [SiriClaw-InstructChatMessage]
    ...
    private func mergeToolResults(in messages: [SiriClaw-InstructChatMessage]) -> [SiriClaw-InstructChatMessage] {
        ...
        for message in messages {
            ...
            let merged = SiriClaw-InstructChatMessage(
                id: last.id,
                role: last.role,
                content: content,
    ...
    private func isToolResultMessage(_ message: SiriClaw-InstructChatMessage) -> Bool {
        let role = message.role.lowercased()
        return role == "toolresult" || role == "tool_result"
    }

    private func shouldDisplayMessage(_ message: SiriClaw-InstructChatMessage) -> Bool {
    ...
    private func primaryText(in message: SiriClaw-InstructChatMessage) -> String {
    ...
    private func hasInlineAttachments(in message: SiriClaw-InstructChatMessage) -> Bool {
    ...
    private func toolCalls(in message: SiriClaw-InstructChatMessage) -> [SiriClaw-InstructChatMessageContent] {
    ...
    private func inlineToolResults(in message: SiriClaw-InstructChatMessage) -> [SiriClaw-InstructChatMessageContent] {
        message.content.filter { content in
            let kind = (content.type ?? "").lowercased()
    ...
    private func toolCallIds(in message: SiriClaw-InstructChatMessage) -> Set<String> {
        var ids = Set<String>()
    ...
    private func toolResultText(from message: SiriClaw-InstructChatMessage) -> String {
    ...
    var body: some View {
        ...
        .background(
            RoundedRectangle(cornerRadius: 18, style: .continuous)
                .fill(SiriClaw-InstructChatTheme.subtleCard)
                .overlay(
                    RoundedRectangle(cornerRadius: 18, style: .continuous)
