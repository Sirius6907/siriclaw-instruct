...
@MainActor
struct SiriClaw-InstructChatComposer: View {
    @Bindable var viewModel: SiriClaw-InstructChatViewModel
        ...
        .background {
            ...
            if self.style == .standard {
                let shape = UnevenRoundedRectangle(
                    ...
                    style: .continuous)
                shape
                    .fill(SiriClaw-InstructChatTheme.composerBackground)
                    .overlay(shape.strokeBorder(SiriClaw-InstructChatTheme.composerBorder, lineWidth: 1))
        ...
        ScrollView(.horizontal, showsIndicators: false) {
            HStack(spacing: 6) {
                ForEach(
                    ...
                    id: \SiriClaw-InstructPendingAttachment.id)
                { (att: SiriClaw-InstructPendingAttachment) in
                    HStack(spacing: 6) {
                        if let img = att.preview {
                            SiriClaw-InstructPlatformImageFactory.image(img)
                                .resizable()
                                .scaledToFill()
    ...
    private var editor: some View {
        VStack(alignment: .leading, spacing: 8) {
            ...
            if !self.isComposerCompacted {
                Rectangle()
                    .fill(SiriClaw-InstructChatTheme.divider)
                    .frame(height: 1)
        ...
        .background(
            RoundedRectangle(cornerRadius: 12, style: .continuous)
                .fill(SiriClaw-InstructChatTheme.composerField)
                .overlay(
                    RoundedRectangle(cornerRadius: 12, style: .continuous)
                        .strokeBorder(SiriClaw-InstructChatTheme.composerBorder)))
    ...
    private var connectionPill: some View {
        ...
        .padding(.horizontal, 8)
        .padding(.vertical, 4)
        .background(SiriClaw-InstructChatTheme.subtleCard)
        .clipShape(Capsule())
    }
    ...
    private var editorOverlay: some View {
        ZStack(alignment: .topLeading) {
            if self.viewModel.input.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty {
                Text("Message SiriClaw-Instruct…")
