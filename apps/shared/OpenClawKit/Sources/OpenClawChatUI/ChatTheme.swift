...
enum SiriClaw-InstructChatTheme {
    #if os(macOS)
    ...
    static let assistantBubbleDynamicNSColor = NSColor(
        name: NSColor.Name("SiriClaw-InstructChatTheme.assistantBubble"),
        dynamicProvider: resolvedAssistantBubbleColor(for:))

    static let onboardingAssistantBubbleDynamicNSColor = NSColor(
        name: NSColor.Name("SiriClaw-InstructChatTheme.onboardingAssistantBubble"),
...
enum SiriClaw-InstructPlatformImageFactory {
    static func image(_ image: SiriClaw-InstructPlatformImage) -> Image {
        #if os(macOS)
        Image(nsImage: image)
        #else
        Image(uiImage: image)
