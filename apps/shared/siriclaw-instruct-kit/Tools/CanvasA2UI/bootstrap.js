...
import "@a2ui/lit/ui";
import { themeContext } from "@SiriClaw-Instruct/a2ui-theme-context";
...
class SiriClaw-InstructA2UIHost extends LitElement {
  static properties = {
    surfaces: { state: true },
  ...
  themeProvider = new ContextProvider(this, {
    ...
    initialValue: SiriClaw-InstructTheme,
  });
  ...
  static styles = css`
    :host {
      ...
      padding:
        var(--SiriClaw-Instruct-a2ui-inset-top, 0px)
        var(--SiriClaw-Instruct-a2ui-inset-right, 0px)
        var(--SiriClaw-Instruct-a2ui-inset-bottom, 0px)
        var(--SiriClaw-Instruct-a2ui-inset-left, 0px);
    }
    ...
    #surfaces {
      ...
      height: 100%;
      overflow: auto;
      padding-bottom: var(--SiriClaw-Instruct-a2ui-scroll-pad-bottom, 0px);
    }
    ...
    .status {
      ...
      top: var(--SiriClaw-Instruct-a2ui-status-top, 12px);
      display: inline-flex;
      align-items: center;
      gap: 8px;
      padding: 8px 10px;
    ...
    .toast {
      ...
      left: 50%;
      transform: translateX(-50%);
      bottom: var(--SiriClaw-Instruct-a2ui-toast-bottom, 12px);
      display: inline-flex;
      align-items: center;
      gap: 8px;
      padding: 8px 10px;
    ...
    .empty {
      ...
      left: 50%;
      transform: translateX(-50%);
      top: var(--SiriClaw-Instruct-a2ui-empty-top, var(--SiriClaw-Instruct-a2ui-status-top, 12px));
      text-align: center;
      opacity: 0.8;
      padding: 10px 12px;
      pointer-events: none;
  ...
  connectedCallback() {
    ...
    globalThis.SiriClaw-InstructA2UI = api;
    this.addEventListener("a2uiaction", (evt) => this.#handleA2UIAction(evt));
    this.#statusListener = (evt) => this.#handleActionStatus(evt);
    for (const eventName of ["SiriClaw-Instruct:a2ui-action-status"]) {
  ...
  #handleA2UIAction(evt) {
    ...
    globalThis.__SiriClaw-InstructLastA2UIAction = userAction;

    const handler =
      globalThis.webkit?.messageHandlers?.SiriClaw-InstructCanvasA2UIAction ??
      globalThis.SiriClaw-InstructCanvasA2UIAction;
    if (handler?.postMessage) {
      try {
        // WebKit message handlers support structured objects; Android's JS interface expects strings.
        if (handler === globalThis.SiriClaw-InstructCanvasA2UIAction) {
...
if (!customElements.get("SiriClaw-Instruct-a2ui-host")) {
  customElements.define("SiriClaw-Instruct-a2ui-host", SiriClaw-InstructA2UIHost);
}
