...
import Foundation
import Testing
@testable import SiriClaw-InstructChatUI
...
private func historyPayload(
    ...
    messages: [AnyCodable] = []) -> SiriClaw-InstructChatHistoryPayload
{
    SiriClaw-InstructChatHistoryPayload(
...
private func sessionEntry(key: String, updatedAt: Double) -> SiriClaw-InstructChatSessionEntry {
...
private func makeViewModel(
    sessionKey: String = "main",
    historyResponses: [SiriClaw-InstructChatHistoryPayload],
    sessionsResponses: [SiriClaw-InstructChatSessionsListResponse] = []) async -> (TestChatTransport, SiriClaw-InstructChatViewModel)
{
    let transport = TestChatTransport(historyResponses: historyResponses, sessionsResponses: sessionsResponses)
    let vm = await MainActor.run { SiriClaw-InstructChatViewModel(sessionKey: sessionKey, transport: transport) }
    return (transport, vm)
}

private func loadAndWaitBootstrap(
    vm: SiriClaw-InstructChatViewModel,
...
private func sendUserMessage(_ vm: SiriClaw-InstructChatViewModel, text: String = "hi") async {
    await MainActor.run {
        vm.input = text
        vm.send()
    }
    ...
    seq: Int = 1)
{
    transport.emit(
        .agent(
            SiriClaw-InstructAgentEventPayload(
                runId: runId,
                seq: seq,
                stream: "assistant",
    ...
    sessionKey: String = "main")
{
    transport.emit(
        .chat(
            SiriClaw-InstructChatEventPayload(
                runId: runId,
                sessionKey: sessionKey,
                state: "final",
                message: nil,
...
private final class TestChatTransport: @unchecked Sendable, SiriClaw-InstructChatTransport {
    private let state = TestChatTransportState()
    private let historyResponses: [SiriClaw-InstructChatHistoryPayload]
    private let sessionsResponses: [SiriClaw-InstructChatSessionsListResponse]

    private let stream: AsyncStream<SiriClaw-InstructChatTransportEvent>
    private let continuation: AsyncStream<SiriClaw-InstructChatTransportEvent>.Continuation

    init(
        historyResponses: [SiriClaw-InstructChatHistoryPayload],
        sessionsResponses: [SiriClaw-InstructChatSessionsListResponse] = [])
    {
        self.historyResponses = historyResponses
        self.sessionsResponses = sessionsResponses
        var cont: AsyncStream<SiriClaw-InstructChatTransportEvent>.Continuation!
    ...
    func requestHistory(sessionKey: String) async throws -> SiriClaw-InstructChatHistoryPayload {
        ...
        return self.historyResponses.last ?? SiriClaw-InstructChatHistoryPayload(
            sessionKey: sessionKey,
            sessionId: nil,
            messages: [],
            thinkingLevel: "off")
    ...
    func sendMessage(
        ...
        attachments _: [SiriClaw-InstructChatAttachmentPayload]) async throws -> SiriClaw-InstructChatSendResponse
    {
        await self.state.sentRunIdsAppend(idempotencyKey)
        return SiriClaw-InstructChatSendResponse(runId: idempotencyKey, status: "ok")
    }
    ...
    func listSessions(limit _: Int?) async throws -> SiriClaw-InstructChatSessionsListResponse {
        ...
        return self.sessionsResponses.last ?? SiriClaw-InstructChatSessionsListResponse(
            ts: nil,
            path: nil,
            count: 0,
            defaults: nil,
    ...
    func emit(_ evt: SiriClaw-InstructChatTransportEvent) {
...
@Suite struct ChatViewModelTests {
    ...
    @Test func sessionChoicesPreferMainAndRecent() async throws {
        ...
        let sessions = SiriClaw-InstructChatSessionsListResponse(
            ts: now,
            path: nil,
            count: 4,
            defaults: nil,
    ...
    @Test func stripsInboundMetadataFromHistoryMessages() async throws {
        let history = SiriClaw-InstructChatHistoryPayload(
            sessionKey: "main",
            sessionId: "sess-main",
            messages: [
                AnyCodable([
...
"""]],
        ...
        let vm = await MainActor.run { SiriClaw-InstructChatViewModel(sessionKey: "main", transport: transport) }
