...
struct ChatMessageBubble: View {
    let message: SiriClaw-InstructChatMessage
    let style: SiriClaw-InstructChatView.Style
...
private struct ChatMessageBody: View {
    ...
    var body: some View {
        ...
        let textColor = self.isUser ? SiriClaw-InstructChatTheme.userText : SiriClaw-InstructChatTheme.assistantText
    ...
    private var inlineAttachments: [SiriClaw-InstructChatMessageContent] {
    ...
    private var toolCalls: [SiriClaw-InstructChatMessageContent] {
        self.message.content.filter { content in
    ...
    private var inlineToolResults: [SiriClaw-InstructChatMessageContent] {
        self.message.content.filter { content in
            let kind = (content.type ?? "").lowercased()
    ...
    private var bubbleFillColor: Color {
        if self.isUser {
            return self.userAccent ?? SiriClaw-InstructChatTheme.userBubble
        }
        if self.style == .onboarding {
            return SiriClaw-InstructChatTheme.onboardingAssistantBubble
        }
        return SiriClaw-InstructChatTheme.assistantBubble
    }
    ...
    private var bubbleBorderColor: Color {
        ...
        if self.style == .onboarding {
            return SiriClaw-InstructChatTheme.onboardingAssistantBorder
        }
...
private struct AttachmentRow: View {
    let att: SiriClaw-InstructChatMessageContent
    let isUser: Bool
    ...
    var body: some View {
        HStack(spacing: 8) {
            ...
            Text(self.att.fileName ?? "Attachment")
                .font(.footnote)
                .lineLimit(1)
                .foregroundStyle(self.isUser ? SiriClaw-InstructChatTheme.userText : SiriClaw-InstructChatTheme.assistantText)
            Spacer()
        }
        .padding(10)
...
private struct ToolCallCard: View {
    let content: SiriClaw-InstructChatMessageContent
    let isUser: Bool
    ...
    var body: some View {
        ...
        .background(
            RoundedRectangle(cornerRadius: 16, style: .continuous)
                .fill(SiriClaw-InstructChatTheme.assistantBubble))
        .overlay(
            RoundedRectangle(cornerRadius: 16, style: .continuous)
                .strokeBorder(Color.white.opacity(0.08), lineWidth: 1))
...
struct ChatPendingToolsBubble: View {
    let toolCalls: [SiriClaw-InstructChatPendingToolCall]
        ...
        VStack(alignment: .leading, spacing: 10) {
            ForEach(segments) { segment in
                ...
                ChatMarkdownRenderer(
                    ...
                    variant: self.markdownVariant,
                    font: font,
                    textColor: SiriClaw-InstructChatTheme.assistantText)
            }
        }
    }
}
