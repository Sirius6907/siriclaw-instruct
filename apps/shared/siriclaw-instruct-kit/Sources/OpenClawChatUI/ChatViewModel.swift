...
private let chatUILogger = Logger(subsystem: "ai.SiriClaw-Instruct", category: "SiriClaw-InstructChatUI")

@MainActor
@Observable
public final class SiriClaw-InstructChatViewModel {
    public private(set) var messages: [SiriClaw-InstructChatMessage] = []
    ...
    public var attachments: [SiriClaw-InstructPendingAttachment] = []
    ...
    public private(set) var pendingToolCalls: [SiriClaw-InstructChatPendingToolCall] = []
    public private(set) var sessions: [SiriClaw-InstructChatSessionEntry] = []
    private let transport: any SiriClaw-InstructChatTransport
    ...
    private var pendingToolCallsById: [String: SiriClaw-InstructChatPendingToolCall] = [:] {
        didSet {
    ...
    public init(sessionKey: String, transport: any SiriClaw-InstructChatTransport) {
    ...
    public var sessionChoices: [SiriClaw-InstructChatSessionEntry] {
        let now = Date().timeIntervalSince1970 * 1000
        let cutoff = now - (24 * 60 * 60 * 1000)
        let sorted = self.sessions.sorted { ($0.updatedAt ?? 0) > ($1.updatedAt ?? 0) }

        var result: [SiriClaw-InstructChatSessionEntry] = []
    ...
    public func removeAttachment(_ id: SiriClaw-InstructPendingAttachment.ID) {
    ...
    private static func decodeMessages(_ raw: [AnyCodable]) -> [SiriClaw-InstructChatMessage] {
        let decoded = raw.compactMap { item in
            (try? ChatPayloadDecoding.decode(item, as: SiriClaw-InstructChatMessage.self))
                .map { Self.stripInboundMetadata(from: $0) }
        }
        return Self.dedupeMessages(decoded)
    }

    private static func stripInboundMetadata(from message: SiriClaw-InstructChatMessage) -> SiriClaw-InstructChatMessage {
        guard message.role.lowercased() == "user" else {
            return message
        }

        let sanitizedContent = message.content.map { content -> SiriClaw-InstructChatMessageContent in
            guard let text = content.text else { return content }
            let cleaned = ChatMarkdownPreprocessor.preprocess(markdown: text).cleaned
            return SiriClaw-InstructChatMessageContent(
                type: content.type,
                text: cleaned,
        ...
        return SiriClaw-InstructChatMessage(
            id: message.id,
            role: message.role,
    ...
    private static func messageIdentityKey(for message: SiriClaw-InstructChatMessage) -> String? {
    ...
    private static func reconcileMessageIDs(
        previous: [SiriClaw-InstructChatMessage],
        incoming: [SiriClaw-InstructChatMessage]) -> [SiriClaw-InstructChatMessage]
    {
    ...
    private static func dedupeMessages(_ messages: [SiriClaw-InstructChatMessage]) -> [SiriClaw-InstructChatMessage] {
        var result: [SiriClaw-InstructChatMessage] = []
    ...
    private static func dedupeKey(for message: SiriClaw-InstructChatMessage) -> String? {
    ...
    private func performSend() async {
        ...
        var userContent: [SiriClaw-InstructChatMessageContent] = [
            SiriClaw-InstructChatMessageContent(
                type: "text",
                text: messageText,
                thinking: nil,
        ...
        let encodedAttachments = self.attachments.map { att -> SiriClaw-InstructChatAttachmentPayload in
            SiriClaw-InstructChatAttachmentPayload(
                type: att.type,
    ...
    private func placeholderSession(key: String) -> SiriClaw-InstructChatSessionEntry {
        SiriClaw-InstructChatSessionEntry(
            key: key,
            kind: nil,
            displayName: nil,
            surface: nil,
    ...
    private func handleTransportEvent(_ evt: SiriClaw-InstructChatTransportEvent) {
        switch evt {
        case let .health(ok):
            self.healthOK = ok
        case .tick:
    ...
    private func handleChatEvent(_ chat: SiriClaw-InstructChatEventPayload) {
    ...
    private func handleAgentEvent(_ evt: SiriClaw-InstructAgentEventPayload) {
        ...
        case "tool":
            ...
            if phase == "start" {
                ...
                self.pendingToolCallsById[toolCallId] = SiriClaw-InstructChatPendingToolCall(
    ...
    private func addImageAttachment(url: URL?, data: Data, fileName: String, mimeType: String) async {
        ...
        self.attachments.append(
            SiriClaw-InstructPendingAttachment(
                url: url,
                data: data,
                fileName: fileName,
                mimeType: mimeType,
    ...
    private static func previewImage(data: Data) -> SiriClaw-InstructPlatformImage? {
        #if canImport(AppKit)
        NSImage(data: data)
        #elseif canImport(UIKit)
        UIImage(data: data)
