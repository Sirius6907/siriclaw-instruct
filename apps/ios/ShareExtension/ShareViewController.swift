...
final class ShareViewController: UIViewController {
    ...
    private let logger = Logger(subsystem: "ai.SiriClaw-Instruct.ios", category: "ShareExtension")
    ...
    private func setupUI() {
        ...
        self.sendButton.setTitle("Send to SiriClaw-Instruct", for: .normal)
    ...
    private func sendCurrentDraft() async {
        ...
        self.showStatus("Sending to SiriClaw-Instruct gateway…")
        ...
        do {
            ...
            self.showStatus("Sent to SiriClaw-Instruct.")
    ...
    private func sendMessageToGateway(_ message: String, attachments: [ShareAttachment]) async throws {
        guard let config = ShareGatewayRelaySettings.loadConfig() else {
            throw NSError(
                domain: "SiriClaw-InstructShare",
                code: 10,
                userInfo: [NSLocalizedDescriptionKey: "SiriClaw-Instruct is not connected to a gateway yet."])
        }
        ...
        let makeOptions: (String) -> GatewayConnectOptions = { clientId in
            GatewayConnectOptions(
                ...
                clientId: clientId,
                clientMode: "node",
                clientDisplayName: "SiriClaw-Instruct Share",
        ...
        do {
            try await gateway.connect(
                ...
                connectOptions: makeOptions("SiriClaw-Instruct-ios"),
                sessionBox: nil,
                onConnected: {},
                onDisconnected: { _ in },
                onInvoke: { req in
