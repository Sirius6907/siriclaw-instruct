...
@Suite struct VoiceWakePreferencesTests {
    @Test func sanitizeTriggerWordsTrimsAndDropsEmpty() {
        #expect(VoiceWakePreferences.sanitizeTriggerWords([" SiriClaw-Instruct ", "", " \nclaude\t"]) == ["SiriClaw-Instruct", "claude"])
    }
    ...
    @Test func displayStringUsesSanitizedWords() {
        #expect(VoiceWakePreferences.displayString(for: ["", " "]) == "SiriClaw-Instruct, claude")
    }
