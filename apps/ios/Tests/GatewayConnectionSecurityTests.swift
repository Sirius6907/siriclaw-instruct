...
@Suite(.serialized) struct GatewayConnectionSecurityTests {
    ...
    {
        let endpoint: NWEndpoint = .service(name: "Test", type: "_SiriClaw-Instruct-gw._tcp", domain: "local.", interface: nil)
    ...
    private func clearTLSFingerprint(stableID: String) {
        let suite = UserDefaults(suiteName: "ai.SiriClaw-Instruct.shared") ?? .standard
    ...
    @Test @MainActor func manualConnectionsForceTLSForNonLoopbackHosts() async {
        ...
        #expect(controller._test_resolveManualUseTLS(host: "SiriClaw-Instruct.local", useTLS: false) == true)
