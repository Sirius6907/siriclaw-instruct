...
@Suite(.serialized) struct VoiceWakeManagerStateTests {
    ...
    @Test @MainActor func handleRecognitionCallbackDispatchesCommand() async {
        let manager = VoiceWakeManager()
        manager.triggerWords = ["SiriClaw-Instruct"]
        manager.isEnabled = true
        ...
        let transcript = "SiriClaw-Instruct hello"
        let triggerRange = transcript.range(of: "SiriClaw-Instruct")!
        let helloRange = transcript.range(of: "hello")!
        let segments = [
            WakeWordSegment(text: "SiriClaw-Instruct", start: 0.0, duration: 0.2, range: triggerRange),
