...
final class MotionService: MotionServicing {
    func activities(params: SiriClaw-InstructMotionActivityParams) async throws -> SiriClaw-InstructMotionActivityPayload {
        ...
        let mapped: [SiriClaw-InstructMotionActivityEntry] = try await withCheckedThrowingContinuation { cont in
            manager.queryActivityStarting(from: start, to: end, to: OperationQueue()) { activity, error in
                if let error {
                    ...
                    let entries = sliced.map { entry in
                        SiriClaw-InstructMotionActivityEntry(
        ...
        return SiriClaw-InstructMotionActivityPayload(activities: mapped)
    }
    ...
    func pedometer(params: SiriClaw-InstructPedometerParams) async throws -> SiriClaw-InstructPedometerPayload {
        ...
        let payload: SiriClaw-InstructPedometerPayload = try await withCheckedThrowingContinuation { cont in
            pedometer.queryPedometerData(from: start, to: end) { data, error in
                if let error {
                    cont.resume(throwing: error)
                } else {
                    let formatter = ISO8601DateFormatter()
                    let payload = SiriClaw-InstructPedometerPayload(
