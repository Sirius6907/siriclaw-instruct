...
actor CameraController {
    ...
    func snap(params: SiriClaw-InstructCameraSnapParams) async throws -> (
        format: String,
        base64: String,
        width: Int,
        height: Int)
    ...
    func clip(params: SiriClaw-InstructCameraClipParams) async throws -> (
        format: String,
        base64: String,
        durationMs: Int,
        hasAudio: Bool)
    {
        ...
        let movURL = FileManager().temporaryDirectory
            .appendingPathComponent("SiriClaw-Instruct-camera-\(UUID().uuidString).mov")
        let mp4URL = FileManager().temporaryDirectory
            .appendingPathComponent("SiriClaw-Instruct-camera-\(UUID().uuidString).mp4")
        defer {
    ...
    private nonisolated static func pickCamera(
        facing: SiriClaw-InstructCameraFacing,
