...
final class ContactsService: ContactsServicing {
    ...
    func search(params: SiriClaw-InstructContactsSearchParams) async throws -> SiriClaw-InstructContactsSearchPayload {
        ...
        return SiriClaw-InstructContactsSearchPayload(contacts: payload)
    }
    ...
    func add(params: SiriClaw-InstructContactsAddParams) async throws -> SiriClaw-InstructContactsAddPayload {
        ...
        if !phoneNumbers.isEmpty || !emails.isEmpty {
            if let existing = try Self.findExistingContact(
                ...
                emails: emails)
            {
                return SiriClaw-InstructContactsAddPayload(contact: Self.payload(from: existing))
            }
        }
        ...
        return SiriClaw-InstructContactsAddPayload(contact: Self.payload(from: persisted))
    }
    ...
    private static func payload(from contact: CNContact) -> SiriClaw-InstructContactPayload {
        SiriClaw-InstructContactPayload(
