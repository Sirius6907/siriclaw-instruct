...
@MainActor
final class SiriClaw-InstructAppDelegate: NSObject, UIApplicationDelegate, @preconcurrency UNUserNotificationCenterDelegate {
    private let logger = Logger(subsystem: "ai.SiriClaw-Instruct.ios", category: "Push")
    private let backgroundWakeLogger = Logger(subsystem: "ai.SiriClaw-Instruct.ios", category: "BackgroundWake")
    private static let wakeRefreshTaskIdentifier = "ai.SiriClaw-Instruct.ios.bgrefresh"
...
enum WatchPromptNotificationBridge {
    static let typeKey = "SiriClaw-Instruct.type"
    static let typeValue = "watch.prompt"
    static let promptIDKey = "SiriClaw-Instruct.watch.promptId"
    static let sessionKeyKey = "SiriClaw-Instruct.watch.sessionKey"
    static let actionPrimaryIDKey = "SiriClaw-Instruct.watch.action.primary.id"
    static let actionPrimaryLabelKey = "SiriClaw-Instruct.watch.action.primary.label"
    static let actionSecondaryIDKey = "SiriClaw-Instruct.watch.action.secondary.id"
    static let actionSecondaryLabelKey = "SiriClaw-Instruct.watch.action.secondary.label"
    static let actionPrimaryIdentifier = "SiriClaw-Instruct.watch.action.primary"
    static let actionSecondaryIdentifier = "SiriClaw-Instruct.watch.action.secondary"
    static let actionIdentifierPrefix = "SiriClaw-Instruct.watch.action."
    static let actionIDKeyPrefix = "SiriClaw-Instruct.watch.action.id."
    static let actionLabelKeyPrefix = "SiriClaw-Instruct.watch.action.label."
    static let categoryPrefix = "SiriClaw-Instruct.watch.prompt.category."

    @MainActor
    static func scheduleMirroredWatchPromptNotificationIfNeeded(
        invokeID: String,
        params: SiriClaw-InstructWatchNotifyParams,
        sendResult: WatchNotificationSendResult) async
    {
        ...
        let normalizedActions = (params.actions ?? []).compactMap { action -> SiriClaw-InstructWatchAction? in
            let id = action.id.trimmingCharacters(in: .whitespacesAndNewlines)
            let label = action.label.trimmingCharacters(in: .whitespacesAndNewlines)
            guard !id.isEmpty, !label.isEmpty else { return nil }
            return SiriClaw-InstructWatchAction(id: id, label: label, style: action.style)
        }
        ...
        content.title = title.isEmpty ? "SiriClaw-Instruct" : title
        content.body = body
    ...
    private static func categoryActions(_ actions: [SiriClaw-InstructWatchAction]) -> [UNNotificationAction] {
...
@main
struct SiriClaw-InstructApp: App {
    @State private var appModel: NodeAppModel
    @State private var gatewayController: GatewayConnectionController
    @UIApplicationDelegateAdaptor(SiriClaw-InstructAppDelegate.self) private var appDelegate
...
extension SiriClaw-InstructApp {
    private static func installUncaughtExceptionLogger() {
        NSLog("SiriClaw-Instruct: installing uncaught exception handler")
