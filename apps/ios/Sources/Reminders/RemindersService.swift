...
final class RemindersService: RemindersServicing {
    func list(params: SiriClaw-InstructRemindersListParams) async throws -> SiriClaw-InstructRemindersListPayload {
        ...
        let payload: [SiriClaw-InstructReminderPayload] = try await withCheckedThrowingContinuation { cont in
            store.fetchReminders(matching: predicate) { items in
                ...
                let payload = selected.map { reminder in
                    ...
                    return SiriClaw-InstructReminderPayload(
        ...
        return SiriClaw-InstructRemindersListPayload(reminders: payload)
    }
    ...
    func add(params: SiriClaw-InstructRemindersAddParams) async throws -> SiriClaw-InstructRemindersAddPayload {
        ...
        let formatter = ISO8601DateFormatter()
        let due = reminder.dueDateComponents.flatMap { Calendar.current.date(from: $0) }
        let payload = SiriClaw-InstructReminderPayload(
            identifier: reminder.calendarItemIdentifier,
            title: reminder.title,
            dueISO: due.map { formatter.string(from: $0) },
            completed: reminder.isCompleted,
        ...
        return SiriClaw-InstructRemindersAddPayload(reminder: payload)
    }
