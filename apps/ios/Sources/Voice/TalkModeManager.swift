...
final class TalkModeManager: NSObject {
    ...
    private static let redactedConfigSentinel = "__SiriClaw-Instruct_REDACTED__"
    ...
    private var pttCompletion: CheckedContinuation<SiriClaw-InstructTalkPTTStopPayload, Never>?
    ...
    private let logger = Logger(subsystem: "ai.SiriClaw-Instruct", category: "TalkMode")
    ...
    func stop() {
        ...
        if pendingPTT {
            let payload = SiriClaw-InstructTalkPTTStopPayload(
    ...
    func beginPushToTalk() async throws -> SiriClaw-InstructTalkPTTStartPayload {
        guard self.gatewayConnected else {
            self.statusText = "Offline"
        ...
        if self.isPushToTalkActive, let captureId = self.activePTTCaptureId {
            return SiriClaw-InstructTalkPTTStartPayload(captureId: captureId)
        }
    ...
    func endPushToTalk() async -> SiriClaw-InstructTalkPTTStopPayload {
    ...
    func runPushToTalkOnce(maxDurationSeconds: TimeInterval = 12) async throws -> SiriClaw-InstructTalkPTTStopPayload {
        ...
        if self.isPushToTalkActive {
            let captureId = self.activePTTCaptureId ?? UUID().uuidString
            return SiriClaw-InstructTalkPTTStopPayload(
                captureId: captureId,
                transcript: nil,
                status: "busy")
        }
    ...
    func cancelPushToTalk() async -> SiriClaw-InstructTalkPTTStopPayload {
        let captureId = self.activePTTCaptureId ?? UUID().uuidString
        guard self.isPushToTalkActive else {
            let payload = SiriClaw-InstructTalkPTTStopPayload(
                captureId: captureId,
    ...
    private func finishPTTOnce(_ payload: SiriClaw-InstructTalkPTTStopPayload) {
    ...
    private func streamAssistant(runId: String, gateway: GatewayNodeSession) async {
        ...
        for await evt in stream {
            ...
            guard let agentEvent = try? GatewayPayloadDecoding.decode(
                payload,
                as: SiriClaw-InstructAgentEventPayload.self
            ) else {
                continue
            }
