...
final class NodeAppModel {
    ...
    private let deepLinkLogger = Logger(subsystem: "ai.SiriClaw-Instruct.ios", category: "DeepLink")
    private let pushWakeLogger = Logger(subsystem: "ai.SiriClaw-Instruct.ios", category: "PushWake")
    private let locationWakeLogger = Logger(subsystem: "ai.SiriClaw-Instruct.ios", category: "LocationWake")
    private let watchReplyLogger = Logger(subsystem: "ai.SiriClaw-Instruct.ios", category: "WatchReply")
    enum CameraHUDKind {
        case photo
        case recording
        case success
    ...
    private func handleCanvasA2UIAction(body: [String: Any]) async {
        ...
        guard let name = SiriClaw-InstructCanvasA2UIAction.extractActionName(userAction) else { return }
        let actionId: String = {
        ...
        let contextJSON = SiriClaw-InstructCanvasA2UIAction.compactJSON(userAction["context"])
        let sessionKey = self.mainSessionKey

        let messageContext = SiriClaw-InstructCanvasA2UIAction.AgentMessageContext(
            actionName: name,
            session: .init(key: sessionKey, surfaceId: surfaceId),
            component: .init(id: sourceComponentId, host: host, instanceId: instanceId),
            contextJSON: contextJSON)
        let message = SiriClaw-InstructCanvasA2UIAction.formatAgentMessage(messageContext)
        ...
        let js = SiriClaw-InstructCanvasA2UIAction.jsDispatchA2UIActionStatus(actionId: actionId, ok: ok, error: errorText)
        do {
    ...
    func requestLocationPermissions(mode: SiriClaw-InstructLocationMode) async -> Bool {
        ...
        self.gatewayHealthMonitor.start(
            check: { [weak self] in
                ...
                do {
                    ...
                    guard let decoded = try? JSONDecoder().decode(SiriClaw-InstructGatewayHealthOK.self, from: data) else {
                        return false
                    }
    ...
    private func handleInvoke(_ req: BridgeInvokeRequest) async -> BridgeInvokeResponse {
        ...
        if self.isBackgrounded, self.isBackgroundRestricted(command) {
            return BridgeInvokeResponse(
                id: req.id,
                ok: false,
                error: SiriClaw-InstructNodeError(
        ...
        do {
            ...
            case .unknownCommand:
                return BridgeInvokeResponse(
                    id: req.id,
                    ok: false,
                    error: SiriClaw-InstructNodeError(code: .invalidRequest, message: "INVALID_REQUEST: unknown command"))
            case .handlerUnavailable:
                return BridgeInvokeResponse(
                    id: req.id,
                    ok: false,
                    error: SiriClaw-InstructNodeError(code: .unavailable, message: "node handler unavailable"))
            }
        } catch {
            ...
            return BridgeInvokeResponse(
                id: req.id,
                ok: false,
                error: SiriClaw-InstructNodeError(code: .unavailable, message: error.localizedDescription))
        }
    }
    ...
    private func handleLocationInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        ...
        let params = (try? Self.decodeParams(SiriClaw-InstructLocationGetParams.self, from: req.paramsJSON)) ??
            SiriClaw-InstructLocationGetParams()
        ...
        let payload = SiriClaw-InstructLocationPayload(
    ...
    private func handleCanvasInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        switch req.command {
        case SiriClaw-InstructCanvasCommand.present.rawValue:
            // iOS ignores placement hints; canvas always fills the screen.
            let params = (try? Self.decodeParams(SiriClaw-InstructCanvasPresentParams.self, from: req.paramsJSON)) ??
                SiriClaw-InstructCanvasPresentParams()
        ...
        case SiriClaw-InstructCanvasCommand.hide.rawValue:
            self.screen.showDefaultCanvas()
            return BridgeInvokeResponse(id: req.id, ok: true)
        case SiriClaw-InstructCanvasCommand.navigate.rawValue:
            let params = try Self.decodeParams(SiriClaw-InstructCanvasNavigateParams.self, from: req.paramsJSON)
            self.screen.navigate(to: params.url)
            return BridgeInvokeResponse(id: req.id, ok: true)
        case SiriClaw-InstructCanvasCommand.evalJS.rawValue:
            let params = try Self.decodeParams(SiriClaw-InstructCanvasEvalParams.self, from: req.paramsJSON)
            let result = try await self.screen.eval(javaScript: params.javaScript)
            let payload = try Self.encodePayload(["result": result])
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: payload)
        case SiriClaw-InstructCanvasCommand.snapshot.rawValue:
            let params = try? Self.decodeParams(SiriClaw-InstructCanvasSnapshotParams.self, from: req.paramsJSON)
    ...
    private func handleCanvasA2UIInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        let command = req.command
        switch command {
        case SiriClaw-InstructCanvasA2UICommand.reset.rawValue:
            ...
            (() => {
              const host = globalThis.SiriClaw-InstructA2UI;
              if (!host) return JSON.stringify({ ok: false, error: "missing SiriClaw-InstructA2UI" });
        ...
        case SiriClaw-InstructCanvasA2UICommand.push.rawValue, SiriClaw-InstructCanvasA2UICommand.pushJSONL.rawValue:
            ...
            if command == SiriClaw-InstructCanvasA2UICommand.pushJSONL.rawValue {
                ...
                do {
                    let params = try Self.decodeParams(SiriClaw-InstructCanvasA2UIPushParams.self, from: req.paramsJSON)
            ...
            let messagesJSON = try SiriClaw-InstructCanvasA2UIJSONL.encodeMessagesJSONArray(messages)
            let js = """
            (() => {
              try {
                const host = globalThis.SiriClaw-InstructA2UI;
    ...
    private func handleCameraInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        switch req.command {
        case SiriClaw-InstructCameraCommand.list.rawValue:
            ...
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: payload)
        case SiriClaw-InstructCameraCommand.snap.rawValue:
            self.showCameraHUD(text: "Taking photo…", kind: .photo)
            self.triggerCameraFlash()
            let params = (try? Self.decodeParams(SiriClaw-InstructCameraSnapParams.self, from: req.paramsJSON)) ??
                SiriClaw-InstructCameraSnapParams()
            ...
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: payload)
        case SiriClaw-InstructCameraCommand.clip.rawValue:
            let params = (try? Self.decodeParams(SiriClaw-InstructCameraClipParams.self, from: req.paramsJSON)) ??
                SiriClaw-InstructCameraClipParams()
    ...
    private func handleScreenRecordInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        let params = (try? Self.decodeParams(SiriClaw-InstructScreenRecordParams.self, from: req.paramsJSON)) ??
            SiriClaw-InstructScreenRecordParams()
    ...
    private func handleSystemNotify(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        let params = try Self.decodeParams(SiriClaw-InstructSystemNotifyParams.self, from: req.paramsJSON)
        ...
        if title.isEmpty, body.isEmpty {
            return BridgeInvokeResponse(
                id: req.id,
                ok: false,
                error: SiriClaw-InstructNodeError(code: .invalidRequest, message: "INVALID_REQUEST: empty notification"))
        }
        ...
        guard finalStatus == .authorized || finalStatus == .provisional || finalStatus == .ephemeral else {
            return BridgeInvokeResponse(
                id: req.id,
                ok: false,
                error: SiriClaw-InstructNodeError(code: .unavailable, message: "NOT_AUTHORIZED: notifications"))
        }
        ...
        if case let .failure(error) = addResult {
            return BridgeInvokeResponse(
                id: req.id,
                ok: false,
                error: SiriClaw-InstructNodeError(code: .unavailable, message: "NOTIFICATION_FAILED: \(error.message)"))
        }
        return BridgeInvokeResponse(id: req.id, ok: true)
    }

    private func handleChatPushInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        let params = try Self.decodeParams(SiriClaw-InstructChatPushParams.self, from: req.paramsJSON)
        let text = params.text.trimmingCharacters(in: .whitespacesAndNewlines)
        guard !text.isEmpty else {
            return BridgeInvokeResponse(
                id: req.id,
                ok: false,
                error: SiriClaw-InstructNodeError(code: .invalidRequest, message: "INVALID_REQUEST: empty chat.push text"))
        }
        ...
        if finalStatus == .authorized || finalStatus == .provisional || finalStatus == .ephemeral {
            let addResult = await self.runNotificationCall(timeoutSeconds: 2.0) { [notificationCenter] in
                let content = UNMutableNotificationContent()
                content.title = "SiriClaw-Instruct"
                content.body = text
                content.sound = .default
        ...
        let payload = SiriClaw-InstructChatPushPayload(messageId: messageId)
        let json = try Self.encodePayload(payload)
        return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: json)
    }
    ...
    private func handleDeviceInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        switch req.command {
        case SiriClaw-InstructDeviceCommand.status.rawValue:
            let payload = try await self.deviceStatusService.status()
            let json = try Self.encodePayload(payload)
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: json)
        case SiriClaw-InstructDeviceCommand.info.rawValue:
    ...
    private func handlePhotosInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        let params = (try? Self.decodeParams(SiriClaw-InstructPhotosLatestParams.self, from: req.paramsJSON)) ??
            SiriClaw-InstructPhotosLatestParams()
    ...
    private func handleContactsInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        switch req.command {
        case SiriClaw-InstructContactsCommand.search.rawValue:
            let params = (try? Self.decodeParams(SiriClaw-InstructContactsSearchParams.self, from: req.paramsJSON)) ??
                SiriClaw-InstructContactsSearchParams()
            let payload = try await self.contactsService.search(params: params)
            let json = try Self.encodePayload(payload)
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: json)
        case SiriClaw-InstructContactsCommand.add.rawValue:
            let params = try Self.decodeParams(SiriClaw-InstructContactsAddParams.self, from: req.paramsJSON)
    ...
    private func handleCalendarInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        switch req.command {
        case SiriClaw-InstructCalendarCommand.events.rawValue:
            let params = (try? Self.decodeParams(SiriClaw-InstructCalendarEventsParams.self, from: req.paramsJSON)) ??
                SiriClaw-InstructCalendarEventsParams()
            let payload = try await self.calendarService.events(params: params)
            let json = try Self.encodePayload(payload)
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: json)
        case SiriClaw-InstructCalendarCommand.add.rawValue:
            let params = try Self.decodeParams(SiriClaw-InstructCalendarAddParams.self, from: req.paramsJSON)
    ...
    private func handleRemindersInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        switch req.command {
        case SiriClaw-InstructRemindersCommand.list.rawValue:
            let params = (try? Self.decodeParams(SiriClaw-InstructRemindersListParams.self, from: req.paramsJSON)) ??
                SiriClaw-InstructRemindersListParams()
            let payload = try await self.remindersService.list(params: params)
            let json = try Self.encodePayload(payload)
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: json)
        case SiriClaw-InstructRemindersCommand.add.rawValue:
            let params = try Self.decodeParams(SiriClaw-InstructRemindersAddParams.self, from: req.paramsJSON)
    ...
    private func handleMotionInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        switch req.command {
        case SiriClaw-InstructMotionCommand.activity.rawValue:
            let params = (try? Self.decodeParams(SiriClaw-InstructMotionActivityParams.self, from: req.paramsJSON)) ??
                SiriClaw-InstructMotionActivityParams()
            let payload = try await self.motionService.activities(params: params)
            let json = try Self.encodePayload(payload)
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: json)
        case SiriClaw-InstructMotionCommand.pedometer.rawValue:
            let params = (try? Self.decodeParams(SiriClaw-InstructPedometerParams.self, from: req.paramsJSON)) ??
                SiriClaw-InstructPedometerParams()
    ...
    private func handleTalkInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        switch req.command {
        case SiriClaw-InstructTalkCommand.pttStart.rawValue:
            ...
            let json = try Self.encodePayload(payload)
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: json)
        case SiriClaw-InstructTalkCommand.pttStop.rawValue:
            ...
            let json = try Self.encodePayload(payload)
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: json)
        case SiriClaw-InstructTalkCommand.pttCancel.rawValue:
            let payload = await self.talkMode.cancelPushToTalk()
            self.voiceWake.resumeAfterExternalAudioCapture(wasSuspended: self.pttVoiceWakeSuspended)
            self.pttVoiceWakeSuspended = false
            let json = try Self.encodePayload(payload)
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: json)
        case SiriClaw-InstructTalkCommand.pttOnce.rawValue:
            self.pttVoiceWakeSuspended = self.voiceWake.suspendForExternalAudioCapture()
            defer {
                self.voiceWake.resumeAfterExternalAudioCapture(wasSuspended: self.pttVoiceWakeSuspended)
                self.pttVoiceWakeSuspended = false
...
private extension NodeAppModel {
    ...
    func buildCapabilityRouter() -> NodeCapabilityRouter {
        ...
        register([SiriClaw-InstructLocationCommand.get.rawValue]) { [weak self] req in
        ...
        register([
            ...
            SiriClaw-InstructCanvasCommand.hide.rawValue,
            SiriClaw-InstructCanvasCommand.navigate.rawValue,
            SiriClaw-InstructCanvasCommand.evalJS.rawValue,
        ...
        register([
            ...
            SiriClaw-InstructCanvasA2UICommand.push.rawValue,
            SiriClaw-InstructCanvasA2UICommand.pushJSONL.rawValue,
        ]) { [weak self] req in
            guard let self else { throw NodeCapabilityRouter.RouterError.handlerUnavailable }
        ...
        register([SiriClaw-InstructScreenCommand.record.rawValue]) { [weak self] req in
            guard let self else { throw NodeCapabilityRouter.RouterError.handlerUnavailable }
            return try await self.handleScreenRecordInvoke(req)
        }

        register([SiriClaw-InstructSystemCommand.notify.rawValue]) { [weak self] req in
            guard let self else { throw NodeCapabilityRouter.RouterError.handlerUnavailable }
            return try await self.handleSystemNotify(req)
        }

        register([SiriClaw-InstructChatCommand.push.rawValue]) { [weak self] req in
            guard let self else { throw NodeCapabilityRouter.RouterError.handlerUnavailable }
        ...
        register([
            SiriClaw-InstructWatchCommand.status.rawValue,
            SiriClaw-InstructWatchCommand.notify.rawValue,
        ]) { [weak self] req in
            guard let self else { throw NodeCapabilityRouter.RouterError.handlerUnavailable }
            return try await self.handleWatchInvoke(req)
        }

        register([SiriClaw-InstructPhotosCommand.latest.rawValue]) { [weak self] req in
            guard let self else { throw NodeCapabilityRouter.RouterError.handlerUnavailable }
        ...
        register([
            SiriClaw-InstructRemindersCommand.list.rawValue,
            SiriClaw-InstructRemindersCommand.add.rawValue,
        ]) { [weak self] req in
            guard let self else { throw NodeCapabilityRouter.RouterError.handlerUnavailable }
        ...
        register([
            SiriClaw-InstructTalkCommand.pttStart.rawValue,
            SiriClaw-InstructTalkCommand.pttStop.rawValue,
            SiriClaw-InstructTalkCommand.pttCancel.rawValue,
            SiriClaw-InstructTalkCommand.pttOnce.rawValue,
        ]) { [weak self] req in
            guard let self else { throw NodeCapabilityRouter.RouterError.handlerUnavailable }
    ...
    func handleWatchInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        switch req.command {
        case SiriClaw-InstructWatchCommand.status.rawValue:
            let status = await self.watchMessagingService.status()
            let payload = SiriClaw-InstructWatchStatusPayload(
            ...
            let json = try Self.encodePayload(payload)
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: json)
        case SiriClaw-InstructWatchCommand.notify.rawValue:
            let params = try Self.decodeParams(SiriClaw-InstructWatchNotifyParams.self, from: req.paramsJSON)
            ...
            do {
                ...
                let payload = SiriClaw-InstructWatchNotifyPayload(
    ...
    func locationMode() -> SiriClaw-InstructLocationMode {
        let raw = UserDefaults.standard.string(forKey: "location.enabledMode") ?? "off"
        return SiriClaw-InstructLocationMode(rawValue: raw) ?? .off
    }
...
private extension NodeAppModel {
                            ...
                            if let requestId, !requestId.isEmpty {
                                self.gatewayStatusText =
                                    "Pairing required (requestId: \(requestId)). "
                                        + "Approve on gateway and return to SiriClaw-Instruct."
                            } else {
                                self.gatewayStatusText =
                                    "Pairing required. Approve on gateway and return to SiriClaw-Instruct."
                            }
                        }
    ...
    func legacyClientIdFallback(currentClientId: String, error: Error) -> String? {
        ...
        guard normalizedClientId == "SiriClaw-Instruct-ios" else { return nil }
...
extension NodeAppModel {
    ...
    func runSharePipelineSelfTest() async {
        ...
        let payload = SharedContentPayload(
            title: "SiriClaw-Instruct Share Self-Test",
            url: URL(string: "https://SiriClaw-Instruct.ai/share-self-test"),
    ...
    func handleSilentPushWake(_ userInfo: [AnyHashable: Any]) async -> Bool {
        ...
        let pushKind = Self.SiriClaw-InstructPushKind(userInfo)
        let receivedMessage =
    ...
    private static func SiriClaw-InstructPushKind(_ userInfo: [AnyHashable: Any]) -> String {
        if let payload = userInfo["SiriClaw-Instruct"] as? [String: Any],
           let kind = payload["kind"] as? String
        {
            let trimmed = kind.trimmingCharacters(in: .whitespacesAndNewlines)
            if !trimmed.isEmpty { return trimmed }
        }
        if let payload = userInfo["SiriClaw-Instruct"] as? [AnyHashable: Any],
           let kind = payload["kind"] as? String
        {
            let trimmed = kind.trimmingCharacters(in: .whitespacesAndNewlines)
            if !trimmed.isEmpty { return trimmed }
