...
extension NodeAppModel {
    ...
    static func normalizeWatchActions(
        _ actions: [SiriClaw-InstructWatchAction]?,
        kind: String?,
        promptId: String?) -> [SiriClaw-InstructWatchAction]
    {
        let provided = (actions ?? []).compactMap { action -> SiriClaw-InstructWatchAction? in
            let id = action.id.trimmingCharacters(in: .whitespacesAndNewlines)
            let label = action.label.trimmingCharacters(in: .whitespacesAndNewlines)
            guard !id.isEmpty, !label.isEmpty else { return nil }
            return SiriClaw-InstructWatchAction(
                id: id,
                label: label,
        ...
        if normalizedKind.contains("approval") || normalizedKind.contains("approve") {
            return [
                SiriClaw-InstructWatchAction(id: "approve", label: "Approve"),
                SiriClaw-InstructWatchAction(id: "decline", label: "Decline", style: "destructive"),
                SiriClaw-InstructWatchAction(id: "open_phone", label: "Open iPhone"),
                SiriClaw-InstructWatchAction(id: "escalate", label: "Escalate"),
            ]
        }

        return [
            SiriClaw-InstructWatchAction(id: "done", label: "Done"),
            SiriClaw-InstructWatchAction(id: "snooze_10m", label: "Snooze 10m"),
            SiriClaw-InstructWatchAction(id: "open_phone", label: "Open iPhone"),
            SiriClaw-InstructWatchAction(id: "escalate", label: "Escalate"),
        ]
    }
    ...
    static func normalizedWatchRisk(
        _ risk: SiriClaw-InstructWatchRisk?,
        priority: SiriClaw-InstructNotificationPriority?) -> SiriClaw-InstructWatchRisk?
    {
    ...
    static func normalizedWatchPriority(
        _ priority: SiriClaw-InstructNotificationPriority?,
        risk: SiriClaw-InstructWatchRisk?) -> SiriClaw-InstructNotificationPriority?
    {
