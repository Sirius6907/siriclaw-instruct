...
final class NetworkStatusService: @unchecked Sendable {
    func currentStatus(timeoutMs: Int = 1500) async -> SiriClaw-InstructNetworkStatusPayload {
        await withCheckedContinuation { cont in
            ...
            let queue = DispatchQueue(label: "ai.SiriClaw-Instruct.ios.network-status")
    ...
    private static func payload(from path: NWPath) -> SiriClaw-InstructNetworkStatusPayload {
        let status: SiriClaw-InstructNetworkPathStatus = switch path.status {
        ...
        var interfaces: [SiriClaw-InstructNetworkInterfaceType] = []
        if path.usesInterfaceType(.wifi) { interfaces.append(.wifi) }
        if path.usesInterfaceType(.cellular) { interfaces.append(.cellular) }
        if path.usesInterfaceType(.wiredEthernet) { interfaces.append(.wired) }
        if interfaces.isEmpty { interfaces.append(.other) }

        return SiriClaw-InstructNetworkStatusPayload(
            status: status,
    ...
    private static func fallbackPayload() -> SiriClaw-InstructNetworkStatusPayload {
        SiriClaw-InstructNetworkStatusPayload(
            status: .unsatisfied,
            isExpensive: false,
            isConstrained: false,
            interfaces: [.other])
