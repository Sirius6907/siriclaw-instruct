...
final class DeviceStatusService: DeviceStatusServicing {
    ...
    func status() async throws -> SiriClaw-InstructDeviceStatusPayload {
        ...
        return SiriClaw-InstructDeviceStatusPayload(
            battery: battery,
            thermal: thermal,
            storage: storage,
            network: network,
    ...
    func info() -> SiriClaw-InstructDeviceInfoPayload {
        ...
        return SiriClaw-InstructDeviceInfoPayload(
    ...
    private func batteryStatus() -> SiriClaw-InstructBatteryStatusPayload {
        let device = UIDevice.current
        device.isBatteryMonitoringEnabled = true
        let level = device.batteryLevel >= 0 ? Double(device.batteryLevel) : nil
        let state: SiriClaw-InstructBatteryState = switch device.batteryState {
        ...
        return SiriClaw-InstructBatteryStatusPayload(
            level: level,
            state: state,
            lowPowerModeEnabled: ProcessInfo.processInfo.isLowPowerModeEnabled)
    }

    private func thermalStatus() -> SiriClaw-InstructThermalStatusPayload {
        let state: SiriClaw-InstructThermalState = switch ProcessInfo.processInfo.thermalState {
        ...
        return SiriClaw-InstructThermalStatusPayload(state: state)
    }

    private func storageStatus() -> SiriClaw-InstructStorageStatusPayload {
        let attrs = (try? FileManager.default.attributesOfFileSystem(forPath: NSHomeDirectory())) ?? [:]
        let total = (attrs[.systemSize] as? NSNumber)?.int64Value ?? 0
        let free = (attrs[.systemFreeSize] as? NSNumber)?.int64Value ?? 0
        let used = max(0, total - free)
        return SiriClaw-InstructStorageStatusPayload(totalBytes: total, freeBytes: free, usedBytes: used)
    }
