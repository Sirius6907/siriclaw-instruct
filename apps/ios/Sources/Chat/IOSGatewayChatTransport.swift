...
struct IOSGatewayChatTransport: SiriClaw-InstructChatTransport, Sendable {
    private static let logger = Logger(subsystem: "ai.SiriClaw-Instruct", category: "ios.chat.transport")
    ...
    func listSessions(limit: Int?) async throws -> SiriClaw-InstructChatSessionsListResponse {
        ...
        return try JSONDecoder().decode(SiriClaw-InstructChatSessionsListResponse.self, from: res)
    }
    ...
    func requestHistory(sessionKey: String) async throws -> SiriClaw-InstructChatHistoryPayload {
        ...
        return try JSONDecoder().decode(SiriClaw-InstructChatHistoryPayload.self, from: res)
    }
        ...
        attachments: [SiriClaw-InstructChatAttachmentPayload]) async throws -> SiriClaw-InstructChatSendResponse
    {
        ...
        struct Params: Codable {
            ...
            var message: String
            var thinking: String
            var attachments: [SiriClaw-InstructChatAttachmentPayload]?
            var timeoutMs: Int
        ...
        do {
            ...
            let decoded = try JSONDecoder().decode(SiriClaw-InstructChatSendResponse.self, from: res)
    ...
    func requestHealth(timeoutMs: Int) async throws -> Bool {
        ...
        return (try? JSONDecoder().decode(SiriClaw-InstructGatewayHealthOK.self, from: res))?.ok ?? true
    }
                ...
                for await evt in stream {
                    ...
                    case "health":
                        ...
                        let ok = (try? GatewayPayloadDecoding.decode(
                            payload,
                            as: SiriClaw-InstructGatewayHealthOK.self))?.ok ?? true
                        continuation.yield(.health(ok: ok))
                    case "chat":
                        guard let payload = evt.payload else { break }
                        if let chatPayload = try? GatewayPayloadDecoding.decode(
                            payload,
                            as: SiriClaw-InstructChatEventPayload.self)
                        {
