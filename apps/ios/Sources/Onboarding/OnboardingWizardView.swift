...
struct OnboardingWizardView: View {
    ...
    private var welcomeStep: some View {
        VStack(spacing: 0) {
            ...
            Text("Connect to your SiriClaw-Instruct gateway")
                .font(.subheadline)
                .foregroundStyle(.secondary)
            ...
            if self.issue.needsPairing {
                Section {
                    ...
                    let requestLine: String = {
                        ...
                        return "Request ID: check `SiriClaw-Instruct devices list`."
                    }()
                    Text(
                        "Approve this device on the gateway.\n"
                            + "1) `SiriClaw-Instruct devices approve` (or `SiriClaw-Instruct devices approve <requestId>`)\n"
                            + "2) `/pair approve` in Telegram\n"
                            + "\(requestLine)\n"
                            + "SiriClaw-Instruct will also retry automatically when you return to this app.")
                }
            }
    ...
    private func initializeState() {
        if self.manualHost.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty {
            if let last = GatewaySettingsStore.loadLastGatewayConnection() {
                ...
                case .discovered:
                    self.manualHost = "SiriClaw-Instruct.local"
        ...
        if self.selectedMode == .developerLocal && self.manualHost == "SiriClaw-Instruct.local" {
    ...
    private func applyModeDefaults(_ mode: OnboardingConnectionMode) {
        ...
        let hostIsDefaultLike = host.isEmpty || host == "SiriClaw-Instruct.local" || host == "localhost"

        switch mode {
        case .homeNetwork:
            if hostIsDefaultLike { self.manualHost = "SiriClaw-Instruct.local" }
            self.manualTLS = true
            if self.manualPort <= 0 || self.manualPort > 65535 { self.manualPort = 18789 }
        case .remoteDomain:
            if host == "SiriClaw-Instruct.local" || host == "localhost" { self.manualHost = "" }
            self.manualTLS = true
            if self.manualPort <= 0 || self.manualPort > 65535 { self.manualPort = 18789 }
        case .developerLocal:
