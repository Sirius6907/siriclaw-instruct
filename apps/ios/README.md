# SiriClaw-Instruct iOS (Super Alpha)

NO TEST FLIGHT AVAILABLE AT THIS POINT

This iPhone app is super-alpha and internal-use only. It connects to an SiriClaw-Instruct Gateway as a `role: node`.
...
cd apps/ios
xcodegen generate
open SiriClaw-Instruct.xcodeproj
```

3. In Xcode:
   - Scheme: `SiriClaw-Instruct`
...
- `apps/ios/Sources/SiriClaw-Instruct.entitlements` sets `aps-environment` to `development`.
