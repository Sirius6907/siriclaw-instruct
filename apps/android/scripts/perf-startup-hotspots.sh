...
Default package/activity target SiriClaw-Instruct Android startup.
EOF
}
...
if [[ -z "$OUTPUT_PERF_DATA" ]]; then
  OUTPUT_PERF_DATA="/tmp/SiriClaw-Instruct-startup-$(date +%Y%m%d-%H%M%S).perf.data"
fi
...
tmp_dir="$(mktemp -d -t SiriClaw-Instruct-android-hotspots.XXXXXX)"
