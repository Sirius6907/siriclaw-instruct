...
private const val NOTIFICATION_CHANNEL_BASE_ID = "SiriClaw-Instruct.system.notify"
  ...
  private fun ensureChannel(priority: String?): String {
    ...
    val (suffix, importance, name) =
      when (normalizedPriority) {
        "passive" -> Triple("passive", NotificationManager.IMPORTANCE_LOW, "SiriClaw-Instruct Passive")
        "timesensitive" -> Triple("timesensitive", NotificationManager.IMPORTANCE_HIGH, "SiriClaw-Instruct Time Sensitive")
        else -> Triple("active", NotificationManager.IMPORTANCE_DEFAULT, "SiriClaw-Instruct Active")
      }
