  ...
  fun refreshNodeCanvasCapability_sendsObjectParamsAndUpdatesScopedUrl() = runBlocking {
    ...
    val server =
      startGatewayServer(json) { webSocket, id, method, frame ->
        when (method) {
          "connect" -> {
            webSocket.send(connectResponseFrame(id, canvasHostUrl = "http://127.0.0.1/__SiriClaw-Instruct__/cap/old-cap"))
          }
    ...
    try {
      ...
      assertEquals(
        "http://127.0.0.1:${server.port}/__SiriClaw-Instruct__/cap/new-cap",
    ...
    session.connect(
      ...
      options =
        GatewayConnectOptions(
          ...
          client =
            GatewayClientInfo(
              id = "SiriClaw-Instruct-android-test",
